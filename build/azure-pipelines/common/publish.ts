...
class ProvisionService {
    constructor(private readonly log: (...args: any[]) => void, private readonly accessToken: string) { }
...
class ESRPClient {
    ...
    constructor(private readonly log: (...args: any[]) => void, private readonly tmp: Temp, tenantId: string, clientId: string, authCertSubjectName: string, requestSigningCertSubjectName: string) {
    ...
    private async SubmitRelease(version: string, filePath: string): Promise<SubmitReleaseResult> {
    ...
    private async ReleaseDetails(releaseId: string): Promise<ReleaseDetailsResult> {
        const inputPath = this.tmp.tmpNameSync();
        fs.writeFileSync(inputPath, JSON.stringify({
            Version: '1.0.0',
...
async function releaseAndProvision(log: (...args: any[]) => void, releaseTenantId: string, releaseClientId: string, releaseAuthCertSubjectName: string, releaseRequestSigningCertSubjectName: string, provisionTenantId: string, provisionAADUsername: string, provisionAADPassword: string, version: string, quality: string, filePath: string): Promise<string> {
