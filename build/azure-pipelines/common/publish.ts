...
type CreateProvisionedFilesResponse =
	| CreateProvisionedFilesSuccessResponse
	| CreateProvisionedFilesErrorResponse;
...
class ProvisionService {
	...
	async provision(releaseId: string, fileId: string, fileName: string) {
		...
		const res = await retry(() =>
			this.request<CreateProvisionedFilesResponse>(
				"POST",
...
class ESRPClient {
	...
	async release(version: string, filePath: string): Promise<Release> {
		const submitReleaseResult = await ESRPClient.Sequencer.queue(
			async () => {
...
const azdoFetchOptions = {
	headers: { Authorization: `Bearer ${e("SYSTEM_ACCESSTOKEN")}` },
};
...
async function uploadAssetLegacy(
	log: (...args: any[]) => void,
	quality: string,
	commit: string,
	filePath: string
): Promise<{ assetUrl: string; mooncakeUrl: string }> {
	...
	if (shouldUploadToMooncake) {
		const mooncakeCredential = new ClientSecretCredential(
			e("AZURE_MOONCAKE_TENANT_ID"),
			e("AZURE_MOONCAKE_CLIENT_ID"),
			e("AZURE_MOONCAKE_CLIENT_SECRET")
		);
		const mooncakeBlobServiceClient = new BlobServiceClient(
...
async function processArtifact(artifact: Artifact): Promise<void> {
	...
	const log = (...args: any[]) =>
		console.log(`[${product} ${os} ${arch} ${unprocessedType}]`, ...args);
	...
	const filePath = await retry(async (attempt) => {
		...
		await downloadSequencer.queue(async () => {
			log(
				`Downloading ${artifact.resource.downloadUrl} (attempt ${attempt})...`
			);
		...
		if (
			artifactSize !== Number(artifact.resource.properties.artifactsize)
		) {
			throw new Error(
				`Artifact size mismatch. Expected ${artifact.resource.properties.artifactsize}. Actual ${artifactSize}`
			);
		}
	...
	log(
		`Publishing (size = ${size}, SHA1 = ${sha1hash}, SHA256 = ${sha256hash})...`
	);
...
async function main() {
	...
	while (true) {
		...
		if (
			stagesCompleted.size === stages.size &&
