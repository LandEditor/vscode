...
class ESRPReleaseService {
	static async create(
		...
		const app = new ConfidentialClientApplication({
			auth: {
				...
				clientCertificate: {
					thumbprintSha256: getThumbprint(
						authCertificate,
						"sha256",
					).toString("hex"),
					privateKey: authKey,
		...
		try {
			...
			for (let i = 0; i < 720; i++) {
				...
				if (releaseStatus.status === "pass") {
					...
					throw new Error(
						`Failed to submit release: ${JSON.stringify(releaseStatus)}`,
					);
				}
			}
	...
	private async submitRelease(
		...
		if (!res.ok) {
			...
			throw new Error(
				`Failed to submit release: ${res.statusText}\n${text}`,
			);
		}
...
async function processArtifact(artifact: Artifact, filePath: string) {
	...
	await retry(async (attempt) => {
		...
		const client = new CosmosClient({
			...
			tokenProvider: () =>
				Promise.resolve(
					`type=aad&ver=1.0&sig=${cosmosDBAccessToken.token}`,
				),
		});
