...
function isCreateProvisionedFilesErrorResponse(
	response: unknown,
): response is CreateProvisionedFilesErrorResponse {
	return (
		(response as CreateProvisionedFilesErrorResponse)?.ErrorDetails
			?.Code !== undefined
	);
}
...
class ProvisionService {
	...
	async provision(releaseId: string, fileId: string, fileName: string) {
		...
		if (
			isCreateProvisionedFilesErrorResponse(res) &&
			res.ErrorDetails.Code === "FriendlyFileNameAlreadyProvisioned"
		) {
			this.log(
				`File already provisioned (most likley due to a re-run), skipping: ${fileName}`,
			);
			return;
		}
		...
		if (!res.IsSuccess) {
			throw new Error(
				`Failed to submit provisioning request: ${JSON.stringify(res.ErrorDetails)}`,
			);
		}
	...
	private async request<T>(
		...
		if (
			(!res.ok || res.status < 200 || res.status >= 500) &&
			res.status !== 400
		) {
			throw new Error(
...
class ESRPClient {
	...
	async release(version: string, filePath: string): Promise<Release> {
		...
		if (details.releaseDetails[0].statusCode !== "pass") {
			throw new Error(
				`Timed out waiting for release ${releaseId}: ${JSON.stringify(details)}`,
			);
		}
