...
async function main() {
	const [, , product, os, arch, unprocessedType, fileName, filePath] =
		process.argv;
	...
	const stat = await new Promise((c, e) =>
		fs.stat(filePath, (err, stat) => (err ? e(err) : c(stat)))
	);
	...
	const shouldUploadToMooncake = /true/i.test(
		process.env["VSCODE_PUBLISH_TO_MOONCAKE"] ?? "true"
	);
	if (shouldUploadToMooncake) {
		const mooncakeCredential = new identity_1.ClientSecretCredential(
			process.env["AZURE_MOONCAKE_TENANT_ID"],
			process.env["AZURE_MOONCAKE_CLIENT_ID"],
			process.env["AZURE_MOONCAKE_CLIENT_SECRET"]
		);
