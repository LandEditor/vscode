...
class SwcTranspiler {
	...
	transpile(file) {
		...
		if (this._cmdLine.options.module === ts.ModuleKind.AMD) {
			...
			this._cmdLine.options.module === ts.ModuleKind.CommonJS ||
			this._cmdLine.options.module === ts.ModuleKind.Node16
		) {
			options = SwcTranspiler._swcrcCommonJS;
		}
		this._jobs.push(
			swc
				.transform(tsSrc, options)
				.then((output) => {
					// check if output of a DTS-files isn't just "empty" and iff so
					// skip this file
					if (
						file.path.endsWith(".d.ts") &&
						_isDefaultEmpty(output.code)
					) {
						return;
					}
