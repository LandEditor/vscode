...
module _nls {
    ...
    function analyze(ts: typeof import('typescript'), contents: string, functionName: 'localize' | 'localize2', options: ts.CompilerOptions = {}): ILocalizeAnalysisResult {
    ...
    function patch(ts: typeof import('typescript'), typescript: string, javascript: string, sourcemap: sm.RawSourceMap, options: {
        preserveEnglish: boolean;
    }): INlsPatchResult {
    ...
    export function patchFile(javascriptFile: File, typescript: string, options: {
        preserveEnglish: boolean;
    }): File {
        ...
        const { javascript, sourcemap, nlsKeys, nlsMessages } = patch(ts, typescript, javascriptFile.contents.toString(), (<any>javascriptFile).sourceMap, options);
