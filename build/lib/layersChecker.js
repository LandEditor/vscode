...
const TS_CONFIG_PATH = (0, path_1.join)(
	__dirname,
	"../../",
	"src",
	"tsconfig.json"
...
function checkFile(program, sourceFile, rule) {
	...
	function checkNode(node) {
		...
		if (rule.disallowedTypes?.some((disallowed) => disallowed === text)) {
			...
			console.log(
				`[build/lib/layersChecker.ts]: Reference to type '${text}' violates layer '${
					rule.target
...
for (const sourceFile of program.getSourceFiles()) {
	for (const rule of RULES) {
		if (
			(0, minimatch_1.match)([sourceFile.fileName], rule.target).length >
			0
		) {
			if (!rule.skip) {
