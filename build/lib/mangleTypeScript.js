...
class ClassData {
	...
	static makeImplicitPublicActuallyPublic(data, reportViolation) {
		...
		for (const [name, info] of data.fields) {
			...
			while (parent) {
				if (
					parent.fields.get(name)?.type ===
					1 /* FieldType.Protected */
				) {
...
class DeclarationData {
	...
	shouldMangle(newName) {
		...
		if (
			skippedFiles.some((file) =>
				this.node.getSourceFile().fileName.endsWith(file)
			)
		) {
			return false;
		}
...
class ConstData {
	...
	get locations() {
		...
		const definitionResult = this.service.getDefinitionAndBoundSpan(
			this.decl.getSourceFile().fileName,
			this.decl.name.getStart()
		);
		if (
	...
	shouldMangle(newName) {
		...
		if (
			skippedFiles.some((file) =>
				this.decl.getSourceFile().fileName.endsWith(file)
			)
		) {
			return false;
		}
...
class Mangler {
	...
	computeNewFileContents(strictImplicitPublicHandling) {
		...
		const visit = (node) => {
			...
			if (
				...
				if (node.name) {
					...
					this.allExportedDeclarationsByKey.set(
						key,
						new DeclarationData(node.getSourceFile().fileName, node)
					);
				}
			}
			if (
				...
				if (node.name && node.body) {
					// On named function and not on the overload
					const anchor = node.name;
					const key = `${
						node.getSourceFile().fileName
					}|${anchor.getStart()}`;
		...
		this.log(
			`Done collecting. Classes: ${this.allClassDataByKey.size}. Exported const/fn: ${this.allExportedDeclarationsByKey.size}`
		);
		...
		for (const item of this.service.getProgram().getSourceFiles()) {
			...
			const sourceMapRoot =
				mapRoot ??
				(0, url_1.pathToFileURL)(sourceRoot ?? projectDir).toString();
			// source maps
			let generator;
			let newFullText;
...
async function _run() {
	...
	for await (const [fileName, contents] of new Mangler(
		projectPath,
		console.log
	).computeNewFileContents(new Set(["saveState"]))) {
