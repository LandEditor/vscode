...
exports.prepareIslFiles =
	exports.prepareI18nPackFiles =
	exports.createXlfFilesForIsl =
	exports.createXlfFilesForExtensions =
	exports.EXTERNAL_EXTENSIONS =
	exports.createXlfFilesForCoreBundle =
	exports.getResource =
	exports.processNlsFiles =
	exports.XLF =
	exports.Line =
...
class XLF {
	...
	addStringItem(file, item) {
		if (!item.id || item.message === undefined || item.message === null) {
			throw new Error(
				`No item ID or value specified: ${JSON.stringify(
					item,
				)}. File: ${file}`,
			);
		}
	...
	static parse = (xlfString) =>
		new Promise((resolve, reject) => {
			...
			const files = [];
			parser.parseString(xlfString, (err, result) => {
				if (err) {
					reject(
						new Error(
					...
					if (transUnits) {
						transUnits.forEach((unit) => {
							...
							if (!key) {
								reject(
									new Error(
										`XLF parsing error: trans-unit ${JSON.stringify(
											unit,
											undefined,
											0,
										)} defined in file ${name} is missing the ID attribute.`,
									),
								);
								return;
							}
...
function processCoreBundleFormat(fileHeader, languages, json, emitter) {
		...
		Object.keys(bundleSection).forEach((bundle) => {
			...
			modules.forEach((module, index) => {
				...
				if (!messages) {
					emitter.emit(
						"error",
						`Didn't find messages for module ${module}.`,
					);
					return;
				}
	...
	sortedLanguages.forEach((language) => {
		...
		if (!stats) {
			log(
				`\tNo translations found for language ${language.id}. Using default language instead.`,
			);
		}
	});
}
		...
		if (basename === "nls.metadata.json") {
			if (file.isBuffer()) {
				...
				for (const resource in xlfs) {
					...
					const filePath = `${xlf.project}/${resource.replace(
						/\//g,
						"_",
					)}.xlf`;
...
function createL10nBundleForExtension(
	...
	return gulp
		...
		.pipe(
			(0, event_stream_1.map)((data, callback) => {
				const file = data;
				if (!file.isBuffer()) {
...
function createXlfFilesForExtensions() {
	...
	return (0, event_stream_1.through)(
		function (extensionFolder) {
			...
			(0, event_stream_1.merge)(
				...
				createL10nBundleForExtension(
					extensionFolderName,
					exports.EXTERNAL_EXTENSIONS.includes(extensionId),
				),
			).pipe(
