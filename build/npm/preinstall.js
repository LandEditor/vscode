...
const minorNodeVersion = Number.parseInt(nodeVersion[2]);
const patchNodeVersion = Number.parseInt(nodeVersion[3]);
...
if (
	...
	(majorYarnVersion === 1 &&
		(minorYarnVersion < 10 ||
			(minorYarnVersion === 10 && patchYarnVersion < 1))) ||
	majorYarnVersion >= 2
) {
...
if (!/yarn[\w-.]*\.c?js$|yarnpkg$/.test(process.env["npm_execpath"])) {
	console.error(
		"\x1b[1;31m*** Please use yarn to install dependencies.\x1b[0;0m",
	);
	err = true;
}
