...
function darkenColor(color) {
	...
	for (let i = 1; i < 7; i += 2) {
		const newVal = Math.round(
			Number.parseInt("0x" + color.substr(i, 2), 16) * 0.9,
		);
				...
				if (preferredDef) {
					...
					if (
						...
						for (let i2 = 0; i2 < exts.length; i2++) {
							...
							if (
								ext2Def[exts[i2]] === preferredDef ||
								ignoreExtAssociation[exts[i2]]
							) {
			...
			return download(colorsFile).then((content) => {
				...
				return getCommitSha("jesseweed/seti-ui").then((info) => {
					try {
						...
						const cgmanifest = fs
							.readFileSync(cgmanifestPath)
							.toString();
						const cgmanifestContent = JSON.parse(cgmanifest);
						cgmanifestContent["registrations"][0]["component"][
							"git"
