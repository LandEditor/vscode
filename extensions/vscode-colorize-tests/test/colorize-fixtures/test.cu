...
#define assertSucceeded(c)                                                     \
	do {                                                                       \
		...
		if (__tmp != cudaSuccess) {                                            \
			fprintf(stderr,                                                    \
					"Operation \"%s\" failed with error code %x. (%s:%d)\n",   \
...
int main() {
	...
	for (size_t i = 0; i < dataLength; i++) {
		...
		for (size_t j = 0; j < sizeof(input[i].structArr); j++) {
			reinterpret_cast<byte *>(input[i].structArr)[j] =
				'0' + static_cast<char>((i + j) % 10);
		}
	...
	assertSucceeded(cudaMallocManaged(reinterpret_cast<void **>(&results),
									  sizeof(unsigned) * dataLength));
	assert(!!results);
