...
const combinePaths: (path: string, ...paths: (string | undefined)[]) => string =
	(ts as any).combinePaths;
...
function createServerHost(
	...
	const ensureTrailingDirectorySeparator: (path: string) => string = (
		ts as any
	).ensureTrailingDirectorySeparator;
	const getDirectoryPath: (path: string) => string = (ts as any)
		.getDirectoryPath;
...
function mapUri(uri: URI, mappedScheme: string): URI {
	...
	uri = uri.with({
		scheme: mappedScheme,
		path: `/${uri.scheme}/${uri.authority || "ts-nul-authority"}${
			uri.path
		}`,
	});
