...
export default class FileConfigurationManager extends Disposable {
	...
	private getPreferences(
		...
		const preferences: Proto.UserPreferences = {
			...
			importModuleSpecifierPreference:
				getImportModuleSpecifierPreference(preferencesConfig),
			importModuleSpecifierEnding:
				getImportModuleSpecifierEndingPreference(preferencesConfig),
			jsxAttributeCompletionStyle:
				getJsxAttributeCompletionStyle(preferencesConfig),
			allowTextChangesInNewFiles:
				document.uri.scheme === fileSchemes.file,
			...
			autoImportSpecifierExcludeRegexes: preferencesConfig.get<string[]>(
				"autoImportSpecifierExcludeRegexes",
			),
			preferTypeOnlyAutoImports: preferencesConfig.get<boolean>(
			...
			includeCompletionsForModuleExports: config.get<boolean>(
				"suggest.autoImports",
			),
	...
	private getAutoImportFileExcludePatternsPreference(
		...
		return (
			workspaceFolder &&
			config.get<string[]>("autoImportFileExcludePatterns")?.map((p) => {
				...
				const wildcardPrefix = this.client.apiVersion.gte(API.v540)
					? ""
					: path.parse(this.client.toTsFilePath(workspaceFolder)!)
							.root;
	...
	private getOrganizeImportsPreferences(
		...
		const organizeImportsCollation = config.get<"ordinal" | "unicode">(
			"organizeImports.unicodeCollation",
		);

		const organizeImportsCaseSensitivity = config.get<
		...
		return {
			...
			organizeImportsTypeOrder: withDefaultAsUndefined(
				config.get<"auto" | "last" | "inline" | "first">(
					"organizeImports.typeOrder",
					"auto",
				),
				"auto",
			),
							...
							organizeImportsCaseSensitivity === "caseInsensitive"
								...
								: withDefaultAsUndefined(
										config.get<
											...
											| "lower"
											| false
										>("organizeImports.caseFirst", false),
										"default",
									),
						organizeImportsAccentCollation: config.get<boolean>(
							"organizeImports.accentCollation",
						),
						organizeImportsLocale: config.get<string>(
							"organizeImports.locale",
						),
						organizeImportsNumericCollation: config.get<boolean>(
							"organizeImports.numericCollation",
						),
					}
				: {}),
		};
...
export function getInlayHintsPreferences(
	...
	return {
		includeInlayParameterNameHints:
			getInlayParameterNameHintsPreference(config),
