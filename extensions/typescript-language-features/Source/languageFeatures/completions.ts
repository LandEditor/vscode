...
class MyCompletionItem extends vscode.CompletionItem {
    ...
    private async isValidFunctionCompletionContext(client: ITypeScriptServiceClient, filepath: string, position: vscode.Position, document: vscode.TextDocument, token: vscode.CancellationToken): Promise<boolean> {
...
class TypeScriptCompletionItemProvider implements vscode.CompletionItemProvider<MyCompletionItem> {
    ...
    public async provideCompletionItems(document: vscode.TextDocument, position: vscode.Position, token: vscode.CancellationToken, context: vscode.CompletionContext): Promise<vscode.CompletionList<MyCompletionItem> | undefined> {
    ...
    private logCompletionsTelemetry(duration: number, response: ServerResponse.Response<Proto.CompletionInfoResponse> | undefined, includesPackageJsonImport?: boolean, includesImportStatementCompletion?: boolean) {
        /* __GDPR__
            "completions.execute" : {
                "owner": "mjbvz",
