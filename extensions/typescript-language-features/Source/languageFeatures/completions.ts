...
import type { Command, CommandManager } from "../commands/commandManager";
import type { DocumentSelector } from "../configuration/documentSelector";
import type { LanguageDescription } from "../configuration/languageDescription";
import type { TelemetryReporter } from "../logging/telemetry";
...
import type TypingsStatus from "../ui/typingsStatus";
import { nulToken } from "../utils/cancellation";
import type FileConfigurationManager from "./fileConfigurationManager";
...
class MyCompletionItem extends vscode.CompletionItem {
	...
	constructor(
		...
		if (sourceDisplay) {
			this.label = {
				label,
				description: Previewer.asPlainTextWithLinks(
					sourceDisplay,
					client,
				),
			};
		...
		this.range = this.getRangeFromReplacementSpan(
			tsEntry,
			completionContext,
		);
	...
	public async resolveCompletionItem(
		...
		const promise = (async (): Promise<
			ResolvedCompletionItem | undefined
		> => {
					...
					if (change.fileName === filepath) {
						additionalTextEdits.push(
							...change.textChanges.map(
								typeConverters.TextEdit.fromCodeEdit,
							),
						);
					} else {
						hasRemainingCommandsOrEdits = true;
				...
				arguments: [
					...
					detail.codeActions.map(
						(x): Proto.CodeAction => ({
							...
							changes: x.changes.filter(
								(x) => x.fileName !== filepath,
							),
						}),
					),
				],
			...
			*/
			this.telemetryReporter.logTelemetry("completions.accept", {
				isPackageJsonImport: item.tsEntry.isPackageJsonImport
					? "true"
					: undefined,
				isImportStatementCompletion: item.tsEntry
...
class ApplyCompletionCommand implements Command {
	...
	public async execute(item: MyCompletionItem) {
		const resolved = await item.resolveCompletionItem(
			this.client,
			nulToken,
		);
		if (!resolved) {
	...
	implements vscode.CompletionItemProvider<MyCompletionItem>
{
	...
	constructor(
		...
		commandManager.register(
			new ApplyCompletionCodeActionCommand(this.client),
		);
		commandManager.register(
	...
	public async provideCompletionItems(
		...
		if (this.client.apiVersion.gte(API.v300)) {
			...
			if (isMemberCompletion) {
				const dotMatch =
					line.text.slice(0, position.character).match(/\??\.\s*$/) ||
					undefined;
				if (dotMatch) {
					const range = new vscode.Range(
						position.translate({
							characterDelta: -dotMatch[0].length,
						}),
						position,
					);
			...
			isIncomplete =
				...
				(response.metadata as any)?.isIncomplete;
		...
		*/
		this.telemetryReporter.logTelemetry("completions.execute", {
			...
			updateGraphDurationMs:
				response?.type === "response" &&
				typeof response.performanceData?.updateGraphDurationMs ===
					"number"
			...
			createAutoImportProviderProgramDurationMs:
				...
				typeof response.performanceData
					...
					: undefined,
			includesPackageJsonImport: includesPackageJsonImport
				? "true"
				: undefined,
	...
	private getTsTriggerCharacter(
		...
		switch (context.triggerCharacter) {
			case "@": {
				...
				return this.client.apiVersion.gte(API.v310) &&
					this.client.apiVersion.lt(API.v320)
					? undefined
					: "@";
			}
			case "#": {
				// Workaround for https://github.com/microsoft/TypeScript/issues/36367
