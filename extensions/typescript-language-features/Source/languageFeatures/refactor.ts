...
class MoveToFileRefactorCommand implements Command {
	...
	private async getTargetFile(
		...
		const picked = await new Promise<DestinationItem | undefined>(
			(resolve) => {
		...
		if (picked === selectExistingFileItem) {
			...
			return picked?.length
				? this.client.toTsFilePath(picked[0])
				: undefined;
...
const Rewrite_Arrow_Braces = Object.freeze<CodeActionKind>({
	kind: vscode.CodeActionKind.RefactorRewrite.append("arrow").append(
		"braces",
	),
	matches: (refactor) =>
		refactor.name.startsWith("Convert default export") ||
...
class InlinedCodeAction extends vscode.CodeAction {
	constructor(
		...
		public readonly copilotRename?: (
			info: Proto.RefactorEditInfo,
		) => vscode.Command,
	) {
		...
		this.command = {
			...
			arguments: [
				<DidApplyRefactoringCommand.Args>{ action: action.name },
			],
		};
	}
			...
			if (this.document.uri.scheme !== fileSchemes.walkThroughSnippet) {
				...
				this.command = {
					...
					arguments: coalesce([
						this.command,
						this.copilotRename
							? this.copilotRename(response.body)
							: {
	...
	private static getKind(refactor: Proto.RefactorActionInfo) {
		if ((refactor as Proto.RefactorActionInfo & { kind?: string }).kind) {
			return vscode.CodeActionKind.Empty.append(
				(refactor as Proto.RefactorActionInfo & { kind?: string })
					.kind!,
			);
		}
...
class MoveToFileCodeAction extends vscode.CodeAction {
	constructor(
		...
		this.command = {
			...
			arguments: [
				<MoveToFileRefactorCommand.Args>{ action, document, range },
			],
		};
	}
}
...
class TypeScriptRefactorProvider
	implements vscode.CodeActionProvider<TsCodeAction>
{
	...
	constructor(
		...
		const didApplyRefactoringCommand = commandManager.register(
			new DidApplyRefactoringCommand(telemetryReporter),
		);
		commandManager.register(new CompositeCommand());
		commandManager.register(new SelectRefactorCommand(this.client));
		commandManager.register(
			new MoveToFileRefactorCommand(
				this.client,
				...
				Array.from(applicableRefactors, async (action) => {
					...
					if (
						...
						if (
							...
							const navTree = await this.cachedNavTree.execute(
								document,
								() =>
									this.client.execute(
										"navtree",
		...
		for (const refactor of refactors) {
			if (refactor.inlineable === false) {
				...
				for (const action of refactor.actions) {
					yield this.refactorActionToCodeAction(
						document,
						refactor,
						action,
						rangeOrSelection,
	...
	private refactorActionToCodeAction(
		...
		if (action.name === "Move to file") {
			codeAction = new MoveToFileCodeAction(
				document,
				action,
				rangeOrSelection,
			);
			...
			let copilotRename:
				| ((info: Proto.RefactorEditInfo) => vscode.Command)
				| undefined;
			if (
				vscode.workspace
					.getConfiguration("typescript", null)
					.get("experimental.aiCodeActions")
			) {
				if (
					(Extract_Constant.matches(action) &&
						vscode.workspace
							.getConfiguration("typescript")
							.get(
								"experimental.aiCodeActions.extractConstant",
							)) ||
					(Extract_Function.matches(action) &&
						vscode.workspace
							.getConfiguration("typescript")
							.get(
								"experimental.aiCodeActions.extractFunction",
							)) ||
					(Extract_Type.matches(action) &&
						vscode.workspace
							.getConfiguration("typescript")
							.get("experimental.aiCodeActions.extractType")) ||
					(Extract_Interface.matches(action) &&
						vscode.workspace
							.getConfiguration("typescript")
							.get("experimental.aiCodeActions.extractInterface"))
				) {
						...
						arguments: [
							{
								...
								expand: Extract_Constant.matches(action)
									? {
											...
											pos: typeConverters.Position.fromLocation(
												info.renameLocation!,
											),
									  }
									: {
			...
			codeAction = new InlinedCodeAction(
				this.client,
				document,
				refactor,
				action,
		...
		if (Extract_Constant.matches(action)) {
			...
			return allActions
				.filter(
					(otherAtion) =>
						otherAtion !== action &&
						Extract_Constant.matches(otherAtion),
				)
				.every((otherAction) => {
					const otherScope = getScope(otherAction.name);
					return typeof otherScope === "number"
						? scope < otherScope
						: true;
				});
		}
	...
	private appendInvalidActions(
		...
		if (
			!actions.some(
				(action) =>
					action.kind && Extract_Constant.kind.contains(action.kind),
			)
		) {
		...
		if (
			!actions.some(
				(action) =>
					action.kind && Extract_Function.kind.contains(action.kind),
			)
		) {
			const disabledAction = new vscode.CodeAction(
	...
	private pruneInvalidActions(
		...
		const topNInvalid = prioritizedActions
			.filter(
				(action) =>
					!only || (action.kind && only.contains(action.kind)),
			)
