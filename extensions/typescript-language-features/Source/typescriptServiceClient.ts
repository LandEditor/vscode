				...
				if (item === reportIssueItem) {
					...
					if (tsVersion.isYarnPnp()) {
						const reportIssue: vscode.MessageItem = {
							title: vscode.l10n.t(
								"Report issue against Yarn PnP",
							),
						};
						const response = await vscode.window.showWarningMessage(
							vscode.l10n.t(
								"Please report an issue against Yarn PnP",
							),
							{
								modal: true,
						...
						if (response === reportIssue) {
							vscode.env.openExternal(
								vscode.Uri.parse(
									"https://github.com/yarnpkg/berry/issues",
								),
							);
						}
					}
		...
		switch (event.event) {
			...
			case EventName.createDirectoryWatcher: {
				...
				if (
					...
					fpath === path.join(homedir(), "Library")
				) {
			...
			case EventName.createFileWatcher: {
				const path = (event.body as Proto.CreateFileWatcherEventBody)
					.path;
					...
					this.diagnosticsManager.logDiagnosticsPerformanceTelemetry(
						diagnosticsDuration.map((fileData) => {
							...
							return {
								...fileData,
								fileLineCount:
									this.bufferSyncSupport.lineCount(resource),
							};
						}),
					);
				}
