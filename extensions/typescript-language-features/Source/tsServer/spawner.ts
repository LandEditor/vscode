...
import { API } from "./api";
import type { ILogDirectoryProvider } from "./logDirectoryProvider";
import type { NodeVersionManager } from "./nodeManager";
import type { TypeScriptPluginPathsProvider } from "./pluginPathsProvider";
...
import {
	...
	type TsServerDelegate,
	type TsServerLog,
	type TsServerProcessFactory,
	TsServerProcessKind,
} from "./server";
...
export class TypeScriptServerSpawner {
	...
	private spawnTsServer(
		...
		const { args, tsServerLog, tsServerTraceDirectory } =
		...
		if (TypeScriptServerSpawner.isLoggingEnabled(configuration)) {
			if (tsServerLog?.type === "file") {
				this._logger.info(
					`<${kind}> Log file: ${tsServerLog.uri.fsPath}`,
				);
		...
		if (configuration.enableTsServerTracing) {
			if (tsServerTraceDirectory) {
				...
				this._logger.error(
					`<${kind}> Could not create trace directory`,
				);
			}
		}
	...
	private getTsServerArgs(
		...
		if (pluginManager.plugins.length) {
			...
			const isUsingBundledTypeScriptVersion =
				currentVersion.path ===
				this._versionProvider.defaultVersion.path;
			for (const plugin of pluginManager.plugins) {
				if (
					...
					pluginPaths.push(
						isWeb() ? plugin.uri.toString() : plugin.uri.fsPath,
					);
				}
			}
		}
