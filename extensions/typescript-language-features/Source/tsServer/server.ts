...
import type { TypeScriptServiceConfiguration } from "../configuration/configuration";
import type { TelemetryReporter } from "../logging/telemetry";
import type Tracer from "../logging/tracer";
...
import type { OngoingRequestCanceller } from "./cancellation";
import type { NodeVersionManager } from "./nodeManager";
import type * as Proto from "./protocol/protocol";
import { EventName } from "./protocol/protocol.const";
import type { TypeScriptVersionManager } from "./versionManager";
import type { TypeScriptVersion } from "./versionProvider";
...
export enum TsServerProcessKind {
	Main = "main",
	Syntax = "syntax",
	Semantic = "semantic",
			...
			switch (message.type) {
				...
				case "event": {
					...
					if (event.event === "requestCompleted") {
						const seq = (event as Proto.RequestCompletedEvent).body
							.request_seq;
		...
		if (
			...
			if (executeInfo.token) {
				...
				executeInfo.token.onCancellationRequested(() => {
					if (
						requestStates.some(
							(state) => state === RequestState.Resolved,
						)
					) {
						// Don't cancel.
					...
					request.then(
						(result) => {
							...
							const erroredRequest = requestStates.find(
								(state) =>
									state.type === RequestState.Type.Errored,
							) as RequestState.Errored | undefined;
							if (erroredRequest) {
								// We've gone out of sync
								this.delegate.onFatalError(
									command,
									erroredRequest.err,
								);
							}
						...
						(err) => {
							requestStates[serverIndex] =
								new RequestState.Errored(err);
							if (
								requestStates.some(
									(state) => state === RequestState.Resolved,
								)
			...
			[
				{
					...
					canRun: (command) =>
						["geterr", "geterrForProject"].includes(command),
				},
				{
	...
	implements ITypeScriptServer
{
	...
	private static readonly semanticCommands = new Set<
		keyof TypeScriptRequests
	>(["geterr", "geterrForProject", "projectInfo", "configurePlugin"]);
			...
			[
				{
					...
					canRun: (command, execInfo) => {
						...
						if (
							...
							this.projectLoading &&
							SyntaxRoutingTsServer.syntaxAllowedCommands.has(
								command,
							)
						) {
							return true;
