...
suite("CachedResponse", () => {
	test("should cache simple response for same document", async () => {
		...
		assertResult(
			await response.execute(doc, respondWith("test-0")),
			"test-0",
		);
		assertResult(
			await response.execute(doc, respondWith("test-1")),
			"test-0",
		);
	});
	...
	test("should invalidate cache for new document", async () => {
		...
		assertResult(
			await response.execute(doc1, respondWith("test-0")),
			"test-0",
		);
		assertResult(
			await response.execute(doc1, respondWith("test-1")),
			"test-0",
		);
		assertResult(
			await response.execute(doc2, respondWith("test-2")),
			"test-2",
		);
		assertResult(
			await response.execute(doc2, respondWith("test-3")),
			"test-2",
		);
		assertResult(
			await response.execute(doc1, respondWith("test-4")),
			"test-4",
		);
		assertResult(
			await response.execute(doc1, respondWith("test-5")),
			"test-4",
		);
	});
	...
	test("should not cache cancelled responses with document changes", async () => {
		...
		const result1 = response.execute(
			doc1,
			() => cancelledResponder.promise,
		);
		const result2 = response.execute(doc1, respondWith("test-0"));
		const result3 = response.execute(doc1, respondWith("test-1"));
		const result4 = response.execute(
			doc2,
			() => cancelledResponder2.promise,
		);
