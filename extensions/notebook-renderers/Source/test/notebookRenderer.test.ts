...
suite("Notebook builtin output renderer", () => {
	...
	function createContext(
		settings?: optionalRenderOptions
	): IRichRenderContext {
	...
	test(`Render with wordwrap and scrolling for error output`, async () => {
		...
		assert.ok(
			inserted.innerHTML.indexOf(
				">Expected type `str`, but received type"
			) > -1,
	...
	test(`Streaming outputs interleaved with other mime types will produce separate outputs`, async () => {
		...
		assert.ok(
			firstOutputElement.innerHTML.indexOf(">first stream content</") >
				-1,
			`Content was not added to output element: ${outputHtml.cellElement.innerHTML}`
		);
		assert.ok(
		...
		assert.ok(
			thirdOutputElement.innerHTML.indexOf(">second stream content</") >
				-1,
			`Content was not added to output element: ${outputHtml.cellElement.innerHTML}`
		);
	});
	...
	test(`Should clean up raw IPython error stack traces`, async () => {
		...
		assert.ok(
			outputElement.innerHTML.indexOf(
				'class="code-background-colored"'
			) === -1,
			`inner HTML:\n ${outputElement.innerHTML}`
		);
	});
