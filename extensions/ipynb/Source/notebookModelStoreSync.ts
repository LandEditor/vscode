...
import { type CellMetadata, useCustomPropertyInMetadata } from "./common";
	...
	const disposable = workspace.onDidChangeNotebookDocument((e) => {
		...
		if (!mergedEvents) {
			...
			mergedEvents = {
				...
				contentChanges: e.contentChanges.concat(
					mergedEvents.contentChanges,
				),
				notebook: e.notebook,
			};
		} else {
...
function onDidChangeNotebookCells(e: NotebookDocumentChangeEventEx) {
	...
	const preferredCellLanguage =
		notebookMetadata.metadata?.language_info?.name;
	const updates: {
		cell: NotebookCell;
		metadata: CellMetadata & { vscode?: { languageId: string } };
	}[] = [];
