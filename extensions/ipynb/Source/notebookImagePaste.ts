...
class DropOrPasteEditProvider implements vscode.DocumentPasteEditProvider, vscode.DocumentDropEditProvider {
    ...
    async provideDocumentPasteEdits(document: vscode.TextDocument, _ranges: readonly vscode.Range[], dataTransfer: vscode.DataTransfer, _context: vscode.DocumentPasteEditContext, token: vscode.CancellationToken): Promise<vscode.DocumentPasteEdit[] | undefined> {
    ...
    private async createInsertImageAttachmentEdit(document: vscode.TextDocument, dataTransfer: vscode.DataTransfer, token: vscode.CancellationToken): Promise<{
...
async function getDroppedImageData(dataTransfer: vscode.DataTransfer, token: vscode.CancellationToken): Promise<readonly ImageAttachmentData[]> {
...
function buildAttachment(cell: vscode.NotebookCell, attachments: readonly ImageAttachmentData[]): {
    metadata: {
        [key: string]: any;
    };
    filenames: string[];
...
export function notebookImagePasteSetup(): vscode.Disposable {
    ...
    return vscode.Disposable.from(vscode.languages.registerDocumentPasteEditProvider(JUPYTER_NOTEBOOK_MARKDOWN_SELECTOR, provider, {
