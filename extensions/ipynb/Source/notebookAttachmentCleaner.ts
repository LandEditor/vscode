...
import { deepClone, Delayer, objectEquals } from "./helper";
...
export class AttachmentCleaner implements vscode.CodeActionProvider {
	...
	private cleanNotebookAttachments(
		...
		const markdownAttachmentsRefedInCell =
			this.getAttachmentNames(document);
		...
		for (const [
			currFilename,
			attachment,
		] of markdownAttachmentsRefedInCell) {
			if (attachment.valid) {
	...
	private checkMetadataHasAttachmentsField(metadata: {
		...
		return (
			!!metadata.attachments && typeof metadata.attachments === "object"
		);
	}
	...
	private saveAllAttachmentsToCache(
		...
		this._attachmentCache.set(notebookUri, documentCache);
		const cellCache =
			documentCache.get(cellFragment) ??
			new Map<string, IAttachmentData>();
		documentCache.set(cellFragment, cellCache);
	...
	private getAttachmentNames(document: vscode.TextDocument) {
		...
		const filenames: Map<
			string,
			{ valid: boolean; ranges: vscode.Range[] }
		> = new Map();
