...
import { HtmlNode, Node, Property, Rule, Stylesheet } from "EmmetFlatNode";
...
export async function wrapWithAbbreviation(args: any): Promise<boolean> {
	...
	const operationRanges = Array.from(editor.selections)
		...
		.map((selection) => {
			...
			// wrap around the node if the selection falls inside its open or close tag
			{
				...
				if (
					startNode &&
					isOffsetInsideOpenOrCloseTag(startNode, startOffset)
				) {
				...
				if (
					endNode &&
					isOffsetInsideOpenOrCloseTag(endNode, endOffset)
				) {
					const nodeStartPosition = document.positionAt(
						endNode.start,
					);
					start = nodeStartPosition.isBefore(start)
						? nodeStartPosition
						: start;
			...
			(builder) => {
				...
				for (let i = 0; i < rangesToReplace.length; i++) {
					...
					const oldPreviewLines =
						oldPreviewRange.end.line -
						oldPreviewRange.start.line +
						1;
					const newLinesInserted =
						expandedTextLines.length - oldPreviewLines;
					...
					if (
						i > 0 &&
						newPreviewLineEnd === lastNewPreviewRange.end.line
					) {
	...
	async function makeChanges(
		...
		if (previewChanges) {
			const expandAbbrList: ExpandAbbreviationInput[] =
				rangesToReplace.map((rangesAndContent) => ({
					syntax,
					abbreviation,
...
export function expandEmmetAbbreviation(
	...
	const getAbbreviation = (
		...
		if (!rangeToReplace.isEmpty) {
			const extractedResults = helper.extractAbbreviationFromText(
				abbr,
				syntax,
			);
			if (extractedResults) {
...
export function isValidLocationForEmmetAbbreviation(
	...
	if (isStyleSheet(syntax)) {
		...
		if (
			...
			const tokenEndPos = document.positionAt(
				currentCssNode.selectorToken.end,
			);
			if (
...
function expandAbbr(input: ExpandAbbreviationInput): string | undefined {
	...
	try {
		expandedText = helper.expandAbbreviation(
			input.abbreviation,
			expandOptions,
		);
	} catch (e) {
...
export function getSyntaxFromArgs(args: { [x: string]: string }):
	| string
	| undefined {
