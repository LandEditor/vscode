...
export async function wrapWithAbbreviation(args: any): Promise<boolean> {
    ...
    async function makeChanges(inputAbbreviation: string | undefined, previewChanges: boolean): Promise<boolean> {
        ...
        if (previewChanges) {
            const expandAbbrList: ExpandAbbreviationInput[] = rangesToReplace.map(rangesAndContent => ({ syntax, abbreviation, rangeToReplace: rangesAndContent.originalRange, textToWrap: rangesAndContent.textToWrapInPreview, filter, indent, baseIndent: rangesAndContent.baseIndent }));
            inPreviewMode = true;

            return applyPreview(expandAbbrList);
        }
        const expandAbbrList: ExpandAbbreviationInput[] = rangesToReplace.map(rangesAndContent => ({ syntax, abbreviation, rangeToReplace: rangesAndContent.originalRange, textToWrap: rangesAndContent.textToWrapInPreview, filter, indent }));
...
export function expandEmmetAbbreviation(args: any): Thenable<boolean | undefined> {
    ...
    const getAbbreviation = (document: vscode.TextDocument, selection: vscode.Selection, position: vscode.Position, syntax: string): [
        vscode.Range | null,
        string,
        string | undefined
    ] => {
