...
import { getFlatNode, parsePartialStylesheet } from "../util";
...
suite("Tests for partial parse of Stylesheets", () => {
	...
	test("Ignore block comment inside rule", (): any => {
		const cssContents = `
p {
	margin: p ;
	/*dn: none; p */ p
...
`;
	...
	test("Ignore commented braces", (): any => {
		const sassContents = `
.foo
// .foo { brs
/* .foo { op.3
...
`;
	...
	test("Block comment between selector and open brace", (): any => {
		const cssContents = `
p
/* First line
of a multiline
...
`;
	...
	test("Nested and consecutive rulesets with errors", (): any => {
		const sassContents = `
.foo{
	a
	a
...
`;
	...
	test("Variables and interpolation", (): any => {
		const sassContents = `
p.#{dn} {
	p.3
	#{$attr}-color: blue;
