...
import type { HtmlNode } from "EmmetFlatNode";
...
export function nextItemHTML(
	...
	if (currentNode.type !== "comment") {
		...
		if (
			currentNode.open &&
			selectionEndOffset <=
				currentNode.open.start + currentNode.name.length
		) {
	...
	while (!prevNode && currentNode) {
		if (currentNode.previousSibling) {
			if (currentNode.previousSibling.type !== "comment") {
				prevNode = <HtmlNode>(
					getDeepestFlatNode(currentNode.previousSibling)
				);
			} else {
		...
		if (pos !== undefined) {
			...
			if (newSelectionStartOffset >= 0 && newSelectionEndOffset >= 0) {
				...
				const newSelectionEnd =
					attr.value.start + newSelectionEndOffset;
				return offsetRangeToSelection(
					document,
					newSelectionStart,
					newSelectionEnd,
