...
export class ExtensionLinter {
    ...
    private async lintWhenClauses(contributesNode: JsonNode | undefined, document: TextDocument): Promise<Diagnostic[]> {
        ...
        const parseResults = await commands.executeCommand<{
            ...
            offset: number;
            length: number;
        }[][]>('_validateWhenClauses', whenClauses.map(w => w.value as string /* we make sure to capture only if `w.value` is string above */));
        const diagnostics: Diagnostic[] = [];
