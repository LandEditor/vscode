...
function fixBadRegex(grammar) {
	...
	if (scopeResolution) {
		...
		if (
			match ===
			"(?i)([a-z_\\x{7f}-\\x{10ffff}\\\\][a-z0-9_\\x{7f}-\\x{10ffff}\\\\]*)(?=\\s*::)"
		) {
			scopeResolution.patterns[0].match =
				"([A-Za-z_\\x{7f}-\\x{10ffff}\\\\][A-Za-z0-9_\\x{7f}-\\x{10ffff}\\\\]*)(?=\\s*::)";
		} else {
	...
	if (functionCall) {
		...
		if (
			begin1 ===
			"(?i)(\\\\)?(?<![a-z0-9_\\x{7f}-\\x{10ffff}])([a-z_\\x{7f}-\\x{10ffff}][a-z0-9_\\x{7f}-\\x{10ffff}]*)\\s*(\\()"
		) {
			functionCall.patterns[1].begin =
