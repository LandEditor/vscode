...
import { type API as GitAPI, RefType, type Repository } from "./typings/git";
...
export async function getLink(
	...
	gitRepo.state.remotes.find((remote) => {
		if (remote.fetchUrl) {
			...
			if (
				foundRepo &&
				remote.name === gitRepo.state.HEAD?.upstream?.remote
			) {
				repo = foundRepo;
				return;
...
export function getVscodeDevHost(): string {
	return `https://${
		vscode.env.appName.toLowerCase().includes("insiders") ? "insiders." : ""
	}vscode.dev/github`;
}
