...
import { type FileSystemProvider, serveFileSystemRequests } from "./requests";
		...
		if (!config.get("linkedEditing") && !config.get("renameOnType")) {
			const activeEditorListener = window.onDidChangeActiveTextEditor(
				async (e) => {
					if (
						e &&
						languageParticipants.hasLanguage(e.document.languageId)
					) {
				...
				provideDocumentSemanticTokens(doc) {
					...
					return client
						...
						.then((data) => {
							return (
								data &&
								new SemanticTokens(new Uint32Array(data))
							);
						});
				},
