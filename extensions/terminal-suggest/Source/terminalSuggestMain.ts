			...
			async provideTerminalCompletions(
				...
				for (const spec of specs) {
					...
					if (terminalContext.commandLine.startsWith(specName)) {
						if (
							"options" in codeInsidersCompletionSpec &&
							codeInsidersCompletionSpec.options
						) {
							for (const option of codeInsidersCompletionSpec.options) {
								...
								if (
									...
									(prefix.length > specName.length &&
										prefix.trim() === specName)
								) {
									result.push(
								...
								if (option.args !== undefined) {
									const args = Array.isArray(option.args)
										? option.args
										: [option.args];
									...
									for (const arg of args) {
										...
										if (
											...
											precedingText.includes(expectedText)
										) {
