	...
	context.subscriptions.push(
		vscode.window.registerTerminalCompletionProvider({
			...
			> {
				...
				const specCompletions = await getCompletionItemsFromSpecs(
					availableSpecs,
					terminalContext,
					commands,
					prefix,
				...
				if (filesRequested || foldersRequested) {
					return new vscode.TerminalCompletionList(items, {
						filesRequested,
						foldersRequested,
						cwd: terminal.shellIntegration?.cwd,
...
function createCompletionItem(
	...
	label: string,
	description?: string,
	hasSpaceBeforeCursor?: boolean,
	kind?: vscode.TerminalCompletionItemKind,
): vscode.TerminalCompletionItem {
	return {
		label,
		detail: description ?? "",
		replacementIndex: hasSpaceBeforeCursor
			? cursorPosition
			: cursorPosition - 1,
...
export function getCompletionItemsFromSpecs(
	specs: Fig.Spec[],
			...
			for (const suggestion of arg.suggestions) {
				...
				for (const suggestionLabel of suggestionLabels) {
					...
					if (
						...
						suggestionLabel !== currentPrefix.trim()
					) {
						const hasSpaceBeforeCursor =
							terminalContext.commandLine[
								terminalContext.cursorPosition - 1
							] === " ";
