 ...
 */
export async function resolveCwdFromPrefix(
	prefix: string,
...
function createCompletionItem(
	...
	return {
		...
		detail: description ?? "",
		replacementIndex:
			commandLine.length - prefix.length >= 0
				? commandLine.length - prefix.length
		...
		try {
			...
			for (const [file, fileType] of files) {
				if (
					...
					(await isExecutable(path + pathSeparator + file))
				) {
...
function getPrefix(commandLine: string, cursorPosition: number): string {
	...
	if (
		...
		/\S/.test(commandLine[cursorPosition])
	) {
		return "";
	}
...
export async function getCompletionItemsFromSpecs(
	...
	for (const spec of specs) {
		...
		for (const specLabel of specLabels) {
			if (
				!availableCommands.includes(specLabel) ||
			...
			const precedingText = terminalContext.commandLine.slice(
				0,
				terminalContext.cursorPosition + 1,
			);
			if ("options" in spec && spec.options) {
				for (const option of spec.options) {
					...
					for (const optionLabel of optionLabels) {
						if (
							(!items.find((i) => i.label === optionLabel) &&
								optionLabel.startsWith(prefix)) ||
							(prefix.length > specLabel.length &&
								prefix.trim() === specLabel)
						) {
							items.push(
								createCompletionItem(
									terminalContext.commandLine,
						...
						const indexOfPrecedingText =
							terminalContext.commandLine.lastIndexOf(
								expectedText,
							);
						const currentPrefix = precedingText.slice(
							indexOfPrecedingText + expectedText.length,
						);
						const argsCompletions = getCompletionItemsFromArgs(
							option.args,
							currentPrefix,
							terminalContext,
						);
						...
						foldersRequested =
							foldersRequested ||
							argsCompletions.foldersRequested;
						filesRequested =
							filesRequested || argsCompletions.filesRequested;
						...
						if (
							...
							cwd =
								(await resolveCwdFromPrefix(
									prefix,
									shellIntegrationCwd,
								)) ?? shellIntegrationCwd;
						}
						specificSuggestionsProvided =
							argsCompletions.specificSuggestionsProvided;
						return {
							items: argCompletions,
							filesRequested,
							foldersRequested,
							cwd,
						};
			...
			if ("args" in spec && asArray(spec.args)) {
				...
				const indexOfPrecedingText =
					terminalContext.commandLine.lastIndexOf(expectedText);
				const currentPrefix = precedingText.slice(
					indexOfPrecedingText + expectedText.length,
				);
				const argsCompletions = getCompletionItemsFromArgs(
				...
				filesRequested =
					filesRequested || argsCompletions.filesRequested;
				foldersRequested =
					foldersRequested || argsCompletions.foldersRequested;
			}
		}
	}
	...
	if (!specificSuggestionsProvided && filesRequested === foldersRequested) {
		...
		for (const command of availableCommands) {
			if (
				(!terminalContext.commandLine.trim() ||
					(firstCommand && command.startsWith(firstCommand))) &&
				!items.find((item) => item.label === command)
			) {
				items.push(
					createCompletionItem(
						terminalContext.commandLine,
	...
	const shouldShowResourceCompletions =
		...
		(terminalContext.commandLine.trim().length === 0 ||
			...
			(items.length &&
				items.every((i) => [".", ".."].includes(i.label)))) &&
	...
	if (shellIntegrationCwd && (filesRequested || foldersRequested)) {
		cwd =
			(await resolveCwdFromPrefix(prefix, shellIntegrationCwd)) ??
			shellIntegrationCwd;
	}
	return { items, filesRequested, foldersRequested, cwd };
}

function getCompletionItemsFromArgs(
	...
	| {
			...
			filesRequested: boolean;
			foldersRequested: boolean;
			specificSuggestionsProvided: boolean;
	  }
	| undefined {
	if (!args) {
		return;
	...
	for (const arg of asArray(args)) {
		...
		if (arg.suggestions?.length) {
			...
			for (const suggestion of arg.suggestions) {
				...
				const twoWordsBefore = terminalContext.commandLine
					.slice(0, terminalContext.cursorPosition)
					.split(" ")
					.at(-2);
				const wordBefore = terminalContext.commandLine
					.slice(0, terminalContext.cursorPosition)
					.split(" ")
					.at(-1);
				for (const suggestionLabel of suggestionLabels) {
					...
					if (
						!arg.isVariadic &&
						twoWordsBefore === suggestionLabel &&
						wordBefore?.trim() === ""
					) {
						return {
							items: [],
							filesRequested,
							foldersRequested,
							specificSuggestionsProvided: false,
						};
					}
					if (
						suggestionLabel &&
						suggestionLabel.startsWith(currentPrefix.trim())
					) {
						const description =
