...
import { type RequestService, getRequestService } from "./requests";
...
export function startServer(
		...
		dataProvidersReady = fetchDataProviders(dataPaths, requestService).then(
			(customDataProviders) => {
				for (const lang in languageServices) {
					languageServices[lang].setDataProviders(
						true,
						customDataProviders,
					);
				}
	...
	connection.onDocumentLinks(async (documentLinkParams, token) => {
		return runSafeAsync(
			...
			async () => {
				const document = documents.get(
					documentLinkParams.textDocument.uri,
				);
				if (document) {
					await dataProvidersReady;
					const documentContext = getDocumentContext(
	...
	connection.onRenameRequest((renameParameters, token) => {
		return runSafeAsync(
			...
			async () => {
				const document = documents.get(
					renameParameters.textDocument.uri,
				);
				if (document) {
					await dataProvidersReady;
					const stylesheet = stylesheets.get(document);
	...
	connection.onFoldingRanges((params, token) => {
		return runSafeAsync(
			...
			async () => {
				...
				if (document) {
					await dataProvidersReady;
					return getLanguageService(document).getFoldingRanges(
						document,
