...
} from "@azure/ms-rest-azure-env";
import TelemetryReporter from "@vscode/extension-telemetry";
import * as vscode from "vscode";
import { AzureActiveDirectoryService, type IStoredSession } from "./AADHelper";
...
async function initMicrosoftSovereignCloudAuthProvider(
	...
	if (environment === "custom") {
		const customEnv = vscode.workspace
			.getConfiguration("microsoft-sovereign-cloud")
		...
		vscode.authentication.registerAuthenticationProvider(
			...
			"Microsoft",
			{
				...
				getSessions: (scopes: string[]) =>
					loginService.getSessions(scopes),
				createSession: async (scopes: string[]) => {
					try {
						/* __GDPR__
					"login" : {
