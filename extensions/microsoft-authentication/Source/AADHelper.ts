...
import type {
	BetterTokenStorage,
	IDidChangeInOtherWindowEvent,
} from "./betterSecretStorage";
import {
	IntervalTimer,
	SequencerByKey,
...
const defaultActiveDirectoryEndpointUrl =
	Environment.AzureCloud.activeDirectoryEndpointUrl;
...
export class AzureActiveDirectoryService {
	...
	public async initialize(): Promise<void> {
		...
		const refreshes = sessions.map(async (session) => {
			this._logger.trace(
				`[${session.scope}] '${session.id}' Read stored session`,
			);
	...
	public getSessions(
		...
		if (!scopes) {
			...
			const sessions = this._tokens.map((token) =>
				this.convertToSessionSync(token),
			);
			this._logger.info(
				`Got ${sessions.length} sessions for all scopes...`,
			);
		...
		return this._sequencer.queue(modifiedScopesStr, () =>
			this.doGetSessions(scopeData),
		);
	}
	...
	private async doGetSessions(
		...
		const matchingTokens = this._tokens.filter(
			(token) => token.scope === scopeData.scopeStr,
		);
		...
		if (!matchingTokens.length) {
			...
			const token =
				scopeData.clientId === DEFAULT_CLIENT_ID
					? this._tokens.find(
							(t) =>
								t.refreshToken &&
								!t.scope.includes("VSCODE_CLIENT_ID"),
						)
					: this._tokens.find(
							(t) =>
								t.refreshToken &&
								t.scope.includes(
									`VSCODE_CLIENT_ID:${scopeData.clientId}`,
								),
						);
			...
			if (token) {
				...
				try {
					const itoken = await this.doRefreshToken(
						token.refreshToken,
						scopeData,
					);
					...
					this._logger.error(
						`[${
							scopeData.scopeStr
						}] Attempted to get a new session using the existing session with scopes '${
							token.scope
						}' but it failed due to: ${err.message ?? err}`,
					);
				}
			}
		}
		...
		const results = await Promise.allSettled(
			matchingTokens.map((token) =>
				this.convertToSession(token, scopeData),
			),
		);
		return results
			...
			.map(
				(result) =>
					(
						result as PromiseFulfilledResult<vscode.AuthenticationSession>
					).value,
			);
	}
	...
	public createSession(
		...
		return this._sequencer.queue(scopeData.scopeStr, () =>
			this.doCreateSession(scopeData),
		);
	}
	...
	private async doCreateSession(
		...
		if (
			...
			this._env.activeDirectoryEndpointUrl !==
				defaultActiveDirectoryEndpointUrl
		) {
			throw new Error(
		...
		return await vscode.window.withProgress(
			{
				...
				title: vscode.l10n.t("Signing in to your account..."),
				cancellable: true,
			},
			async (_progress, token) => {
				if (runsRemote || runsServerless) {
					return await this.createSessionWithoutLocalServer(
						scopeData,
						token,
					);
				}
				...
				try {
					...
					this._logger.error(
						`[${scopeData.scopeStr}] Error creating session: ${e}`,
					);
					...
					if (
						...
						return this.createSessionWithoutLocalServer(
							scopeData,
							token,
						);
					}
	...
	private async createSessionWithLocalServer(
		...
		this._logger.trace(
			`[${scopeData.scopeStr}] Starting login flow with local server`,
		);
		...
		const loginUrl = new URL(
			`${scopeData.tenant}/oauth2/v2.0/authorize?${qs}`,
			this._env.activeDirectoryEndpointUrl,
		).toString();
		...
		try {
			vscode.env.openExternal(
				vscode.Uri.parse(
					`http://127.0.0.1:${
						server.port
					}/signin?nonce=${encodeURIComponent(server.nonce)}`,
				),
			);
			const { code } = await raceCancellationAndTimeoutError(
	...
	private async createSessionWithoutLocalServer(
		...
		this._logger.trace(
			`[${scopeData.scopeStr}] Starting login flow without local server`,
		);
		let callbackUri = await vscode.env.asExternalUri(
			vscode.Uri.parse(
				`${vscode.env.uriScheme}://vscode.microsoft-authentication`,
			),
		);
		...
		const signInUrl = new URL(
			`${scopeData.tenant}/oauth2/v2.0/authorize`,
			this._env.activeDirectoryEndpointUrl,
		);
		...
		let existingPromise = this._codeExchangePromises.get(
			scopeData.scopeStr,
		);
	...
	public async removeSessionById(
		...
		return this._sequencer.queue(token.scope, () =>
	...
	private convertToTokenSync(
		...
		try {
			if (json.id_token) {
				...
				this._logger.warn(
					`[${scopeData.scopeStr}] '${
						existingId ?? "new"
					}' Attempting to parse access_token instead since no id_token was included in the response.`,
				);
				claims = JSON.parse(
		...
		return {
			...
			account: {
				...
				id,
				type:
					claims.tid === MSA_TID || claims.tid === MSA_PASSTHRU_TID
	...
	private async doRefreshToken(
		...
		this._logger.trace(
			`[${scopeData.scopeStr}] '${sessionId ?? "new"}' Refreshing token`,
		);
		...
		try {
			...
			this._logger.error(
				`[${scopeData.scopeStr}] '${
					sessionId ?? "new"
				}' Refreshing token failed: ${e.message}`,
			);
			throw e;
		}
	}
	...
	private async handleCodeResponse(
		...
		return new Promise(
			(
				...
				uriEventListener = this._uriHandler.event(
	...
	private async exchangeCodeForSession(
		...
		this._logger.trace(
			`[${scopeData.scopeStr}] Exchanging login code for session`,
		);
		...
		try {
			...
			this._logger.error(
				`[${scopeData.scopeStr}] Error exchanging code for token: ${e}`,
			);
			throw e;
		}
	...
	private async fetchTokenResponse(
		...
		while (attempts <= 3) {
			...
			if (!result || result.status > 499) {
				if (attempts > 3) {
					this._logger.error(
						`[${scopeData.scopeStr}] Fetching token failed: ${
							result ? await result.text() : errorMessage
						}`,
					);
					break;
				}
				...
				await new Promise((resolve) =>
					setTimeout(resolve, 5 * attempts * attempts * 1000),
				);
				continue;
			} else if (!result.ok) {
	...
	private setToken(token: IToken, scopeData: IScopeData): void {
		this._logger.trace(
			`[${scopeData.scopeStr}] '${token.sessionId}' Setting token`,
		);

		const existingTokenIndex = this._tokens.findIndex(
			(t) => t.sessionId === token.sessionId,
		);
	...
	private async storeToken(
		...
		if (!vscode.window.state.focused) {
			if (this._pendingTokensToStore.has(token.sessionId)) {
				this._logger.trace(
					`[${scopeData.scopeStr}] '${token.sessionId}' Window is not focused, replacing token to be stored`,
				);
			} else {
				this._logger.trace(
	...
	private async checkForUpdates(
		...
		for (const key of e.added) {
			...
			if (!session) {
				this._logger.error(
					"session not found that was apparently just added",
				);
				continue;
			}
		...
		for (const { value } of e.removed) {
			...
			if (!this.sessionMatchesEndpoint(value)) {
				// If the session wasn't made for this login endpoint, ignore this update
				this._logger.trace(
					`[${value.scope}] '${value.id}' Session doesn't match endpoint. Skipping...`,
				);
				continue;
			}
		...
		for (const sessionId of e.updated) {
			if (this._pendingTokensToStore.delete(sessionId)) {
				this._logger.trace(
					`'${sessionId}' Cancelled pending token storage because token was updated in another window`,
				);
			}
		}
	}
