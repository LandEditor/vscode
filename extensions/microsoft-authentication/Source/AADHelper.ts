...
export class AzureActiveDirectoryService {
	...
	private async doGetSessions(
		...
		this._logger.info(
			`[${scopeData.scopeStr}] Getting sessions` + account
				? ` for ${account?.label}`
				: "",
		);
		...
		const matchingTokens = this._tokens
			...
			.filter(
				(token) =>
					!account?.label || token.account.label === account.label,
			);
		...
		if (!matchingTokens.length) {
			...
			if (token) {
				...
				try {
					...
					this._logger.error(
						`[${scopeData.scopeStr}] Attempted to get a new session using the existing session with scopes '${token.scope}' but it failed due to: ${err.message ?? err}`,
					);
				}
			}
		}
	...
	private async doCreateSession(
		...
		this._logger.info(
			`[${scopeData.scopeStr}] Creating session` + account
				? ` for ${account?.label}`
				: "",
		);
	...
	private async createSessionWithLocalServer(
		...
		try {
			vscode.env.openExternal(
				vscode.Uri.parse(
					`http://127.0.0.1:${server.port}/signin?nonce=${encodeURIComponent(server.nonce)}`,
				),
			);
	...
	private async convertToSession(
		...
		if (
			...
			this._logger.trace(
				`[${scopeData.scopeStr}] '${token.sessionId}' Token available from cache${token.expiresAt ? `, expires in ${token.expiresAt - Date.now()} milliseconds` : ""}.`,
			);
			return {
				id: token.sessionId,
				accessToken: token.accessToken,
	...
	private refreshToken(
		...
		this._logger.trace(
			`[${scopeData.scopeStr}] '${sessionId ?? "new"}' Queued refreshing token`,
		);
		return this._sequencer.queue(scopeData.scopeStr, () =>
	...
	private async fetchTokenResponse(
		...
		while (attempts <= 3) {
			...
			if (!result || result.status > 499) {
				if (attempts > 3) {
					this._logger.error(
						`[${scopeData.scopeStr}] Fetching token failed: ${result ? await result.text() : errorMessage}`,
					);
					break;
				}
				// Exponential backoff
