	...
	implements ICachedPublicClientApplicationManager
{
	...
	constructor(
		...
		this._pcasSecretStorage = new PublicClientApplicationsSecretStorage(
			_secretStorage,
			_cloudName,
		);
	...
	async initialize() {
		...
		for (const key of keys) {
			try {
				const { clientId, authority } = JSON.parse(
					key,
				) as IPublicClientApplicationInfo;
				...
				this._logger.error(
					"[initialize] Error intitializing PCA:",
					key,
				);
			}
		}
	...
	async getOrCreate(
		...
		if (pca) {
			this._logger.debug(
				`[getOrCreate] [${clientId}] [${authority}] PublicClientApplicationManager cache hit`,
			);
		} else {
			this._logger.debug(
		...
		if (refreshTokensToMigrate?.length) {
			this._logger.debug(
				`[getOrCreate] [${clientId}] [${authority}] Migrating refresh tokens to PCA...`,
			);
			...
			for (const refreshToken of refreshTokensToMigrate) {
				try {
					...
					if (result?.account) {
						this._logger.debug(
							`[getOrCreate] [${clientId}] [${authority}] Refresh token migrated to PCA.`,
						);
					}
				} catch (e) {
					this._logger.error(
						`[getOrCreate] [${clientId}] [${authority}] Error migrating refresh token:`,
						e,
					);
				}
			}
	...
	private async _doCreatePublicClientApplication(
		...
		const pca = new CachedPublicClientApplication(
			clientId,
			authority,
			this._cloudName,
			this._globalMemento,
		...
		const disposable = Disposable.from(
			pca,
			pca.onDidAccountsChange((e) =>
				this._onDidAccountsChangeEmitter.fire(e),
			),
	...
	private async _handleSecretStorageChange() {
		...
		for (const newPca of pcaKeysFromStorage) {
			try {
				...
				this._logger.error(
					`Failed to parse new PublicClientApplication: ${newPca}`,
				);
