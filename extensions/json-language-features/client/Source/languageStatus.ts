...
import type { JSONLanguageStatus, JSONSchemaSettings } from "./jsonClient";
	...
	if (workspaceFolder && Array.isArray(folderSettingSchemas)) {
		for (const setting of folderSettingSchemas) {
			...
			if (typeof uri === "string") {
				let fullUri = uri;
				if (uri[0] === "." && uri[1] === "/") {
					fullUri = Uri.joinPath(workspaceFolder.uri, uri).toString(
						false,
					);
				}
...
function showSchemaList(input: ShowSchemasInput) {
	...
	for (const schemaUri of input.schemas) {
		const extensionEntry =
			extensionSchemaAssocations.findExtension(schemaUri);
		if (extensionEntry) {
