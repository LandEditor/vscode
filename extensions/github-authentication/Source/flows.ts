...
import { commands, env, l10n, ProgressLocation, Uri, window } from "vscode";
	...
	new (class UrlHandlerFlow implements IFlow {
		...
		async trigger({
			...
			return await window.withProgress<string>(
				...
				async (_, token) => {
					const promise = uriHandler.waitForCode(
						logger,
						scopes,
						nonce,
						token,
					...
					const searchParams = new URLSearchParams([
						...
						[
							"state",
							encodeURIComponent(callbackUri.toString(true)),
						],
					]);
					if (existingLogin) {
			...
			return await window.withProgress<string>(
				...
				async (_, token) => {
					...
					for (let i = 0; i < attempts; i++) {
						...
						try {
							accessTokenResult = await fetching(
								refreshTokenUri.toString(true),
								{
									method: "POST",
									headers: {
										Accept: "application/json",
