...
import type { ExperimentationTelemetry } from "./common/experimentationService";
import type { Log } from "./common/logger";
import { Config } from "./config";
import { ExtensionHost, GitHubTarget, getFlows } from "./flows";
import { AuthProviderType, type UriEventHandler } from "./github";
	...
	private async getRedirectEndpoint(): Promise<string> {
		...
		if (this._type === AuthProviderType.github) {
			...
			if (
				...
				new URL(proxyEndpoints.github).hostname ===
					"insiders.vscode.dev"
			) {
