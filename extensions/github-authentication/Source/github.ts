...
import { GitHubServer, type IGitHubServer } from "./githubServer";
	...
	async getSessions(
		...
		this._logger.info(
			`Getting sessions for ${
				sortedScopes.length ? sortedScopes.join(",") : "all scopes"
			}...`,
		);
		...
		try {
			...
			*/
			...
			const accounts = new Set(
				sessions.map((session) => session.account.label),
			);
			const existingLogin =
				accounts.size <= 1
					? sessions[0]?.account.label
					: await vscode.window.showQuickPick([...accounts], {
							placeHolder:
								"Choose an account that you would like to log in to",
						});
