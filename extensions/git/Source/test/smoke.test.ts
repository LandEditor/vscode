...
suite("git smoke test", () => {
	...
	suiteSetup(async () => {
		...
		assert.strictEqual(
			fs.realpathSync(git.repositories[0].rootUri.fsPath),
			cwd,
		);
	...
	test("reflects working tree changes", async () => {
		...
		repository.state.workingTreeChanges.some(
			(r) =>
				r.uri.path === appjs.uri.path && r.status === Status.MODIFIED,
		);
		...
		repository.state.workingTreeChanges.some(
			(r) =>
				r.uri.path === newfile.uri.path &&
				r.status === Status.UNTRACKED,
		);
	});

	test("opens diff editor", async () => {
	...
	test("stages correctly", async () => {
		...
		assert.strictEqual(repository.state.workingTreeChanges.length, 1);
		repository.state.workingTreeChanges.some(
			(r) => r.uri.path === newfile.path && r.status === Status.UNTRACKED,
		);
		assert.strictEqual(repository.state.indexChanges.length, 1);
		repository.state.indexChanges.some(
			(r) =>
				r.uri.path === appjs.path && r.status === Status.INDEX_MODIFIED,
		);

		await commands.executeCommand("git.unstage", appjs);
		assert.strictEqual(repository.state.workingTreeChanges.length, 2);
		repository.state.workingTreeChanges.some(
			(r) => r.uri.path === appjs.path && r.status === Status.MODIFIED,
		);
		repository.state.workingTreeChanges.some(
			(r) => r.uri.path === newfile.path && r.status === Status.UNTRACKED,
		);
	...
	test("stages, commits changes and verifies outgoing change", async () => {
		const appjs = uri("app.js");
		const newfile = uri("newfile.txt");
	...
	test("rename/delete conflict", async () => {
