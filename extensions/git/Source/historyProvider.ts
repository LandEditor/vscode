	...
	switch (ref.type) {
		case RefType.RemoteHead:
			return {
				...
				description: ref.commit
					? l10n.t("Remote branch at {0}", ref.commit.substring(0, 8))
					: undefined,
				revision: ref.commit,
		...
		case RefType.Tag:
			return {
				...
				name: ref.name ?? "",
				description: ref.commit
					? l10n.t("Tag at {0}", ref.commit.substring(0, 8))
					: undefined,
				revision: ref.commit,
		...
		IDisposable
{
	...
	private async onDidRunWriteOperation(
		...
		const delta = deltaHistoryItemRefs(
	...
	async provideHistoryItemChanges(
		...
		const changes = await this.repository.diffTrees(
			historyItemParentId,
			historyItemId,
		);
