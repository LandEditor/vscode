...
import {
	...
	type SourceControlHistoryItemChange,
	type SourceControlHistoryItemGroup,
	type SourceControlHistoryOptions,
	type SourceControlHistoryProvider,
	ThemeIcon,
	type Uri,
	window,
} from "vscode";
import { type Branch, RefType, type UpstreamRef } from "./api/git";
import { emojify, ensureEmojis } from "./emoji";
import { Operation } from "./operation";
import { type Repository, Resource } from "./repository";
import { toGitUri } from "./uri";
import { type IDisposable, dispose, filterEvent } from "./util";
	...
	implements SourceControlHistoryProvider, FileDecorationProvider, IDisposable
{
	...
	private async onDidRunGitStatus(force = false): Promise<void> {
		...
		if (
			...
			this._HEAD?.upstream?.name ===
				this.repository.HEAD?.upstream?.name &&
			this._HEAD?.upstream?.remote ===
				this.repository.HEAD?.upstream?.remote &&
		...
		this.logger.trace(
			`GitHistoryProvider:onDidRunGitStatus - currentHistoryItemGroup (${force}): ${JSON.stringify(
				this.currentHistoryItemGroup,
			)}`,
		);
	}
	...
	async provideHistoryItems(
		...
		const refId =
			(await this.repository.revParse(historyItemGroupId)) ?? "";
	...
	async resolveHistoryItemGroupCommonAncestor(
		...
		if (!historyItemId2) {
			const upstreamRef =
				await this.resolveHistoryItemGroupBase(historyItemId1);
			if (!upstreamRef) {
				this.logger.info(
		...
		try {
			...
			this.logger.trace(
				`GitHistoryProvider:resolveHistoryItemGroupCommonAncestor - Resolved common ancestor for '${historyItemId1}' and '${historyItemId2}': ${JSON.stringify(
					{
						id: ancestor,
	...
	private async resolveHistoryItemGroupBase(
		...
		try {
			...
			const remoteBranch =
				await this.repository.getBranchBase(historyItemId);
			if (
				!remoteBranch?.remote ||
				!remoteBranch?.name ||
				!remoteBranch?.commit ||
