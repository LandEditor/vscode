...
import { fromGitUri, isGitUri, toGitUri, toMergeUris } from "./uri";
...
class CheckoutItem implements QuickPickItem {
	...
	get label(): string {
		return `${
			this.repository.isBranchProtected(this.ref)
				? "$(lock)"
				: "$(git-branch)"
		} ${this.ref.name || this.shortCommit}`;
	}
	...
	constructor(
		...
		protected ref: Ref,
		protected _buttons?: QuickInputButton[],
	) {}
...
class CheckoutRemoteHeadItem extends CheckoutItem {
	...
	override async run(opts?: { detached?: boolean }): Promise<void> {
		...
		const branches = await this.repository.findTrackingBranches(
			this.ref.name,
		);
...
class MergeItem implements QuickPickItem {
	...
	get description(): string {
		return this.ref.type === RefType.RemoteHead
			? l10n.t("Remote branch at {0}", this.shortCommit)
			: this.shortCommit;
	}
...
class CreateBranchItem implements QuickPickItem {
	get label(): string {
		return `$(plus) ${l10n.t("Create new branch...")}`;
	}
	get description(): string {
		return "";
	}
...
class CreateBranchFromItem implements QuickPickItem {
	get label(): string {
		return `$(plus) ${l10n.t("Create new branch from...")}`;
	}
	get description(): string {
		return "";
	}
...
class CheckoutDetachedItem implements QuickPickItem {
	get label(): string {
		return `$(debug-disconnect) ${l10n.t("Checkout detached...")}`;
	}
	get description(): string {
		return "";
	}
...
async function createCheckoutItems(
	...
	const processors = checkoutTypes
		.map((type) => getCheckoutProcessor(repository, type))
	...
	return processors.reduce<CheckoutItem[]>(
		(r, p) =>
			r.concat(
				...p.items.map((item) => {
					if (item.refRemote) {
						const matchingRemote = repository.remotes.find(
							(remote) => remote.name === item.refRemote,
						);
						const remoteUrl =
							matchingRemote?.pushUrl ?? matchingRemote?.fetchUrl;
						if (remoteUrl) {
...
class CheckoutProcessor {
	...
	get items(): CheckoutItem[] {
		return this.refs.map((r) => new this.ctor(this.repository, r));
	}
	constructor(
		private repository: Repository,
		private type: RefType,
		private ctor: { new (repository: Repository, ref: Ref): CheckoutItem },
	) {}
...
function getCheckoutProcessor(
	repository: Repository,
	type: string,
): CheckoutProcessor | undefined {
	switch (type) {
		case "local":
...
export class CommandCenter {
	...
	constructor(
		...
		this.disposables = Commands.map(
			({ commandId, key, method, options }) => {
				...
				if (options.diff) {
					return commands.registerDiffInformationCommand(
						commandId,
						command,
					);
				} else {
	...
	async openMergeEditor(uri: unknown) {
		...
		try {
			...
			incoming.description = `$(git-commit) ${rebaseOrMergeHead.hash.substring(
				0,
				7,
			)}`;
		...
		function findFirstConflictMarker(
			...
			for (let lineIdx = 0; lineIdx < doc.lineCount; lineIdx++) {
				...
				} else if (lineStr.startsWith(conflictMarkerStart)) {
		...
		if (!url || typeof url !== "string") {
			url = await pickRemoteSource({
				providerLabel: (provider) =>
					l10n.t("Clone from {0}", provider.name),
			...
			*/
			...
			if (
				...
				*/
				...
				/Cancelled/i.test(err && (err.message || err.stderr || ""))
			) {
				return;
			} else {
				/* __GDPR__
	...
	async continueInLocalClone(): Promise<Uri | void> {
		...
		const items = this.model.repositories.reduce<
			(QuickPickItem & { repository: Repository })[]
		>((items, repository) => {
		...
		if (uri !== undefined) {
			let target = `${
				env.uriScheme
			}://vscode.git/clone?url=${encodeURIComponent(uri)}`;
	...
	async rename(
		...
		let to = await window.showInputBox({
			value: from,
			valueSelection: [
				from.length - path.basename(from).length,
				from.length,
			],
		});
	...
	async stage(
		...
		if (
			...
			this.logger.debug(
				`git.stage.getSCMResource ${
					resource ? resource.resourceUri.toString() : null
				} `,
			);
		...
		const selectedChanges = changes
			.map((diff) =>
				selectedLines.reduce<LineChange | null>(
					(result, range) =>
						result ||
						intersectDiffWithRange(modifiedDocument, diff, range),
					null,
				),
			)
	...
	private async _revertChanges(
		...
		edit.replace(
			...
			new Range(
				new Position(0, 0),
				modifiedDocument.lineAt(modifiedDocument.lineCount - 1).range
					.end,
			),
			result,
		);
	...
	async clean(
		...
		const untrackedCount = scmResources.reduce(
		...
		if (scmResources.length === 1) {
			...
			} else if (scmResources[0].type === Status.DELETED) {
			...
			if (
				scmResources.every(
					(resource) => resource.type === Status.DELETED,
				)
			) {
	...
	async cleanAll(repository: Repository): Promise<void> {
		...
		if (untrackedResources.length === 0) {
			...
			// resources.length > 1 && untrackedResources.length > 0 && trackedResources.length > 0
			const untrackedMessage =
				untrackedResources.length === 1
					? l10n.t(
							"The following untracked file will be DELETED FROM DISK if discarded: {0}.",
							path.basename(
								untrackedResources[0].resourceUri.fsPath,
							),
					  )
					: l10n.t(
	...
	private async _cleanTrackedChanges(
		repository: Repository,
		resources: Resource[],
	): Promise<void> {
		const message =
	...
	private async _cleanUntrackedChange(
		repository: Repository,
		resource: Resource,
	): Promise<void> {
		const message = l10n.t(
	...
	private async _cleanUntrackedChanges(
		repository: Repository,
		resources: Resource[],
	): Promise<void> {
		const message = l10n.t(
	...
	private async commitWithAnyInput(
		...
		const getCommitMessage = async () => {
			...
			if (!(_message || config.get<boolean>("useEditorAsCommitInput"))) {
				...
				if (opts?.amend && repository.HEAD && repository.HEAD.commit) {
					return undefined;
				}
	...
	private async promptForBranchName(
		...
		if (!rawBranchName) {
			...
			if (!initialValue) {
				...
				const branchName = branchRandomNameEnabled
					? await this.generateRandomBranchName(
							repository,
							branchWhitespaceChar,
					  )
					: "";
			...
			const initialValueSelection: [number, number] | undefined =
				initialValue.startsWith(branchPrefix)
					? [branchPrefix.length, initialValue.length]
					: undefined;
	...
	private async _branch(
		...
		if (from) {
			const getRefPicks = async () => {
				return [
					new HEADItem(repository),
					...(await createCheckoutItems(repository)),
				];
			};
	...
	async deleteBranch(
		...
		if (typeof name === "string") {
			...
			const getBranchPicks = async () => {
				...
				const currentHead = repository.HEAD?.name;
			...
			const choice = await window.showQuickPick<BranchDeleteItem>(
				getBranchPicks(),
				{ placeHolder },
			);

			if (!choice?.branchName) {
				return;
			}
	...
	async renameBranch(repository: Repository): Promise<void> {
		const currentBranchName = repository.HEAD?.name;
		const branchName = await this.promptForBranchName(
			repository,
			undefined,
			currentBranchName,
	...
	async merge(repository: Repository): Promise<void> {
		...
		const getBranchPicks = async (): Promise<MergeItem[]> => {
			...
			const remoteHeads = (
				includeRemotes
					? refs.filter((ref) => ref.type === RefType.RemoteHead)
					: []
			)
				.filter((ref) => ref.name || ref.commit)
				.map((ref) => new MergeItem(repository, ref as Branch));
		...
		const choice = await window.showQuickPick<MergeItem>(getBranchPicks(), {
			placeHolder,
		});
	...
	async rebase(repository: Repository): Promise<void> {
		...
		const getBranchPicks = async () => {
			...
			const picks = [...heads, ...remoteHeads].map(
				(ref) => new RebaseItem(ref),
			);
		...
		const choice = await window.showQuickPick<RebaseItem>(
			getBranchPicks(),
			{ placeHolder },
		);
	...
	async deleteTag(repository: Repository): Promise<void> {
		const tagPicks = async (): Promise<TagItem[] | QuickPickItem[]> => {
			...
			return remoteTags.length === 0
				...
				: remoteTags.map((ref) => new TagItem(ref));
		};

		const placeHolder = l10n.t("Select a tag to delete");
		const choice = await window.showQuickPick<TagItem | QuickPickItem>(
			tagPicks(),
			{ placeHolder },
		);
	...
	async deleteRemoteTag(repository: Repository): Promise<void> {
		...
		if (
			remoteTagPick &&
			remoteTagPick instanceof TagItem &&
			remoteTagPick.ref.name
		) {
			await repository.deleteRemoteTag(
				remoteName,
				remoteTagPick.ref.name,
			);
		}
		...
		if (repository.HEAD?.upstream?.remote) {
			...
			if (defaultRemoteIndex !== -1) {
				remoteItems.splice(
					0,
					0,
					...remoteItems.splice(defaultRemoteIndex, 1),
				);
			}
		}
	...
	async pullFrom(repository: Repository): Promise<void> {
		...
		const getBranchPicks = async (): Promise<QuickPickItem[]> => {
			...
			const remoteRefsFiltered = remoteRefs.filter(
				(r) => r.remote === remoteName,
			);
			return remoteRefsFiltered.map((r) => ({ label: r.name! }));
		};

		const branchPlaceHolder = l10n.t("Pick a branch to pull from");
		const branchPick = await window.showQuickPick(getBranchPicks(), {
		...
		await repository.pullFrom(
			false,
			remoteName,
			branchPick.label.slice(remoteCharCnt + 1),
		);
	}
	...
	async addRemote(repository: Repository): Promise<string | undefined> {
		const url = await pickRemoteSource({
			providerLabel: (provider) =>
				l10n.t("Add remote from {0}", provider.name),
	...
	private async _sync(
		...
		const isReadonly = remote?.isReadOnly;
	...
	private async _stash(
		...
		} else if (noUnstagedChanges && noStagedChanges) {
			window.showInformationMessage(
		...
		try {
			...
			if (
				/You do not have the initial commit yet/.test(err.stderr || "")
			) {
				window.showInformationMessage(
					l10n.t(
	...
	private async _resolveTimelineOpenCommitCommand(
		...
		for (const commitFile of commitFiles) {
			const commitFileUri = Uri.file(
				path.join(repository.root, commitFile),
			);
			args.push([
				commitFileUri,
				toGitUri(commitFileUri, item.previousRef),
				toGitUri(commitFileUri, item.ref),
			]);
		}
	...
	async timelineCompareWithSelected(
		...
		if (
			!(GitTimelineItem.is(item) && uri && this._selectedForCompare) ||
	...
	private createCommand(
		...
		const result = (...args: any[]) => {
			...
			*/
			this.telemetryReporter.sendTelemetryEvent("git.command", {
				command: id,
			});
	...
	private getSCMResource(uri?: Uri): Resource | undefined {
		...
		this.logger.debug(`git.getSCMResource.uri ${uri?.toString()}`);
		...
		const groups = resources.reduce(
			(result, resource) => {
				...
				if (pathEquals(resource.fsPath, repository.root)) {
					repository =
						this.model.getRepositoryForSubmodule(resource) ||
						repository;
				}

				const tuple = result.filter(
