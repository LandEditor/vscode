			...
			for (const item of processor.items) {
				...
				if (item.refRemote) {
					...
					if (
						...
						buttons.push(
							...(this.buttons.get(matchingRemote.fetchUrl) ??
								[]),
						);
					}
					if (buttons.length) {
...
export class CommandCenter {
	...
	async stageSelectedChanges(changes: LineChange[]): Promise<void> {
		...
		this.logger.debug(
			"[CommandCenter][stageSelectedChanges] changes:",
			changes,
		);
		this.logger.debug(
			"[CommandCenter][stageSelectedChanges] diffInformation.changes:",
			textEditor?.diffInformation?.changes,
		);
		this.logger.debug(
			"[CommandCenter][stageSelectedChanges] diffInformation.isStale:",
			textEditor?.diffInformation?.isStale,
		);
		...
		const selectedChanges = changes
			.map((change) =>
				selectedLines.reduce<LineChange | null>(
					(result, range) =>
						result ||
						intersectDiffWithRange(modifiedDocument, change, range),
					null,
				),
			)
	...
	async revertSelectedRanges(changes: LineChange[]): Promise<void> {
		...
		this.logger.debug(
			"[CommandCenter][revertSelectedRanges] changes:",
			changes,
		);
		this.logger.debug(
			"[CommandCenter][revertSelectedRanges] diffInformation.changes:",
			textEditor?.diffInformation?.changes,
		);
		this.logger.debug(
			"[CommandCenter][revertSelectedRanges] diffInformation.isStale:",
			textEditor?.diffInformation?.isStale,
		);
	...
	async unstageSelectedRanges(changes: LineChange[]): Promise<void> {
		...
		this.logger.debug(
			"[CommandCenter][unstageSelectedRanges] changes:",
			changes,
		);
		this.logger.debug(
			"[CommandCenter][unstageSelectedRanges] diffInformation.changes:",
			textEditor?.diffInformation?.changes,
		);
		this.logger.debug(
			"[CommandCenter][unstageSelectedRanges] diffInformation.isStale:",
			textEditor?.diffInformation?.isStale,
		);
	...
	async checkoutRefDetached(
		repository: Repository,
		historyItem?: SourceControlHistoryItemRef,
	): Promise<boolean> {
		if (!historyItem) {
			return false;
		}
	...
	async viewCommit(
		repository: Repository,
		historyItem1: SourceControlHistoryItem,
		historyItem2?: SourceControlHistoryItem,
	): Promise<void> {
		...
		if (historyItem2) {
			const mergeBase = await repository.getMergeBase(
				historyItem1.id,
				historyItem2.id,
			);
		...
		const multiDiffSourceUri = toGitUri(
			Uri.file(repository.root),
			`${historyItemParentId}..${historyItem1.id}`,
			{ scheme: "git-commit" },
		);
	...
	private getSCMResource(uri?: Uri): Resource | undefined {
		...
		for (const r of this.model.repositories.map((r) => r.root)) {
			this.logger.debug(
				`[CommandCenter][getSCMResource] repo root: ${r}`,
			);
		}
