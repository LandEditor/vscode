...
class RefProcessor {
	...
	constructor(
		...
		protected readonly ctor: { new (ref: Ref): QuickPickItem } = RefItem,
	) {}
...
export class CommandCenter {
	...
	async stageFile(uri: Uri): Promise<void> {
		...
		const resources = [
			...
			.filter(
				(r) =>
					r.multiFileDiffEditorModifiedUri?.toString() ===
					uri.toString(),
			)
			.map((r) => r.resourceUri);
			...
			disposables.push(
				quickPick.onDidTriggerItemButton((e) => {
					...
					if (button.actual && item.refName) {
						button.actual.run(
							item.refRemote
								? item.refName.substring(
										item.refRemote.length + 1,
								  )
								: item.refName,
						);
					}
	...
	async deleteTag(repository: Repository): Promise<void> {
		const tagPicks = async (): Promise<
			TagDeleteItem[] | QuickPickItem[]
		> => {
	...
	async deleteRemoteTag(repository: Repository): Promise<void> {
		...
		> => {
			...
			return remoteTags.length === 0
				...
				: remoteTags.map((ref) => new RemoteTagDeleteItem(ref));
		};
	...
	async stashView(repository: Repository): Promise<void> {
		...
		for (const file of stashFiles) {
			...
			resources.push({
				originalUri: fileUri,
				modifiedUri: toGitUri(fileUri, `stash@{${stash.index}}`),
			});
		}
	...
	private async _resolveTimelineOpenCommitCommand(
		...
		for (const commitFile of commitFiles) {
			...
			resources.push({
				originalUri: toGitUri(commitFileUri, item.previousRef),
				modifiedUri: toGitUri(commitFileUri, item.ref),
			});
		}
	...
	async _viewChanges(
		...
		const historyItemParentId =
			historyItem.parentIds.length > 0
				...
				: undefined;
		const files = await historyProvider.provideHistoryItemChanges(
			historyItem.id,
			historyItemParentId,
		);
