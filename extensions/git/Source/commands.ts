...
import {
	...
	RefType,
	type Remote,
	type RemoteSourcePublisher,
	Status,
} from "./api/git";
import type { RemoteSourceAction } from "./api/git-base";
import type { Git, Stash } from "./git";
import type { Model } from "./model";
import { getRemoteSourceActions, pickRemoteSource } from "./remoteSource";
import {
	type GitResourceGroup,
	type Repository,
	Resource,
	ResourceGroupType,
} from "./repository";
...
export class CommandCenter {
	...
	private async getStashFromUri(
		...
		const stash = stashes.find(
			(stash) => stash.index === Number.parseInt(index),
		);
		if (!stash) {
			return undefined;
		}
	...
	async _viewChanges(
		...
		const resources = changes.map((c) =>
			toMultiFileDiffEditorUris(c, historyItemParentId, historyItem.id),
		);
