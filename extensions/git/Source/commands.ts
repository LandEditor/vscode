...
class CheckoutItem extends RefItem {
    async run(repository: Repository, opts?: {
        detached?: boolean;
    }): Promise<void> {
        if (!this.ref.name) {
            return;
...
class RefProcessor {
    ...
    constructor(protected readonly type: RefType, protected readonly ctor: {
        new (ref: Ref): QuickPickItem;
    } = RefItem) { }
...
class CheckoutItemsProcessor extends RefItemsProcessor {
    ...
    constructor(processors: RefProcessor[], private readonly repository: Repository, private readonly buttons: Map<string, RemoteSourceActionButton[]>, private readonly detached = false) {
        super(processors);
        // Default button(s)
...
export class CommandCenter {
    ...
    constructor(private git: Git, private model: Model, private globalState: Memento, private logger: LogOutputChannel, private telemetryReporter: TelemetryReporter) {
    ...
    async openChanges(repository: Repository): Promise<void> {
        for (const resource of [...repository.workingTreeGroup.resourceStates, ...repository.untrackedGroup.resourceStates]) {
            if (resource.type === Status.DELETED || resource.type === Status.DELETED_BY_THEM ||
                resource.type === Status.DELETED_BY_US || resource.type === Status.BOTH_DELETED) {
                continue;
            }
            void commands.executeCommand('vscode.open', resource.resourceUri, { background: true, preview: false, });
        }
    }
    ...
    async openMergeEditor(uri: unknown) {
        ...
        function findFirstConflictMarker(doc: TextDocument): {
    ...
    async cloneRepository(url?: string, parentPath?: string, options: {
        recursive?: boolean;
        ref?: string;
    } = {}): Promise<void> {
        ...
        try {
            ...
            const repositoryPath = await window.withProgress(opts, (progress, token) => this.git.clone(url!, { parentPath: parentPath!, progress, recursive: options.recursive, ref: options.ref }, token));
            const config = workspace.getConfiguration('git');
    ...
    async continueInLocalClone(): Promise<Uri | void> {
        ...
        const items = this.model.repositories.reduce<(QuickPickItem & {
    ...
    @command('git.clone')
    async clone(url?: string, parentPath?: string, options?: {
        ref?: string;
    }): Promise<void> {
    ...
    async acceptMerge(_uri: Uri | unknown): Promise<void> {
        ...
        const result = await commands.executeCommand('mergeEditor.acceptMerge') as {
            successful: boolean;
        };
    ...
    private async runGitMerge(diff3: boolean): Promise<void> {
        ...
        e.replace(input.result, new Range(new Position(0, 0), new Position(doc.lineCount, 0)), result);
    ...
    private async smartCommit(repository: Repository, getCommitMessage: () => Promise<string | undefined>, opts: CommitOptions): Promise<void> {
        const config = workspace.getConfiguration('git', Uri.file(repository.root));
    ...
    private async _checkout(repository: Repository, opts?: {
        detached?: boolean;
        treeish?: string;
    }): Promise<boolean> {
    ...
    private async promptForBranchName(repository: Repository, defaultName?: string, initialValue?: string): Promise<string> {
        ...
        const getValueSelection = (value: string): [
            number,
            number
        ] | undefined => {
    ...
    async _stashDrop(repository: Repository, stash: Stash): Promise<boolean> {
        const yes = l10n.t('Yes');
        const result = await window.showWarningMessage(l10n.t('Are you sure you want to drop the stash: {0}?', stash.description), { modal: true }, yes);
        if (result !== yes) {
            return false;
        }
    ...
    private async getStashFromUri(uri: Uri | undefined): Promise<{
        repository: Repository;
        stash: Stash;
    } | undefined> {
    ...
    async manageUnsafeRepositories(): Promise<void> {
        ...
        quickpick.items = this.model.unsafeRepositories.length === 1 ? [...repositoriesQuickPickItems] :
            [...repositoriesQuickPickItems, { label: '', kind: QuickPickItemKind.Separator }, allRepositoriesQuickPickItem];
        quickpick.show();
        const repositoryItem = await new Promise<RepositoryItem | QuickPickItem | undefined>(resolve => {
