...
abstract class AbstractActionButton {
	...
	protected getPublishBranchActionButton():
		| SourceControlActionButton
		| undefined {
	...
	protected getSyncChangesActionButton():
		| SourceControlActionButton
		| undefined {
		const branchIsAheadOrBehind =
			(this.state.HEAD?.behind ?? 0) > 0 ||
			(this.state.HEAD?.ahead ?? 0) > 0;
...
export class CommitActionButton extends AbstractActionButton {
	...
	constructor(
		...
		this.disposables.push(
			workspace.onDidChangeConfiguration((e) => {
				...
				if (
					e.affectsConfiguration("scm.experimental.showSyncView") ||
	...
	protected override getPublishBranchActionButton():
		| SourceControlActionButton
		| undefined {
	...
	protected override getSyncChangesActionButton():
		| SourceControlActionButton
		| undefined {
		const scmConfig = workspace.getConfiguration("scm");
		if (scmConfig.get<boolean>("experimental.showSyncView", false)) {
