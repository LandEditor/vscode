...
import {
	GenerateCommitMessageActionButton,
	ICommitMessageProviderRegistry,
...
function wrapCommitMessage(
	...
	for (let index = 0; index < messageLines.length; index++) {
		...
		while (messageLine.length - position > threshold) {
			const lastSpaceBeforeThreshold = messageLine.lastIndexOf(
				" ",
				position + threshold
			);
			if (
				...
				const firstSpaceAfterThreshold = messageLine.indexOf(
					" ",
					position
				);
	...
	private onDidChangeLogLevel(logLevel: LogLevel): void {
		...
		this.eventDisposable = combinedDisposable([
			this.onWorkspaceWorkingTreeFileChange((uri) =>
				this.logger.debug(`[wt] Change: ${uri.fsPath}`)
			),
			this.onDotGitFileChange((uri) =>
				this.logger.debug(`[.git] Change: ${uri.fsPath}`)
			),
		]);
	}
...
class DotGitWatcher implements IFileWatcher {
	...
	constructor(
		...
		const filteredRootWatcher = filterEvent(
			...
			(uri) =>
				uri.scheme === "file" &&
				!/\/\.git(\/index\.lock)?$|\/\.watchman-cookie-/.test(uri.path)
		);
	...
	private updateTransientWatchers() {
		...
		try {
			...
			this.logger.warn(
				`Failed to watch ref '${upstreamPath}', is most likely packed.`
			);
		}
	}
...
class ResourceCommandResolver {
	...
	getResources(resource: Resource): [Uri | undefined, Uri | undefined] {
		...
		return [
			this.getLeftResource(resource),
			this.getRightResource(resource),
		];
	}
	...
	private getRightResource(resource: Resource): Uri | undefined {
		switch (resource.type) {
			...
			case Status.TYPE_CHANGED: {
				...
				const [indexStatus] =
					this.repository.indexGroup.resourceStates.filter(
...
export class Repository implements Disposable {
	...
	readonly onDidStartCommitMessageGeneration: Event<void> =
		this._onDidStartCommitMessageGeneration.event;

	private _onDidEndCommitMessageGeneration = new EventEmitter<void>();
	readonly onDidEndCommitMessageGeneration: Event<void> =
		this._onDidEndCommitMessageGeneration.event;
	...
	set rebaseCommit(rebaseCommit: Commit | undefined) {
		if (this._rebaseCommit && !rebaseCommit) {
			...
			rebaseCommit &&
			(!this._rebaseCommit ||
				this._rebaseCommit.hash !== rebaseCommit.hash)
		) {
	...
	private updateModelStateCancellationTokenSource:
		| CancellationTokenSource
		| undefined;
	private generateCommitMessageCancellationTokenSource:
		| CancellationTokenSource
		| undefined;
	...
	constructor(
		...
		const onRepositoryWorkingTreeFileChange = filterEvent(
			...
			(uri) =>
				!/\.git($|\\|\/)/.test(
					relativePath(repository.root, uri.fsPath)
				)
		);
		...
		try {
			...
			logger.error(
				`Failed to watch path:'${this.dotGit.path}' or commonPath:'${
					this.dotGit.commonPath
		...
		const inputActionButton = new GenerateCommitMessageActionButton(
			this,
			commitMessageProviderRegistry
		);
		this.disposables.push(inputActionButton);
		inputActionButton.onDidChange(
			() =>
				(this._sourceControl.inputBox.actionButton =
					inputActionButton.button)
		);
		...
		workspace.onDidChangeConfiguration(
			(e) => {
				if (e.affectsConfiguration("git.mergeEditor")) {
					this.mergeGroup.resourceStates =
						this.mergeGroup.resourceStates.map((r) => r.clone());
				}
			},
			undefined,
			this.disposables
		...
		this.disposables.push(
			this.branchProtectionProviderRegistry.registerBranchProtectionProvider(
				root,
				new GitBranchProtectionProvider(root)
			)
		);
		...
		const commitActionButton = new CommitActionButton(
			this,
			this.commitCommandCenter
		);
		this.disposables.push(commitActionButton);
		commitActionButton.onDidChange(
	...
	validateInput(
		...
		const setting = config.get<"always" | "warn" | "off">(
			"inputValidation"
		);
		...
		if (lineNumber === 0) {
			const inputValidationSubjectLength = config.get<number | null>(
	...
	provideOriginalResource(uri: Uri): Uri | undefined {
		...
		if (
			this.mergeGroup.resourceStates.some(
				(r) => r.resourceUri.path === uri.path
			)
		) {
			return undefined;
		}
	...
	getConfigs(): Promise<{ key: string; value: string }[]> {
		return this.run(Operation.Config, () =>
	...
	getConfig(key: string): Promise<string> {
		return this.run(Operation.Config, () =>
			this.repository.config("local", key)
		);
	}

	getGlobalConfig(key: string): Promise<string> {
		return this.run(Operation.Config, () =>
			this.repository.config("global", key)
		);
	}

	setConfig(key: string, value: string): Promise<string> {
		return this.run(Operation.Config, () =>
			this.repository.config("local", key, value)
		);
	}
	...
	logFile(uri: Uri, options?: LogFileOptions): Promise<Commit[]> {
		...
		return this.run(Operation.LogFile, () =>
			this.repository.logFile(uri, options)
		);
	}
	...
	diffWithHEAD(path?: string | undefined): Promise<string | Change[]> {
		return this.run(Operation.Diff, () =>
			this.repository.diffWithHEAD(path)
		);
	}
	...
	diffWith(ref: string, path?: string): Promise<string | Change[]> {
		return this.run(Operation.Diff, () =>
			this.repository.diffWith(ref, path)
		);
	}
	...
	diffBetweenShortStat(ref1: string, ref2: string): Promise<string> {
		return this.run(Operation.Diff, () =>
			this.repository.diffBetweenShortStat(ref1, ref2)
		);
	}

	getMergeBase(ref1: string, ref2: string): Promise<string> {
		return this.run(Operation.MergeBase, () =>
			this.repository.getMergeBase(ref1, ref2)
		);
	}
			...
			() => {
				...
				for (const resource of [
					...
					if (
						...
						!indexGroupResourcePaths.includes(
							resource.resourceUri.fsPath
						)
					) {
						addedResourceStates.push(
							resource.clone(ResourceGroupType.Index)
						);
					}
				}
				...
				const workingTreeGroup =
					this.workingTreeGroup.resourceStates.filter(
						(r) => !resourcePaths.includes(r.resourceUri.fsPath)
					);

				// Remove resource(s) from untracked group
				const untrackedGroup =
					this.untrackedGroup.resourceStates.filter(
						(r) => !resourcePaths.includes(r.resourceUri.fsPath)
					);
					...
					if (resourcePaths.includes(resource.resourceUri.fsPath)) {
						if (resource.type === Status.INDEX_ADDED) {
							untrackedResources.push(
								resource.clone(
									untrackedChangesResourceGroupType
								)
							);
						} else {
	...
	async getBranchBase(ref: string): Promise<Branch | undefined> {
		...
		if (branchFromReflog) {
			await this.setConfig(
				branchMergeBaseConfigKey,
				branchFromReflog.name!
			);
	...
	private async getBranchBaseFromReflog(
		...
		try {
			...
			const match = reflogEntries[0].match(
				/branch: Created from (?<name>.*)$/
			);
			...
			const match2 = headReflogEntries[
				headReflogEntries.length - 1
			].match(/checkout: moving from ([^\s]+)\s/);
	...
	private async getDefaultBranch(): Promise<Branch | undefined> {
		try {
			const defaultBranchResult = await this.repository.exec([
				"symbolic-ref",
				"--short",
	...
	async deleteTag(name: string): Promise<void> {
		await this.run(Operation.DeleteTag, () =>
			this.repository.deleteTag(name)
		);
	}

	async deleteRemoteTag(remoteName: string, tagName: string): Promise<void> {
		await this.run(Operation.DeleteRemoteTag, () =>
			this.repository.deleteRemoteTag(remoteName, tagName)
		);
	}
	...
	async checkoutTracking(
		...
		const refLabel = opts.detached ? treeish.substring(0, 8) : treeish;
		await this.run(Operation.CheckoutTracking(refLabel), () =>
	...
	async getCommitCount(
		...
		return await this.run(Operation.RevList, () =>
			this.repository.getCommitCount(range)
		);
	}
	...
	async reset(treeish: string, hard?: boolean): Promise<void> {
		await this.run(Operation.Reset, () =>
			this.repository.reset(treeish, hard)
		);
	}

	async deleteRef(ref: string): Promise<void> {
		await this.run(Operation.DeleteRef, () =>
			this.repository.deleteRef(ref)
		);
	}
	...
	async removeRemote(name: string): Promise<void> {
		await this.run(Operation.Remote, () =>
			this.repository.removeRemote(name)
		);
	}
	...
	async blame(path: string): Promise<string> {
		return await this.run(Operation.Blame, () =>
	...
	private async _sync(head: Branch, rebase: boolean): Promise<void> {
		...
		await this.run(Operation.Sync, async () => {
			await this.maybeAutoStash(async () => {
				...
				const supportCancellation = config.get<boolean>(
					"supportCancellation"
				);
	...
	private async checkIfMaybeRebased(currentBranch?: string) {
		const config = workspace.getConfiguration("git");
		const shouldIgnore =
			config.get<boolean>("ignoreRebaseWarning") === true;
	...
	async apply(patch: string, reverse?: boolean): Promise<void> {
		return await this.run(Operation.Apply, () =>
			this.repository.apply(patch, reverse)
		);
	}
	...
	async createStash(
		...
		const workingGroupResources = [
			...(!staged
				...
				: []),
			...(includeUntracked
				? this.untrackedGroup.resourceStates.map(
						(r) => r.resourceUri.fsPath
				  )
				: []),
		];
	...
	async popStash(index?: number): Promise<void> {
		return await this.run(Operation.Stash, () =>
			this.repository.popStash(index)
		);
	}
	...
	async applyStash(index?: number): Promise<void> {
		return await this.run(Operation.Stash, () =>
			this.repository.applyStash(index)
		);
	}
	...
	async ignore(files: Uri[]): Promise<void> {
		return await this.run(Operation.Ignore, async () => {
			...
			const document = (await new Promise((c) =>
				fs.exists(ignoreFile, c)
			))
				...
				const onExit = (exitCode: number) => {
					if (exitCode === 1) {
						...
						if (/ is in submodule /.test(stderr)) {
							...
							reject(
								new GitError({ stdout: data, stderr, exitCode })
							);
						}
					}
				};
	...
	async generateCommitMessage(): Promise<void> {
		...
		this.generateCommitMessageCancellationTokenSource =
		...
		try {
			...
			const token =
				this.generateCommitMessageCancellationTokenSource.token;
			const provider =
				this.commitMessageProviderRegistry.commitMessageProvider;
			const commitMessage = await provider.provideCommitMessage(
				new ApiRepository(this),
				diff,
				token
			);
			if (commitMessage) {
				const config = workspace.getConfiguration("git");
				const subjectLineLength = config.get<number | null>(
					"inputValidationSubjectLength",
					null
				);
				const bodyLineLength = config.get<number>(
					"inputValidationLength",
					50
				);

				this.inputBox.value = wrapCommitMessage(
					commitMessage,
					subjectLineLength ?? bodyLineLength,
					bodyLineLength
				);
			}
		} catch (err) {
	...
	private async _push(
		...
		try {
			...
			const remoteObj = repository.state.remotes.find(
				(r) => r.name === remote
			);
	...
	private _updateResourceGroupsState(
		resourcesGroups: GitResourceGroups
	): void {
		// set resource groups
		...
		if (resourcesGroups.workingTreeGroup) {
			this.workingTreeGroup.resourceStates =
				resourcesGroups.workingTreeGroup;
		}
	...
	private async getStatus(
		...
		const untrackedChanges = scopedConfig.get<
			"mixed" | "separate" | "hidden"
		>("untrackedChanges");
	...
	private isMergeInProgress(): Promise<boolean> {
		...
		return new Promise<boolean>((resolve) =>
			fs.exists(mergeHeadPath, resolve)
		);
	}

	private async maybeAutoStash<T>(
	...
	private onFileChange(_uri: Uri): void {
		...
		if (!autorefresh) {
			this.logger.trace(
				"Skip running git status because autorefresh setting is disabled."
			);
			return;
		}
		...
		if (this.isRepositoryHuge) {
			this.logger.trace(
				"Skip running git status because repository is huge."
			);
			return;
		}
		...
		if (!this.operations.isIdle()) {
			this.logger.trace(
				"Skip running git status because an operation is running."
			);
			return;
		}
	...
	private updateBranchProtectionMatchers(root: Uri): void {
		...
		for (const provider of this.branchProtectionProviderRegistry.getBranchProtectionProviders(
			...
			for (const {
				remote,
				rules,
			} of provider.provideBranchProtection()) {
	...
	private async handleTagConflict(
		...
		if (!remote) {
			throw new Error(
				"Unable to resolve tag conflict due to missing remote."
			);
		}
		...
		if (tags.length === 0) {
			throw new Error(
				`Unable to extract tag names from error message: ${raw}`
			);
		}
	...
	public isBranchProtected(branch = this.HEAD): boolean {
		if (branch?.name) {
			...
			const defaultBranchProtectionMatcher =
				this.branchProtection.get("");
			if (
