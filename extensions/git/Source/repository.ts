...
export class Resource implements SourceControlResourceState {
    ...
    constructor(private _commandResolver: ResourceCommandResolver, private _resourceGroupType: ResourceGroupType, private _resourceUri: Uri, private _type: Status, private _useIcons: boolean, private _renameResourceUri?: Uri) { }
...
class FileEventLogger {
    ...
    constructor(private onWorkspaceWorkingTreeFileChange: Event<Uri>, private onDotGitFileChange: Event<Uri>, private logger: LogOutputChannel) {
...
export class Repository implements Disposable {
    ...
    constructor(private readonly repository: BaseRepository, private readonly repositoryResolver: IRepositoryResolver, private pushErrorHandlerRegistry: IPushErrorHandlerRegistry, remoteSourcePublisherRegistry: IRemoteSourcePublisherRegistry, postCommitCommandsProviderRegistry: IPostCommitCommandsProviderRegistry, private readonly branchProtectionProviderRegistry: IBranchProtectionProviderRegistry, globalState: Memento, private readonly logger: LogOutputChannel, private telemetryReporter: TelemetryReporter) {
        ...
        filterEvent(workspace.onDidChangeConfiguration, e => e.affectsConfiguration('git.branchSortOrder', root)
    ...
    diffBetweenShortStat(ref1: string, ref2: string): Promise<{
        files: number;
        insertions: number;
        deletions: number;
    }> {
    ...
    async add(resources: Uri[], opts?: {
        update?: boolean;
    }): Promise<void> {
        await this.run(Operation.Add(!this.optimisticUpdateEnabled()), async () => {
    ...
    private commitOperationGetOptimisticResourceGroups(opts: CommitOptions): GitResourceGroups {
        let untrackedGroup: Resource[] | undefined = undefined, workingTreeGroup: Resource[] | undefined = undefined;
        if (opts.all === 'tracked') {
    ...
    async clean(resources: Uri[]): Promise<void> {
        await this.run(Operation.Clean(!this.optimisticUpdateEnabled()), async () => {
    ...
    async getRemoteRefs(remote: string, opts?: {
        heads?: boolean;
        tags?: boolean;
    }): Promise<Ref[]> {
    ...
    async checkout(treeish: string, opts?: {
        detached?: boolean;
        pullBeforeCheckout?: boolean;
    }): Promise<void> {
    ...
    async checkoutTracking(treeish: string, opts: {
        detached?: boolean;
    } = {}): Promise<void> {
        const refLabel = opts.detached ? treeish.substring(0, 8) : treeish;
    ...
    private async checkIfMaybeRebased(currentBranch?: string) {
        ...
        const result = await window.showWarningMessage(currentBranch
            ...
            : l10n.t('It looks like the current branch might have been rebased. Are you sure you still want to pull into it?'), always, pull, cancel);
        if (result === pull) {
            return true;
        }
        if (result === always) {
    ...
    private async run<T>(operation: Operation, runOperation: () => Promise<T> = () => Promise.resolve<any>(null), getOptimisticResourceGroups: () => GitResourceGroups | undefined = () => undefined): Promise<T> {
    ...
    private async retryRun<T>(operation: Operation, runOperation: () => Promise<T> = () => Promise.resolve<any>(null)): Promise<T> {
        ...
        while (true) {
            try {
                ...
                const shouldRetry = attempt <= 10 && ((err.gitErrorCode === GitErrorCodes.RepositoryIsLocked)
    ...
    private async getStatus(cancellationToken?: CancellationToken): Promise<GitResourceGroups> {
        ...
        const indexGroup: Resource[] = [], mergeGroup: Resource[] = [], untrackedGroup: Resource[] = [], workingTreeGroup: Resource[] = [];
        status.forEach(raw => {
            ...
            switch (raw.x) {
                case 'M':
                    indexGroup.push(new Resource(this.resourceCommandResolver, ResourceGroupType.Index, uri, Status.INDEX_MODIFIED, useIcons));
                    break;
                case 'A':
                    indexGroup.push(new Resource(this.resourceCommandResolver, ResourceGroupType.Index, uri, Status.INDEX_ADDED, useIcons));
                    break;
                case 'D':
                    indexGroup.push(new Resource(this.resourceCommandResolver, ResourceGroupType.Index, uri, Status.INDEX_DELETED, useIcons));
                    break;
                case 'R':
                    indexGroup.push(new Resource(this.resourceCommandResolver, ResourceGroupType.Index, uri, Status.INDEX_RENAMED, useIcons, renameUri));
                    break;
                case 'C':
                    indexGroup.push(new Resource(this.resourceCommandResolver, ResourceGroupType.Index, uri, Status.INDEX_COPIED, useIcons, renameUri));
                    break;
            }
            switch (raw.y) {
                case 'M':
                    ...
                    break;
                case 'D':
                    workingTreeGroup.push(new Resource(this.resourceCommandResolver, ResourceGroupType.WorkingTree, uri, Status.DELETED, useIcons, renameUri));
                    break;
                case 'A':
                    workingTreeGroup.push(new Resource(this.resourceCommandResolver, ResourceGroupType.WorkingTree, uri, Status.INTENT_TO_ADD, useIcons, renameUri));
                    break;
                case 'R':
                    workingTreeGroup.push(new Resource(this.resourceCommandResolver, ResourceGroupType.WorkingTree, uri, Status.INTENT_TO_RENAME, useIcons, renameUri));
                    break;
                case 'T':
                    workingTreeGroup.push(new Resource(this.resourceCommandResolver, ResourceGroupType.WorkingTree, uri, Status.TYPE_CHANGED, useIcons, renameUri));
                    break;
            }
            return undefined;
        });
