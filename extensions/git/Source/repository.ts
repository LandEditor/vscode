...
export class Repository implements Disposable {
	...
	closeDiffEditors(
		...
		for (const tab of window.tabGroups.all.flatMap((g) => g.tabs)) {
			const { input } = tab;
			if (
	...
	async getBranchBase(ref: string): Promise<Branch | undefined> {
		...
		const branchFromReflogUpstream = branchFromReflog
			? await this.getUpstreamBranch(branchFromReflog)
			: undefined;
	...
	private async getUpstreamBranch(
		branch: Branch,
	): Promise<Branch | undefined> {
		if (!branch.upstream) {
			return undefined;
		...
		try {
			...
			this.logger.warn(
				`Failed to get branch details for 'refs/remotes/${branch.upstream.remote}/${branch.upstream.name}': ${err.message}.`,
			);
			return undefined;
		}
	}
