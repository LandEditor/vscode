...
export class Git {
    ...
    open(repositoryRoot: string, repositoryRootRealPath: string | undefined, dotGit: {
        path: string;
        commonPath?: string;
    }, logger: LogOutputChannel): Repository {
        ...
        if (isWindows) {
            ...
            if (repoUri.authority.length !== 0 && pathUri.authority.length === 0) {
                ...
                if (match !== null) {
                    const [, letter] = match;
                    try {
                        const networkPath = await new Promise<string | undefined>(resolve => realpath.native(`${letter}:\\`, { encoding: 'utf8' }, (err, resolvedPath) => resolve(err !== null ? undefined : resolvedPath)));
...
export class Repository {
    ...
    constructor(private _git: Git, private repositoryRoot: string, private repositoryRootRealPath: string | undefined, readonly dotGit: {
        path: string;
        commonPath?: string;
    ...
    async getConfigs(scope: string): Promise<{
        key: string;
        value: string;
    }[]> {
        const args = ['config'];
    ...
    async getObjectDetails(treeish: string, path: string): Promise<{
        mode: string;
        object: string;
        size: number;
    }> {
    ...
    async getGitRelativePath(ref: string, relativePath: string): Promise<string> {
        ...
        const elements: {
            file: string;
        }[] = ref ? await this.lstree(ref, dirname) : await this.lsfiles(dirname);
    ...
    async detectObjectType(object: string): Promise<{
        mimetype: string;
        encoding?: string;
    }> {
    ...
    async diffBetweenShortStat(ref1: string, ref2: string): Promise<{
        files: number;
        insertions: number;
        deletions: number;
    }> {
    ...
    async checkout(treeish: string, paths: string[], opts: {
        track?: boolean;
        detached?: boolean;
