...
import { type Branch, RefType, type RemoteSourcePublisher } from "./api/git";
import {
	type CheckoutOperation,
	type CheckoutTrackingOperation,
	OperationKind,
} from "./operation";
...
class CheckoutStatusBar {
	...
	get command(): Command | undefined {
		...
		const label =
			...
			`${this.repository.headLabel}${
				rebasing ? ` (${l10n.t("Rebasing")})` : ""
			}`;
		const command =
	...
	private getIcon(): string {
		...
		if (
			...
			return this.repository.isBranchProtected()
				? "$(lock)"
				: "$(git-branch)";
		}
