...
(env.uiKind === UIKind.Web ? suite.skip : suite)(
	...
	() => {
		...
		suiteSetup(async () => {
			...
			await extensions
				.getExtension("vscode.vscode-api-tests")
				?.activate();
		...
		suite("Terminal", () => {
			...
			test("creationOptions should be set and readonly for TerminalOptions terminals", async () => {
				...
				const terminalOptions =
					terminal.creationOptions as TerminalOptions;
				const result = await new Promise<Terminal>((r) => {
					disposables.push(
						window.onDidOpenTerminal((t) => {
							if (t === terminal) {
				...
				test.skip("should respect dimension overrides", async () => {
					...
					const pty: Pseudoterminal = {
						...
						onDidOverrideDimensions:
							overrideDimensionsEmitter.event,
						open: () =>
							overrideDimensionsEmitter.fire({
								columns: 10,
								rows: 5,
							}),
						close: () => {},
			...
			(process.platform === "win32" ? suite.skip : suite)(
				...
				() => {
					...
					test("get and forEach should work (scope)", () => {
						...
						const expectedScopedCollection =
							collection.getScoped(scope);
						...
						const entries: [string, EnvironmentVariableMutator][] =
							[];
						expectedScopedCollection.forEach((v, m) =>
							entries.push([v, m])
						);
						deepStrictEqual(
							entries.map((v) => v[1]),
