...
suite("Notebook Document", function () {
	...
	test("document basics", async function () {
		const uri = await utils.createRandomFile(
			undefined,
			undefined,
			".nbdtest"
		);
			...
			.then((notebook) => {
				for (let i = 0; i < notebook.cellCount; i++) {
					...
					const doc = vscode.workspace.textDocuments.find(
						(doc) =>
							doc.uri.toString() === cell.document.uri.toString()
					);
					assert.ok(doc);
	...
	test("open untitled notebook", async function () {
		const nb =
			await vscode.workspace.openNotebookDocument("notebook.nbdtest");
	...
	test("workspace edit API (notebookMetadata)", async function () {
		...
		assert.ok(
			document.metadata.custom.extraNotebookMetadata,
	...
	test("setTextDocumentLanguage when notebook editor is not open", async function () {
		...
		cellDoc = await vscode.languages.setTextDocumentLanguage(
			cellDoc,
			"css"
		);
