...
suite("vscode API - workspace", () => {
				...
				return vscode.window.showTextDocument(doc).then(() => {
					return doc.save().then((didSave: boolean) => {
						...
						const closed = closedDocuments.filter(
							(close) =>
								close.uri.toString() === doc.uri.toString()
						)[0];
						assert.ok(closed);
	...
	test("openTextDocument, uri scheme/auth/path", function () {
		const registration =
			vscode.workspace.registerTextDocumentContentProvider("sc", {
	...
	test("openTextDocument, actual casing first", async function () {
		...
		fs.writeFile(uriOne, Buffer.from("one"), {
			create: true,
			overwrite: true,
		});
		fs.writeFile(uriTwo, Buffer.from("two"), {
			create: true,
			overwrite: true,
		});
	...
	test("eol, change via applyEdit", () => {
		return createRandomFile("foo\nbar\nbar").then((file) => {
			return vscode.workspace.openTextDocument(file).then((doc) => {
				...
				edit.set(file, [
					vscode.TextEdit.setEndOfLine(vscode.EndOfLine.CRLF),
				]);
	...
	test("registerTextDocumentContentProvider, simple", function () {
		const registration =
			vscode.workspace.registerTextDocumentContentProvider("foo", {
	...
	test("applyEdit", async () => {
		const doc = await vscode.workspace.openTextDocument(
			vscode.Uri.parse(
				"untitled:" +
					join(vscode.workspace.rootPath || "", "./new2.txt")
			)
		);
	...
	function nameWithUnderscore(uri: vscode.Uri) {
		return uri.with({
			path: posix.join(
				posix.dirname(uri.path),
				`_${posix.basename(uri.path)}`
			),
		});
	}
	...
	async function test77735(withOpenedEditor: boolean): Promise<void> {
		...
		if (withOpenedEditor) {
			const document =
				await vscode.workspace.openTextDocument(docUriOriginal);
	...
	test("SnippetString in WorkspaceEdit", async function (): Promise<any> {
		...
		assert.deepStrictEqual(edt.selections, [
