	...
	suite("Tasks", () => {
		...
		suite("ShellExecution", () => {
			...
			test.skip("dependsOn task should start with a different processId (#118256)", async () => {
				...
				const waitForStartEvent1 = new Promise<TaskProcessStartEvent>(
					(r) => {
						// Listen for first task and verify valid process ID
						const listener = tasks.onDidStartTaskProcess(
							async (e) => {
								if (
									e.execution.task.name === "taskToDependOn"
								) {
									listener.dispose();
									r(e);
								}
				...
				const waitForStartEvent2 = new Promise<TaskProcessStartEvent>(
					(r) => {
				...
				assert.notStrictEqual(
					startEvent1.processId,
					startEvent2.processId
				);
