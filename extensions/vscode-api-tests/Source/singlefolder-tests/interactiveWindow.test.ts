...
async function createInteractiveWindow(kernel: Kernel) {
	...
	assert.ok(
		notebookEditor,
		"Interactive Window was not created successfully"
	);
...
async function addCellAndRun(code: string, notebook: vscode.NotebookDocument) {
	...
	try {
		...
		assert.fail(
			`Notebook change event was not triggered after executing newly added cell. Initial Cell count: ${initialCellCount}. Current cell count: ${
				notebook.cellCount
			}. execution summary: ${JSON.stringify(result.executionSummary)}`
		);
	}
	assert.strictEqual(
		cell.outputs.length,
		1,
		`Executed cell has no output. Initial Cell count: ${initialCellCount}. Current cell count: ${
			notebook.cellCount
		}. execution summary: ${JSON.stringify(cell.executionSummary)}`
	);
	return cell;
}
...
(vscode.env.uiKind === vscode.UIKind.Web ? suite.skip : suite)(
	...
	function () {
		...
		test("Interactive window scrolls after execute", async () => {
			...
			assert.strictEqual(
				notebookEditor.visibleRanges[
					notebookEditor.visibleRanges.length - 1
				].end,
				notebookEditor.notebook.cellCount,
		...
		test("Interactive window has the correct kernel", async () => {
			...
			const { notebookEditor } =
				await createInteractiveWindow(secondKernel);
