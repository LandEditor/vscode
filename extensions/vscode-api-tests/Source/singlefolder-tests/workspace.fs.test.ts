...
suite("vscode API - workspace-fs", () => {
	...
	test("fs.stat - bad scheme", async function () {
		try {
			await vscode.workspace.fs.stat(
				vscode.Uri.parse("foo:/bar/baz/test.txt")
			);
			assert.ok(false);
		} catch {
			assert.ok(true);
	...
	test("throws FileSystemError (1)", async function () {
		try {
			...
			assert.strictEqual(
				e.name,
				vscode.FileSystemError.FileNotFound().name
			);
		}
	});
	...
	test("throws FileSystemError (2)", async function () {
		try {
			...
			assert.strictEqual(
				e.name,
				vscode.FileSystemError.Unavailable().name
			);
		}
	});
	...
	test("vscode.workspace.fs.remove() (and copy()) succeed unexpectedly. #84177 (2)", async function () {
		...
		try {
			await vscode.workspace.fs.copy(folder, someFolder, {
				overwrite: true,
			});
			assert.ok(true);
		} catch (err) {
	...
	test("vscode.workspace.fs error reporting is weird #132981", async function () {
		...
		try {
			...
			assert.strictEqual(
				err.code,
				vscode.FileSystemError.FileNotFound().code
			);
		}
