...
async function assertKernel(
	...
	assert.ok(
		success,
		`expected selected kernel to be ${kernel.controller.id}`
	);
		...
		test("onDidChangeCellExecutionState is fired", async () => {
			...
			testDisposables.push(
				vscode.notebooks.onDidChangeNotebookCellExecutionState((e) => {
					try {
						assert.strictEqual(
							e.cell.document.uri.toString(),
							cell.document.uri.toString(),
							"event should be fired for the executing cell"
						);

						if (eventCount === 0) {
							assert.strictEqual(
								e.state,
								vscode.NotebookCellExecutionState.Pending,
								"should be set to Pending"
							);
						} else if (eventCount === 1) {
							assert.strictEqual(
								e.state,
								vscode.NotebookCellExecutionState.Executing,
								"should be set to Executing"
							);
							assert.strictEqual(
								cell.outputs.length,
								0,
								"no outputs yet: " +
									JSON.stringify(cell.outputs[0])
							);
						} else if (
		...
		test("Output changes are applied once the promise resolves", async function () {
			...
			const verifyOutputSyncKernel = new (class extends Kernel {
				...
				override async _execute(cells: vscode.NotebookCell[]) {
					...
					assert.strictEqual(
						cell.notebook.cellAt(0).outputs.length,
						1
					);
					assert.deepStrictEqual(
