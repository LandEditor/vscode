...
class VsCodeDocument implements md.ITextDocument {
	...
	constructor(
		uri: string,
		init: { inMemoryDoc?: TextDocument; onDiskDoc?: TextDocument },
	) {
		this.uri = uri;
...
export class VsCodeClientWorkspace implements md.IWorkspaceWithWatching {
	private readonly _onDidCreateMarkdownDocument =
		new Emitter<md.ITextDocument>();
	public readonly onDidCreateMarkdownDocument =
		this._onDidCreateMarkdownDocument.event;

	private readonly _onDidChangeMarkdownDocument =
		new Emitter<md.ITextDocument>();
	public readonly onDidChangeMarkdownDocument =
		this._onDidChangeMarkdownDocument.event;

	private readonly _onDidDeleteMarkdownDocument = new Emitter<URI>();
	public readonly onDidDeleteMarkdownDocument =
		this._onDidDeleteMarkdownDocument.event;
	...
	constructor(
		...
		documents.onDidOpen((e) => {
			...
			this.logger.log(
				md.LogLevel.Trace,
				"VsCodeClientWorkspace.TextDocument.onDidOpen",
			...
			if (doc) {
				...
				const doc = new VsCodeDocument(e.document.uri, {
		...
		documents.onDidChangeContent((e) => {
			...
			this.logger.log(
				md.LogLevel.Trace,
				"VsCodeClientWorkspace.TextDocument.onDidChanceContent",
				{ document: e.document.uri },
			);
		...
		documents.onDidClose(async (e) => {
			...
			this.logger.log(
				md.LogLevel.Trace,
				"VsCodeClientWorkspace.TextDocument.onDidClose",
				{ document: e.document.uri },
			);
			...
			if (!(await this.statBypassingCache(uri))) {
				if (
					this._documentCache.get(uri) === doc &&
					!doc.hasInMemoryDoc()
				) {
					this.doDeleteDocument(uri);
					return;
		...
		connection.onDidChangeWatchedFiles(async ({ changes }) => {
			for (const change of changes) {
				...
				this.logger.log(
					md.LogLevel.Trace,
					"VsCodeClientWorkspace.onDidChangeWatchedFiles",
					{ type: change.type, resource: resource.toString() },
				);
				switch (change.type) {
					case FileChangeType.Changed: {
						...
						if (entry) {
							...
							const document =
								await this.openMarkdownDocumentFromFs(resource);
							if (document) {
					...
					case FileChangeType.Created: {
						...
						if (entry) {
							...
							const document =
								await this.openMarkdownDocumentFromFs(resource);
							if (document) {
								this._onDidCreateMarkdownDocument.fire(
									document,
								);
							}
						}
		...
		connection.onRequest(protocol.fs_watcher_onChange, (params) => {
			this.logger.log(
				md.LogLevel.Trace,
				"VsCodeClientWorkspace.fs_watcher_onChange",
				{ kind: params.kind, uri: params.uri },
			);
	...
	async getAllMarkdownDocuments(): Promise<Iterable<md.ITextDocument>> {
		...
		const openTextDocumentResults = this.documents
			.all()
		...
		const limiter = new Limiter<md.ITextDocument | undefined>(
			maxConcurrent,
		);
		const resources = await this.connection.sendRequest(
			protocol.findMarkdownFilesInWorkspace,
			{},
		);
		await Promise.all(
			resources.map((strResource) => {
	...
	async openMarkdownDocument(
		resource: URI,
	): Promise<md.ITextDocument | undefined> {
	...
	private async openMarkdownDocumentFromFs(
		resource: URI,
	): Promise<md.ITextDocument | undefined> {
	...
	private async statBypassingCache(
		resource: URI,
	): Promise<md.FileStat | undefined> {
		const uri = resource.toString();
		if (this.documents.get(uri)) {
			return { isDirectory: false };
		}
		const fsResult = await this.connection.sendRequest(protocol.fs_stat, {
			uri,
		});
	...
	async readDirectory(resource: URI): Promise<[string, md.FileStat][]> {
		...
		return this.connection.sendRequest(protocol.fs_readDirectory, {
			uri: resource.toString(),
		});
	}
	...
	getContainingDocument(
		resource: URI,
	): md.ContainingDocumentContext | undefined {
		if (resource.scheme === Schemes.notebookCell) {
			const nb = this.notebooks.findNotebookDocumentForCell(
				resource.toString(),
			);
			if (nb) {
				return {
					uri: URI.parse(nb.uri),
					children: nb.cells.map((cell) => ({
						uri: URI.parse(cell.document),
					})),
				};
			}
		}
	...
	watchFile(
		...
		return {
			...
			dispose: () => {
				this.logger.log(
					md.LogLevel.Trace,
					"VsCodeClientWorkspace.disposeWatcher",
	...
	private isRelevantMarkdownDocument(doc: TextDocument) {
		return (
			isMarkdownFile(doc) &&
			URI.parse(doc.uri).scheme !== "vscode-bulkeditpreview"
		);
	}
	...
	private doDeleteDocument(uri: URI) {
		this.logger.log(
			md.LogLevel.Trace,
			"VsCodeClientWorkspace.deleteDocument",
