...
import { type LsConfiguration, getLsConfiguration } from "./config";
import { ConfigurationManager, type Settings } from "./configuration";
import { registerValidateSupport } from "./languageFeatures/diagnostics";
import { LogFunctionLogger } from "./logging";
import * as protocol from "./protocol";
import type { IDisposable } from "./util/dispose";
...
export async function startVsCodeServer(connection: Connection) {
	...
	const workspaceFactory: WorkspaceFactory = ({
		...
		const workspace = new VsCodeClientWorkspace(
			connection,
			config,
			documents,
			notebooks,
...
export async function startServer(
	...
	connection.onInitialize((params: InitializeParams): InitializeResult => {
		const initOptions = params.initializationOptions as
			| MdServerInitializationOptions
			| undefined;

		const mdConfig = getLsConfiguration(initOptions ?? {});

		const workspace = serverConfig.workspaceFactory({
			connection,
			config: mdConfig,
		...
		mdLs = md.createLanguageService({
			...
			get preferredMdPathExtensionStyle() {
				switch (
					serverConfig.configurationManager.getSettings()?.markdown
						.preferredMdPathExtensionStyle
				) {
					case "includeExtension":
						return md.PreferredMdPathExtensionStyle
							.includeExtension;
	...
	connection.onDocumentLinks(
		async (params, token): Promise<lsp.DocumentLink[]> => {
	...
	connection.onDocumentSymbol(
		async (params, token): Promise<lsp.DocumentSymbol[]> => {
			const document = documents.get(params.textDocument.uri);
			if (!document) {
				return [];
			}
	...
	connection.onFoldingRanges(
		async (params, token): Promise<lsp.FoldingRange[]> => {
			const document = documents.get(params.textDocument.uri);
			if (!document) {
				return [];
			}
	...
	connection.onSelectionRanges(
		async (params, token): Promise<lsp.SelectionRange[] | undefined> => {
			const document = documents.get(params.textDocument.uri);
			if (!document) {
				return [];
			}
	...
	connection.onDefinition(
		async (params, token): Promise<lsp.Definition | undefined> => {
			const document = documents.get(params.textDocument.uri);
			if (!document) {
				return undefined;
			}
	...
	connection.onRequest(
		...
		async (params, token: CancellationToken) => {
			...
			return {
				edit: result.edit,
				participatingRenames: result.participatingRenames.map(
					(rename) => ({
	...
	connection.onRequest(
		protocol.prepareUpdatePastedLinks,
		async (params, token: CancellationToken) => {
			...
			return mdLs!.prepareUpdatePastedLinks(
				document,
				params.ranges,
				token,
			);
	...
	connection.onRequest(
		protocol.getUpdatePastedLinksEdit,
		async (params, token: CancellationToken) => {
			...
			const edits = params.edits.map((edit: any) =>
				lsp.TextEdit.replace(
					lsp.Range.create(
						edit.range[0].line,
						edit.range[0].character,
						edit.range[1].line,
						edit.range[1].character,
					),
					edit.newText,
				),
			);
			return mdLs!.getUpdatePastedLinksEdit(
				document,
				edits,
				params.metadata,
				token,
...
function registerCompletionsSupport(
	...
	function getIncludeWorkspaceHeaderCompletions(): md.IncludeWorkspaceHeaderCompletions {
		switch (
			config.getSettings()?.markdown.suggest.paths
				.includeWorkspaceHeaderCompletions
		) {
			case "onSingleOrDoubleHash":
				return md.IncludeWorkspaceHeaderCompletions
	...
	connection.onCompletion(
		async (params, token): Promise<lsp.CompletionItem[]> => {
			const settings = config.getSettings();
	...
	return registerDynamicClientFeature(
		config,
		(settings) => !!settings?.markdown.suggest.paths.enabled,
		() => {
...
function registerDocumentHighlightSupport(
	...
	return registerDynamicClientFeature(
		configurationManager,
		(settings) => !!settings?.markdown.occurrencesHighlight.enabled,
		() => {
