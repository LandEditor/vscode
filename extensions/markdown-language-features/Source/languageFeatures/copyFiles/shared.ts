...
export const externalUriSchemes = ["http", "https", "mailto"];
...
> {
	...
	for (const range of ranges) {
		...
		if (useSmartPaste) {
			pasteAsMarkdownLink = checkSmartPaste(
				document,
				selectedRange,
				range
			);
		...
		const snippet = await tryGetUriListSnippet(
			document,
			urlList,
			token,
...
export function checkSmartPaste(
	document: SkinnyTextDocument,
	...
	if (
		...
		validateLink(document.getText(range)).isValid
	) {
		return false;
	}
	if (
	...
	for (const regex of smartPasteRegexes) {
		...
		for (const match of matches) {
			if (match.index !== undefined) {
				const useDefaultPaste =
					selectedRange.start.character > match.index &&
					selectedRange.end.character < match.index + match[0].length;
				if (useDefaultPaste) {
					return false;
				}
			}
...
export function validateLink(urlList: string): {
	isValid: boolean;
	...
	const splitUrlList = trimmedUrlList
		.split(" ")
		.filter((item) => item !== ""); //split on spaces and remove empty strings
	if (uri) {
		isValid =
			splitUrlList.length === 1 &&
			!splitUrlList[0].includes("\n") &&
			externalUriSchemes.includes(
				vscode.Uri.parse(splitUrlList[0]).scheme
			) &&
			!!vscode.Uri.parse(splitUrlList[0]).authority;
	}
	return { isValid, cleanedUrlList: splitUrlList[0] };
}

export async function tryGetUriListSnippet(
	document: SkinnyTextDocument,
	urlList: String,
	token: vscode.CancellationToken,
	title = "",
...
): Promise<{ snippet: vscode.SnippetString; label: string } | undefined> {
...
export function appendToLinkSnippet(
	...
	snippet.appendText(
		`](${escapeMarkdownLinkPath(uriString, isExternalLink)})`
	);
	return snippet;
}
		...
		if (insertAsVideo) {
			...
			if (insertAsMedia) {
				...
				if (pasteAsMarkdownLink) {
					...
					snippet.appendText(
						`](${escapeMarkdownLinkPath(mdPath, isExternalLink)})`
					);
				} else {
					snippet.appendText(
						escapeMarkdownLinkPath(mdPath, isExternalLink)
...
> {
	...
	const snippet = createUriListSnippet(
		document,
		fileEntries.map((entry) => entry.uri)
	);
	if (!snippet) {
		return;
	}
...
function escapeMarkdownLinkPath(
	mdPath: string,
	isExternalLink: boolean
): string {
