...
import type { TextDocumentEdit } from "vscode-languageclient";
...
enum UpdateLinksOnFileMoveSetting {
	Prompt = "prompt",
	Always = "always",
	Never = "never",
}
...
class UpdateLinksOnFileRenameHandler extends Disposable {
			...
			vscode.workspace.onDidRenameFiles(async (e) => {
				await Promise.all(
					e.files.map(async (rename) => {
						if (
							await this._shouldParticipateInLinkUpdate(
								rename.newUri,
							)
						) {
						...
						vscode.window.withProgress(
							{
								...
								title: vscode.l10n.t(
									"Checking for Markdown links to update",
								),
							},
							() => this._flushRenames(),
						);
	...
	> {
		...
		for (const change of result.edit
			.documentChanges as TextDocumentEdit[]) {
			const uri = vscode.Uri.parse(change.textDocument.uri);
			for (const edit of change.edits) {
				workspaceEdit.replace(
					uri,
					convertRange(edit.range),
					edit.newText,
				);
			}
		}
