...
import type { MarkdownContributionProvider } from "../markdownExtensions";
import { Disposable } from "../util/dispose";
import { isMarkdownFile } from "../util/file";
import type { MdLinkOpener } from "../util/openDocumentLink";
import type { WebviewResourceProvider } from "../util/resources";
import { urlToUri } from "../util/url";
import type { ImageInfo, MdDocumentRenderer } from "./documentRenderer";
import type { MarkdownPreviewConfigurationManager } from "./previewConfig";
import {
	StartingScrollFragment,
	StartingScrollLine,
	type StartingScrollLocation,
	scrollEditorToLine,
} from "./scrolling";
import {
	type LastScrollLocation,
	type TopmostLineMonitor,
	getVisibleLine,
...
class MarkdownPreview extends Disposable implements WebviewResourceProvider {
	private static readonly _unwatchedImageSchemes = new Set([
		"https",
		"http",
		"data",
	]);
		...
		if (!this._throttleTimer) {
			if (this._firstUpdate) {
				...
				this._throttleTimer = setTimeout(
					() => this._updatePreview(forceUpdate),
					this._delay,
				);
			}
		}
	...
	private _updateImageWatchers(srcs: Set<string>) {
		...
		for (const src of srcs) {
			...
			if (
				uri &&
				!MarkdownPreview._unwatchedImageSchemes.has(uri.scheme) &&
				!this._fileWatchersBySrc.has(src)
			) {
	...
	private async _onDidClickPreviewLink(href: string) {
		...
		if (openLinks === "inPreview") {
			const resolved = await this._opener.resolveDocumentLink(
				href,
				this.resource,
			);
			if (resolved.kind === "file") {
				try {
					...
					if (isMarkdownFile(doc)) {
						return this._delegate.openPreviewLinkToMarkdownFile(
							doc.uri,
							resolved.fragment
	...
	implements IManagedMarkdownPreview
{
	...
	public static create(
		...
		const webview = vscode.window.createWebviewPanel(
			...
			DynamicMarkdownPreview._getPreviewTitle(
				input.resource,
				input.locked,
			),
			previewColumn,
			...
			startingScroll,
			{
				...
				openPreviewLinkToMarkdownFile: (
					...
					this.update(
						link,
						fragment
							? new StartingScrollFragment(fragment)
							: undefined,
					);
				},
			},
