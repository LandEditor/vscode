...
import type { MarkdownPreviewManager } from "./previewManager";
	...
	implements ContentSecurityPolicyArbiter
{
	...
	public getSecurityLevelForResource(
		...
		const level = this._globalState.get<
			MarkdownPreviewSecurityLevel | undefined
		>(this._security_level_key + this._getRoot(resource), undefined);
...
export class PreviewSecuritySelector {
	...
	public async showSecuritySelectorForResource(
						...
						label:
							markActiveWhen(
								currentSecurityLevel ===
									MarkdownPreviewSecurityLevel.Strict,
		...
		if (selection.type === "toggle") {
			...
			await this._cspArbiter.setSecurityLevelForResource(
				resource,
				selection.type,
			);
		}
