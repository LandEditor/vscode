	...
	if (previous) {
		...
		if (next) {
			const progressBetweenElements =
				offsetFromPrevious /
				(getElementBounds(next).top - previousBounds.top);
			return (
				previous.line +
				progressBetweenElements * (next.line - previous.line)
			);
		} else {
			const progressWithinElement =
				offsetFromPrevious / previousBounds.height;
