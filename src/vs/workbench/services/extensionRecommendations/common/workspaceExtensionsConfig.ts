	...
	implements IWorkspaceExtensionsConfigService
{
	...
	private async addOrRemoveWorkspaceFolderRecommendation(
		...
		if (add) {
			...
			if (
				...
				extensionsConfigContent.unwantedRecommendations.some(
					(e) => e === extensionId
				)
			) {
				values.push({
					path: ["unwantedRecommendations"],
					value: extensionsConfigContent.unwantedRecommendations.filter(
						(e) => e !== extensionId
					),
				});
			}
			...
			values.push({
				path: ["recommendations"],
				value: extensionsConfigContent.recommendations.filter(
					(e) => e !== extensionId
				),
			});
		}
		...
		if (extensionsConfigContent) {
			if (add) {
				...
				if (
					...
					values.push({
						path: ["extensions", "unwantedRecommendations"],
						value: extensionsConfigContent.unwantedRecommendations.filter(
							(e) => e !== extensionId
						),
					});
	...
	private async addOrRemoveWorkspaceFolderUnwantedRecommendation(
		...
		if (add) {
			...
			if (
				extensionsConfigContent.recommendations &&
				extensionsConfigContent.recommendations.some(
					(e) => e === extensionId
				)
			) {
				values.push({
