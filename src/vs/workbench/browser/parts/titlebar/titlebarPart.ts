...
export class TitlebarPart extends Part implements ITitleService {
	...
	get minimumHeight(): number {
		const value =
			this.isCommandCenterVisible || (isWeb && isWCOEnabled()) ? 35 : 30;
	...
	readonly onDidChangeCommandCenterVisibility: Event<void> =
		this._onDidChangeCommandCenterVisibility.event;
	...
	private editorActionsChangeDisposable = this._register(
		new DisposableStore()
	);
	...
	constructor(
		...
		this.windowTitle = this._register(
			instantiationService.createInstance(WindowTitle, mainWindow, "main")
		);
	...
	protected onConfigurationChanged(event: IConfigurationChangeEvent): void {
		...
		if (this.titleBarStyle !== "native" && this.actionToolBar) {
			...
			if (
				affectsEditorActions ||
				affectsLayoutControl ||
				affectsActivityControl
			) {
				this.createActionToolBarMenus({
					editorActions: affectsEditorActions,
	...
	protected override createContentArea(parent: HTMLElement): HTMLElement {
		...
		[EventType.CONTEXT_MENU, EventType.MOUSE_DOWN].forEach((event) => {
			this._register(
				addDisposableListener(this.rootContainer, event, (e) => {
					if (
						e.type === EventType.CONTEXT_MENU ||
						(e.target === this.title && e.metaKey)
					) {
						EventHelper.stop(e);
						this.onContextMenu(
							e,
	...
	private actionViewItemProvider(
		...
		const activeEditorPane =
			this.editorGroupService.mainPart.activeGroup?.activeEditorPane;
	...
	private createActionToolBar() {
		...
		this.actionToolBar = this.instantiationService.createInstance(
			WorkbenchToolBar,
		...
		if (this.editorActionsEnabled) {
			this.actionToolBarDisposable.add(
				this.editorGroupService.mainPart.onDidChangeActiveGroup(() =>
		...
		if (update.editorActions) {
			...
			if (
				...
				const context: IEditorCommandsContext = {
					groupId: this.editorGroupService.mainPart.activeGroup.id,
				};
	...
	protected get useCounterZoom(): boolean {
		...
		return (
			zoomFactor < 1 || (noMenubar && noCommandCenter && noLayoutControls)
		);
	}
...
registerAction2(
	class ToggleEditorActions extends Action2 {
		...
		run(accessor: ServicesAccessor, ...args: any[]): void {
			...
			const value = configService.getValue<string>(
