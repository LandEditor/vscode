		...
		disposables.add(
			new DragAndDropObserver(tab, {
				onDragStart: (e) => {
					...
					scheduleAtNextAnimationFrame(getWindow(tab), () =>
		...
		if (this.visible) {
			if (!this.layoutScheduler.value) {
				...
				if (this.lifecycleService.phase >= LifecyclePhase.Restored) {
					scheduledLayout = scheduleAtNextAnimationFrame(
						getWindow(this.tabsContainer),
						layoutFunction
					);
				} else {
					scheduledLayout = runWhenWindowIdle(
						getWindow(this.tabsContainer),
						layoutFunction
					);
				}
