...
} from "vs/workbench/browser/parts/notifications/notificationsTelemetry";
import {
...
export function registerNotificationCommands(
	...
	KeybindingsRegistry.registerCommandAndKeybindingRule({
		...
		handler: (accessor) => {
			...
			const notification =
				...
				firstOrDefault(model.notifications);
			if (!notification) {
				return;
			}
			const primaryAction = notification.actions?.primary
				? firstOrDefault(notification.actions.primary)
				: undefined;
			if (!primaryAction) {
				return;
			}
	...
	CommandsRegistry.registerCommand(TOGGLE_DO_NOT_DISTURB_MODE, (accessor) => {
		...
		notificationService.doNotDisturbMode =
			!notificationService.doNotDisturbMode;
	});

	// Commands for Command Palette
	const category = {
		value: localize("notifications", "Notifications"),
	...
	MenuRegistry.appendMenuItem(MenuId.CommandPalette, {
		command: {
			id: SHOW_NOTIFICATIONS_CENTER,
			title: {
				value: localize("showNotifications", "Show Notifications"),
				original: "Show Notifications",
			},
			category,
		},
	});
	MenuRegistry.appendMenuItem(MenuId.CommandPalette, {
		command: {
			id: HIDE_NOTIFICATIONS_CENTER,
			title: {
				value: localize("hideNotifications", "Hide Notifications"),
				original: "Hide Notifications",
			},
			category,
		},
		when: NotificationsCenterVisibleContext,
