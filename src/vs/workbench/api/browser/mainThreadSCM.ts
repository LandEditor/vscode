...
import {
	...
	ISCMHistoryItemGroupDetails,
	ISCMHistoryItemGroupEntry,
	ISCMHistoryOptions,
	ISCMHistoryProvider,
...
function getSCMInputBoxActionButtonIcon(
	...
	if (!actionButton.icon) {
		...
		const icon = actionButton.icon as {
			light: UriComponents;
			dark: UriComponents;
		};
...
function getSCMHistoryItemIcon(
	historyItem: SCMHistoryItemDto
): URI | { light: URI; dark: URI } | ThemeIcon | undefined {
	if (!historyItem.icon) {
		return undefined;
		...
		const icon = historyItem.icon as {
			light: UriComponents;
			dark: UriComponents;
		};
		return { light: URI.revive(icon.light), dark: URI.revive(icon.dark) };
...
class MainThreadSCMHistoryProvider implements ISCMHistoryProvider {
	...
	readonly onDidChangeCurrentHistoryItemGroup =
	...
	async provideHistoryItems(
		...
		return historyItems?.map((historyItem) => ({
			...historyItem,
			icon: getSCMHistoryItemIcon(historyItem),
		}));
	}
...
class MainThreadSCMProvider implements ISCMProvider, QuickDiffProvider {
	...
	readonly onDidChangeCommitTemplate: Event<string> =
		this._onDidChangeCommitTemplate.event;

	private readonly _onDidChangeStatusBarCommands = new Emitter<
		readonly Command[]
	>();
	get onDidChangeStatusBarCommands(): Event<readonly Command[]> {
	...
	readonly onDidChangeHistoryProvider: Event<void> =
		this._onDidChangeHistoryProvider.event;
	...
	$onDidChangeHistoryProviderCurrentHistoryItemGroup(
		...
		this._historyProvider.currentHistoryItemGroup =
			currentHistoryItemGroup ?? undefined;
	}
...
class MainThreadSCMInputBoxValueProvider implements ISCMInputValueProvider {
	...
	provideValue(
		repositoryId: string,
		context: ISCMInputValueProviderContext[]
	): Promise<string | undefined> {
		return this.proxy.$provideInputBoxValue(
			this.handle,
			repositoryId,
			context
		);
...
export class MainThreadSCM implements MainThreadSCMShape {
	...
	$registerSourceControlInputBoxValueProvider(
		...
		const provider = new MainThreadSCMInputBoxValueProvider(
			this._proxy,
			handle,
			label,
			getIconFromIconDto(icon)
		);
		const disposable =
			this.scmService.registerSCMInputValueProvider(provider);
	...
	$setInputBoxActionButton(
		...
		repository.input.actionButton = actionButton
			? {
					...actionButton,
					icon: getSCMInputBoxActionButtonIcon(actionButton),
			  }
			: undefined;
	}
