	...
	implements MainThreadLanguageFeaturesShape
{
		...
		this._registrations.set(
			...
			>{
				provideHover: (
					...
					return this._proxy.$provideHover(
						handle,
						model.uri,
						position,
						token
	...
	$registerEvaluatableExpressionProvider(
		...
		this._registrations.set(
			...
			this._languageFeaturesService.evaluatableExpressionProvider.register(
				selector,
				<languages.EvaluatableExpressionProvider>{
		...
		this._registrations.set(
			...
			combinedDisposable(
				this._languageFeaturesService.documentHighlightProvider.register(
					...
					<languages.DocumentHighlightProvider>{
						provideDocumentHighlights: (
							...
							languages.DocumentHighlight[] | undefined
						> => {
				...
				this._languageFeaturesService.multiDocumentHighlightProvider.register(
					selector,
					<languages.MultiDocumentHighlightProvider>{
						provideMultiDocumentHighlights: (
							model: ITextModel,
							position: EditorPosition,
							otherModels: ITextModel[],
							token: CancellationToken
						): Promise<Map<URI, languages.DocumentHighlight[]>> => {
							const word = model.getWordAtPosition({
								lineNumber: position.lineNumber,
								column: position.column,
							});

							const res = this._proxy.$provideDocumentHighlights(
								handle,
								model.uri,
								position,
								token
	...
	$registerDocumentOnDropEditProvider(
		...
		this._registrations.set(
			handle,
			combinedDisposable(
				this._languageFeaturesService.documentOnDropEditProvider.register(
					selector,
					provider
				),
				toDisposable(() =>
		...
		if (metadata.supportsPaste) {
			this.provideDocumentPasteEdits = async (
				...
				try {
					...
					const result = await this._proxy.$providePasteEdits(
						this._handle,
						request.id,
						model.uri,
						selections,
					...
					return {
						...result,
						additionalEdit: result.additionalEdit
							? reviveWorkspaceEditDto(
									result.additionalEdit,
									this._uriIdentService,
									(dataId) =>
...
class MainThreadDocumentOnDropEditProvider
	implements languages.DocumentOnDropEditProvider
{
	private readonly dataTransfers = new DataTransferFileCache();
	...
	async provideDocumentOnDropEdits(
		...
		dataTransfer: IReadonlyVSDataTransfer,
		token: CancellationToken
	): Promise<languages.DocumentOnDropEdit | null | undefined> {
		const request = this.dataTransfers.add(dataTransfer);
		try {
			const dataTransferDto =
				await typeConvert.DataTransfer.from(dataTransfer);
			...
			const edit = await this._proxy.$provideDocumentOnDropEdits(
				this._handle,
				request.id,
				model.uri,
				position,
