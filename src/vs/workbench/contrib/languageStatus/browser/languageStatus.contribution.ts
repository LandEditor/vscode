...
class EditorStatusContribution implements IWorkbenchContribution {
	...
	constructor(
		...
		_storageService.onDidChangeValue(
			StorageScope.PROFILE,
			EditorStatusContribution._keyDedicatedItems,
			this._disposables
	...
	private _update(): void {
		...
		if (model.combined.length === 0) {
			...
			const text = EditorStatusContribution._severityToComboCodicon(
				first.severity
			);
			...
			for (const status of model.combined) {
				...
				ariaLabels.push(
					EditorStatusContribution._accessibilityInformation(status)
						.label
				);
			...
			const node = mainWindow.document.querySelector(
			...
			if (!userHasInteractedWithStatus) {
				const hoverTarget = mainWindow.document.querySelector(
	...
	private _renderStatus(
		...
		const severityText = EditorStatusContribution._severityToSingleCodicon(
			status.severity
		);
	...
	private static _asStatusbarEntry(item: ILanguageStatus): IStatusbarEntry {
		...
		return {
			...
			text: item.busy
				...
				: textValue,
			ariaLabel:
				EditorStatusContribution._accessibilityInformation(item).label,
