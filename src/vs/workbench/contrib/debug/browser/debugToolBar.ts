...
import {
	...
	debugToolBarBorder,
} from "vs/workbench/contrib/debug/browser/debugColors";
import {
	CONTINUE_ID,
	CONTINUE_LABEL,
...
export class DebugToolBar extends Themable implements IWorkbenchContribution {
	...
	private registerListeners(): void {
		...
		this._register(
			dom.addDisposableListener(mainWindow, dom.EventType.RESIZE, () =>
				this.setCoordinates()
			)
		);
			...
			dom.addDisposableGenericMouseUpListener(
				...
				(event: MouseEvent) => {
					...
					if (mouseClickEvent.detail === 2) {
						...
						this.setCoordinates(
							0.5 * mainWindow.innerWidth - 0.5 * widgetWidth,
							0
						);
						this.storePosition();
					}
	...
	private setCoordinates(x?: number, y?: number): void {
		...
		if (x === undefined) {
			...
			x =
				positionPercentage !== undefined
					? parseFloat(positionPercentage) * mainWindow.innerWidth
					: 0.5 * mainWindow.innerWidth - 0.5 * widgetWidth;
		}
		...
		if (y === undefined) {
			y = this.storageService.getNumber(
				DEBUG_TOOLBAR_Y_KEY,
				StorageScope.PROFILE,
				0
			);
