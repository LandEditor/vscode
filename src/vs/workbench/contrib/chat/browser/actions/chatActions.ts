...
class QuickChatGlobalAction extends Action2 {
	constructor() {
		super({
			...
			title: {
				value: localize("quickChat", "Quick Chat"),
				original: "Quick Chat",
			},
...
export function registerChatActions() {
	...
	registerEditorAction(
		class ChatAcceptInput extends EditorAction {
			constructor() {
				super({
	...
	registerEditorAction(
		class ChatSubmitSecondaryAgent extends EditorAction {
			constructor() {
				super({
				...
				super({
					...
					keybinding: {
						...
						when: ContextKeyExpr.and(
							CONTEXT_IN_CHAT_SESSION,
							ContextKeyExpr.not(EditorContextKeys.focus.key)
						),
					},
				});
			}
...
export function getHistoryAction(viewId: string, providerId: string) {
	return class HistoryAction extends ViewAction<ChatViewPane> {
		...
		async runInView(accessor: ServicesAccessor, view: ChatViewPane) {
			...
			const picks = items.map(
				(i) =>
					<IQuickPickItem & { chat: IChatDetail }>{
						label: i.title,
						chat: i,
						buttons: [
							{
