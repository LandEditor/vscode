...
async function executeMoveToAction(
	...
	switch (moveTo) {
		...
		case MoveToNewLocation.Window: {
			await openInNewWindow(instantiationService, editorGroupService, {
				target: { sessionId },
			});
		}
		default: {
...
async function openInNewWindow(
	intstantiationService: IInstantiationService,
	editorGroupService: IEditorGroupsService,
	options: IChatEditorOptions
) {
	const auxiliaryEditorPart =
		await editorGroupService.createAuxiliaryEditorPart();
	const chatEditorInput = intstantiationService.createInstance(
		ChatEditorInput,
		ChatEditorInput.getNewEditorUri(),
		options
	);
	await auxiliaryEditorPart.activeGroup.openEditor(chatEditorInput, {
		pinned: true,
	});
}
