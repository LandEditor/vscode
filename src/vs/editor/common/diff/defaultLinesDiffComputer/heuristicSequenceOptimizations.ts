...
function shiftSequenceDiffs(
	...
	for (let i = 0; i < sequenceDiffs.length; i++) {
		...
		const seq1ValidRange = new OffsetRange(
			prevDiff ? prevDiff.seq1Range.start + 1 : 0,
			nextDiff ? nextDiff.seq1Range.endExclusive - 1 : sequence1.length
		);
		const seq2ValidRange = new OffsetRange(
			prevDiff ? prevDiff.seq2Range.start + 1 : 0,
			nextDiff ? nextDiff.seq2Range.endExclusive - 1 : sequence2.length
		);
...
export function extendDiffsToEntireWordIfAppropriate(
	...
	function maybePushWordToAdditional() {
		...
		const originalLength1 =
			lastModifiedWord.s1Range.length - lastModifiedWord.deleted;
		const originalLength2 =
		...
		if (
			...
			additional.push(
				new SequenceDiff(
					lastModifiedWord.s1Range,
					lastModifiedWord.s2Range
				)
			);
		}
		...
		function processWord(s1Range: OffsetRange, s2Range: OffsetRange) {
			if (
				...
				if (
					...
					lastModifiedWord.s1Range =
						lastModifiedWord.s1Range.join(s1Range);
					lastModifiedWord.s2Range =
						lastModifiedWord.s2Range.join(s2Range);
				} else {
