...
import {
	LineRangeFragment,
	isSpace,
} from "vs/editor/common/diff/defaultLinesDiffComputer/utils";
...
function computeUnchangedMoves(
	...
	for (const change of changes) {
		for (
			...
			const key = `${hashedOriginalLines[i - 1]}:${
				hashedOriginalLines[i + 1 - 1]
			}:${hashedOriginalLines[i + 2 - 1]}`;
	...
	for (const change of changes) {
		...
		for (
			...
			const key = `${hashedModifiedLines[i - 1]}:${
				hashedModifiedLines[i + 1 - 1]
			}:${hashedModifiedLines[i + 2 - 1]}`;
