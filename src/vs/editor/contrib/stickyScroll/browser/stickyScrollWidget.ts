...
export class StickyScrollWidget extends Disposable implements IOverlayWidget {
	...
	setState(
		...
		foldingModel: FoldingModel | null,
		rebuildFromLine: number = Infinity
	): void {
		if (
			((!this._previousState && !state) ||
				(this._previousState && this._previousState.equals(state))) &&
	...
	private async _renderRootNode(
		previousStickyLines: RenderedStickyLine[],
		foldingModel: FoldingModel | null,
		rebuildFromLine: number = Infinity
	): Promise<void> {
		const layoutInfo = this._editor.getLayoutInfo();
		for (const [index, line] of this._lineNumbers.entries()) {
			...
			const stickyLine =
				line >= rebuildFromLine ||
				previousStickyLine?.lineNumber !== line
	...
	private _updateTopAndZIndexOfStickyLine(
		...
		const lastLineTop = `${
			index * this._lineHeight +
			this._lastLineRelativePosition +
			(stickyLine.foldingIcon?.isCollapsed ? 1 : 0)
		}px`;
	...
	private _updateMinContentWidth() {
		...
		for (const stickyLine of this._stickyLines) {
			if (
				stickyLine.lineDomNode.scrollWidth > this._minContentWidthInPx
			) {
