...
impl CodeVersionManager {
	...
	pub async fn try_get_entrypoint(&self, version:&RequestedVersion) -> Option<PathBuf> {
		...
		// For simple quality requests, see if that's installed already on the
		// system
...
pub fn prompt_to_install(version:&RequestedVersion) {
	...
	if let RequestedVersion::Default = version {
		if let Some(uri) = PRODUCT_DOWNLOAD_URL {
			// todo: on some platforms, we may be able to help automate
			// installation. For example, we can unzip the app ourselves on
...
fn detect_installed_program(log:&log::Logger) -> io::Result<Vec<PathBuf>> {
	...
	// _Much_ slower detection using the system_profiler (~10s for me). While
	// the profiler can output nicely structure plist xml, pulling in an xml
	// parser just for this is overkill. The default output looks something
	// like...
	//
	...
	// 2.app
	//
	// So, use a simple state machine that looks for the first line, and then
...
fn detect_installed_program(log:&log::Logger) -> io::Result<Vec<PathBuf>> {
	...
	for dir in path.split(':') {
		...
		// note: intentionally store the non-canonicalized version, since if
		// it's a symlink, (1) it's probably desired to use it and (2)
		// resolving the link breaks snap installations.
		output.push(target);
	}
...
mod tests {
	...
	fn test_detect_installed_program() {
		// developers can run this test and debug output manually; VS Code will
		// not be installed in CI, so the test only makes sure it doesn't
		// error out
