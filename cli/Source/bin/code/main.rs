...
use opentelemetry::{sdk::trace::TracerProvider as SdkTracerProvider, trace::TracerProvider};
...
async fn main() -> Result<(), std::convert::Infallible> {
	...
	// gets a command context with the global logger installer. Usually what
	// most commands want.
	macro_rules! context {
		() => {{
	...
	let result = match parsed {
		...
		| args::AnyCli::Integrated(args::IntegratedCli { core: c, .. }) => {
			match c.subcommand {
				...
				Some(args::Commands::Version(version_args)) => {
					match version_args.subcommand {
				...
				Some(args::Commands::Tunnel(tunnel_args)) => {
					match tunnel_args.subcommand {
						Some(args::TunnelSubcommand::Prune) => tunnels::prune(context!()).await,
						Some(args::TunnelSubcommand::Unregister) => {
							tunnels::unregister(context!()).await
						},
...
async fn start_code(context:CommandContext, args:Vec<String>) -> Result<i32, AnyError> {
	// todo: once the integrated CLI takes the place of the Node.js CLI, this
	// should redirect to the current installation without using the
	// CodeVersionManager.
