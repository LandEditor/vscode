...
pub struct ServeWebArgs {
	...
	/// be started without a user prompt.
	#[clap(long)]
...
pub struct CommandShellArgs {
	...
	/// Optional parent process id. If provided, the server will be stopped
	/// when the process of the given pid no longer exists
	#[clap(long, hide = true)]
...
pub struct InstallExtensionArgs {
	...
	/// Update to the latest version of the extension if it's already
	/// installed.
	#[clap(long)]
	pub force:bool,
}
