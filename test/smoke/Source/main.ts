...
if (!opts.web) {
	...
	if (!fs.existsSync(electronPath || "")) {
		fail(
			`Can't find VSCode at ${electronPath}. Please run VSCode once first (scripts/code.sh, scripts\\code.bat) and try again.`
		);
	}
	...
	if (opts.remote) {
		logger.log(
			`Running desktop remote smoke tests against ${electronPath}`
		);
	} else {
...
async function setupRepository(): Promise<void> {
	if (opts["test-repo"]) {
		...
		if (process.platform === "win32") {
			cp.execSync(
				`xcopy /E "${opts["test-repo"]}" "${workspacePath}"\\*`
			);
		} else {
		...
		if (!fs.existsSync(workspacePath)) {
			...
			const res = cp.spawnSync(
				"git",
				["clone", testRepoUrl, workspacePath],
				{ stdio: "inherit" }
			);
			if (!fs.existsSync(workspacePath)) {
				throw new Error(
					`Clone operation failed: ${res.stderr.toString()}`
				);
			}
		} else {
			logger.log("Cleaning test project repository...");
			cp.spawnSync("git", ["fetch"], {
				cwd: workspacePath,
				stdio: "inherit",
			});
...
async function ensureStableCode(): Promise<void> {
	...
	if (!stableCodePath) {
		...
		if (!stableVersion) {
			throw new Error(
				`Could not find suitable stable version for ${version}`
			);
		}
					...
					() =>
						vscodetest.download({
							...
							reporter: {
								...
								error: (error) =>
									logger.log(
										`download stable code error: ${error}`
									),
							},
						}),
...
describe(`VSCode Smoke Tests (${opts.web ? "Web" : "Electron"})`, () => {
	...
	} // Not stable on desktop/remote https://github.com/microsoft/vscode/issues/146811
