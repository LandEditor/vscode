		...
		async (
			...
			try {
				for (const log of logs) {
					const absoluteLogsPath = path.join(
						logsPath,
						log.relativePath
					);
...
async function launchServer(
	...
	if (process.env.VSCODE_REMOTE_SERVER_PATH) {
		...
		if (args.debug) {
			console.log(
				`Starting server out of sources from '${serverLocation}'`
			);
		}
	}
	...
	const serverProcess = cp.spawn(serverLocation, serverArgs, { env, stdio });
...
launchServer(args.browser).then(
