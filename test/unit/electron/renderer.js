...
Object.assign(globalThis, {
	...
	__writeFileInTests: (path, contents) =>
...
function initLoader(opts) {
	...
	const loaderConfig = {
		nodeRequire: require,
		catchError: true,
		baseUrl: bootstrap.fileUriFromPath(
...
function createCoverageReport(opts) {
	if (opts.coverage) {
		return coverage.createReport(
			opts.run || opts.runGlob,
			opts.coveragePath,
			opts.coverageFormats,
		);
	}
...
function loadTests(opts) {
	...
	for (const consoleFn of [
		...
		console[consoleFn.name] = (msg) => {
			if (
				!(
					_allowedTestOutput.some((a) => a.test(msg)) ||
					_allowedTestsWithOutput.has(currentTestTitle)
				)
			) {
	...
	loader.require(["vs/base/common/errors"], (errors) => {
		...
		teardown(() => {
			...
			if (errors.length) {
				for (const error of errors) {
					console.error(
						`Error: Test run should not have unexpected errors:\n${error}`,
					);
				}
				assert.ok(
					false,
...
class IPCReporter {
	constructor(runner) {
		...
		runner.on("suite", (suite) =>
			ipcRenderer.send("suite", serializeSuite(suite)),
		);
		runner.on("suite end", (suite) =>
			ipcRenderer.send("suite end", serializeSuite(suite)),
		);
		runner.on("test", (test) =>
			ipcRenderer.send("test", serializeRunnable(test)),
		);
		runner.on("test end", (test) =>
			ipcRenderer.send("test end", serializeRunnable(test)),
		);
		runner.on("hook", (hook) =>
			ipcRenderer.send("hook", serializeRunnable(hook)),
		);
		runner.on("hook end", (hook) =>
			ipcRenderer.send("hook end", serializeRunnable(hook)),
		);
		runner.on("pass", (test) =>
			ipcRenderer.send("pass", serializeRunnable(test)),
		);
		...
		runner.on("pending", (test) =>
			ipcRenderer.send("pending", serializeRunnable(test)),
		);
	}
}
