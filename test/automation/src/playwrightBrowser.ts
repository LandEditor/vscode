...
async function launchServer(options: LaunchOptions) {
	...
	const serverProcess = spawn(serverLocation, args, { env });
...
async function launchBrowser(options: LaunchOptions, endpoint: string) {
	...
	const browser = await measureAndLog(
		() =>
			playwright[options.browser ?? "chromium"].launch({
			...
			await measureAndLog(
				() =>
					context.tracing.start({
						screenshots:
							true /* remaining options are off for perf reasons */,
					}),
				"context.tracing.start()",
				logger
			);
	...
	if (options.verbose) {
		...
		context.on("requestfailed", (e) =>
			logger.log(
				`Playwright (Browser): context.on('requestfailed') [${e.failure()
					?.errorText} for ${e.url()}]`
			)
		);
		...
		page.on("console", (e) =>
			logger.log(
				`Playwright (Browser): window.on('console') [${e.text()}]`
			)
		);
		page.on("dialog", () =>
		...
		page.on("load", () =>
			logger.log(`Playwright (Browser): page.on('load')`)
		);
		page.on("popup", () =>
		...
		page.on("requestfailed", (e) =>
			logger.log(
				`Playwright (Browser): page.on('requestfailed') [${e.failure()
					?.errorText} for ${e.url()}]`
			)
		);
	}
	...
	page.on("crash", () =>
		logger.log("Playwright (Browser) ERROR: page crash")
	);
	...
	const gotoPromise = measureAndLog(
		() =>
			page.goto(
				`${endpoint}&${
					workspacePath.endsWith(".code-workspace")
						? "workspace"
						: "folder"
				}=${URI.file(workspacePath!).path}&payload=${payloadParam}`
			),
		"page.goto()",
		logger
	);
