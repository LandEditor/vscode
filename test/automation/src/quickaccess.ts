...
export class QuickAccess {
			...
			try {
				...
				await this.quickInput.waitForQuickInputElements(
					(elementNames) => {
						...
						if (
							typeof expectedFirstElementNameOrExpectedResultCount ===
							"number"
						) {
							if (
								elementNames.length ===
	...
	async runCommand(
		...
		const openCommandPalletteAndTypeCommand =
			async (): Promise<boolean> => {
				...
				const text =
					await this.quickInput.waitForQuickInputElementText();
