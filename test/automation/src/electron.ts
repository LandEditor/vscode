...
export async function resolveElectronConfiguration(
	...
	if (remote) {
		...
		args[0] = `--${
			workspacePath.endsWith(".code-workspace") ? "file" : "folder"
		}-uri=vscode-remote://test+test/${URI.file(workspacePath).path}`;
...
export function getBuildVersion(root: string): string {
	switch (process.platform) {
		...
		default:
			return require(join(root, "resources", "app", "package.json"))
				.version;
	}
}
