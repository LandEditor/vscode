...
export class Code {
					...
					(async () => {
						...
						while (!done) {
							...
							switch (retries) {
								...
								case 20: {
									this.logger.log(
										"Smoke test exit call did not terminate process after 5-10s, gracefully trying to exit the application again..."
									);
									this.driver.exitApplication();
									break;
								}
								...
								case 40: {
									this.logger.log(
										"Smoke test exit call did not terminate process after 20s, forcefully exiting the application..."
									);
									...
									treekill(pid, (err) => {
										try {
											...
											this.logger.log(
												"Failed to kill Electron process tree:",
												err?.message
											);
										} catch (error) {
								...
								case 60: {
									...
									this.logger.log(
										"Smoke test exit call did not terminate process after 30s, giving up"
									);
									resolve();
								}
							}
				...
				this.driver
					...
					.then((els) =>
						els.length > 0
							...
							: Promise.reject(
									new Error(
										"Element not found for textContent"
									)
							  )
					),
	...
	async waitForElement(
		selector: string,
		accept: (result: IElement | undefined) => boolean = (result) =>
			!!result,
		retryCount: number = 200
	): Promise<IElement> {
	...
	private async poll<T>(
		...
		while (true) {
			if (trial > retryCount) {
				...
				this.logger.log(
					`Timeout: ${timeoutMessage} after ${
						(retryCount * retryInterval) / 1000
					} seconds.`
				);
			...
			try {
				...
				lastError = Array.isArray(e.stack)
					? e.stack.join(os.EOL)
					: e.stack;
			}
