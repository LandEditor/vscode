...
export class Extensions extends Viewlet {
	...
	async searchForExtension(id: string): Promise<any> {
		...
		await this.code.waitForTypeInEditor(
			'div.extensions-viewlet[id="workbench.view.extensions"] .monaco-editor textarea',
			`@id:${id}`
		);
	...
	async installExtension(
		...
		if (waitUntilEnabled) {
			await this.code.waitForElement(
				`.extension-editor .monaco-action-bar .action-item:not(.disabled)[title="Disable this extension"]`
			);
		}
	}
}
