	...
	async selectTab(fileName: string): Promise<void> {
		...
		while (retries < 10) {
			...
			await this.code.dispatchKeybinding(
				process.platform === "darwin" ? "cmd+1" : "ctrl+1"
			); // make editor really active if click failed somehow

			try {
				await this.waitForEditorFocus(
					fileName,
					50 /* 50 retries * 100ms delay = 5s */
				);
				return;
			} catch (e) {
				error = e;
	...
	async waitForActiveTab(
		...
		await this.code.waitForElement(
			`.tabs-container div.tab.active${
				isDirty ? ".dirty" : ""
			}[aria-selected="true"][data-resource-name$="${fileName}"]`,
			undefined,
			retryCount
		);
	}
