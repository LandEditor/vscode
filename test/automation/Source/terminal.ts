...
export class Terminal {
	...
	async getTerminalGroups(): Promise<TerminalGroup[]> {
		...
		for (let i = 0; i < tabCount; i++) {
			const title = await this.code.waitForElement(
				`${Selector.Tabs}[data-index="${i}"] ${Selector.TabsEntry}`,
				(e) =>
					e?.textContent?.length ? e?.textContent?.length > 1 : false
			);
			const description: IElement | undefined =
	...
	private async assertTabExpected(
		...
		if (listIndex) {
			if (nameRegex) {
				await this.code.waitForElement(
					`${Selector.Tabs}[data-index="${listIndex}"] ${Selector.TabsEntry}`,
			...
			if (icon) {
				selector =
					selector === Selector.EditorTab
						? selector
						: `${selector} .codicon`;
				await this.code.waitForElement(
					`${selector}`,
					(singleTab) =>
						!!singleTab && !!singleTab.className.includes(icon)
				);
			}
		}
	}
	...
	async assertCommandDecorations(
		...
		if (expectedCounts) {
			...
			await this.code.waitForElements(
				...
				(decorations) =>
					decorations &&
					decorations.length === expectedCounts.placeholder
			);
		...
		if (customIcon) {
			await this.code.waitForElements(
				...
				true,
				(decorations) =>
					decorations && decorations.length === customIcon.count
			);
		}
	}
