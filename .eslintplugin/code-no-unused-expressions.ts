...
module.exports = {
	...
	create(context: eslint.Rule.RuleContext) {
		...
		const allowShortCircuit = config.allowShortCircuit;
		const allowTernary = config.allowTernary;
		const allowTaggedTemplates = config.allowTaggedTemplates;
			...
			ExpressionStatement(node: TSESTree.ExpressionStatement) {
				if (
					!(
						...
						isDirective(
							node,
							<TSESTree.Node[]>context.getAncestors(),
						)
					)
				) {
					context.report({
