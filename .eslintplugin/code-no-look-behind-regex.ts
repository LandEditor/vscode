...
function _containsLookBehind(pattern: string | unknown): boolean {
	...
	return (
		_positiveLookBehind.test(pattern) || _negativeLookBehind.test(pattern)
	);
}
			...
			["Literal[regex]"]: (node: any) => {
				...
				if (_containsLookBehind(pattern)) {
					context.report({
						node,
						message:
							"Look behind assertions are not yet supported in all browsers",
					});
				}
			},
			// new Regex("...")
			['NewExpression[callee.name="RegExp"] Literal']: (
				node: ESTree.Literal,
			) => {
