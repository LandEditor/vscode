...
export = new (class NoUnexternalizedStrings implements eslint.Rule.RuleModule {
	...
	readonly meta: eslint.Rule.RuleMetaData = {
		messages: {
			...
			duplicateKey:
				"Duplicate key '{{key}}' with different message value.",
			badMessage:
				"Message argument to '{{message}}' must be a string literal.",
		},
	};
	...
	create(context: eslint.Rule.RuleContext): eslint.Rule.RuleListener {
		...
		function visitLocalizeCall(node: TSESTree.CallExpression) {
			...
			const [keyNode, messageNode] = (<TSESTree.CallExpression>node)
				.arguments;
					...
					if (
						...
						if (
							...
							if (isStringLiteral(property.value)) {
								doubleQuotedStringLiterals.delete(
									property.value,
								);
			...
			if (!isStringLiteral(messageNode)) {
				context.report({
					...
					data: {
						message: context.getSourceCode().getText(<any>node),
					},
				});
			}
		}
				...
				if (values.length > 1) {
					for (let i = 1; i < values.length; i++) {
						if (
							context
								.getSourceCode()
								.getText(<any>values[i - 1].message) !==
							context
								.getSourceCode()
								.getText(<any>values[i].message)
						) {
							context.report({
