	...
	implements IWorkbenchContribution
{
	...
	private async migrateConfigurationsForFolder(
		...
		await Promise.all(
			migrations.map((migration) =>
				this.migrateConfigurationsForFolderAndOverride(migration, {
					resource: folder?.uri,
				})
			)
		);
	...
	private async migrateConfigurationsForFolderAndOverride(
		...
		const data = this.configurationService.inspect(
			migration.key,
			overrides
		);

		await this.migrateConfigurationForFolderOverrideAndTarget(
			migration,
			overrides,
			data,
			"userValue",
		...
		if (
			typeof overrides.overrideIdentifier === "undefined" &&
			typeof data.overrideIdentifiers !== "undefined"
		) {
			for (const overrideIdentifier of data.overrideIdentifiers) {
				await this.migrateConfigurationsForFolderAndOverride(
					migration,
					{ resource: overrides.resource, overrideIdentifier }
				);
			}
		}
	}
	...
	private async migrateConfigurationForFolderOverrideAndTarget(
		...
		const valueAccessor = (key: string) =>
			this.configurationService.inspect(key, overrides)[dataKey];
		const result = await migration.migrateFn(value, valueAccessor);
		const keyValuePairs: ConfigurationKeyValuePairs = Array.isArray(result)
			? result
...
export class DynamicWorkbenchConfigurationWorkbenchContribution
	extends Disposable
	implements IWorkbenchContribution
{
		...
		(async () => {
			if (!isWindows) {
				const remoteEnvironment =
					await remoteAgentService.getEnvironment();
