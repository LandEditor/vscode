...
class InspectContextKeysAction extends Action2 {
	constructor() {
		super({
			...
			title: {
				value: localize("inspect context keys", "Inspect Context Keys"),
...
class ToggleScreencastModeAction extends Action2 {
	...
	run(accessor: ServicesAccessor): void {
		...
		const onMouseDown = disposables.add(
			new DomEmitter(container, "mousedown", true)
		);
		const onMouseUp = disposables.add(
			new DomEmitter(container, "mouseup", true)
		);
		const onMouseMove = disposables.add(
			new DomEmitter(container, "mousemove", true)
		);
		...
		disposables.add(
			onMouseDown.event((e) => {
				mouseMarker.style.top = `${
					e.clientY - mouseIndicatorSize / 2
				}px`;
				mouseMarker.style.left = `${
					e.clientX - mouseIndicatorSize / 2
				}px`;
		...
		const onKeyDown = disposables.add(
			new DomEmitter(window, "keydown", true)
		);
		const onCompositionStart = disposables.add(
			new DomEmitter(window, "compositionstart", true)
		);
		const onCompositionUpdate = disposables.add(
			new DomEmitter(window, "compositionupdate", true)
		);
		const onCompositionEnd = disposables.add(
			new DomEmitter(window, "compositionend", true)
		);
		...
		disposables.add(
			onKeyDown.event((e) => {
				...
				if (
					...
					(commandDetails && (options.showKeybindings ?? true))
				) {
