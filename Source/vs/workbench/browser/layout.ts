...
import {
	...
	IViewSize,
	Sizing,
} from "vs/base/browser/ui/grid/grid";
	...
	implements IWorkbenchLayoutService
{
	...
	private readonly _onDidChangeCenteredLayout = this._register(
		new Emitter<boolean>()
	);
	...
	private readonly _onDidRemoveContainer = this._register(
		new Emitter<HTMLElement>()
	);
	...
	private registerLayoutListeners(): void {
		...
		this.editorGroupService.mainPart.whenRestored.then(() => {
			...
			this._register(
				this.editorService.onDidVisibleEditorsChange(showEditorIfHidden)
			);
			this._register(
				this.editorGroupService.onDidActivateGroup(showEditorIfHidden)
			);

			// Revalidate center layout when active editor changes: diff editor quits centered mode.
			this._register(
				this.editorService.onDidActiveEditorChange(() =>
					this.centerEditorLayout(
						this.stateModel.getRuntimeValue(
							LayoutStateKeys.EDITOR_CENTERED
						)
					)
				)
			);
		...
		// Title Menu changes
		this._register(
			this.titleService.onDidChangeCommandCenterVisibility(() =>
				this.doUpdateLayoutConfiguration()
			)
		);
		...
		// Theme changes
		this._register(
			this.themeService.onDidColorThemeChange(() => this.updateStyles())
		);

		// Window focus changes
		this._register(
			this.hostService.onDidChangeFocus((e) =>
				this.onWindowFocusChanged(e)
			)
		);
		...
		this._register(
			this.auxiliaryWindowService.onDidOpenAuxiliaryWindow(
				({ window, disposables }) => {
					...
					disposables.add(
						toDisposable(() =>
							this._onDidRemoveContainer.fire(window.container)
						)
					);
				}
			)
	...
	private onFullscreenChanged(): void {
		...
		if (this.state.runtime.fullscreen) {
			...
			const zenModeActive = this.stateModel.getRuntimeValue(
	...
	private updateWindowBorder(skipLayout: boolean = false) {
		...
		if (
			!this.state.runtime.fullscreen &&
			!this.state.runtime.maximized &&
			(activeBorder || inactiveBorder)
		) {
			windowBorder = true;

			// If the inactive color is missing, fallback to the active one
			const borderColor = this.state.runtime.hasFocus
				? activeBorder
	...
	protected restoreParts(): void {
		...
		layoutReadyPromises.push(
			(async () => {
				...
				if (this.state.initialization.layout?.editors) {
					this.editorGroupService.applyLayout(
				...
				layoutRestoredPromises.push(
					Promise.all([
						...
						this.editorGroupService.mainPart.whenRestored.finally(
							() =>
		...
		// Restore Zen Mode
		const zenModeWasActive = this.stateModel.getRuntimeValue(
			LayoutStateKeys.ZEN_MODE_ACTIVE
		);
	...
	private shouldShowTitleBar(): boolean {
		...
		if (
			this.configurationService.getValue<boolean>("window.commandCenter")
		) {
			return true;
		}
	...
	getMaximumEditorDimensions(): Dimension {
		const panelPosition = this.getPanelPosition();
		const isColumn =
			panelPosition === Position.RIGHT || panelPosition === Position.LEFT;
		const takenWidth =
			...
			(this.isVisible(Parts.SIDEBAR_PART)
				...
				: 0) +
			(this.isVisible(Parts.PANEL_PART) && isColumn
		...
		const takenHeight =
			(this.isVisible(Parts.TITLEBAR_PART)
		...
		const availableHeight =
			this._mainContainerDimension.height - takenHeight;
	...
	toggleZenMode(skipLayout?: boolean, restoring = false): void {
		...
		const setLineNumbers = (lineNumbers?: LineNumbersType) => {
			...
			if (!lineNumbers) {
				...
				for (const editorControl of this.editorService
		...
		if (this.stateModel.getRuntimeValue(LayoutStateKeys.ZEN_MODE_ACTIVE)) {
			...
			if (config.hideLineNumbers) {
				setLineNumbers("off");
				this.state.runtime.zenMode.transitionDisposables.add(
			...
			if (
				...
				this.state.runtime.zenMode.transitionDisposables.add(
					this.editorGroupService.enforcePartOptions({
				...
				this.configurationService.onDidChangeConfiguration((e) => {
					if (
						e.affectsConfiguration(
							WorkbenchLayoutSettings.ZEN_MODE_SILENT_NOTIFICATIONS
						)
					) {
			...
			this.storageService.onWillSaveState((willSaveState) => {
				if (willSaveState.reason === WillSaveStateReason.SHUTDOWN) {
					...
					const auxiliaryBarSize = this.stateModel.getRuntimeValue(
						...
						: this.workbenchGrid.getViewSize(
								this.auxiliaryBarPartView
						  ).width;
					this.stateModel.setInitializationValue(
	...
	centerEditorLayout(active: boolean, skipLayout?: boolean): void {
		...
		if (activeEditor instanceof DiffEditorInput) {
			...
			activeEditor?.hasCapability(EditorInputCapabilities.MultipleEditors)
		) {
		...
		this._onDidChangeCenteredLayout.fire(
			this.stateModel.getRuntimeValue(LayoutStateKeys.EDITOR_CENTERED)
		);
	}
		...
		switch (part) {
			...
			case Parts.EDITOR_PART:
				...
				if (this.editorGroupService.count === 1) {
					...
					const { width, height } =
						this.editorGroupService.getSize(activeGroup);
					this.editorGroupService.setSize(activeGroup, {
		...
		// If panel part becomes visible, show last active panel or default panel
			...
			if (!panelToOpen || !this.hasViews(panelToOpen)) {
				panelToOpen = this.viewDescriptorService
					...
					.find((viewContainer) => this.hasViews(viewContainer.id))
					?.id;
			}
	...
	getWindowBorderRadius(): string | undefined {
		return this.state.runtime.windowBorder && isMacintosh
			? "5px"
			: undefined;
	}
	...
	updateMenubarVisibility(skipLayout: boolean): void {
		...
		if (
			!skipLayout &&
			this.workbenchGrid &&
			shouldShowTitleBar !== this.isVisible(Parts.TITLEBAR_PART)
		) {
			this.workbenchGrid.setViewVisible(
				this.titleBarPartView,
				shouldShowTitleBar
	...
	setPanelPosition(position: Position): void {
		...
		if (newPositionValue !== oldPositionValue && !editorHidden) {
			...
			if (position === Position.BOTTOM) {
				...
				positionFromString(oldPositionValue) === Position.BOTTOM
			) {
				this.stateModel.setRuntimeValue(
					LayoutStateKeys.PANEL_LAST_NON_MAXIMIZED_HEIGHT,
					size.height
	...
	private arrangeMiddleSectionNodes(
		...
		if (
			...
			result.push({
				type: "branch",
				data: [
					this.arrangeEditorNodes(
						{
							editor: nodes.editor,
							sideBar: sideBarNextToEditor
								? nodes.sideBar
								: undefined,
...
const LayoutStateKeys = {
	// Editor
	EDITOR_CENTERED: new RuntimeStateKey<boolean>(
		"editor.centered",
		StorageScope.WORKSPACE,
	...
	load(): void {
		...
		LayoutStateKeys.PANEL_SIZE.defaultValue =
			(this.stateCache.get(LayoutStateKeys.PANEL_POSITION.name) ??
				LayoutStateKeys.PANEL_POSITION.defaultValue) === "bottom"
