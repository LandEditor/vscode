...
import { compare, format, splitLines } from "vs/base/common/strings";
...
} from "vs/editor/contrib/indentation/browser/indentation";
...
class EditorStatus extends Disposable {
	...
	private readonly currentProblemStatus = this._register(
	...
	private updateState(update: StateDelta): void {
		...
		if (this.toRender) {
			...
			this.delayedRender.value = runAtThisOrScheduleAtNextAnimationFrame(
				getWindowById(this.targetWindowId)?.window ?? mainWindow,
				() => {
	...
	implements IWorkbenchContribution
{
	constructor(
		...
		// Main Editor Status
		const mainInstantiationService = instantiationService.createChild(
		...
		this._register(
			editorGroupService.onDidCreateAuxiliaryEditorPart(
				({ part, instantiationService, disposables }) => {
					disposables.add(
...
export class ChangeLanguageAction extends Action2 {
	...
	constructor() {
		super({
			...
			title: {
				value: localize("changeMode", "Change Language Mode"),
				original: "Change Language Mode",
			},
			f1: true,
			keybinding: {
...
export class ChangeEncodingAction extends Action2 {
	constructor() {
		super({
			...
			title: {
				value: localize("changeEncoding", "Change File Encoding"),
