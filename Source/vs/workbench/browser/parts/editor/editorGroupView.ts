...
export class EditorGroupView extends Themable implements IEditorGroupView {
	...
	private createContainerToolbar(): void {
		...
		const updateContainerToolbar = () => {
			const actions: { primary: IAction[]; secondary: IAction[] } = {
				primary: [],
				secondary: [],
			};
	...
	private doMoveOrCopyEditorAcrossGroups(
		...
		if (!keepCopy) {
			this.doCloseEditor(
				editor,
				false /* do not focus next one behind if any */,
	...
	private async doCloseEditorWithConfirmationHandling(
		...
		this.doCloseEditor(
			editor,
			options?.preserveFocus ? false : undefined,
			internalOptions
		);
	...
	private doCloseEditor(
		editor: EditorInput,
		focusNext = this.groupsView.activeGroup === this,
		internalOptions?: IInternalEditorCloseOptions
	): void {
