...
function registerDiffEditorCommands(): void {
	...
	MenuRegistry.appendMenuItem(MenuId.CommandPalette, {
		command: {
			id: GOTO_NEXT_CHANGE,
			title: {
				value: localize("compare.nextChange", "Go to Next Change"),
				original: "Go to Next Change",
			},
		},
	});
	...
	MenuRegistry.appendMenuItem(MenuId.CommandPalette, {
		command: {
			...
			title: {
				...
				original: "Go to Previous Change",
			},
		},
	});
	...
	KeybindingsRegistry.registerCommandAndKeybindingRule({
		...
		primary: undefined,
		handler: (accessor) =>
			focusInDiffEditor(accessor, FocusTextDiffEditorMode.Modified),
	});
	...
	KeybindingsRegistry.registerCommandAndKeybindingRule({
		...
		primary: undefined,
		handler: (accessor) =>
			focusInDiffEditor(accessor, FocusTextDiffEditorMode.Original),
	});
	...
	KeybindingsRegistry.registerCommandAndKeybindingRule({
		...
		primary: undefined,
		handler: (accessor) =>
			focusInDiffEditor(accessor, FocusTextDiffEditorMode.Toggle),
	});
...
function registerCloseEditorCommands() {
	...
	KeybindingsRegistry.registerCommandAndKeybindingRule({
		...
		handler: (
			...
			if (
				commandsContext &&
				typeof commandsContext.groupId === "number"
			) {
				...
				groups.map(async (group) => {
					if (group) {
						...
						const editorsToClose = group
							...
							.filter(
								(editor) => !editorsToKeep.includes(editor),
							);
	...
	registerAction2(
		class extends Action2 {
			...
			async run(
				...
				if (editor instanceof SideBySideEditorInput) {
					await joinEditorInGroup(
						accessor,
						resourceOrContext,
						context,
					);
				} else if (editor) {
					await splitEditorInGroup(
						accessor,
						resourceOrContext,
						context,
					);
		...
		class extends Action2 {
			constructor() {
				super({
					...
					title: {
						value: localize("lockEditorGroup", "Lock Editor Group"),
	...
	KeybindingsRegistry.registerCommandAndKeybindingRule({
		...
		handler: (
			...
			if (
				...
				const group = editorGroupService.getGroup(
					commandsContext.groupId,
				);
				if (group) {
	...
	if (list instanceof List && list.getHTMLElement() === getActiveElement()) {
		const elementToContext = (
			...
			return {
				...
				editorIndex: group
					? group.getIndexOfEditor(element.editor)
					: -1,
			};
		};

		const onlyEditorGroupAndEditor = (
			e: IEditorIdentifier | IEditorGroup,
