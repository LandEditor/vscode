...
import {
	BreadcrumbsConfig,
	IBreadcrumbsService,
} from "vs/workbench/browser/parts/editor/breadcrumbs";
import {
	BreadcrumbsModel,
	FileElement,
	OutlineElement2,
...
import {
	...
	BreadcrumbsPicker,
} from "vs/workbench/browser/parts/editor/breadcrumbsPicker";
import {
	IEditorPartOptions,
			...
			const items = model
				...
				.map((element) =>
					element instanceof FileElement
						? new FileItem(model, element, options, this._labels)
	...
	private _onSelectEvent(event: IBreadcrumbsItemEvent): void {
		...
		this._contextViewService.showContextView({
			render: (parent: HTMLElement) => {
				...
				const zoomListener = PixelRatio.onDidChange(() =>
					this._contextViewService.hideContextView({ source: this })
				);
