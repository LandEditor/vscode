	...
	implements IWorkbenchContribution
{
	...
	private readonly waitingOnErrorAutoSaveWorkingCopies = new ResourceMap<{
		readonly workingCopy: IWorkingCopy;
		readonly reason: SaveReason;
	}>((resource) => this.uriIdentityService.extUri.getComparisonKey(resource));
	private readonly waitingOnErrorAutoSaveEditors = new ResourceMap<{
	...
	private onMarkerChanged(resources: readonly URI[]): void {
		for (const resource of resources) {
			...
			const workingCopyResult =
				this.waitingOnErrorAutoSaveWorkingCopies.get(resource);
			...
			// Waiting editors
				...
				const editorResult =
					this.waitingOnErrorAutoSaveEditors.get(resource);
				if (
