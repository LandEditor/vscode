...
import {
	...
	IToolbarActions,
	EditorTabsControl,
} from "vs/workbench/browser/parts/editor/editorTabsControl";
...
export class MultiEditorTabsControl extends EditorTabsControl {
	...
	private readonly closeEditorAction = this._register(
		this.instantiationService.createInstance(
			CloseOneEditorAction,
			CloseOneEditorAction.ID,
			CloseOneEditorAction.LABEL
		)
	);
				...
				onDragEnter: (e) => {
					...
					if (
						...
						if (Array.isArray(data)) {
							...
							if (
								this.groupView.id ===
									localDraggedEditor.groupId &&
								this.tabsModel.isLast(localDraggedEditor.editor)
							) {
								if (e.dataTransfer) {
									e.dataTransfer.dropEffect = "none";
								}
	...
	private registerTabListeners(
		...
		disposables.add(
			addDisposableListener(tab, EventType.MOUSE_UP, (e) => {
				EventHelper.stop(e);
		...
		// Drag support
		disposables.add(
			addDisposableListener(tab, EventType.DRAG_START, (e) => {
				const editor = this.tabsModel.getEditorByIndex(tabIndex);
				if (!editor) {
					return;
				}
				...
				onDragEnter: (e) => {
					...
					if (
						...
						if (Array.isArray(data)) {
							const localDraggedEditor = data[0].identifier;
							if (
								localDraggedEditor.editor ===
									this.tabsModel.getEditorByIndex(tabIndex) &&
								localDraggedEditor.groupId === this.groupView.id
							) {
								if (e.dataTransfer) {
									e.dataTransfer.dropEffect = "none";
								}
	...
	private updateDropFeedback(
		...
		const editor =
			typeof tabIndex === "number"
				...
				: undefined;
		const isActiveTab =
			isTab && !!editor && this.tabsModel.isActive(editor);
	...
	private redrawTab(
		...
		// Settings
		const tabActionsVisibility =
			isTabSticky && options.pinnedTabSizing === "compact"
				? "off" /* treat sticky compact tabs as tabCloseButton: 'off' */
				: options.tabCloseButton;
		for (const option of ["off", "left", "right"]) {
			tabContainer.classList.toggle(
				`tab-actions-${option}`,
				tabActionsVisibility === option
			);
		}
		...
		if (this.tabsModel.isActive(editor)) {
			...
			if (activeTabBorderColorBottom) {
				...
				tabContainer.style.setProperty(
					...
					activeTabBorderColorBottom.toString()
				);
			} else {
			...
			const activeTabBorderColorTop = allowBorderTop
				? this.getColor(
						isGroupActive
							? TAB_ACTIVE_BORDER_TOP
							: TAB_UNFOCUSED_ACTIVE_BORDER_TOP
			...
			if (activeTabBorderColorTop) {
				...
				tabContainer.style.setProperty(
					...
					activeTabBorderColorTop.toString()
				);
			} else {
	...
	layout(
		...
		if (this.visible) {
			...
			if (!this.layoutScheduler.value) {
				...
				this.layoutScheduler.value = {
					options,
					dispose: () => scheduledLayout.dispose(),
				};
			}
...
registerThemingParticipant((theme, collector) => {
	...
	if (
		...
		const makeTabHoverBackgroundRule = (
			...
			.monaco-workbench .part.editor > .content:not(.dragged-over) .editor-group-container${
				hasFocus ? ".active" : ""
			} > .title .tabs-container > .tab.sizing-shrink:not(.dragged):not(.sticky-compact):hover > .tab-label > .monaco-icon-label-container::after,
			.monaco-workbench .part.editor > .content:not(.dragged-over) .editor-group-container${
				hasFocus ? ".active" : ""
			} > .title .tabs-container > .tab.sizing-fixed:not(.dragged):not(.sticky-compact):hover > .tab-label > .monaco-icon-label-container::after {
				background: linear-gradient(to left, ${color}, transparent) !important;
			}

			.monaco-workbench .part.editor > .content.dragged-over .editor-group-container${
				hasFocus ? ".active" : ""
			} > .title .tabs-container > .tab.sizing-shrink:not(.dragged):not(.sticky-compact):hover > .tab-label > .monaco-icon-label-container::after,
			.monaco-workbench .part.editor > .content.dragged-over .editor-group-container${
				hasFocus ? ".active" : ""
		...
		const makeTabBackgroundRule = (
			...
			active: boolean
				...
				.monaco-workbench .part.editor > .content:not(.dragged-over) .editor-group-container${
					focused ? ".active" : ":not(.active)"
				} > .title .tabs-container > .tab.sizing-shrink${
					active ? ".active" : ""
				}:not(.dragged):not(.sticky-compact) > .tab-label > .monaco-icon-label-container::after,
				.monaco-workbench .part.editor > .content:not(.dragged-over) .editor-group-container${
					focused ? ".active" : ":not(.active)"
				} > .title .tabs-container > .tab.sizing-fixed${
					active ? ".active" : ""
				}:not(.dragged):not(.sticky-compact) > .tab-label > .monaco-icon-label-container::after {
