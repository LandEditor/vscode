...
export class PaneCompositeBar extends Disposable {
	...
	private onDidChangeViewContainerLocation(
		...
		if (to === this.location) {
			...
			if (
				...
				this.layoutService.isVisible(this.paneCompositePart.partId) &&
				visibleComposites.length
			) {
	...
	private toCompositeBarActionItem(
		...
		if (this.options.icon) {
			if (URI.isUri(icon)) {
				...
				hash.update(cssUrl);
				const iconId = `activity-${id.replace(
					/\./g,
					"-"
				)}-${hash.digest()}`;
	...
	private shouldBeHidden(
		...
		if (
			...
			!(this.environmentService.remoteAuthority && isNative)
		) {
			cachedViewContainer =
				cachedViewContainer ||
			...
			cachedViewContainers.map(
				({ id, pinned, order }) =>
					<IPinnedViewContainer>{
						id,
						pinned,
						visible: pinnedViewContainers.find(
