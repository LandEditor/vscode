	...
	implements IPaneCompositePart
{
	...
	private registerListeners(): void {
		...
		this._register(
			this.registry.onDidDeregister(
				async (viewletDescriptor: PaneCompositeDescriptor) => {
	...
	protected override createTitleArea(parent: HTMLElement): HTMLElement {
		...
		this.globalToolBar = this._register(
			new ToolBar(globalTitleActionsContainer, this.contextMenuService, {
	...
	private layoutEmptyMessage(): void {
		this.emptyPaneMessageElement?.classList.toggle(
			"visible",
			!!this.paneCompositeBar.value &&
	...
	protected getToolbarWidth(): number {
		...
		return (
			...
			5 +
			(this.globalToolBar?.getItemsWidth() ?? 0)
		); // 5px toolBar padding-left
	}
