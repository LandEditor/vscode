...
export class BrowserWindow extends Disposable {
	...
	private onWillShutdown(): void {
		...
		Event.toPromise(
			Event.any(
				Event.once(
					new DomEmitter(document.body, EventType.KEY_DOWN, true)
						.event
				),
				Event.once(
					new DomEmitter(document.body, EventType.MOUSE_DOWN, true)
						.event
				)
			)
		).then(async () => {
		...
		this.openerService.setDefaultExternalOpener({
			openExternal: async (href: string) => {
				...
				if (
					...
					for (const trustedPopupPrefix of this.environmentService
