...
export class WorkbenchContextKeysHandler extends Disposable {
	...
	constructor(
		...
		this.maximizedEditorGroupContext = MaximizedEditorGroupContext.bindTo(
			this.contextKeyService
		);
		...
		// Editor Area
		this.editorAreaVisibleContext = EditorAreaVisibleContext.bindTo(
			this.contextKeyService
		);
	...
	private registerListeners(): void {
		...
		this._register(
			this.editorGroupService.onDidChangeGroupMaximized((maximized) =>
				this.maximizedEditorGroupContext.set(maximized)
			)
		);
		...
		this._register(
			this.layoutService.onDidChangePartVisibility(() => {
				this.editorAreaVisibleContext.set(
					this.layoutService.isVisible(Parts.EDITOR_PART)
				);
				this.panelVisibleContext.set(
					this.layoutService.isVisible(Parts.PANEL_PART)
				);
	...
	private updateInputContextKeys(ownerDocument: Document): void {
		function activeElementIsInput(): boolean {
			return (
				...
				(ownerDocument.activeElement.tagName === "INPUT" ||
					ownerDocument.activeElement.tagName === "TEXTAREA")
			);
		}
	...
	private updateTitleBarContextKeys(): void {
		this.titleAreaVisibleContext.set(
			this.layoutService.isVisible(Parts.TITLEBAR_PART)
		);
