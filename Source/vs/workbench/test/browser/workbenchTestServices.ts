...
} from "vs/platform/remote/common/remoteSocketFactoryService";
import {
	IStorageService,
	StorageScope,
	StorageTarget,
...
import {
	...
	IAuxiliaryEditorPartCreateEvent,
...
import {
	EncodingOracle,
	IEncodingOverride,
} from "vs/workbench/services/textfile/browser/textFileService";
import {
	UTF8_with_bom,
	UTF16be,
	UTF16le,
} from "vs/workbench/services/textfile/common/encoding";
import {
	ITextEditorService,
	TextEditorService,
} from "vs/workbench/services/textfile/common/textEditorService";
...
} from "vs/workbench/services/untitled/common/untitledTextEditorService";
...
} from "vs/workbench/services/workingCopy/common/workingCopyBackup";
...
export function workbenchInstantiationService(
	...
	instantiationService.stub(IAudioCueService, {
		playAudioCue: async () => {},
		isEnabled(cue: unknown) {
			return false;
		},
	} as any);
	instantiationService.stub(
	...
	const hoverService = instantiationService.stub(
		IHoverService,
		instantiationService.createInstance(TestHoverService),
	);
	instantiationService.stub(
		IQuickInputService,
		disposables.add(
...
export class TestMenuService implements IMenuService {
	...
	createMenu(
		_id: MenuId,
		_scopedKeybindingService: IContextKeyService,
	): IMenu {
		return {
...
export class TestEditorGroupsService implements IEditorGroupsService {
	...
	onDidCreateAuxiliaryEditorPart: Event<IAuxiliaryEditorPartCreateEvent> =
		Event.None;
	...
	mergeAllGroups(_group: number | IEditorGroup): IEditorGroup {
		throw new Error("not implemented");
	}
	copyGroup(
		_group: number | IEditorGroup,
...
export class TestEditorService extends Disposable implements EditorServiceImpl {
	...
	public get activeTextEditorControl():
		| ICodeEditor
		| IDiffEditor
		| undefined {
		return this._activeTextEditorControl;
	}
	public set activeTextEditorControl(value:
		| ICodeEditor
		| IDiffEditor
		| undefined) {
...
export class InMemoryTestWorkingCopyBackupService extends BrowserWorkingCopyBackupService {
	...
	joinBackupResource(): Promise<void> {
		return new Promise((resolve) =>
			this.backupResourceJoiners.push(resolve),
		);
	}

	joinDiscardBackup(): Promise<void> {
		return new Promise((resolve) =>
			this.discardBackupJoiners.push(resolve),
		);
	}
...
export class RemoteFileSystemProvider implements IFileSystemProvider {
	...
	readonly onDidChangeCapabilities: Event<void> =
		this.wrappedFsp.onDidChangeCapabilities;

	readonly onDidChangeFile: Event<readonly IFileChange[]> = Event.map(
	...
	implements IFileEditorInput
{
	...
	constructor(public resource: URI, private _typeId: string) {
		super();
	}
...
export async function createEditorPart(
	...
	class TestEditorParts extends EditorParts {
		...
		protected override createMainEditorPart(): MainEditorPart {
			this.testMainPart = instantiationService.createInstance(
				TestEditorPart,
				this,
			);
	...
	const part = disposables.add(
		instantiationService.createInstance(TestEditorParts),
	).testMainPart;
	...
	implements IRemoteExtensionsScannerService
{
	...
	scanSingleExtension(): Promise<IExtensionDescription | null> {
		throw new Error("Method not implemented.");
	}
}
	...
	implements IWorkbenchExtensionManagementService
{
	...
	installVSIX(
		...
		installOptions?: InstallVSIXOptions | undefined,
