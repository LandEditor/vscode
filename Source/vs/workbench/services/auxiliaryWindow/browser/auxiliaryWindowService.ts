	...
	implements IAuxiliaryWindowService
{
	...
	async open(options?: { position?: IRectangle }): Promise<IAuxiliaryWindow> {
		...
		const { container, onDidLayout, onDidClose } = this.create(
			auxiliaryWindow,
			disposables
		);
	...
	private async doOpen(options?: {
		position?: IRectangle;
	}): Promise<AuxiliaryWindow | undefined> {
		let position: IRectangle | undefined = options?.position;
		if (!position) {
			const activeWindow = getActiveWindow();
			position = {
				x:
					activeWindow.screen.availWidth / 2 -
					BrowserAuxiliaryWindowService.DEFAULT_SIZE.width / 2,
				y:
					activeWindow.screen.availHeight / 2 -
					BrowserAuxiliaryWindowService.DEFAULT_SIZE.height / 2,
		...
		const auxiliaryWindow = window.open(
			"about:blank",
			undefined,
			`popup=yes,left=${position.x},top=${position.y},width=${position.width},height=${position.height}`
		);
	...
	protected create(
		...
		const { onDidLayout, onDidClose } = this.registerListeners(
			auxiliaryWindow,
			container,
			disposables
		);
	...
	private applyMeta(auxiliaryWindow: AuxiliaryWindow): void {
		...
		const originalCSPMetaTag = document.querySelector(
	...
	protected applyCSS(
		...
		for (const originalNode of document.head.querySelectorAll(
	...
	private applyHTML(
		...
		disposables.add(
			trackAttributes(document.body, auxiliaryWindow.document.body)
		);
		disposables.add(
			trackAttributes(this.layoutService.container, container, ["class"])
	...
	private registerListeners(
		...
		disposables.add(
			addDisposableListener(auxiliaryWindow, "unload", () => {
				onDidClose.fire();
			})
		);
		...
		disposables.add(
			addDisposableListener(auxiliaryWindow, EventType.RESIZE, () => {
		...
		if (isWeb) {
			disposables.add(
				addDisposableListener(container, EventType.DROP, (e) =>
			...
			disposables.add(
				addDisposableListener(container, EventType.CONTEXT_MENU, (e) =>
					EventHelper.stop(e, true)
				)
