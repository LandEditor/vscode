...
import type { IStoredWorkspaceFolder } from "vs/platform/workspaces/common/workspaces";
import {
...
import type { IJSONEditingService } from "vs/workbench/services/configuration/common/jsonEditing";
				...
				resources.map(async (resource) => {
					try {
						...
						this.logService.trace(
							`Error while resolving configuration file '${resource.toString()}': ${errors.getErrorMessage(
								error,
							)}`,
						);
						if (
