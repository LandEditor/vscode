...
} from "vs/workbench/services/environment/browser/environmentService";
...
suite("WorkspaceContextService - Folder", () => {
	...
	test("getWorkspaceFolder() - queries in workspace folder", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			const folder = joinPath(ROOT, folderName).with({
				query: "myquery=1",
			});
			await fileService.createFolder(folder);
	...
	(isMacintosh ? test.skip : test)(
		...
		() =>
			runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
				...
				assert.deepStrictEqual(
					[
						...(<IConfigurationChangeEvent>target.args[0][0])
							.affectedKeys,
					],
					["initialization.testSetting1"]
				);
		...
		() =>
			runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
				...
				assert.deepStrictEqual(
					(<IWorkspaceFoldersChangeEvent>(
						target.args[1][0]
					)).removed.map((folder_2) => folder_2.uri.toString()),
					[joinPath(ROOT, "a").toString()]
				);
				assert.deepStrictEqual(
...
suite("WorkspaceConfigurationService - Folder", () => {
	...
	test("update language configuration for multiple languages", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await testObject.updateValue(
				"configurationService.folder.languageSetting",
				"multiLangValue",
				{ overrideIdentifiers: ["deflang", "xyzlang"] },
				ConfigurationTarget.USER
			);
			assert.strictEqual(
				testObject.getValue(
	...
	test("update user configuration to default value when target is not passed", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			assert.strictEqual(
				testObject.inspect("configurationService.folder.testSetting")
					.userValue,
				undefined
			);
		}));
		...
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			assert.strictEqual(
				testObject.getValue(
					...
					{
						resource:
							workspaceContextService.getWorkspace().folders[0]
								.uri,
					}
				),
				"userValue"
				...
				[
					{
						path: ["settings"],
						value: {
							"configurationService.workspace.unknownSetting":
								"workspaceValue",
						},
					},
				],
...
suite("WorkspaceConfigurationService - Remote Folder", () => {
	...
	test("remote settings override globals", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await fileService.writeFile(
	...
	test("remote settings override globals after remote provider is registered on activation", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await fileService.writeFile(
				machineSettingsResource,
				VSBuffer.fromString(
	...
	test("remote settings override globals after remote environment is resolved", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			await fileService.writeFile(
				machineSettingsResource,
				VSBuffer.fromString(
