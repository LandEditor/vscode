	...
	implements IFilesConfigurationService
{
	...
	private readonly _onAutoSaveConfigurationChange = this._register(
		new Emitter<IAutoSaveConfiguration>()
	);
	readonly onAutoSaveConfigurationChange =
		this._onAutoSaveConfigurationChange.event;

	private readonly _onFilesAssociationChange = this._register(
		new Emitter<void>()
	);
	readonly onFilesAssociationChange = this._onFilesAssociationChange.event;

	private readonly _onReadonlyConfigurationChange = this._register(
		new Emitter<void>()
	);
	...
	protected onFilesConfigurationChange(
		...
		this.autoSaveAfterShortDelayContext.set(
			this.getAutoSaveMode() === AutoSaveMode.AFTER_SHORT_DELAY
		);
	...
	getAutoSaveMode(): AutoSaveMode {
		...
		if (
			typeof this.configuredAutoSaveDelay === "number" &&
