...
import { coalesce, tail } from "vs/base/common/arrays";
import type { IStringDictionary } from "vs/base/common/collections";
import { Emitter, type Event } from "vs/base/common/event";
import { type JSONVisitor, visit } from "vs/base/common/json";
import { Disposable, type IReference } from "vs/base/common/lifecycle";
...
export class DefaultSettings extends Disposable {
	...
	private parse(): ISettingsGroup[] {
		...
		const mostCommonlyUsed =
			this.getMostCommonlyUsedSettings(settingsGroups);
...
export class DefaultRawSettingsEditorModel extends Disposable {
	...
	constructor(private defaultSettings: DefaultSettings) {
		super();
		this._register(
			defaultSettings.onDidChange(() => (this._content = null)),
		);
	}
