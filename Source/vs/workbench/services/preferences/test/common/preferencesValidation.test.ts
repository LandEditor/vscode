...
import {
	createValidator,
	getInvalidTypeError,
} from "vs/workbench/services/preferences/common/preferencesValidation";
...
suite("Preferences Validation", () => {
	class Tester {
		...
		public accepts(input: any) {
			assert.strictEqual(
				...
				`Expected ${JSON.stringify(
					this.settings
				)} to accept \`${JSON.stringify(input)}\`. Got ${this.validator(
					input
				)}.`
			);
		}
		...
		public rejects(input: any) {
			assert.notStrictEqual(
				...
				`Expected ${JSON.stringify(
					this.settings
				)} to reject \`${JSON.stringify(input)}\`.`
			);
			return {
				withMessage: (message: string) => {
					...
					assert(
						...
						`Expected error of ${JSON.stringify(
							this.settings
						)} on \`${input}\` to contain ${message}. Got ${this.validator(
							input
						)}.`
					);
				},
	...
	test("getInvalidTypeError", () => {
		function testInvalidTypeError(
			...
			const message = `value: ${value}, type: ${JSON.stringify(
				type
			)}, expected: ${shouldValidate ? "valid" : "invalid"}`;
			if (shouldValidate) {
