...
import type { OutputChannel } from "vs/workbench/services/search/node/ripgrepSearchUtils";
	...
	provideTextSearchResults(
		...
		if (options.folder.scheme === Schemas.vscodeUserData) {
			...
			const translatedOptions = {
				...options,
				folder: options.folder.with({ scheme: Schemas.file }),
			};
			const progressTranslator = new Progress<TextSearchResult>((data) =>
				progress.report({
					...data,
