...
flakySuite("TextSearch-integration", () => {
	...
	test("Multiroot: e with folder exclude", () => {
		const config: ITextQuery = {
			...
			folderQueries: [
				{
					folder: URI.file(EXAMPLES_FIXTURES),
					excludePattern: makeExpression("**/e*.js"),
				},
				{ folder: URI.file(MORE_FIXTURES) },
			],
			contentPattern: { pattern: "e" },
	...
	test("Text: 语", () => {
		...
		return doSearchTest(config, 1).then((results) => {
			const matchRange = (<ITextSearchMatch>results[0].results?.[0])
				.ranges;
	...
	test("Search with context matches", () => {
		...
		return doSearchTest(config, 4).then((results) => {
			...
			assert.strictEqual(
				(<ITextSearchContext>results[2].results?.[0]).lineNumber,
				26,
			);
			assert.strictEqual(
			...
			assert.strictEqual(
				(<ITextSearchContext>results[3].results?.[0]).lineNumber,
				27,
			);
			assert.strictEqual(
				(<ITextSearchContext>results[3].results?.[0]).text,
				"",
			);
		});
	});
