	...
	return (prefix: string) => {
		...
		if (traverse) {
			...
			if (shouldMatch) {
				assert(
					traverses,
					`${ignoreFileLocation}: ${ignoreFile} should traverse ${
						isDir ? "dir" : "file"
					} ${prefix}${rawInput}`
				);
			} else {
				assert(
					!traverses,
					`${ignoreFileLocation}: ${ignoreFile} should not traverse ${
						isDir ? "dir" : "file"
					} ${prefix}${rawInput}`
				);
			}
			...
			if (shouldMatch) {
				assert(
					ignores,
					`${ignoreFileLocation}: ${ignoreFile} should ignore ${
						isDir ? "dir" : "file"
					} ${prefix}${rawInput}`
				);
			} else {
				assert(
					!ignores,
					`${ignoreFileLocation}: ${ignoreFile} should not ignore ${
						isDir ? "dir" : "file"
					} ${prefix}${rawInput}`
				);
			}
