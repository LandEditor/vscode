...
suite("TextSearchResult", () => {
	...
	function assertOneLinePreviewRangeText(
		...
		assert.strictEqual(
			result.preview.text.substring(
				(<SearchRange>result.preview.matches).startColumn,
				(<SearchRange>result.preview.matches).endColumn
			),
			text
		);
	}
	...
	test("one line of multiline match", () => {
		...
		assert.strictEqual(
			(<SearchRange>result.preview.matches).startLineNumber,
			0
		);
		assert.strictEqual(
			(<SearchRange>result.preview.matches).startColumn,
		...
		assert.strictEqual(
			(<SearchRange>result.preview.matches).endLineNumber,
			1
		);
	...
	test("compacts multiple ranges on long lines", () => {
		...
		assert.deepStrictEqual(result.preview.matches, [
