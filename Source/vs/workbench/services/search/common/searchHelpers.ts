...
import type { FindMatch, ITextModel } from "vs/editor/common/model";
import {
	type ITextQuery,
...
function getMatchStartEnd(match: ITextSearchMatch): {
	...
	const matchStartLine = Array.isArray(matchRanges)
		? matchRanges[0].startLineNumber
		: matchRanges.startLineNumber;
	const matchEndLine = Array.isArray(matchRanges)
		? matchRanges[matchRanges.length - 1].endLineNumber
		: matchRanges.endLineNumber;
