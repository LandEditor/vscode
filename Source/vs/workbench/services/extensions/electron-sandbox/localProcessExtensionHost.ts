...
export class NativeLocalProcessExtensionHost implements IExtensionHost {
	...
	protected async _start(): Promise<IMessagePassingProtocol> {
		const communication = this._toDispose.add(
			new ExtHostMessagePortCommunication(this._logService)
		);
		return this._startWithCommunication(communication);
	}

	protected async _startWithCommunication<T>(
		communication: IExtHostCommunication<T>
	): Promise<IMessagePassingProtocol> {
		const [
			extensionHostCreationResult,
			communicationPreparedData,
			portNumber,
			processEnv,
		] = await Promise.all([
		...
		const protocol = await communication.establishProtocol(
			communicationPreparedData,
...
export class ExtHostMessagePortCommunication
	extends Disposable
	implements IExtHostCommunication<void>
{
