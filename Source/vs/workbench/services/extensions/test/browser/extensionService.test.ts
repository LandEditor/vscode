...
} from "vs/workbench/services/extensions/common/extensionManifestPropertiesService";
...
suite("ExtensionService", () => {
	class MyTestExtensionService extends AbstractExtensionService {
		constructor(
			...
			const extensionHostFactory = new (class
				implements IExtensionHostFactory
			{
				createExtensionHost(
		...
		): Promise<Readonly<IRelaxedExtensionDescription> | null> {
			throw new Error("Method not implemented.");
		}
	...
	test("Extension host not disposed when vetoed (async)", async () => {
		...
		disposables.add(
			extService.onWillStop((e) =>
				e.veto(Promise.resolve(true), "test 2")
			)
		);
		disposables.add(
			extService.onWillStop((e) =>
				e.veto(Promise.resolve(false), "test 3")
			)
		);
