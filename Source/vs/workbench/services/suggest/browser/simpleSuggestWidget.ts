...
import type { SimpleCompletionItem } from "vs/workbench/services/suggest/browser/simpleCompletionItem";
import type {
	LineContext,
...
export class SimpleSuggestWidget extends Disposable {
	...
	private _layout(size: dom.Dimension | undefined): void {
		...
		if (this._preference === WidgetPositionPreference.Above) {
			this.element.domNode.style.top = `${
				this._cursorPosition.top - height - info.borderHeight
			}px`;
		} else {
			this.element.domNode.style.top = `${
				this._cursorPosition.top + this._cursorPosition.height
			}px`;
		}
