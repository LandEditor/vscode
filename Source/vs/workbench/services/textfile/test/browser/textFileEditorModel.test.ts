...
suite("Files - TextFileEditorModel", () => {
	...
	setup(() => {
		...
		disposables.add(
			<TextFileEditorModelManager>accessor.textFileService.files
		);
		disposables.add(
	...
	teardown(async () => {
		for (const textFileEditorModel of accessor.textFileService.files
			.models) {
	...
	test("save() and isDirty() - proper with check for mtimes", async function () {
		...
		if (isWeb) {
			...
			assert.ok(
				assertIsDefined(getLastResolvedFileStat(model1)).mtime >=
					m1Mtime
			);
			assert.ok(
				assertIsDefined(getLastResolvedFileStat(model2)).mtime >=
					m2Mtime
			);
		} else {
