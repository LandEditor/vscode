...
import { type ReadableStream, consumeStream } from "vs/base/common/stream";
import type { URI } from "vs/base/common/uri";
import { ICodeEditorService } from "vs/editor/browser/services/codeEditorService";
import { ILanguageService } from "vs/editor/common/languages/language";
import { PLAINTEXT_LANGUAGE_ID } from "vs/editor/common/languages/modesRegistry";
import type { ITextModel, ITextSnapshot } from "vs/editor/common/model";
import {
	...
	suggestFilename(languageId: string, untitledName: string) {
		...
		if (primaryExtension) {
			if (untitledExtension) {
				return `${untitledName.substring(
					0,
					untitledName.indexOf(untitledExtension),
				)}${primaryExtension}`;
			}
