...
class WorkbenchHostService extends Disposable implements IHostService {
	...
	get onDidChangeFocus(): Event<boolean> {
		...
		this._register(
			Event.filter(
				this.nativeHostService.onDidFocusWindow,
		...
		this._register(
			Event.filter(
				this.nativeHostService.onDidBlurWindow,
				(id) => id === this.nativeHostService.windowId,
				this._store
			)(() => emitter.fire(this.hasFocus))
		);
		...
		this._register(
			onDidRegisterWindow(({ window, disposables }) => {
				...
				const onVisibilityChange = disposables.add(
					new DomEmitter(window.document, "visibilitychange")
				);

				disposables.add(
					focusTracker.onDidFocus(() => emitter.fire(this.hasFocus))
				);
				disposables.add(
					focusTracker.onDidBlur(() => emitter.fire(this.hasFocus))
				);
				disposables.add(
					onVisibilityChange.event(() => emitter.fire(this.hasFocus))
				);
			})
		);
	...
	async moveTop(window: Window & typeof globalThis): Promise<void> {
		...
		return this.nativeHostService.moveWindowTop(
			isAuxiliaryWindow(window)
				? { targetWindowId: window.vscodeWindowId }
				: undefined
		);
	}
