...
suite("EditorGroupsService", () => {
	...
	test("copy/merge groups", async () => {
		...
		part.mergeGroup(rootGroup, rightGroup, {
	...
	test("moveEditor (same group)", async () => {
		...
		assert.strictEqual(
			(moveEvents[0] as IGroupEditorOpenEvent).editorIndex,
			0
		);
		assert.strictEqual(
		...
		assert.strictEqual(
			(moveEvents[1] as IGroupEditorOpenEvent).editorIndex,
			1
		);
		assert.strictEqual(
	...
	test("locked groups - auto locking via setting", async () => {
		...
		await configurationService.setUserConfiguration("workbench", {
			"editor": {
				"autoLockGroups": {
					"testEditorInputForEditorGroupService": true,
				},
			},
		});
