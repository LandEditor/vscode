...
suite("EditorService", () => {
	...
	async function testOpenBasics(service: IEditorService) {
		...
		disposables.add(
			service.onDidActiveEditorChange(() => {
		...
		disposables.add(
			service.onDidVisibleEditorsChange(() => {
		...
		assert.strictEqual(
			input,
			service.getEditors(EditorsOrder.MOST_RECENTLY_ACTIVE)[0].editor
		);
		assert.strictEqual(
			input,
			service.getEditors(EditorsOrder.SEQUENTIAL)[0].editor
		);
		...
		assert.strictEqual(
			0,
			service.getEditors(EditorsOrder.MOST_RECENTLY_ACTIVE).length
		);
		assert.strictEqual(
			0,
			service.getEditors(EditorsOrder.SEQUENTIAL).length
		);
		...
		assert.strictEqual(
			input,
			service.getEditors(EditorsOrder.MOST_RECENTLY_ACTIVE)[1].editor
		);
		assert.strictEqual(
			input,
			service.getEditors(EditorsOrder.SEQUENTIAL)[0].editor
		...
		assert.strictEqual(
			otherInput,
			service.getEditors(EditorsOrder.SEQUENTIAL)[1].editor
		);
		assert.strictEqual(service.visibleEditorPanes.length, 1);
		assert.strictEqual(service.isOpened(input), true);
		assert.strictEqual(
		...
		const allSequentialEditors = service.getEditors(
		...
		const sequentialEditorsExcludingSticky = service.getEditors(
			EditorsOrder.SEQUENTIAL,
			{ excludeSticky: true }
		);
		...
		const mruEditorsExcludingSticky = service.getEditors(
	...
	async function testOpenEditors(useOpenEditors: boolean) {
		...
		let lastEditorFactoryEditor: IResourceEditorInput | undefined =
			undefined;
		...
		// untyped
		{
			...
			// untyped resource editor, options (sticky: true, preserveFocus: true), no group
			{
				...
				assert.strictEqual(
					(
						lastEditorFactoryEditor as IResourceEditorInput
					).resource.toString(),
					untypedEditor.resource.toString()
				);
				assert.strictEqual(
	...
	test("openEditors() handles workspace trust (typed editors)", async () => {
		...
		try {
			...
			accessor.workspaceTrustRequestService.requestOpenUrisHandler =
				async (uris) => WorkspaceTrustUriResponse.OpenInNewWindow;
			...
			// Trust: allow
			accessor.workspaceTrustRequestService.requestOpenUrisHandler =
				async (uris) => WorkspaceTrustUriResponse.Open;
	...
	test("openEditors() ignores trust when `validateTrust: false", async () => {
		...
		try {
			// Trust: cancel
			accessor.workspaceTrustRequestService.requestOpenUrisHandler =
				async (uris) => WorkspaceTrustUriResponse.Cancel;
		...
		try {
			...
			assert.strictEqual(
				trustEditorUris.some(
					(uri) =>
						uri.toString() ===
						otherInput.original.resource?.toString()
				),
				true
			);
			assert.strictEqual(
				trustEditorUris.some(
					(uri) =>
						uri.toString() ===
						otherInput.modified.resource?.toString()
				),
				true
			);
		} finally {
