...
import {
	EditorWorkerClient,
	EditorWorkerHost,
} from "vs/editor/browser/services/editorWorkerService";
...
import type { LanguageDetectionSimpleWorker } from "vs/workbench/services/languageDetection/browser/languageDetectionSimpleWorker";
import {
	...
	async sendTelemetryEvent(
		...
		type LanguageDetectionStatsClassification = {
			...
			languages: {
				...
				comment: "The languages that are guessed";
			};
			confidences: {
				classification: "SystemMetaData";
				purpose: "FeatureInsight";
				comment: "The confidences of each language guessed";
			};
			timeSpent: {
				classification: "SystemMetaData";
				purpose: "FeatureInsight";
...
export class LanguageDetectionWorkerClient extends EditorWorkerClient {
	private workerPromise:
		| Promise<IWorkerClient<LanguageDetectionSimpleWorker>>
		| undefined;
	...
	private _getOrCreateLanguageDetectionWorker(): Promise<
		IWorkerClient<LanguageDetectionSimpleWorker>
	> {
	...
	public async detectLanguage(
		...
		const modelId = await (await this._getProxy()).detectLanguage(
			resource.toString(),
			langBiases,
			preferHistory,
			supportedLangs,
