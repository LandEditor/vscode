...
import {
	TreeView,
	TreeViewPane,
} from "vs/workbench/browser/parts/views/treeView";
...
} from "vs/workbench/services/userDataProfile/browser/extensionsResource";
import {
	GlobalStateResource,
	GlobalStateResourceExportTreeItem,
	GlobalStateResourceImportTreeItem,
	GlobalStateResourceTreeItem,
} from "vs/workbench/services/userDataProfile/browser/globalStateResource";
...
} from "vs/workbench/services/userDataProfile/browser/keybindingsResource";
import {
	SettingsResource,
...
} from "vs/workbench/services/userDataProfile/browser/snippetsResource";
import {
	TasksResource,
	TasksResourceTreeItem,
} from "vs/workbench/services/userDataProfile/browser/tasksResource";
import {
...
import {
	DEFAULT_ICON,
	ICONS,
} from "vs/workbench/services/userDataProfile/common/userDataProfileIcons";
...
export class UserDataProfileImportExportService
	extends Disposable
	implements IUserDataProfileImportExportService, IURLHandler
{
	...
	constructor(
		...
		this.isProfileExportInProgressContextKey =
			IS_PROFILE_EXPORT_IN_PROGRESS_CONTEXT.bindTo(contextKeyService);
		this.isProfileImportInProgressContextKey =
			IS_PROFILE_IMPORT_IN_PROGRESS_CONTEXT.bindTo(contextKeyService);

		this.viewContainer = Registry.as<IViewContainersRegistry>(
	...
	private isProfileURL(uri: URI): boolean {
		return (
			...
			new RegExp(
				`^${UserDataProfileImportExportService.PROFILE_URL_AUTHORITY_PREFIX}`,
			).test(uri.authority)
		);
	}
	...
	async importProfile(
		...
		if (this.isProfileImportInProgressContextKey.get()) {
			this.notificationService.warn(
				"Profile import already in progress.",
			);
			return;
		}
		...
		disposables.add(
			toDisposable(() =>
				this.isProfileImportInProgressContextKey.set(false),
			),
		);
		...
		try {
			...
			const profileTemplate = await this.progressService.withProgress(
				{
					...
					title: localize(
						...
						options?.mode
							? localize("preview profile", "Preview Profile")
							: localize("import profile", "Create Profile"),
					),
				},
			...
			if (mode === "preview") {
				...
				await this.createAndSwitch(
					profileTemplate,
					false,
					true,
					options,
					localize("create profile", "Create Profile"),
				);
			} else if (mode === "both") {
				await this.importAndPreviewProfile(
					uri,
					profileTemplate,
					options,
				);
	...
	private saveProfile(
		...
		source?: IUserDataProfile | URI | IUserDataProfileTemplate,
	): Promise<void>;
	private async saveProfile(
		profile?: IUserDataProfile,
		source?: IUserDataProfile | URI | Mutable<IUserDataProfileTemplate>,
	): Promise<void> {
		...
		if (profile) {
			this.telemetryService.publicLog2<{}, SaveProfileInfoClassification>(
				"userDataProfile.startEdit",
			);
		} else {
			this.telemetryService.publicLog2<
				CreateProfileInfoEvent,
				CreateProfileInfoClassification
			>("userDataProfile.startCreate", createProfileTelemetryData);
		}

		const disposables = new DisposableStore();
		const title = profile
			? localize("save profile", "Edit {0} Profile...", profile.name)
			: localize("create new profle", "Create New Profile...");

		const settings: IQuickPickItem & { id: ProfileResourceType } = {
			id: ProfileResourceType.Settings,
			label: localize("settings", "Settings"),
			picked: !profile?.useDefaultFlags?.settings,
		};
		const keybindings: IQuickPickItem & { id: ProfileResourceType } = {
			id: ProfileResourceType.Keybindings,
			label: localize("keybindings", "Keyboard Shortcuts"),
			picked: !profile?.useDefaultFlags?.keybindings,
		};
		const snippets: IQuickPickItem & { id: ProfileResourceType } = {
			id: ProfileResourceType.Snippets,
			label: localize("snippets", "User Snippets"),
			picked: !profile?.useDefaultFlags?.snippets,
		};
		const tasks: IQuickPickItem & { id: ProfileResourceType } = {
			id: ProfileResourceType.Tasks,
			label: localize("tasks", "User Tasks"),
			picked: !profile?.useDefaultFlags?.tasks,
		};
		const extensions: IQuickPickItem & { id: ProfileResourceType } = {
			id: ProfileResourceType.Extensions,
			label: localize("extensions", "Extensions"),
			picked: !profile?.useDefaultFlags?.extensions,
		};
		...
		quickPick.value =
			profile?.name ??
			(isUserDataProfileTemplate(source)
				? this.generateProfileName(source.name)
				: "");
		...
		disposables.add(
			quickPick.onDidChangeSelection((items) => {
				...
				for (const resource of resources) {
					...
					const description = resource.picked
						? undefined
						: localize(
								"use default profile",
		...
		if (
			icon.id !== DEFAULT_ICON.id &&
			!ICONS.some(({ id }) => id === icon.id) &&
			!getAllCodicons().some(({ id }) => id === icon.id)
		) {
			icon = DEFAULT_ICON;
		}
		let result:
		...
		disposables.add(
			Event.any(
				...
				result = {
					...
					icon: icon.id === DEFAULT_ICON.id ? null : icon.id,
				};
				quickPick.hide();
				quickPick.severity = Severity.Ignore;
				quickPick.validationMessage = undefined;
		...
		DOM.append(
			...
			DOM.$(".profile-icon-label", undefined, localize("icon", "Icon:")),
		);
		const profileIconElement = DOM.append(
			profileIconContainer,
			DOM.$(`.profile-icon${ThemeIcon.asCSSSelector(icon)}`),
		);
		profileIconElement.tabIndex = 0;
		profileIconElement.role = "button";
		profileIconElement.ariaLabel = localize(
			"select icon",
			"Icon: {0}",
			icon.id,
		);
		...
		const updateIcon = (updated: ThemeIcon | undefined) => {
			...
			profileIconElement.className = `profile-icon ${ThemeIcon.asClassName(
				icon,
			)}`;
			profileIconElement.ariaLabel = localize(
				"select icon",
		...
		if (!profile && !isUserDataProfileTemplate(source)) {
			...
			const profileOptions: (ISelectOptionItem & {
				id?: string;
				source?: IUserDataProfile | URI;
			})[] = [];
			profileOptions.push({ text: localize("empty profile", "None") });
			const templates =
				await this.userDataProfileManagementService.getBuiltinProfileTemplates();
			if (templates.length) {
				profileOptions.push({
					...separator,
			...
			const selectBox = disposables.add(
			...
			selectBox.render(
				DOM.append(
					profileTypeContainer,
					DOM.$(".profile-type-select-container"),
				),
			);

			if (profileOptions[initialIndex].source) {
				quickPick.value = this.generateProfileName(
					profileOptions[initialIndex].text,
				);
			}
			...
			const updateOptions = () => {
				...
				for (const resource of resources) {
					resource.picked =
						option.source && !(option.source instanceof URI)
							? !option.source?.useDefaultFlags?.[resource.id]
							: true;
				}
				updateIcon(
					!(option.source instanceof URI) && option.source?.icon
						? ThemeIcon.fromId(option.source.icon)
						: undefined,
				);
				update();
			};
		...
		if (!result) {
			if (profile) {
				this.telemetryService.publicLog2<
					{},
					SaveProfileInfoClassification
				>("userDataProfile.cancelEdit");
			} else {
				this.telemetryService.publicLog2<
					CreateProfileInfoEvent,
					CreateProfileInfoClassification
				>("userDataProfile.cancelCreate", createProfileTelemetryData);
			}
			return;
		}
		...
		try {
			const useDefaultFlags: UseDefaultProfileFlags | undefined =
				result.items.length === resources.length
					? undefined
					: {
			...
			if (profile) {
				...
				await this.importProfile(source, {
					mode: "apply",
					name: result.name,
					useDefaultFlags,
				...
				await this.createFromProfile(source, result.name, {
					useDefaultFlags,
					icon: result.icon ? result.icon : undefined,
				});
				...
				this.telemetryService.publicLog2<
					...
					CreateProfileInfoClassification
				>(
					"userDataProfile.createFromExternalTemplate",
					createProfileTelemetryData,
				);
				await this.createAndSwitch(
					source,
				...
				await this.userDataProfileManagementService.createAndEnterProfile(
					result.name,
					{
						useDefaultFlags,
						icon: result.icon ? result.icon : undefined,
	...
	async showProfileContents(): Promise<void> {
		const view = this.viewsService.getViewWithId(
			EXPORT_PROFILE_PREVIEW_VIEW,
		);
		if (view) {
		...
		try {
			...
			const exportAction = new BarrierAction(
				barrier,
				new Action(
					"export",
			...
			const closeAction = new BarrierAction(
				barrier,
				new Action("close", localize("close", "Close")),
				this.notificationService,
			);
			await this.showProfilePreviewView(
				EXPORT_PROFILE_PREVIEW_VIEW,
				userDataProfilesExportState.profile.name,
				exportAction,
				closeAction,
				true,
				userDataProfilesExportState,
		...
		try {
			...
			await this.progressService.withProgress(
				...
				async (progress) => {
					...
					const createdProfile = await this.doCreateProfile(
						profileTemplate,
						false,
						false,
						{
					...
					if (createdProfile) {
						...
						await this.instantiationService
							...
							.copy(profile, createdProfile, false);
	...
	private async doExportProfile(
		...
		disposables.add(
			toDisposable(() =>
				this.isProfileExportInProgressContextKey.set(false),
			),
		);
	...
	private async resolveProfileTemplate(
		uri: URI,
		options?: IProfileImportOptions,
		...
		const profileTemplate: Mutable<IUserDataProfileTemplate> =
	...
	private async importAndPreviewProfile(
		uri: URI,
		...
		try {
			const userDataProfileImportState = disposables.add(
				this.instantiationService.createInstance(
			...
			profileTemplate =
				await userDataProfileImportState.getProfileTemplateToImport();

			const importedProfile = await this.createAndSwitch(
				profileTemplate,
				true,
				false,
				options,
			...
			const barrier = new Barrier();
			const importAction = this.getCreateAction(
				barrier,
				userDataProfileImportState,
			);
			...
			const view = await this.showProfilePreviewView(
				IMPORT_PROFILE_PREVIEW_VIEW,
			...
			message.appendMarkdown(
				`[${localize(
					"learn more",
					"Learn more",
				)}](https://aka.ms/vscode-extension-marketplace#_can-i-trust-extensions-from-the-marketplace).`,
			);
							...
							return that.progressService.withProgress(
								...
								async (progress) => {
									...
									if (profileTemplate.extensions) {
										await that.instantiationService
											.createInstance(ExtensionsResource)
											.apply(
	...
	private async previewProfile(
		...
		try {
			...
			if (userDataProfileImportState.isEmpty()) {
				...
				const barrier = new Barrier();
				const cancelAction = new BarrierAction(
					barrier,
	...
	private getCreateAction(
		barrier: Barrier,
		userDataProfileImportState: UserDataProfileImportState,
		cancelAction?: IAction,
	): IAction {
		const importAction = new BarrierAction(
			barrier,
			new Action(
				"title",
				localize("import", "Create Profile"),
				undefined,
				true,
				async () => {
	...
	private async createAndSwitch(
		...
		return this.progressService.withProgress(
			...
			async (progress) => {
				...
				const reportProgress = (message: string) =>
					progress.report({ message: `${title}: ${message}` });
				const profile = await this.doCreateProfile(
					profileTemplate,
					temporaryProfile,
					extensions,
	...
	private async resolveProfileContent(resource: URI): Promise<string | null> {
		...
		if (context.res.statusCode === 200) {
			...
			throw new Error(
				`Failed to get profile from URL: ${resource.toString()}. Status code: ${
	...
	private async getProfileToImport(
		...
		if (profile) {
			...
			const { result } =
				await this.dialogService.prompt<ImportProfileChoice>({
					type: Severity.Info,
					message: localize(
			...
			const name = await this.quickInputService.input({
				...
				value: `${profileName} ${this.getProfileNameIndex(
					profileName,
				)}`,
	...
	private generateProfileName(profileName: string): string {
		const existingProfile = this.userDataProfilesService.profiles.find(
			(p) => p.name === profileName,
		);
	...
	private async showProfilePreviewView(
		id: string,
		name: string,
		primary: IAction,
		secondary: IAction,
		refreshAction: boolean,
		userDataProfilesData: UserDataProfileImportExportState,
	): Promise<UserDataProfilePreviewViewPane> {
		...
		const descriptor: ITreeViewDescriptor = {
			...
			ctorDescriptor: new SyncDescriptor(UserDataProfilePreviewViewPane, [
				userDataProfilesData,
				primary,
				secondary,
				actionRunner,
			]),
		...
		return (await this.viewsService.openView<UserDataProfilePreviewViewPane>(
			id,
			true,
		))!;
	}
	...
	private async closeAllImportExportPreviewEditors(): Promise<void> {
		const editorsToColse = this.editorService
			...
			.filter(
				({ editor }) =>
					...
					editor.resource?.scheme ===
						USER_DATA_PROFILE_IMPORT_PREVIEW_SCHEME,
			);
...
class UserDataProfilePreviewViewPane extends TreeViewPane {
	...
	protected override renderTreeView(container: HTMLElement): void {
		...
		super.renderTreeView(
			DOM.append(container, DOM.$(".profile-view-tree-container")),
		);
		this.messageContainer = DOM.append(
			container,
			DOM.$(".profile-view-message-container.hide"),
		);
		...
		this._register(
			Event.any(
				this.userDataProfileData.onDidChangeRoots,
				this.treeView.onDidCollapseItem,
				this.treeView.onDidExpandItem,
			)(() => this.computeAndLayout()),
		);
	}
		...
		const children = await Promise.all(
			roots.map(async (root) => {
				...
				try {
					...
					/* Ignore because element might not be added yet */
				}
				if (expanded) {
	...
	private createButtons(container: HTMLElement): void {
		this.buttonsContainer = DOM.append(
			container,
			DOM.$(".profile-view-buttons-container"),
		);

		this.primaryButton = this._register(
		...
		this._register(
			this.primaryButton.onDidClick(() =>
				this.actionRunner.run(this.primaryAction),
			),
		);
		this._register(
			this.primaryAction.onDidChange((e) => {
		...
		this.secondaryButton = this._register(
			new Button(this.buttonsContainer, {
				secondary: true,
				...defaultButtonStyles,
			}),
		);
		...
		this._register(
			this.secondaryButton.onDidClick(() =>
				this.actionRunner.run(this.secondaryAction),
			),
		);
		this._register(
			this.secondaryAction.onDidChange((e) => {
				if (e.enabled !== undefined) {
	...
	protected override layoutTreeView(height: number, width: number): void {
		...
		if (!this.messageContainer.classList.contains("hide")) {
			messageContainerHeight = DOM.getClientArea(
		...
		super.layoutTreeView(
			Math.min(
				height - buttonContainerHeight - messageContainerHeight,
				22 * this.totalTreeItemsCount,
			),
			width,
		);
	}
	...
	private updateConfirmButtonEnablement(): void {
		this.primaryButton.enabled =
			this.primaryAction.enabled && this.userDataProfileData.isEnabled();
	}
	...
	setMessage(message: MarkdownString | undefined): void {
		...
		if (message) {
			...
			const rendered = this.renderDisposables.add(
...
class UserDataProfileImportState extends UserDataProfileImportExportState {
	...
	protected async fetchRoots(): Promise<IProfileResourceTreeItem[]> {
		...
		const inMemoryProvider = this._register(
			new InMemoryFileSystemProvider(),
		);
		this.disposables.add(
			this.fileService.registerProvider(
		...
		const roots: IProfileResourceTreeItem[] = [];
		const importPreviewProfle = toUserDataProfile(
			generateUuid(),
			this.profile.name,
			URI.file("/root").with({
		...
		if (this.profile.snippets) {
			const snippetsResource =
				this.instantiationService.createInstance(SnippetsResource);
			await snippetsResource.apply(
				this.profile.snippets,
				importPreviewProfle,
			);
			const snippetsResourceTreeItem =
				this.instantiationService.createInstance(
		...
		if (this.profile.globalState) {
			const globalStateResource = joinPath(
				importPreviewProfle.globalStorageHome,
				"globalState.json",
			);
			const content = VSBuffer.fromString(
				JSON.stringify(
					JSON.parse(this.profile.globalState),
					null,
					"\t",
				),
			);
