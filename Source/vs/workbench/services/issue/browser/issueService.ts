...
export class WebIssueService implements IWorkbenchIssueService {
	...
	async openReporter(options: Partial<IssueReporterData>): Promise<void> {
		...
		if (this._handlers.has(extensionId)) {
			try {
				const uri = await this.getIssueUriFromHandler(
					extensionId,
	...
	private async getIssueUriFromHandler(
		extensionId: string,
		...
		if (!handler) {
			throw new Error(
				`No handler registered for extension ${extensionId}`
			);
		}
...
<!-- generated by web issue reporter -->`;
		...
		return `${baseUri}?body=${encodeURIComponent(
			issueDescription
		)}&labels=web`;
	}
}
