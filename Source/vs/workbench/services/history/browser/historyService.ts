...
export class HistoryService extends Disposable implements IHistoryService {
				...
				this._register(
					Event.runAndSubscribe(
						...
						(container) => {
							...
							this._register(
								Event.filter(
									this.layoutService.onDidRemoveContainer,
	...
	private onDidActiveEditorChange(): void {
		...
		this.lastActiveEditor =
			activeEditorPane?.input && activeEditorPane.group
				? {
	...
	private traceStack(): void {
		...
		for (const entry of this.stack) {
			if (typeof entry.selection?.log === "function") {
				entryLabels.push(
					`- group: ${
						entry.groupId
					}, editor: ${entry.editor.resource?.toString()}, selection: ${entry.selection.log()}`
				);
			} else {
				entryLabels.push(
					`- group: ${
						entry.groupId
					}, editor: ${entry.editor.resource?.toString()}, selection: <none>`
				);
			}
		}
		...
		if (entryLabels.length === 0) {
			this.trace(
				`index: ${
					this.index
				}, navigating: ${this.isNavigating()}: <empty>`
			);
		} else {
	...
	private trace(
		...
		if (editor !== null) {
			this.logService.trace(
				`[History stack ${filterLabel}-${scopeLabel}]: ${msg} (editor: ${editor?.resource?.toString()}, event: ${this.traceEvent(
					event
				)})`
			);
		} else {
