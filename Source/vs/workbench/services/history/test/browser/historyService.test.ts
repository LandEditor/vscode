...
import {
	...
	HistoryService,
} from "vs/workbench/services/history/browser/historyService";
import {
	IEditorService,
	SIDE_GROUP,
...
suite("HistoryService", function () {
	...
	test("back / forward: editor navigation stack - navigation", async function () {
		const [, , editorService, , instantiationService] =
			await createServices();
	...
	test("getHistory", async () => {
		...
		const [part, historyService, , , instantiationService] =
			await createServices();
		...
		assert.strictEqual(
			isResourceEditorInput(history[0]) &&
				!(history[0] instanceof EditorInput),
			true
		);
		assert.strictEqual(
