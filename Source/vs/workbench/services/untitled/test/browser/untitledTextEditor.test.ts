...
suite("Untitled text editors", () => {
	...
	test("associated resource is dirty", async () => {
		...
		let onDidChangeDirtyModel: IUntitledTextEditorModel | undefined =
			undefined;
		disposables.add(
	...
	test("initial content is dirty", async () => {
		...
		const backup = (await untitled.model.backup(CancellationToken.None))
			.content;
		if (isReadableStream(backup)) {
			const value = await streamToBuffer(
				backup as VSBufferReadableStream
			);
