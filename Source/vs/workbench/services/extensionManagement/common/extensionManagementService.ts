	...
	implements IWorkbenchExtensionManagementService
{
	...
	async installResourceExtension(
		...
		this.logService.info(
			...
			} from ${extension.location.toString()} in workspace`,
		);
		...
		try {
			...
			this.logService.info(
				`Successfully installed the extension ${
					workspaceExtension.identifier.id
				} from ${extension.location.toString()} in the workspace`,
			);
	...
	private async uninstallExtensionFromWorkspace(
		...
		this.logService.info(
			`Uninstalling the workspace extension ${
				extension.identifier.id
			} from ${extension.location.toString()}`,
		);
		const server = this.getWorkspaceExtensionsServer();
		...
		try {
			...
			this.logService.info(
				`Successfully uninstalled the workspace extension ${
					extension.identifier.id
				} from ${extension.location.toString()}`,
			);
			...
			this.logService.error(
				`Failed to uninstall the workspace extension ${
					extension.identifier.id
				} from ${extension.location.toString()}`,
				getErrorMessage(error),
			);
...
class WorkspaceExtensionsManagementService extends Disposable {
	...
	getInstalledWorkspaceExtensionsLocations(): URI[] {
		...
		try {
			...
			this.logService.warn(
				`Error parsing workspace extensions locations: ${getErrorMessage(
					error,
				)}`,
			);
		}
