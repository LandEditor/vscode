...
export class TestExtensionEnablementService extends ExtensionEnablementService {
	...
	public reset(): void {
		...
		const workspaceEnabledExtensions =
			this._getWorkspaceEnabledExtensions();
		if (workspaceEnabledExtensions.length) {
			extensions = extensions.filter(
				(r) =>
					!workspaceEnabledExtensions.some((e) =>
						areSameExtensions(e, r)
					)
			);
		}
...
suite("ExtensionEnablementService Test", () => {
	...
	const didChangeProfileExtensionsEvent =
		new Emitter<DidChangeProfileEvent>();
			...
			anExtensionManagementServerService(
				{
					...
					>{
						...
						onDidChangeProfile:
							didChangeProfileExtensionsEvent.event,
		...
		return testObject
			...
			.then(() => {
				...
				assert.deepStrictEqual(
					(<IExtension>target.args[0][0][0]).identifier,
					{ id: "pub.a" }
				);
			});
	});
