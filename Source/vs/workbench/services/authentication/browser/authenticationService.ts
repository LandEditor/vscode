...
export function addAccountUsage(
	storageService: IStorageService,
	providerId: string,
	accountName: string,
	extensionId: string,
...
let placeholderMenuItem: IDisposable | undefined = MenuRegistry.appendMenuItem(
	...
	implements IAuthenticationService
{
	...
	private _onDidChangeDeclaredProviders: Emitter<
		AuthenticationProviderInformation[]
	> = this._register(new Emitter<AuthenticationProviderInformation[]>());
	readonly onDidChangeDeclaredProviders: Event<
		AuthenticationProviderInformation[]
	...
	unregisterAuthenticationProvider(id: string): void {
		...
		if (provider) {
			...
			const accessRequests =
				this._sessionAccessRequestItems.get(id) || {};
		...
		if (!this._authenticationProviders.size) {
			placeholderMenuItem = MenuRegistry.appendMenuItem(
				MenuId.AccountsContext,
				{
					command: {
						id: "noAuthenticationProviders",
	...
	async selectSession(
		...
		return new Promise((resolve, reject) => {
			...
			const items: { label: string; session?: AuthenticationSession }[] =
			...
			quickPick.onDidAccept(async (_) => {
				const session =
					quickPick.selectedItems[0].session ??
					(await this.createSession(providerId, scopes));
	...
	async completeSessionAccessRequest(
		providerId: string,
		extensionId: string,
		extensionName: string,
		scopes: string[],
		...
		const supportsMultipleAccounts =
			this.supportsMultipleAccounts(providerId);
	...
	async requestNewSession(
		...
		if (!provider) {
			...
			await new Promise<void>((resolve, _) => {
				const dispose = this.onDidRegisterAuthenticationProvider(
					(e) => {
						if (e.id === providerId) {
							provider =
		...
		const commandId = `${providerId}:${extensionId}:signIn${
			Object.keys(providerRequests || []).length
		}`;
		const menuItem = MenuRegistry.appendMenuItem(MenuId.AccountsContext, {
