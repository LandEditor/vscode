...
import type { IValidGrammarDefinition } from "vs/workbench/services/textMate/common/TMScopeRegistry";
...
export class ThreadedBackgroundTokenizerFactory implements IDisposable {
	...
	private _workerProxyPromise: Promise<TextMateTokenizationWorker | null> | null =
		null;
	...
	private async _createWorkerProxy(): Promise<TextMateTokenizationWorker | null> {
		...
		const textmateLocation: AppResourcePath = useAsar
			? textmateModuleLocationAsar
			: textmateModuleLocation;
		...
		const createData: ICreateData = {
			...
			textmateMainUri: uri,
			onigurumaMainUri:
				FileAccess.asBrowserUri(onigurumaMain).toString(true),
			onigurumaWASMUri:
