...
suite("ViewDescriptorService", () => {
	...
	test("move view events", async () => {
		...
		const locationMoveString = (
			...
			return `Moved ${view.id} from ${
				from === ViewContainerLocation.Sidebar ? "Sidebar" : "Panel"
			} to ${
				to === ViewContainerLocation.Sidebar ? "Sidebar" : "Panel"
			}\n`;
		};
		disposables.add(
	...
	test("initialize with custom locations", async () => {
		...
		const generateViewContainer1 = `workbench.views.service.${ViewContainerLocationToString(
			ViewContainerLocation.Sidebar,
		)}.${generateUuid()}`;
	...
	test("orphan view containers", async () => {
		const storageService = instantiationService.get(IStorageService);
		const generatedViewContainerId = `workbench.views.service.${ViewContainerLocationToString(
			ViewContainerLocation.Sidebar,
		)}.${generateUuid()}`;
		const viewsCustomizations = {
			viewContainerLocations: {
	...
	test("storage change also updates locations even if views do not exists and views are registered later", async () => {
		...
		const generateViewContainerId = `workbench.views.service.${ViewContainerLocationToString(
			ViewContainerLocation.AuxiliaryBar,
		)}.${generateUuid()}`;
		const viewsCustomizations = {
			viewContainerLocations: {
		...
		const viewContainer1Views =
			testObject.getViewContainerModel(viewContainer);
		assert.deepStrictEqual(
			viewContainer1Views.allViewDescriptors.map((v) => v.id),
			["view2"],
		);
