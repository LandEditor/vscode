...
import { type ILogger, ILoggerService } from "vs/platform/log/common/log";
...
} from "vs/workbench/services/views/common/viewContainerModel";
	...
	implements IViewDescriptorService
{
	...
	moveViewsToContainer(
		...
		this.logger.value.info(
			`moveViewsToContainer: views:${views
				.map((view) => view.id)
				.join(",")} viewContainer:${viewContainer.id} reason:${reason}`,
		);
	...
	private generateContainerId(location: ViewContainerLocation): string {
		return `${
			ViewDescriptorService.COMMON_CONTAINER_ID_PREFIX
		}.${ViewContainerLocationToString(location)}.${generateUuid()}`;
	}
