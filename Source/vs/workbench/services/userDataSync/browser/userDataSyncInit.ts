...
} from "vs/platform/userDataSync/common/extensionsSync";
import {
	GlobalStateInitializer,
	UserDataSyncStoreTypeSynchronizer,
} from "vs/platform/userDataSync/common/globalStateSync";
...
import type { IUserDataInitializer } from "vs/workbench/services/userData/browser/userDataInit";
		...
		await Promises.settled(
			syncResources.map(async (syncResource) => {
				try {
					...
					this.logService.info(
						`Error while initializing ${getSyncAreaLabel(
							syncResource,
						)}`,
					);
					this.logService.error(error);
