...
import type { IKeyboardMapper } from "vs/platform/keyboardLayout/common/keyboardMapper";
...
} from "vs/workbench/services/keybinding/browser/navigatorKeyboard";
...
export class WorkbenchKeybindingService extends AbstractKeybindingService {
		...
		disposables.add(
			dom.addDisposableListener(
				...
				(e: KeyboardEvent) => {
					...
					this._log(
						`| Converted keydown event - ${printStandardKeyboardEvent(
							keyEvent,
						)}`,
					);
	...
	private _printResolvedKeybindings(
		...
		for (const resolvedKeybinding of resolvedKeybindings) {
			if (isFirst) {
				output.push(
					`${firstRow}${this._printResolvedKeybinding(
						resolvedKeybinding,
					).padStart(padLength, " ")}`,
				);
			} else {
