	...
	implements IKeybindingEditingService
{
	...
	private resolveModelReference(): Promise<
	...
	> {
		return this.fileService
			...
			.then((exists) => {
				const EOL = this.configurationService.getValue<{ eol: string }>(
					"files",
					{ overrideIdentifier: "json" }
				)["eol"];
				const result: Promise<any> = exists
					? Promise.resolve(null)
					: this.textFileService.write(
	...
	private resolveAndValidate(): Promise<
		IReference<IResolvedTextEditorModel>
	> {
