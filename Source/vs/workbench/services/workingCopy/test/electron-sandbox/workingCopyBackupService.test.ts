...
import {
	...
	hashIdentifier,
} from "vs/workbench/services/workingCopy/common/workingCopyBackupService";
...
suite("WorkingCopyBackupService", () => {
	...
	suite("backup", () => {
		function toExpectedPreamble(
			...
			return `${identifier.resource.toString()} ${JSON.stringify({
				...meta,
		...
		async function testLargeTextFile(
			...
			await service.backup(identifier, buffer, undefined, {
				largeTest: true,
			});
			assert.strictEqual(
				(
		...
		test("multiple same resource, different type id", async () => {
			...
			for (const backupId of [backupId1, backupId2, backupId3]) {
				...
				assert.strictEqual(
					(
						await fileService.readFile(fooBackupPath)
					).value.toString(),
	...
	suite("resolve", () => {
		...
		async function doTestResolveBackup(
			...
			const backup =
				await service.resolve<IBackupTestMetaData>(identifier);
			assert.ok(backup);
			assert.strictEqual(
				contents,
		...
		async function testShouldRestoreOriginalContentsWithBrokenBackup(
			...
			const newFileContents = `${fileContents.substring(
				0,
				metaIndex,
		...
		test("file with binary data", async () => {
			...
			const buffer = Uint8Array.from([
				...
				0, 0, 73, 0, 0, 0, 67, 8, 2, 0, 0, 0, 95, 138, 191, 237, 0, 0,
				0, 1, 115, 82, 71, 66, 0, 174, 206, 28, 233, 0, 0, 0, 4, 103,
				65, 77, 65, 0, 0, 177, 143, 11, 252, 97, 5, 0, 0, 0, 9, 112, 72,
				89, 115, 0, 0, 14, 195, 0, 0, 14, 195, 1, 199, 111, 168, 100, 0,
				0, 0, 71, 116, 69, 88, 116, 83, 111, 117, 114, 99, 101, 0, 83,
				104, 111, 116, 116, 121, 32, 118, 50, 46, 48, 46, 50, 46, 50,
				49, 54, 32, 40, 67, 41, 32, 84, 104, 111, 109, 97, 115, 32, 66,
				97, 117, 109, 97, 110, 110, 32, 45, 32, 104, 116, 116, 112, 58,
				47, 47, 115, 104, 111, 116, 116, 121, 46, 100, 101, 118, 115,
				45, 111, 110, 46, 110, 101, 116, 44, 132, 21, 213, 0, 0, 0, 84,
				73, 68, 65, 84, 120, 218, 237, 207, 65, 17, 0, 0, 12, 2, 32,
				211, 217, 63, 146, 37, 246, 218, 65, 3, 210, 191, 226, 230, 230,
				230, 230, 230, 230, 230, 230, 230, 230, 230, 230, 230, 230, 230,
				230, 230, 230, 230, 230, 230, 230, 230, 230, 230, 230, 230, 230,
				230, 230, 230, 230, 230, 230, 230, 230, 230, 230, 230, 230, 230,
				230, 230, 230, 230, 230, 230, 230, 230, 230, 230, 230, 230, 230,
				230, 118, 100, 169, 4, 173, 8, 44, 248, 184, 40, 0, 0, 0, 0, 73,
	...
	suite("typeId migration", () => {
		test("works (when meta is missing)", async () => {
			...
			assert.ok(
				backups.some((backup) =>
					isEqual(backup.resource, untitledFile),
				),
			);
			assert.ok(
		...
		test("works (when typeId in meta is missing)", async () => {
			...
			await fileService.writeFile(
				fooBackupPath,
				VSBuffer.fromString(
					`${fooFile.toString()} ${JSON.stringify({
						foo: "bar",
					})}\ntest file`,
				),
			);
			await fileService.writeFile(
				untitledBackupPath,
				VSBuffer.fromString(
					`${untitledFile.toString()} ${JSON.stringify({
						foo: "bar",
					})}\ntest untitled`,
				),
			);
			await fileService.writeFile(
				customFileBackupPath,
				VSBuffer.fromString(
					`${customFile.toString()} ${JSON.stringify({
						foo: "bar",
					})}\ntest custom`,
				),
			);
