...
export class TestStoredFileWorkingCopyModelFactory
	implements
		IStoredFileWorkingCopyModelFactory<TestStoredFileWorkingCopyModel>
{
	async createModel(
		resource: URI,
		contents: VSBufferReadableStream,
...
export class TestStoredFileWorkingCopyModelWithCustomSaveFactory
	implements
		IStoredFileWorkingCopyModelFactory<TestStoredFileWorkingCopyModelWithCustomSave>
{
	async createModel(
		resource: URI,
		contents: VSBufferReadableStream,
...
suite("StoredFileWorkingCopy (with custom save)", function () {
	...
	test("save (custom implemented)", async () => {
		let savedCounter = 0;
		let lastSaveEvent: IStoredFileWorkingCopySaveEvent | undefined =
			undefined;
		disposables.add(
...
suite("StoredFileWorkingCopy", function () {
	...
	test("resolve (FILE_NOT_MODIFIED_SINCE still updates readonly state)", async () => {
		...
		try {
			accessor.fileService.readShouldThrowError =
				new NotModifiedSinceFileOperationError(
					"file not modified since",
					{ ...stat, readonly: true }
				);
			await workingCopy.resolve();
	...
	test("backup", async () => {
		...
		if (backup.content instanceof VSBuffer) {
			...
			backupContents = (
				await consumeStream(backup.content, (chunks) =>
	...
	test("save (no errors) - save reason", async () => {
		...
		assert.strictEqual(workingCopy.isDirty(), false);
		assert.strictEqual(
			(lastSaveEvent! as IStoredFileWorkingCopySaveEvent).reason,
			SaveReason.AUTO
		);
		assert.strictEqual(
			(lastSaveEvent! as IStoredFileWorkingCopySaveEvent).source,
			source
		);
	});
