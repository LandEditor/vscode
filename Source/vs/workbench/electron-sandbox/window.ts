...
export class NativeWindow extends BaseWindow {
	...
	private registerListeners(): void {
		...
		if (isMacintosh) {
			...
			this._register(
				this.auxiliaryWindowService.onDidOpenAuxiliaryWindow(
					({ window, disposables }) => {
						const auxiliaryWindowEditorPart =
							this.editorGroupService.getPart(window.container);
						if (auxiliaryWindowEditorPart) {
							const auxiliaryEditorService =
								this.editorService.createScoped(
									auxiliaryWindowEditorPart,
									disposables
								);
							disposables.add(
		...
		if (
			...
			this._register(
				addDisposableListener(titlePart, EventType.DBLCLICK, (e) => {
					EventHelper.stop(e);
		...
		this._register(
			Event.any(
				Event.map(
					Event.filter(
						this.nativeHostService.onDidMaximizeMainWindow,
				...
				Event.map(
					Event.filter(
						this.nativeHostService.onDidUnmaximizeMainWindow,
						(id) => id === this.nativeHostService.windowId
					),
					() => false
				)
	...
	private maybeCloseWindow(): void {
		const closeWhenEmpty =
			...
			this.environmentService.args.wait;
		if (!closeWhenEmpty) {
		...
		mainWindow.focus = function () {
			...
			if (getActiveWindow() !== mainWindow) {
				that.nativeHostService.focusWindow({
					targetWindowId: that.nativeHostService.windowId,
				});
			}
		};
	}
	...
	private async handleWarnings(): Promise<void> {
		...
		if (isMacintosh) {
			const majorVersion =
				this.environmentService.os.release.split(".")[0];
			...
			resolveExternalUri: async (uri: URI, options?: OpenOptions) => {
				if (options?.allowTunneling) {
					...
					if (portMappingRequest) {
						...
						const addressProvider: IAddressProvider | undefined =
							remoteAuthority
								? {
										getAddress:
