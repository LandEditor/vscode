...
import { isCI, isMacintosh, isWindows } from "vs/base/common/platform";
import { dirname } from "vs/base/common/resources";
import { ThemeIcon } from "vs/base/common/themables";
import { assertIsDefined } from "vs/base/common/types";
import { URI } from "vs/base/common/uri";
import type { AuthInfo } from "vs/base/parts/sandbox/electron-sandbox/electronTypes";
import {
	ipcRenderer,
	process,
...
export class NativeWindow extends BaseWindow {
	...
	protected registerListeners(): void {
		...
		this._register(
			this.editorGroupService.onDidCreateAuxiliaryEditorPart(
				({ instantiationService, disposables, part }) => {
	...
	private createWindowZoomStatusEntry(
		...
		this.mapWindowIdToZoomStatusEntry.set(
			targetWindowId,
			disposables.add(
				instantiationService.createInstance(ZoomStatusEntry),
			),
		);
		disposables.add(
			toDisposable(() =>
