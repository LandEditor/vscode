...
} from "vs/workbench/contrib/snippets/browser/snippetCompletionProvider";
...
suite("SnippetsService", function () {
	...
	test("snippet completions - simple", function () {
		...
		return provider
			.provideCompletionItems(model, new Position(1, 1), context)!
			.then((result) => {
	...
	test("snippet completions - with prefix", function () {
		...
		return provider
			.provideCompletionItems(model, new Position(1, 4), context)!
			.then((result) => {
				assert.strictEqual(result.incomplete, undefined);
	...
	test("snippet completions - with different prefixes", async function () {
		...
		await provider
			.provideCompletionItems(model, new Position(1, 3), context)!
			.then((result) => {
				assert.strictEqual(result.incomplete, undefined);
				assert.strictEqual(result.suggestions.length, 2);
				assert.deepStrictEqual(result.suggestions[0].label, {
		...
		await provider
			.provideCompletionItems(model, new Position(1, 5), context)!
			.then((result) => {
				assert.strictEqual(result.incomplete, undefined);
				assert.strictEqual(result.suggestions.length, 2);
		...
		await provider
			.provideCompletionItems(model, new Position(1, 6), context)!
			.then((result) => {
				assert.strictEqual(result.incomplete, undefined);
				assert.strictEqual(result.suggestions.length, 2);
				assert.deepStrictEqual(result.suggestions[0].label, {
	...
	test('Cannot use "<?php" as user snippet prefix anymore, #26275', function () {
		...
		return provider
			.provideCompletionItems(model, new Position(1, 7), context)!
			.then((result) => {
				assert.strictEqual(result.suggestions.length, 1);
				model.dispose();
