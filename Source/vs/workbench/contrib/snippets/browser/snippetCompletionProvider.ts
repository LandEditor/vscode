...
export class SnippetCompletion implements CompletionItem {
	...
	constructor(
		...
		this.range = range;
		this.sortText = `${
			snippet.snippetSource === SnippetSource.Extension ? "z" : "a"
		}-${snippet.prefix}`;
	...
	async provideCompletionItems(
		...
		snippet: for (const snippet of snippets) {
			...
			const word = getWordAtText(
				1,
				languageConfig.getWordDefinition(),
				snippet.prefixLow,
				0,
			);
			...
			const minPos =
				firstNonWhitespaceIndex(snippet.prefixLow) === 0
					? Math.max(
							0,
			...
			column: for (
				let pos = Math.max(
					minPos,
					columnOffset - snippet.prefixLow.length,
				);
				pos < lineContentLow.length;
				pos++
			) {
				if (
					!isPatternInWord(
				...
				if (columnOffset < lineContentLow.length) {
					const autoClosingPairs =
						languageConfig.getAutoClosingPairs();
				...
				const replace = Range.fromPositions(startPosition, {
