...
import type { ILink, LinkedText } from "vs/base/common/linkedText";
...
import type { IWindowOpenable } from "vs/platform/window/common/window";
import {
...
export class GettingStartedPage extends EditorPane {
	...
	private hideVideos() {
		this.setHiddenCategories([
			...this.getHiddenCategories().add("getting-started-videos"),
		]);
	...
	private async buildCategoriesSlide() {
		...
		const showVideoTutorials = await Promise.race([
			this.tasExperimentService?.getTreatment<boolean>(
				"gettingStarted.showVideoTutorials",
			),
		...
		const layoutRecentList = () => {
			if (
				...
				this.container.classList.contains("noVideos")
			) {
	...
	private buildVideosList(): GettingStartedIndexList<IWelcomePageStartEntry> {
		const renderFeaturedExtensions = (
			...
			return $(
				"button.getting-started-category" + ".featured",
				{
