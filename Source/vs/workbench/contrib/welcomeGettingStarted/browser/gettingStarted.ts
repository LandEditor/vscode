...
} from "vs/workbench/contrib/welcomeGettingStarted/browser/gettingStartedIcons";
...
} from "vs/workbench/contrib/welcomeGettingStarted/browser/gettingStartedService";
import {
...
export class GettingStartedPage extends EditorPane {
	...
	constructor(
		...
		private readonly featuredExtensionService: IFeaturedExtensionsService,
		@IConfigurationService
		...
		const rerender = () => {
			...
			this.featuredExtensions =
				this.featuredExtensionService.getExtensions();
			...
			this.extensionManagementService.onDidInstallExtensions(
				async (result) => {
					for (const e of result) {
						const installedFeaturedExtension = (
							await this.featuredExtensions
						)?.find((ext) =>
							ExtensionIdentifier.equals(ext.id, e.identifier.id)
						);
			...
			this.gettingStartedService.onDidProgressStep((step) => {
				...
				if (category.id === this.currentWalkthrough?.id) {
					const badgeelements = assertIsDefined(
						getWindow(this.container).document.querySelectorAll(
	...
	private registerDispatchListeners() {
		...
		this.container.querySelectorAll("[x-dispatch]").forEach((element) => {
			const [command, argument] = (
				element.getAttribute("x-dispatch") ?? ""
			).split(":");
			if (command) {
	...
	private hideExtension(extensionId: string) {
		this.setHiddenCategories([
			...this.getHiddenCategories().add(extensionId),
		]);
		...
		if (stepToExpand.media.type === "image") {
			...
			this.stepDisposables.add(
				addDisposableListener(this.stepMediaComponent, "click", () => {
					const hrefs = flatten(
						stepToExpand.description.map((lt) =>
							lt.nodes
								.filter(
									(node): node is ILink =>
										typeof node !== "string"
	...
	private async buildCategoriesSlide() {
		...
		const layoutLists = () => {
			if (gettingStartedList.itemCount) {
				...
				reset(
					rightColumn,
					featuredExtensionList.getDomElement(),
					gettingStartedList.getDomElement()
				);
			} else {
		...
		const layoutRecentList = () => {
			if (
				this.container.classList.contains("noWalkthroughs") &&
				this.container.classList.contains("noExtensions")
			) {
	...
	private buildFeaturedExtensionsList(): GettingStartedIndexList<IFeaturedExtension> {
		const renderFeaturedExtensions = (
			entry: IFeaturedExtension
		): HTMLElement => {
			const descriptionContent = $(".featured-description-content", {});

			reset(
				descriptionContent,
				...renderLabelWithIcons(entry.description)
			);

			const titleContent = $("h3.category-title.max-lines-3", {
				"x-category-title-for": entry.id,
			});
		...
		const featuredExtensionsList = (this.featuredExtensionsList =
			new GettingStartedIndexList({
				title: this.featuredExtensionService.title,
				klass: "featured-extensions",
				limit: 5,
				renderElement: renderFeaturedExtensions,
				rankElement: (extension) => {
					if (this.getHiddenCategories().has(extension.id)) {
						return null;
					}
					return 0;
				},
	...
	private updateCategoryProgress() {
		getWindow(this.container)
			.document.querySelectorAll(".category-progress")
			.forEach((element) => {
	...
	private runStepCommand(href: string) {
		...
		const fullSize = this.group
			? this.groupsService.getPart(this.group).contentDimension
			: undefined;
		...
		if (toSide && fullSize && fullSize.width > 700) {
			if (this.groupsService.count === 1) {
				...
				this.groupsService.setSize(
					assertIsDefined(gettingStartedGroup),
					{ width: gettingStartedSize, height: fullSize.height }
				);
			}
