...
import type { ExplorerViewPaneContainer } from "vs/workbench/contrib/files/browser/explorerViewlet";
import {
	IExplorerService,
...
import type { ExplorerView } from "vs/workbench/contrib/files/browser/views/explorerView";
				...
				actions: {
					primary: [
						...
						toAction({
							...
							label: nls.localize("discard", "Discard"),
...
CommandsRegistry.registerCommand({
	...
	handler: (
		...
		if (contexts.length) {
			...
			groups = editorGroupService.getGroups(
