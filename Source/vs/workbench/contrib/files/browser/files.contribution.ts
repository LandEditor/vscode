...
} from "vs/workbench/contrib/files/browser/editors/fileEditorHandler";
...
import {
	ExplorerService,
	UNDO_REDO_SOURCE,
} from "vs/workbench/contrib/files/browser/explorerService";
...
Registry.as<IWorkbenchContributionsRegistry>(
	WorkbenchExtensions.Workbench,
).registerWorkbenchContribution(TextFileEditorTracker, LifecyclePhase.Starting);
...
Registry.as<IWorkbenchContributionsRegistry>(
	WorkbenchExtensions.Workbench,
).registerWorkbenchContribution(WorkspaceWatcher, LifecyclePhase.Restored);

// Register Dirty Files Indicator
Registry.as<IWorkbenchContributionsRegistry>(
	WorkbenchExtensions.Workbench,
).registerWorkbenchContribution(DirtyFilesIndicator, LifecyclePhase.Starting);
	...
	properties: {
		...
		"files.autoSaveWhenNoErrors": {
			...
			markdownDescription: nls.localize(
				...
				"When enabled, will limit [auto save](https://code.visualstudio.com/docs/editor/codebasics#_save-auto-save) of editors to files that have no errors reported in them. Only applies when `#files.autoSave#` is enabled.",
			),
			scope: ConfigurationScope.LANGUAGE_OVERRIDABLE,
		},
