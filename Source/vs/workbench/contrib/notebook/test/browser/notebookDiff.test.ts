		...
		await withTestNotebookDiffModel(
			[
				[
					...
					{
						custom: { metadata: { collapsed: false } },
						executionOrder: 3,
					},
				],
			],
			...
			(model, disposables, accessor) => {
				...
				const diffViewModels = NotebookTextDiffEditor.computeDiff(
					accessor,
				...
				assert.strictEqual(
					diffViewModels.viewModels[0].type,
					"modified"
				);
	...
	test("diff different output", async () => {
		await withTestNotebookDiffModel(
			...
			(model, disposables, accessor) => {
				...
				assert.strictEqual(
					diffViewModels.viewModels[1].type,
					"unchanged"
				);
	...
	test("diff foo/foe", async () => {
		await withTestNotebookDiffModel(
			...
			(model, disposables, accessor) => {
				...
				assert.strictEqual(
					diffViewModels.viewModels[2].type,
					"unchanged"
				);
	...
	test("diff insert", async () => {
		await withTestNotebookDiffModel(
			...
			(model, disposables, accessor) => {
				...
				const diffResult = NotebookTextDiffEditor.computeDiff(
					accessor,
					configurationService,
					model,
					eventDispatcher,
		...
		await withTestNotebookDiffModel(
			[
				...
				[
					...
					[],
					{
						custom: { metadata: { collapsed: true } },
						executionOrder: 1,
					},
				],
				[
	...
	test("diff output", async () => {
		await withTestNotebookDiffModel(
			...
			(model, disposables, accessor) => {
				...
				assert.strictEqual(
					diffViewModels.viewModels[0].checkIfOutputsModified(),
					false
				);
				assert.strictEqual(
					diffViewModels.viewModels[1].type,
	...
	test("diff output fast check", async () => {
		await withTestNotebookDiffModel(
			...
			(model, disposables, accessor) => {
				...
				assert.strictEqual(diffViewModels.viewModels.length, 2);
				assert.strictEqual(
					diffViewModels.viewModels[0].original!.textModel.equal(
						diffViewModels.viewModels[0].modified!.textModel
					),
					true
				);
				assert.strictEqual(
					diffViewModels.viewModels[1].original!.textModel.equal(
						diffViewModels.viewModels[1].modified!.textModel
					),
					false
				);
				diffViewModels.viewModels.forEach((vm) => {
