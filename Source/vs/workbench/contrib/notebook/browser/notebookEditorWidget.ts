...
import {
	registerZIndex,
	ZIndex,
} from "vs/platform/layout/browser/zIndexRegistry";
import {
...
} from "vs/workbench/contrib/notebook/browser/view/notebookCellList";
...
} from "vs/workbench/contrib/notebook/browser/view/renderers/cellRenderer";
import {
	IAckOutputHeight,
	IMarkupCellInitialization,
} from "vs/workbench/contrib/notebook/browser/view/renderers/webviewMessages";
import {
	CodeCellViewModel,
	outputDisplayLimit,
} from "vs/workbench/contrib/notebook/browser/viewModel/codeCellViewModel";
import { NotebookEventDispatcher } from "vs/workbench/contrib/notebook/browser/viewModel/eventDispatcher";
import { MarkupCellViewModel } from "vs/workbench/contrib/notebook/browser/viewModel/markupCellViewModel";
import {
	CellViewModel,
	NotebookViewModel,
} from "vs/workbench/contrib/notebook/browser/viewModel/notebookViewModelImpl";
	...
	implements INotebookEditorDelegate, INotebookEditor
{
	...
	constructor(
		...
		this.scopedContextKeyService = contextKeyService.createScoped(
		...
		this._notebookOutline = this._register(
			this.instantiationService.createInstance(
		...
		this._outputInputFocus = NOTEBOOK_OUPTUT_INPUT_FOCUSED.bindTo(
			this.scopedContextKeyService
		);
	...
	private _createLayoutStyles(): void {
		...
		if (compactView) {
			styleSheets.push(
				`.notebookOverlay .cell-list-container > .monaco-list > .monaco-scrollable-element > .monaco-list-rows > .markdown-cell-row div.cell.code { margin-left: ${
					codeCellLeftMargin + cellRunGutter
				}px; }`
			);
		} else {
			styleSheets.push(
		...
		if (focusIndicator === "border") {
			...
			.monaco-workbench .notebookOverlay .monaco-list.selection-multiple .monaco-list-row.code-cell-row.selected .cell-focus-indicator-right:before {
				top: -${cellTopMargin}px; height: calc(100% + ${
					cellTopMargin + cellBottomMargin
				}px)
			}`);
		} else {
			styleSheets.push(`
		...
		styleSheets.push(
			`.notebookOverlay .output { margin: 0px ${cellRightMargin}px 0px ${
				codeCellLeftMargin + cellRunGutter
			}px; }`
		);
		styleSheets.push(
			`.notebookOverlay .output { width: calc(100% - ${
				codeCellLeftMargin + cellRunGutter + cellRightMargin
			}px); }`
		);

		// comment
		styleSheets.push(
			`.notebookOverlay .cell-list-container > .monaco-list > .monaco-scrollable-element > .monaco-list-rows > .monaco-list-row .cell-comment-container { left: ${
				codeCellLeftMargin + cellRunGutter
			}px; }`
		);
		styleSheets.push(
			`.notebookOverlay .cell-list-container > .monaco-list > .monaco-scrollable-element > .monaco-list-rows > .monaco-list-row .cell-comment-container { width: calc(100% - ${
				codeCellLeftMargin + cellRunGutter + cellRightMargin
			}px); }`
		);
		...
		// show more container
		styleSheets.push(
			`.notebookOverlay .output-show-more-container { margin: 0px ${cellRightMargin}px 0px ${
				codeCellLeftMargin + cellRunGutter
			}px; }`
		);
		styleSheets.push(
		...
		styleSheets.push(
			`.monaco-workbench .notebookOverlay > .cell-list-container .notebook-folding-indicator { left: ${
				(markdownCellGutter - 20) / 2 + markdownCellLeftMargin
			}px; }`
		);
		styleSheets.push(
			`.notebookOverlay > .cell-list-container .notebook-folded-hint { left: ${
				markdownCellGutter + markdownCellLeftMargin + 8
			}px; }`
		);
		styleSheets.push(
		...
		styleSheets.push(
			`.notebookOverlay .monaco-list .monaco-list-row.code-cell-row .cell-focus-indicator-left { width: ${
				codeCellLeftMargin + cellRunGutter
			}px; }`
		);
		styleSheets.push(
		...
		styleSheets.push(
			`.monaco-workbench .notebookOverlay > .cell-list-container > .monaco-list > .monaco-scrollable-element > .monaco-list-rows > .cell-list-top-cell-toolbar-container .monaco-toolbar { height: ${bottomToolbarHeight}px }`
		);
	...
	private _createCellList(): void {
		...
		this._list = this.instantiationService.createInstance(
			...
			this.scopedContextKeyService,
			{
				...
				paddingTop: this._notebookOptions.computeTopInsertToolbarHeight(
					this.viewModel?.viewType
				),
				paddingBottom: 0,
				...
				accessibilityProvider: {
					getAriaLabel: (element: CellViewModel) => {
						...
						if (index >= 0) {
							return `Cell ${index}, ${
								element.cellKind === CellKind.Markup
									? "markdown"
									: "code"
							}  cell`;
						}
	...
	async setModel(
		...
		if (this.viewModel === undefined || !this.viewModel.equal(textModel)) {
			const oldTopInsertToolbarHeight =
			...
			const newTopInsertToolbarHeight =
				this._notebookOptions.computeTopInsertToolbarHeight(
					this.viewModel?.viewType
				);
			...
			if (
				oldTopInsertToolbarHeight !== newTopInsertToolbarHeight ||
		...
		if (cellOptions) {
			...
			if (cell) {
				...
				if (selection) {
					...
					CellRevealType.NearTopIfOutsideViewport
				) {
					await this._list.revealCellAsync(
						cell,
						CellRevealType.NearTopIfOutsideViewport
					);
				} else {
		...
		if (this._list.getRenderHeight() < newCellListHeight) {
			...
			this._list.updateOptions({
				paddingBottom: this._allowScrollBeyondLastLine()
					...
					: 0,
				paddingTop: topInserToolbarHeight,
			});
	...
	private layoutContainerOverShadowElement(
		...
		this._overlayContainer.style.top = `${
			this._shadowElementViewInfo.top - (elementContainerRect?.top || 0)
		}px`;
		this._overlayContainer.style.left = `${
			this._shadowElementViewInfo.left - (elementContainerRect?.left || 0)
		}px`;
		this._overlayContainer.style.width = `${
			dimension ? dimension.width : this._shadowElementViewInfo.width
		}px`;
		this._overlayContainer.style.height = `${
			dimension ? dimension.height : this._shadowElementViewInfo.height
		}px`;
	}
	...
	async revealLineInViewAsync(
		...
		return this._list.revealCellRangeAsync(
			cell,
	...
	async revealCellOffsetInCenterAsync(
		cell: ICellViewModel,
		offset: number
	): Promise<void> {
