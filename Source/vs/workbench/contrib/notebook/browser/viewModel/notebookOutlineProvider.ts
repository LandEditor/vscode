...
} from "vs/workbench/contrib/notebook/browser/viewModel/notebookOutlineEntryFactory";
import {
	CellKind,
...
export class NotebookCellOutlineProvider {
	...
	private _recomputeState(): void {
		...
		if (this._target === OutlineTarget.Breadcrumbs) {
			notebookCells = notebookEditorWidget
				.getViewModel()
				.viewCells.filter(
					(cell) =>
						cell.cellKind === CellKind.Markup || includeCodeCells,
				);
		} else {
			...
			!(
				(
					(showMarkdownHeadersOnly &&
						newActive?.cell.cellKind === CellKind.Markup &&
						newActive?.level ===
							NotebookOutlineConstants.NonHeaderOutlineLevel) || // show headers only + cell is mkdn + is level 7 (no header)
					(!showCodeCells &&
						newActive?.cell.cellKind === CellKind.Code) || // show code cells   + cell is code
					(!showCodeCellSymbols &&
						newActive?.cell.cellKind === CellKind.Code &&
						newActive?.level >
							NotebookOutlineConstants.NonHeaderOutlineLevel)
				) // show code symbols + cell is code + has level > 7 (nb symbol levels)
			)
		) {
