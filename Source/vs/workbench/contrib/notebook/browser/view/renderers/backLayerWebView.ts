...
import type { INotebookKernel } from "vs/workbench/contrib/notebook/common/notebookKernelService";
import { INotebookLoggingService } from "vs/workbench/contrib/notebook/common/notebookLoggingService";
import type { IScopedRendererMessaging } from "vs/workbench/contrib/notebook/common/notebookRendererMessagingService";
...
export class BackLayerWebView<T extends ICommonCellInfo> extends Themable {
	...
	private _generateStyles() {
		return {
			"notebook-output-left-margin": `${
				this.options.leftMargin + this.options.runGutter
			}px`,
			"notebook-output-width": `calc(100% - ${
				this.options.leftMargin +
				this.options.rightMargin +
				this.options.runGutter
			}px)`,
			...
			"notebook-markdown-min-height": `${
				this.options.previewNodePadding * 2
			}px`,
			...
			"notebook-cell-output-font-size": `${
				this.options.outputFontSize || this.options.fontSize
			}px`,
			"notebook-cell-output-line-height": `${this.options.outputLineHeight}px`,
			"notebook-cell-output-max-height": `${
				this.options.outputLineHeight * this.options.outputLineLimit
			}px`,
