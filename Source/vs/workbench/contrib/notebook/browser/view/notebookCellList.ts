...
import {
	...
	INotebookCellList,
} from "vs/workbench/contrib/notebook/browser/view/notebookRenderingCommon";
	...
	implements IDisposable, IStyleController, INotebookCellList
{
	...
	protected override createListView(
		...
		return new NotebookCellListView(
			container,
			virtualDelegate,
			renderers,
			viewOptions
	...
	async revealCellAsync(cell: ICellViewModel, revealType: CellRevealType) {
		...
		const revealPosition =
			revealType === CellRevealType.NearTopIfOutsideViewport
	...
	async revealCellRangeAsync(
		...
		switch (revealType) {
			case CellRevealRangeType.Default:
				return this._revealRangeInternalAsync(
					index,
					range,
					CellEditorRevealType.Range
				);
			case CellRevealRangeType.Center:
				return this._revealRangeInCenterInternalAsync(
					index,
					range,
					CellEditorRevealType.Range
				);
	...
	private async _revealRangeInternalAsync(
		viewIndex: number,
		range: Selection | Range,
		revealType: CellEditorRevealType
	): Promise<void> {
		const scrollTop = this.getViewScrollTop();
		const wrapperBottom = this.getViewScrollBottom();
		const elementTop = this.view.elementTop(viewIndex);
	...
	updateElementHeight2(
		...
		if (anchorElementIndex !== null) {
			return this.view.updateElementHeight(
				index,
				size,
				anchorElementIndex
			);
