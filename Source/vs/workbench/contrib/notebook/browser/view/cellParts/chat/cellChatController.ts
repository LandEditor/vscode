...
export class NotebookCellChatController extends Disposable {
	private static _cellChatControllers = new WeakMap<
		ICellViewModel,
	...
	private _initialize(editor: IActiveCodeEditor) {
		...
		this._toolbar = this._register(
			this._instantiationService.createInstance(
				...
				this._toolbarDOM.editorToolbar,
...
class EditStrategy {
	...
	async cancel() {
		...
		while (
			targetAltVersion < modelN.getAlternativeVersionId() &&
			modelN.canUndo()
		) {
			modelN.undo();
		}
