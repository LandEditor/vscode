...
import {
	ClickTargetType,
	type IClickTarget,
} from "vs/workbench/contrib/notebook/browser/view/cellParts/cellWidgets";
...
export class CellEditorStatusBar extends CellContentPart {
	...
	constructor(
		...
		this._register(DOM.addDisposableListener(this.statusBarContainer, DOM.EventType.CLICK, e => {
			...
			} else if ((e.target as HTMLElement).classList.contains('cell-status-item-has-command')) {
					this._onDidClick.fire({
			...
			const hoverContent =
				typeof item.tooltip === "string"
					...
					: ({
						...
						} as IUpdatableHoverTooltipMarkdownString);
			this._itemDisposables.add(
