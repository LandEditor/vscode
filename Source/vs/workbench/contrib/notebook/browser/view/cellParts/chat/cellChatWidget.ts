...
export class CellChatWidget extends Disposable {
	...
	private readonly _elements = h("div.cell-chat-container@root", [
		h("div.body", [
	...
	constructor(
		...
		const uri = URI.from({
			...
			path: `/notebook-cell-chat/model${CellChatWidget._modelPool++}.txt`,
		});
		this._inputModel = this._register(
		...
		this._toolbar = this._register(
			this._instantiationService.createInstance(
				...
				MENU_NOTEBOOK_CELL_CHAT_WIDGET,
				{
		...
		const copilotGeneratedCodeSpan = $(
			"span.copilot-generated-code",
			{},
		...
		this._register(
			this._inputEditor.onDidFocusEditorWidget(() => {
				if (this._activeCell) {
		...
		this._ctxInputEditorFocused = CTX_NOTEBOOK_CELL_CHAT_FOCUSED.bindTo(
