...
} from "../../scm/browser/dirtydiffDecorator.js";
...
class NotebookChatEditorController extends Disposable {
	...
	constructor(
		...
		const entryObs = observableValue<IModifiedFileEntry | undefined>(
			"fileentry",
			undefined,
		);

		const notebookDiff = observableValue<
			{ cellDiff: CellDiffInfo[]; modelVersion: number } | undefined
		>("cellDiffInfo", undefined);
		...
		this._register(
			autorunWithStore(async (r, store) => {
				...
				const result = this._register(
					await this.originalModelRefFactory.getOrCreate(
						entry,
						model.viewType,
					),
				);
		...
		const decorators = new Map<
			...
			{ editor: ICodeEditor } & IDisposable
		>();
				...
				diffInfo.cellDiff.forEach((diff) => {
					if (diff.type === "modified") {
						...
						const originalCellValue =
							original.cells[diff.originalCellIndex].getValue();
						...
						if (
							editor &&
							decorators.get(modifiedCell)?.editor !== editor
						) {
		...
		this._register(
			autorun((r) => {
				...
				if (
					...
					!modified ||
					!original ||
					(diffInfo && diffInfo.modelVersion !== modified.versionId)
				) {
					return;
				}
				if (!diffInfo) {
...
class NotebookCellDiffDecorator extends DisposableStore {
	...
	constructor(
		...
		const shouldBeReadOnly = derived(this, (r) => {
			...
			return value.entries
				.read(r)
				.some((e) =>
					isEqual(e.modifiedURI, this.editor.getModel()?.uri),
				);
		});
	...
	private async _updateImpl() {
		...
		if (
			...
			this.editor.getModel()?.getVersionId() === version
		) {
	...
	private getOrCreateOriginalModel() {
		...
		return (this._originalModel = this.add(
	...
	private _updateWithDiff(
		originalModel: ITextModel | undefined,
		diff: IDocumentDiff,
	): void {
		...
		const chatDiffWholeLineAddDecoration =
			ModelDecorationOptions.createDynamic({
				...diffWholeLineAddDecoration,
				stickiness: TrackedRangeStickiness.NeverGrowsWhenTypingAtEdges,
			});
		...
		this.editor.changeViewZones((viewZoneChangeAccessor) => {
			...
			for (const diffEntry of diff.changes) {
				...
				const isCreatedContent =
					...
					decorations[0].range.startLineNumber === 1;
...
class NotebookModelSynchronizer extends Disposable {
	...
	constructor(
		...
		let cancellationToken = cancellationTokenStore.add(
		...
		const updateNotebookModel = (
			...
			this.throttledUpdateNotebookModel.trigger(() =>
				this.updateNotebookModel(entry, viewType, token),
			);
		};
		...
		this._register(
			modifiedModel.onDidChangeContent(async () => {
				...
				if (
					...
					modifiedModel.getValue() === entry.originalModel.getValue()
				) {
					if (await this.revert()) {
						this._onDidRevert.fire(true);
					}
	...
	private async accept() {
		const modifiedModel = (this.entry as ChatEditingModifiedFileEntry)
			.modifiedModel;
	...
	private async updateNotebookModel(
		...
		const model = await this.getModifiedModelForDiff(
			entry,
			viewType,
			token,
		);
		...
		if (
			...
			originalModelVersion !== original.versionId
		) {
			return;
		}
...
const ttPolicy = createTrustedTypesPolicy("notebookChatEditController", {
...
class NotebookDeletedCellDecorator extends Disposable {
	...
	private async _createWidgetImpl(
		...
		const widgets = cells.map(
			(cell) =>
				new NotebookDeletedCellWidget(
		...
		const totalHeight = heights.reduce<number>(
			(prev, curr) => prev + curr,
			0,
		);
