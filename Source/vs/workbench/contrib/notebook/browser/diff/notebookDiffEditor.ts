...
import type { IDiffChange, IDiffResult } from "vs/base/common/diff/diff";
...
} from "vs/workbench/contrib/notebook/browser/diff/eventDispatcher";
import {
	DIFF_CELL_MARGIN,
	DiffSide,
	type IDiffCellInfo,
...
} from "vs/workbench/contrib/notebook/browser/diff/notebookDiffList";
...
import type { NotebookLayoutInfo } from "vs/workbench/contrib/notebook/browser/notebookViewEvents";
import {
	BackLayerWebView,
	type INotebookDelegateForWebview,
} from "vs/workbench/contrib/notebook/browser/view/renderers/backLayerWebView";
...
import type { NotebookDiffEditorInput } from "../../common/notebookDiffEditorInput";
		...
		IEditorPaneWithScrolling
{
	...
	async updateLayout(token: CancellationToken, selections?: number[]) {
		...
		const { viewModels, firstChangeIndex } =
			NotebookTextDiffEditor.computeDiff(
		...
		if (
			this._revealFirst &&
			firstChangeIndex !== -1 &&
			firstChangeIndex < this._list.length
		) {
	...
	private _isViewModelTheSame(viewModels: DiffElementViewModelBase[]) {
		...
		if (this._diffElementViewModels.length === viewModels.length) {
			for (let i = 0; i < viewModels.length; i++) {
				...
				if (
					a.original?.textModel.getHashValue() !==
						b.original?.textModel.getHashValue() ||
					a.modified?.textModel.getHashValue() !==
						b.modified?.textModel.getHashValue()
				) {
					isSame = false;
					break;
				}
	...
	static computeDiff(
		...
		for (let i = 0; i < cellChanges.length; i++) {
			...
			for (let j = 0; j < change.originalStart - originalCellIndex; j++) {
				...
				if (
					originalCell.getHashValue() === modifiedCell.getHashValue()
				) {
					diffElementViewModels.push(
						new SideBySideDiffElementViewModel(
			...
			diffElementViewModels.push(
				new SideBySideDiffElementViewModel(
					...
					instantiationService.createInstance(
						DiffNestedCellViewModel,
						modifiedModel.cells[
							i - originalCellIndex + modifiedCellIndex
						],
					),
					"unchanged",
					eventDispatcher,
	...
	static computeModifiedLCS(
		...
		for (let j = 0; j < modifiedLen; j++) {
			const isTheSame = originalModel.cells[
				change.originalStart + j
			].equal(modifiedModel.cells[change.modifiedStart + j]);
			result.push(
				new SideBySideDiffElementViewModel(
					modifiedModel,
					originalModel,
		...
		for (let j = modifiedLen; j < change.originalLength; j++) {
			// deletion
			result.push(
				new SingleSideDiffElementViewModel(
					originalModel,
					modifiedModel,
					instantiationService.createInstance(
						DiffNestedCellViewModel,
	...
	nextChange(): void {
		...
		if (nextChangeIndex < this._diffElementViewModels.length) {
			...
			// go to the first one
			const index = this._diffElementViewModels.findIndex(
				(vm) => vm.type !== "unchanged",
			);
			if (index >= 0) {
				this._list.setFocus([index]);
	...
	getCellOutputLayoutInfo(nestedCell: DiffNestedCellViewModel) {
		...
		const belongToOriginalDocument =
			this._model.original.notebook.uri.toString() ===
			documentModel.notebook.toString();
		const viewModel = this._diffElementViewModels.find((element) => {
			const textModel = belongToOriginalDocument
				? element.original
				: element.modified;
			if (!textModel) {
				return false;
		...
		if (!viewModel) {
			throw new Error(
				"Nested cell in the diff editor does not match any diff element",
			);
		}
