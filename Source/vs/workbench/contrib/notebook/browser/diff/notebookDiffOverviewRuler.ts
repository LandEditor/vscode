...
import type { DiffElementViewModelBase } from "vs/workbench/contrib/notebook/browser/diff/diffElementViewModel";
import type { NotebookDiffEditorEventDispatcher } from "vs/workbench/contrib/notebook/browser/diff/eventDispatcher";
import type { INotebookTextDiffEditor } from "vs/workbench/contrib/notebook/browser/diff/notebookDiffEditorBrowser";
...
export class NotebookDiffOverviewRuler extends Themable {
	...
	updateViewModels(
		elements: DiffElementViewModelBase[],
	...
	private _layoutNow() {
		...
		const contentHeight = this._diffElementViewModels
			.map((view) => view.layoutInfo.totalHeight)
			.reduce((a, b) => a + b, 0);
	...
	private _renderCanvas(
		...
		for (let i = 0; i < this._diffElementViewModels.length; i++) {
			...
			const cellHeight = Math.round(
				(element.layoutInfo.totalHeight / scrollHeight) *
					ratio *
					height,
			);
