...
import type { ToolBar } from "vs/base/browser/ui/toolbar/toolbar";
...
import type * as editorCommon from "vs/editor/common/editorCommon";
...
import {
	...
	fixedEditorOptions,
	fixedEditorPadding,
} from "vs/workbench/contrib/notebook/browser/diff/diffCellEditorOptions";
...
import {
	collapsedIcon,
	expandedIcon,
} from "vs/workbench/contrib/notebook/browser/notebookIcons";
...
class PropertyHeader extends Disposable {
	...
	buildHeader(): void {
		...
		this._toolbar = new WorkbenchToolBar(
		...
		const propertyChanged = NOTEBOOK_DIFF_CELL_PROPERTY.bindTo(
		...
		this._menu = this.menuService.createMenu(
		...
		this._register(
			this.notebookEditor.onMouseUp((e) => {
				...
				if (
					...
					if (
						!parent.classList.contains("property-folding-indicator")
					) {
						return;
					}
...
abstract class AbstractElementRenderer extends Disposable {
	...
	updateBorders() {
		this.templateData.leftBorder.style.height = `${
			this.cell.layoutInfo.totalHeight - 32
		}px`;
		this.templateData.rightBorder.style.height = `${
			this.cell.layoutInfo.totalHeight - 32
		}px`;
		this.templateData.bottomBorder.style.top = `${
			this.cell.layoutInfo.totalHeight - 32
		}px`;
	}
...
export class DeletedElement extends SingleSideDiffElement {
	...
	updateSourceEditor(): void {
		...
		this.textModelService
			.createModelReference(originalCell.uri)
			.then((ref) => {
				if (this._isDisposed) {
					return;
				}
		...
		DOM.scheduleAtNextAnimationFrame(
			...
			() => {
				...
				if (this._diagonalFill) {
					this._diagonalFill.style.height = `${
						this.cell.layoutInfo.totalHeight - 32
					}px`;
				}
...
export class InsertElement extends SingleSideDiffElement {
	...
	updateSourceEditor(): void {
		...
		this.textModelService
			.createModelReference(modifiedCell.uri)
			.then((ref) => {
				if (this._isDisposed) {
					return;
				}
				...
				this._editor.restoreViewState(
					this.cell.getSourceEditorViewState() as editorCommon.ICodeEditorViewState,
				);
				this.cell.editorHeight = this._editor.getContentHeight();
			});
	}
...
export class ModifiedElement extends AbstractElementRenderer {
	...
	updateSourceEditor(): void {
		...
		if (
			this.cell.modified.textModel.getValue() !==
			this.cell.original.textModel.getValue()
		) {
			...
			() => {
				...
				if (state.outputTotalHeight || state.outerWidth) {
					...
					if (this._outputMetadataContainer) {
						...
						this._outputMetadataContainer.style.top = `${
							this.cell.layoutInfo.outputTotalHeight -
							this.cell.layoutInfo.outputMetadataHeight
						}px`;
						this._outputMetadataEditor?.layout();
					}
				}
