		...
		const _toolbar = instaService.createInstance(
			...
			MenuId.ChatEditingEditorContent,
			{
				...
				actionViewItemProvider: (action, options) => {
					...
					if (
						action.id === "chatEditor.action.accept" ||
						action.id === "chatEditor.action.reject"
					) {
...
class NextPreviousChangeActionRunner extends ActionRunner {
	...
	protected override async runAction(
		...
		const activeCellIndex = viewModel.viewCells.findIndex(
		...
		if (typeof index === "number") {
			return this.notebookEditor.focusNotebookCell(
		...
		index = this.getNextCellDiff(
			this.direction === "next" ? -1 : viewModel.viewCells.length + 1,
			cellDiff,
		);
	...
	private getNextCellDiff(
		...
		if (this.direction === "next") {
			return cellDiffInfo
				.filter(
					(d) =>
						(d.type === "insert" || d.type === "modified") &&
						d.modifiedCellIndex > currentCellIndex,
				)
				.map((d) =>
					d.type === "insert" || d.type === "modified"
						? d.modifiedCellIndex
