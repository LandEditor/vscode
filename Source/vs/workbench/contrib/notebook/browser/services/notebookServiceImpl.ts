	...
	private _registerContributionPoint(
		...
		for (const selector of notebookProviderInfo.selectors) {
			...
			const notebookEditorInputFactory: EditorInputFactoryFunction = ({
				...
				return {
					editor: NotebookEditorInput.create(
...
export class NotebookService extends Disposable implements INotebookService {
	...
	constructor(
		...
		const onDidAddDecorationType = (e: string) => {
			...
			const options = this._codeEditorService.resolveDecorationOptions(
				e,
				true
			);
			if (
				...
				const cssRules =
					this._codeEditorService.resolveDecorationCSSRules(e);
				if (cssRules !== null) {
		...
		this._register(
			this._codeEditorService.onDecorationTypeRegistered(
	...
	*getStaticPreloads(viewType: string): Iterable<INotebookStaticPreloadInfo> {
	...
	createNotebookTextModel(
		...
		this._models.set(
			uri,
			new ModelData(notebookModel, this._onWillDisposeDocument.bind(this))
		);
