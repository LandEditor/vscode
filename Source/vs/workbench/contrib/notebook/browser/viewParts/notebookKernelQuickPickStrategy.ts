...
abstract class KernelPickerStrategyBase implements IKernelPickerStrategy {
		...
		if (wantedId) {
			...
			if (!newKernel) {
				this._logService.warn(
					`wanted kernel DOES NOT EXIST, wanted: ${wantedId}, all: ${all.map(
						(k) => k.id,
					)}`,
				);
				return false;
	...
	private getSuggestedLanguage(
		...
		let suggestedKernelLanguage: string | undefined = (
			metaData.custom as any
...
export class KernelPickerMRUStrategy extends KernelPickerStrategyBase {
	constructor(
		...
		_paneCompositePartService: IPaneCompositePartService,
		@IExtensionsWorkbenchService
