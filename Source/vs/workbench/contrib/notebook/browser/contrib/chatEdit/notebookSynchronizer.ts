...
import {
	...
	computeDiff,
	prettyChanges,
} from "../../diff/notebookDiffViewModel.js";
import { CellEditState } from "../../notebookBrowser.js";
import { INotebookEditorService } from "../../services/notebookEditorService.js";
import { INotebookOriginalModelReferenceFactory } from "./notebookOriginalModelRefFactory.js";

export const INotebookModelSynchronizerFactory =
	createDecorator<INotebookModelSynchronizerFactory>(
...
export interface INotebookModelSynchronizerFactory {
	...
	): IReference<NotebookModelSynchronizer>;
}
...
class NotebookModelSynchronizerReferenceCollection extends ReferenceCollection<NotebookModelSynchronizer> {
	...
	protected override destroyReferencedObject(
		_key: string,
		object: NotebookModelSynchronizer,
	): void {
		object.dispose();
	}
}
...
export class NotebookModelSynchronizerFactory
	implements INotebookModelSynchronizerFactory
{
	readonly _serviceBrand: undefined;
...
export class NotebookModelSynchronizer extends Disposable {
	...
	private _diffInfo = observableValue<
		{ cellDiff: CellDiffInfo[]; modelVersion: number } | undefined
	>("diffInfo", undefined);
	public get diffInfo(): IObservable<
		{ cellDiff: CellDiffInfo[]; modelVersion: number } | undefined
	> {
	...
	constructor(
		...
		private readonly notebookModelResolverService: INotebookEditorModelResolverService,
		...
		const entryObs = derived((r) => {
			const session =
				_chatEditingService.currentEditingSessionObs.read(r);
			if (!session) {
				return;
			}
		...
		this._register(
			chatService.onDidPerformUserAction(async (e) => {
				const entry = entryObs.read(undefined);
				if (!entry) {
					return;
				}
				if (
					e.action.kind === "chatEditingSessionAction" &&
					!e.action.hasRemainingEdits &&
					isEqual(e.action.uri, entry.modifiedURI)
				) {
		...
		const updateNotebookModel = (
			entry: IModifiedFileEntry,
			token: CancellationToken,
		) => {
			this.throttledUpdateNotebookModel.trigger(() =>
				this.updateNotebookModel(entry, token),
			);
		};
		...
		this._register(
			model.onDidChangeContent(() => {
				...
				if (!this.isEditFromUs && this.snapshot) {
					...
					if (entry) {
						this.throttledTextModelUpdate.trigger(() =>
							this.updateTextDocumentModel(entry),
						);
					}
				}
			}),
	...
	private async createSnapshot() {
		...
		try {
			...
			const indentAmount =
				this.model.metadata.indentAmount ||
				ref.object.notebook.metadata.indentAmount ||
				undefined;
	...
	private async updateNotebook(bytes: VSBuffer, saveForRevert: boolean) {
		...
		const ref = await this.notebookModelResolverService.resolve(
			this.model.uri,
		);
		try {
	...
	private async accept(entry: IModifiedFileEntry) {
		...
		const stream =
			await this.notebookService.createNotebookTextDocumentSnapshot(
				this.model.uri,
				SnapshotContext.Save,
				CancellationToken.None,
			);
		const originalModel = await this.getOriginalModel(entry);
		await this.notebookService.restoreNotebookTextModelFromSnapshot(
			originalModel.uri,
			originalModel.viewType,
			stream,
		);
	}
	...
	async getNotebookSerializer() {
		const info = await this.notebookService.withNotebookDataProvider(
			this.model.viewType,
		);
	...
	private async getOriginalModel(
		entry: IModifiedFileEntry,
	): Promise<NotebookTextModel> {
		if (!this._originalModel) {
			this._originalModel = this.originalModelRefFactory
				.getOrCreate(entry, this.model.viewType)
				.then((ref) => {
	...
	private async updateNotebookModel(
		entry: IModifiedFileEntry,
		token: CancellationToken,
	) {
		const modifiedModelVersion = (
			entry as ChatEditingModifiedFileEntry
		).modifiedModel.getVersionId();
		const currentModel = this.model;
		const modelVersion = currentModel?.versionId ?? 0;
		const modelWithChatEdits = await this.getModifiedModelForDiff(
			entry,
			token,
		);
		if (
		...
		const cellDiffInfo = (
			await this.computeDiff(originalModel, modelWithChatEdits, token)
		)?.cellDiffInfo;
		// This is the diff from the current model to the model with chat edits.
		const cellDiffInfoToApplyEdits = (
			await this.computeDiff(currentModel, modelWithChatEdits, token)
		)?.cellDiffInfo;
		...
		if (
			!cellDiffInfo ||
			!cellDiffInfoToApplyEdits ||
			token.isCancellationRequested ||
			currentVersion !== modifiedModelVersion ||
			modelVersion !== currentModel.versionId
		) {
			return;
		}
		...
		try {
			...
			await Promise.all(
				cellDiffInfoToApplyEdits.reverse().map(async (diff) => {
					if (diff.type === "delete") {
			...
			cellDiffInfoToApplyEdits.reverse().forEach((diff) => {
				...
				if (diff.type === "insert") {
					...
					const index = currentModel.cells.length
						? originalIndex + 1
						: originalIndex;
					const cell =
						modelWithChatEdits.cells[diff.modifiedCellIndex];
			...
			if (edits.length) {
				...
				for (const edit of edits.filter(
					(e) => e.editType === CellEditType.Replace,
				)) {
			...
			// Finally update
			await Promise.all(
				cellDiffInfoToApplyEdits.map(async (diff) => {
					if (diff.type === "modified") {
						...
						try {
							const modifiedCell =
								modelWithChatEdits.cells[
									diff.modifiedCellIndex
								];
							if (modifiedCell.cellKind === cell.cellKind) {
								const cellViewModel =
									notebookEditor?.getCellByHandle(
										cell.handle,
									);
								cellViewModel?.updateEditState(
									CellEditState.Editing,
								...
								const newCellDto: ICellDto2 = {
									...
									outputs: modifiedCell.outputs.map(
										(output) => output.asDto(),
									),
								...
								const newCell =
									currentModel.cells[diff.originalCellIndex];
								const cellViewModel =
									notebookEditor?.getCellByHandle(
										newCell.handle,
									);
	...
	private async getModifiedModelForDiff(
		entry: IModifiedFileEntry,
		token: CancellationToken,
	): Promise<NotebookTextModel | undefined> {
		const text = (
			entry as ChatEditingModifiedFileEntry
		).modifiedModel.getValue();
		const bytes = VSBuffer.fromString(text);
		const uri = entry.modifiedURI.with({
			scheme: `NotebookChatEditorController.modifiedScheme${Date.now().toString()}`,
		});
		...
		try {
			const model = await this.notebookService.createNotebookTextModel(
				this.model.viewType,
				uri,
				stream,
			);
