	...
	constructor(
		...
		const shouldBeReadOnly = derived(this, (r) => {
			...
			return value.entries
				.read(r)
				.some((e) => isEqual(e.modifiedURI, editor.getModel()?.uri));
		});
		...
		Array.from(this.deletedCellInfos.keys())
			...
			.forEach((originalIndex) => {
				...
				if (previousDeletedCell) {
					const deletedCell =
						this.deletedCellInfos.get(originalIndex);

					if (deletedCell) {
						deletedCell.offset =
							previousDeletedCell.height +
							previousDeletedCell.offset;
					}
				}
			});
...
function areLineRangeMappinsEqual(
	...
	if (
		a.some((c, i) => {
			...
			if (
				(c.innerChanges || []).length !==
				(bChange.innerChanges || []).length
			) {
				return true;
			}
			if (
				(c.innerChanges || []).some((innerC, innerIdx) => {
					...
					if (
						!innerC.modifiedRange.equalsRange(bInnerC.modifiedRange)
					) {
						return true;
					}
					if (
						!innerC.originalRange.equalsRange(bInnerC.originalRange)
					) {
						return true;
					}
					return false;
