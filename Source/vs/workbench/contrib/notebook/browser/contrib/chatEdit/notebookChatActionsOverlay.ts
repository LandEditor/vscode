...
import {
	AcceptAction,
	RejectAction,
} from "../../../../chat/browser/chatEditorActions.js";
		...
		this._register(
			autorunWithStore((r, store) => {
				...
				const idx = entries.findIndex((e) =>
					isEqual(e.modifiedURI, model.uri),
				);
				if (idx >= 0) {
...
export class NotebookChatActionsOverlay extends Disposable {
	private readonly focusedDiff = observableValue<CellDiffInfo | undefined>(
		"focusedDiff",
		undefined,
	);
	...
	constructor(
		...
		this._register(
			notebookEditor.onDidBlurWidget(() => {
				if (
					getNotebookEditorFromEditorPane(
						_editorService.activeEditorPane,
					) !== notebookEditor
				) {
		...
		const _toolbar = instaService.createInstance(
			MenuWorkbenchToolBar,
			this.toolbarNode,
			MenuId.ChatEditingEditorContent,
			{
				...
				actionViewItemProvider: (action, options) => {
					...
					if (
						...
						return new (class extends ActionViewItem {
							private readonly _reveal = this._store.add(
					...
					if (
						action.id === "chatEditor.action.navigateNext" ||
						action.id === "chatEditor.action.navigatePrevious"
					) {
						return new (class extends ActionViewItem {
							constructor() {
								super(undefined, action, {
							...
							override set actionRunner(_: IActionRunner) {
								...
								const direction =
									...
									"chatEditor.action.navigateNext"
										...
										: "previous";
								super.actionRunner =
									new NextPreviousChangeActionRunner(
										notebookEditor,
										cellDiffInfo,
										entry,
										next,
...
class NextPreviousChangeActionRunner extends ActionRunner {
	constructor(
		private readonly notebookEditor: INotebookEditor,
		private readonly cellDiffInfo: IObservable<
		...
		private readonly focusedDiff: ISettableObservable<
			CellDiffInfo | undefined
		>,
	) {
		super();
	...
	protected override async runAction(
		...
		if (
			!viewModel ||
			!cellDiff?.length ||
			(!activeCell && this.focusedDiff.read(undefined))
		) {
	...
	private async focusDiff(diff: CellDiffInfo, viewModel: INotebookViewModel) {
		if (diff.type === "delete") {
			const top = this.deletedCellDecorator.getTop(
			...
			viewModel.viewCells[index].updateEditState(
	...
	private getNextCellDiff(
		cellDiffInfo: CellDiffInfo[],
		viewModel: INotebookViewModel,
	) {
		const activeCell = this.notebookEditor.activeCellAndCodeEditor;
		const currentCellIndex = activeCell
			? viewModel.viewCells.findIndex(
					(c) => c.handle === activeCell[0].handle,
				)
			: this.direction === "next"
				? 0
				: viewModel.viewCells.length - 1;
		if (this.focusedDiff.read(undefined)) {
			const changes = cellDiffInfo.filter((d) => d.type !== "unchanged");
			const idx = changes.findIndex(
				(d) => d === this.focusedDiff.read(undefined),
			);
			if (idx >= 0) {
		...
		return this.direction === "next"
			? cellDiffInfo[0]
			: cellDiffInfo[cellDiffInfo.length - 1];
	}
		...
		await this.editorService.openEditor(
			{
				...
				options: {
					selection:
						change &&
						Range.fromPositions({
							lineNumber: change.original.startLineNumber,
							column: 1,
						}),
					revealIfOpened: false,
					revealIfVisible: false,
