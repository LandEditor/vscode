...
class NotebookSynchronizerSaveParticipant extends NotebookSaveParticipant {
	...
	override async participate(
		...
		if (
			!(entry && entry instanceof ChatEditingModifiedNotebookEntry) &&
			!inWorkingSet
		) {
...
export class NotebookSynchronizerService
	extends Disposable
	implements INotebookSynchronizerService
{
	...
	constructor(
		...
		super();
		this._register(
			this._workingCopyFileService.addSaveParticipant(
