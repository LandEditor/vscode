...
import { type FuzzyScore, createMatches } from "vs/base/common/filters";
import {
	Disposable,
	DisposableStore,
	type IDisposable,
...
import type { IWorkbenchDataTreeOptions } from "vs/platform/list/browser/listService";
	...
	implements IQuickPickDataSource<OutlineEntry>
{
	...
	getQuickPickElements(): IQuickPickOutlineElement<OutlineEntry>[] {
		...
		const showSymbols = this._configurationService.getValue<boolean>(
		...
		for (let i = 0; i < bucket.length; i++) {
			...
			if (!showSymbols && isSymbol(element)) {
				continue;
			}
	...
	implements IDataSource<NotebookCellOutline, OutlineEntry>
{
	...
	*getChildren(
		...
		const showCodeCells = this._configurationService.getValue<boolean>(
			NotebookSetting.outlineShowCodeCells,
		);
		const showCodeCellSymbols =
...
export class NotebookCellOutline implements IOutline<OutlineEntry> {
	...
	private _outlineProviderReference:
		| IReference<NotebookCellOutlineProvider>
		| undefined;
	...
	async setFullSymbols(cancelToken: CancellationToken) {
		await this._outlineProviderReference?.object?.setFullSymbols(
			cancelToken,
		);
	}
	...
	implements IOutlineCreator<NotebookEditor, OutlineEntry>
{
	...
	async createOutline(
		...
		const showAllGotoSymbols = this._configurationService.getValue<boolean>(
			NotebookSetting.gotoSymbolsAllSymbols,
		);
		const showAllOutlineSymbols =
			this._configurationService.getValue<boolean>(
				NotebookSetting.outlineShowCodeCellSymbols,
			);
