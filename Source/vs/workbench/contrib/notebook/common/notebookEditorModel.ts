...
import type { IFileWorkingCopyManager } from "vs/workbench/services/workingCopy/common/fileWorkingCopyManager";
import {
				...
				if (!token.isCancellationRequested) {
					...
					type notebookSaveErrorClassification = {
						...
						versionMismatch: {
							classification: "SystemMetaData";
							purpose: "PerformanceAndHealth";
							comment: "If the error was because of a version mismatch";
						};
					};
		...
		IUntitledFileWorkingCopyModelFactory<NotebookFileWorkingCopyModel>
{
	...
	async createModel(
		...
		const notebookModel = this._notebookService.createNotebookTextModel(
			info.viewType,
			resource,
			data,
