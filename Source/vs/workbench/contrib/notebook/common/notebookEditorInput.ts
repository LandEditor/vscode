...
export class NotebookEditorInput extends AbstractResourceEditorInput {
	...
	override getDescription(verbosity = Verbosity.MEDIUM): string | undefined {
		if (
			...
			this._editorModelReference?.object.hasAssociatedFilePath()
		) {
	...
	override async saveAs(
		...
		const pathCandidate = this.hasCapability(
			...
			: this._editorModelReference.object.resource;
	...
	override async rename(
		...
		if (this._editorModelReference) {
			const contributedNotebookProviders =
				this._notebookService.getContributedNotebookTypes(target);

			if (
				contributedNotebookProviders.find(
					(provider) =>
						provider.id ===
						this._editorModelReference!.object.viewType
				)
			) {
	...
	private _move(
		...
		const editorInput = NotebookEditorInput.create(
	...
	override async resolve(
		...
		if (!this._editorModelReference) {
			...
			this._register(
				this._editorModelReference.object.onDidChangeDirty(() =>
					this._onDidChangeDirty.fire()
				)
			);
			this._register(
				this._editorModelReference.object.onDidChangeReadonly(() =>
					this._onDidChangeCapabilities.fire()
				)
			);
			this._register(
				this._editorModelReference.object.onDidRevertUntitled(() =>
					this.dispose()
				)
			);
		...
		if (this.options._backupId) {
			const info = await this._notebookService.withNotebookDataProvider(
				this._editorModelReference.object.notebook.viewType
			);
				...
				[
					{
						...
						index: 0,
						count: this._editorModelReference.object.notebook
							.length,
						cells: data.cells,
					},
				],
