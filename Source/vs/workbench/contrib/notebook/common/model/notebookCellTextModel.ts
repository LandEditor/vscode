...
import { StringSHA1, hash } from "vs/base/common/hash";
...
import type { ILanguageDetectionService } from "vs/workbench/services/languageDetection/common/languageDetectionWorkerService";
		...
		for (const key of keys) {
			if (!transientCellMetadata[key as keyof NotebookCellMetadata]) {
				filteredMetadata[key] =
					this.metadata[key as keyof NotebookCellMetadata];
			}
		}
