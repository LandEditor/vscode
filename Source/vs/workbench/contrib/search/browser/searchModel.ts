...
export class Match {
	...
	preview(): { before: string; inside: string; after: string } {
		let before = this._oneLinePreviewText.substring(
				0,
...
export class CellMatch {
	...
	public addContext(textSearchMatches: ITextSearchMatch[]) {
		...
		this.cell.resolveTextModel().then((textModel) => {
			const textResultsWithContext = addContextToEditorMatches(
				textSearchMatches,
				textModel,
...
export class FileMatch extends Disposable implements IFileMatch {
		...
		if (model) {
			...
			if (this.rawMatch.results) {
				this.rawMatch.results
					...
					.forEach((rawMatch) => {
						textSearchResultToMatches(rawMatch, this).forEach((m) =>
							this.add(m)
						);
					});
			}
	...
	private updateMatchesForModel(): void {
		...
		const matches = this._model.findMatches(
			...
			wordSeparators,
			false,
			this._maxResults ?? Number.MAX_SAFE_INTEGER
		);
	...
	private updateMatches(
		...
		textSearchResults.forEach((textSearchResult) => {
			textSearchResultToMatches(textSearchResult, this).forEach(
				(match) => {
	...
	private setNotebookFindMatchDecorationsUsingCellMatches(
		...
		const cellFindMatch: CellFindMatchWithIndex[] = cells.map((cell) => {
			const webviewMatches: CellWebviewFindMatch[] =
				cell.webviewMatches.map((match) => {
	...
	private async highlightCurrentFindMatchDecoration(
		...
		if (
			!this._findMatchDecorationModel ||
			match.cell instanceof CellSearchModel
		) {
	...
	private revealCellRange(
		...
		if (match.webviewIndex !== undefined) {
			...
			if (index !== undefined) {
				this._notebookEditorWidget.revealCellOffsetInCenterAsync(
					match.cell,
					outputOffset ?? 0
				);
			}
...
export class FolderMatch extends Disposable {
	...
	public createIntermediateFolderMatch(
		...
		const folderMatch = this.instantiationService.createInstance(
			FolderMatchWithResource,
			resource,
			id,
			index,
			...
			if (existingFileMatch) {
				if (rawFileMatch.results) {
					rawFileMatch.results.filter(resultIsMatch).forEach((m) => {
						textSearchResultToMatches(m, existingFileMatch).forEach(
...
export class SearchResult extends Disposable {
	...
	private _createBaseFolderMatch(
		...
		if (resource) {
			folderMatch = this.instantiationService.createInstance(
				FolderMatchWorkspaceRoot,
				resource,
				id,
				index,
	...
	private groupFilesByFolder(fileMatches: IFileMatch[]): {
...
export class SearchModel extends Disposable {
	...
	constructor(
		...
		this._searchResultChangedListener = this._register(
	...
	search(
		...
		Promise.race([
			asyncResults,
			Event.toPromise(progressEmitter.event),
		]).finally(() => {
			/* __GDPR__
	...
	private onSearchCompleted(
		completed: ISearchComplete | null,
		duration: number,
		searchInstanceID: string
	): ISearchComplete | null {
		if (!this._searchQuery) {
			throw new Error(
...
function textSearchResultToMatches(
	...
	if (Array.isArray(rawMatch.ranges)) {
		return rawMatch.ranges.map((r, i) => {
			const previewRange: ISearchRange = (<ISearchRange[]>(
				rawMatch.preview.matches
			))[i];
	...
	textSearchMatches.map((textSearchMatch) => {
		...
		if (Array.isArray(textSearchMatch.ranges)) {
			textSearchMatch.ranges.forEach((r, i) => {
				const previewRange: ISearchRange = (<ISearchRange[]>(
					textSearchMatch.preview.matches
				))[i];
