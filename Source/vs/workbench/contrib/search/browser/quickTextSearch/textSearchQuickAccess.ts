...
import type { IWorkbenchEditorConfiguration } from "vs/workbench/common/editor";
import {
...
} from "vs/workbench/contrib/search/browser/searchView";
import {
...
export class TextSearchQuickAccess extends PickerQuickAccessProvider<ITextSearchQuickAccessItem> {
	...
	override provide(
		picker: IQuickPick<ITextSearchQuickAccessItem>,
		...
		for (let fileIndex = 0; fileIndex < matches.length; fileIndex++) {
			...
			for (
				...
				picks.push({
					...
					ariaLabel: `Match at location ${
						element.range().startLineNumber
					}:${element.range().startColumn} - ${previewText}`,
