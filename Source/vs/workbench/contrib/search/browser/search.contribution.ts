...
} from "vs/workbench/contrib/search/browser/quickTextSearch/textSearchQuickAccess";
...
class RegisterSearchViewContribution implements IWorkbenchContribution {
	constructor(
		...
		Registry.as<IConfigurationMigrationRegistry>(
			...
			{
				key: "search.location",
				migrateFn: (value: any) => ({ value: undefined }),
			},
		]);
	}
}
...
CommandsRegistry.registerCommand(
	...
	async (accessor, ...args): Promise<IWorkspaceSymbol[]> => {
		const [query] = args;
