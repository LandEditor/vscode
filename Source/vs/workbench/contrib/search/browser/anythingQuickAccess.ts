...
export class AnythingQuickAccessProvider extends PickerQuickAccessProvider<IAnythingQuickPickItem> {
		...
		async restoreEditorViewState(): Promise<void> {
			if (this.editorViewState) {
				const options: IEditorOptions = {
					...
					preserveFocus:
						true /* import to not close the picker as a result */,
				};

				await this.editorViewState.group.openEditor(
	...
	protected _getPicks(
		...
		return this.doGetPicks(
			filter,
			{
				...
				includeHelp: runOptions?.includeHelp,
				from: runOptions?.from,
			},
			disposables,
			token
		);
	...
	private doGetPicks(
		...
		return {
			...
			> => {
				...
				const additionalPicks = await this.getAdditionalPicks(
					query,
	...
	private createAnythingPick(
		...
		if (isEditorInput(resourceOrEditor)) {
			...
			description = this.labelService.getUriLabel(dirname(resource), {
				relative: true,
			});
			isDirty =
				this.workingCopyService.isDirty(resource) &&
				!configuration.shortAutoSaveDelay;
			extraClasses = [];
		}
