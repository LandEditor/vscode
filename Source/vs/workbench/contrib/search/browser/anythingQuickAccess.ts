...
import { type IEditor, ScrollType } from "vs/editor/common/editorCommon";
...
export class AnythingQuickAccessProvider extends PickerQuickAccessProvider<IAnythingQuickPickItem> {
	...
	private doGetPicks(
		...
		const configuration = {
			...this.configuration,
			includeSymbols:
				options.includeSymbols ?? this.configuration.includeSymbols,
		};
			...
			> => {
				...
				return additionalPicks.length > 0
					? [
							{
								type: "separator",
								label: configuration.includeSymbols
									? localize(
