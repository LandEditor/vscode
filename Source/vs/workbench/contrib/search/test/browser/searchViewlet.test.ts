...
suite("Search - Viewlet", () => {
	...
	setup(() => {
		...
		instantiation.stub(
			...
			stubNotebookEditorService(instantiation)
		);
	...
	test("Advanced Comparer", () => {
		...
		// By type, bar.a < bar.b
		assert(
			searchMatchComparer(fileMatch3, fileMatch4, SearchSortOrder.Type) <
				0
		);
	});
	...
	test("Cross-type Comparer", () => {
		...
		assert(
			searchComparer(fileMatch1, fileMatch3, SearchSortOrder.FileNames) <
				0
		);
		// bar.b < path.c
		assert(
			searchComparer(fileMatch3, fileMatch2, SearchSortOrder.FileNames) <
				0
		);
		// lineMatch4's parent is fileMatch2, "bar.b < path.c"
		assert(
			searchComparer(fileMatch3, lineMatch4, SearchSortOrder.FileNames) <
				0
		);
