...
suite("SearchModel", () => {
	...
	setup(() => {
		...
		instantiationService.stub(
			IUriIdentityService,
			new UriIdentityService(new FileService(new NullLogService()))
		);
	...
	function notebookSearchServiceWithInfo(
		...
		return <INotebookSearchService>{
			...
			notebookSearch(
				...
				token?.onCancellationRequested(() => tokenSource?.cancel());
				const localResults =
					new ResourceMap<IFileMatchWithCells | null>(
						(uri) => uri.path
					);
	...
	test("Search Model: Search can return notebook results", async () => {
		...
		assert(
			(notebookFileMatches[0] as MatchInNotebook).cell.id ===
				"mdInputCell"
		);
		assert(
			(notebookFileMatches[1] as MatchInNotebook).cell.id === "codeCell"
		);
		assert(
			(notebookFileMatches[2] as MatchInNotebook).cell.id === "codeCell"
		);
		assert(
			(notebookFileMatches[3] as MatchInNotebook).cell.id === "codeCell"
		);
		assert(
			(notebookFileMatches[4] as MatchInNotebook).cell.id === "codeCell"
		);

		const mdCellMatchProcessed = (notebookFileMatches[0] as MatchInNotebook)
			.cellParent;
		const codeCellMatchProcessed = (
			notebookFileMatches[1] as MatchInNotebook
		).cellParent;
