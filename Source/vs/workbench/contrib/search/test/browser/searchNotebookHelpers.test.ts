...
suite("searchNotebookHelpers", () => {
	...
	suite("notebookEditorMatchesToTextSearchResults", () => {
		...
		test("convert CellFindMatchModel to ITextSearchMatch and check results", () => {
			...
			assert.strictEqual(
				markdownContentResults[0].preview.text,
				"# Hello World Test\n"
			);
			assertRangesEqual(markdownContentResults[0].preview.matches, [
				new Range(0, 14, 0, 18),
			]);
			assertRangesEqual(markdownContentResults[0].ranges, [
				new Range(0, 14, 0, 18),
			]);

			assert.strictEqual(codeContentResults.length, 2);
			assert.strictEqual(
				codeContentResults[0].preview.text,
				'print("test! testing!!")\n'
			);
			assert.strictEqual(
				codeContentResults[1].preview.text,
				'print("this is a Test")\n'
			);
			assertRangesEqual(codeContentResults[0].preview.matches, [
				new Range(0, 7, 0, 11),
				new Range(0, 13, 0, 17),
			]);
			assertRangesEqual(codeContentResults[0].ranges, [
				new Range(0, 7, 0, 11),
				new Range(0, 13, 0, 17),
			]);
			...
			assert.strictEqual(
				codeWebviewResults[0].preview.text,
				"test! testing!!"
			);
			assert.strictEqual(
				codeWebviewResults[1].preview.text,
				"test! testing!!"
			);
			assert.strictEqual(
				codeWebviewResults[2].preview.text,
				"this is a Test"
			);

			assertRangesEqual(codeWebviewResults[0].preview.matches, [
				new Range(0, 1, 0, 5),
			]);
			assertRangesEqual(codeWebviewResults[1].preview.matches, [
				new Range(0, 7, 0, 11),
			]);
			assertRangesEqual(codeWebviewResults[2].preview.matches, [
				new Range(0, 11, 0, 15),
			]);
			assertRangesEqual(codeWebviewResults[0].ranges, [
				new Range(0, 1, 0, 5),
			]);
			assertRangesEqual(codeWebviewResults[1].ranges, [
				new Range(0, 7, 0, 11),
			]);
			assertRangesEqual(codeWebviewResults[2].ranges, [
				new Range(0, 11, 0, 15),
			]);
		});
		...
		test("convert ITextSearchMatch to MatchInNotebook", () => {
			...
			const markdownCellContentMatchObjs =
				textSearchMatchesToNotebookMatches(
			...
			const codeWebviewContentMatchObjs =
				textSearchMatchesToNotebookMatches(
					codeWebviewResults,
					codeCellMatch
				);
