			...
			registerAction2(
				class extends Action2 {
					...
					async run(accessor: ServicesAccessor): Promise<void> {
						...
						const logFileOutputChannelDescriptor =
							this.getLogFileOutputChannelDescriptor(
								outputService
							);
						if (logFileOutputChannelDescriptor) {
							await fileConfigurationService.updateReadonly(
								logFileOutputChannelDescriptor.file,
								true
							);
					...
					private getLogFileOutputChannelDescriptor(
