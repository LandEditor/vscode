...
export class TimelinePane extends ViewPane {
	...
	private onTimelineChanged(e: TimelineChangeEvent) {
		if (
			e?.uri === undefined ||
			this.uriIdentityService.extUri.isEqual(e.uri, this.uri)
		) {
	...
	private *getItems(): Generator<ITreeElement<TreeElement>, any, any> {
		...
		if (this.timelinesBySource.size === 1) {
			...
			const sources: {
				...
				nextItem: IteratorResult<TimelineItem, TimelineItem>;
			}[] = [];
	...
	implements ITreeRenderer<TreeElement, FuzzyScore, TimelineElementTemplate>
{
	...
	renderElement(
		...
		if (iconUrl) {
			...
			template.icon.className = `custom-view-tree-node-item-icon ${ThemeIcon.asClassName(
				item.themeIcon,
			)}`;
				...
				class extends Action2 {
					constructor() {
						super({
							id: "timeline.refresh",
							title: {
								value: localize("refresh", "Refresh"),
								original: "Refresh",
							},
							icon: timelineRefresh,
							category: {
								value: localize("timeline", "Timeline"),
								original: "Timeline",
							},
							menu: {
			...
			MenuRegistry.appendMenuItem(MenuId.TimelineTitle, {
				command: {
					...
					title: {
						...
						original: "Pin the Current Timeline",
					},
					icon: timelinePin,
					category: {
						value: localize("timeline", "Timeline"),
			...
			MenuRegistry.appendMenuItem(MenuId.TimelineTitle, {
				command: {
					...
					title: {
						...
						original: "Unpin the Current Timeline",
					},
					icon: timelineUnpin,
					category: {
						value: localize("timeline", "Timeline"),
