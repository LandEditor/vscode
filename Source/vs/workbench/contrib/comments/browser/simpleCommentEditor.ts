...
export function calculateEditorHeight(
	...
	const contentHeight =
		editor.getModel()?.getLineCount()! * lineHeight ??
		editor.getContentHeight(); // Can't just call getContentHeight() because it returns an incorrect, large, value when the editor is first created.
	if (
