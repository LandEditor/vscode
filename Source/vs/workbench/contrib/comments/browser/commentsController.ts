...
import {
	...
	ReviewZoneWidget,
} from "vs/workbench/contrib/comments/browser/commentThreadZoneWidget";
...
class CommentingRangeDecorator {
	...
	public getMatchedCommentAction(
		...
		return Array.from(foundHoverActions.values())
			.filter((action) => {
				return (
					action.range.startLineNumber <=
						commentRange.startLineNumber &&
					commentRange.endLineNumber <= action.range.endLineNumber
				);
			})
...
export class CommentController implements IEditorContribution {
	...
	}; // owner -> threadId -> uniqueIdInThread -> pending comment
	...
	private beginCompute(): Promise<void> {
		...
		return this._computePromise.then(
			(commentInfos) => {
		...
		this.localToDispose.add(
			this.commentService.onDidUpdateCommentThreads(async (e) => {
				...
				for (const thread of added) {
					...
					const matchedNewCommentThreadZones =
						this._commentWidgets.filter(
							(zoneWidget) =>
								zoneWidget.owner === e.owner &&
								zoneWidget.commentThread.commentThreadHandle ===
									-1 &&
								Range.equalsRange(
									zoneWidget.commentThread.range,
									thread.range
								)
						);
					...
					const continueOnCommentIndex =
						this._inProcessContinueOnComments
							.get(e.owner)
							?.findIndex((pending) =>
								Range.lift(pending.range).equalsRange(
									thread.range
								)
							);
					...
					const pendingCommentText =
						(this._pendingNewCommentCache[e.owner] &&
							this._pendingNewCommentCache[e.owner][
								thread.threadId!
							]) ??
						continueOnCommentText;
					const pendingEdits =
						this._pendingEditsCache[e.owner] &&
						this._pendingEditsCache[e.owner][thread.threadId!];
					this.displayCommentThread(
						e.owner,
						thread,
						pendingCommentText,
						pendingEdits
					...
					this._commentInfos
						.filter((info) => info.owner === e.owner)[0]
						.threads.push(thread);
					this.tryUpdateReservedSpace();
				}
	...
	private getCommentProvidersQuickPicks(
		commentInfos: {
			...
			extensionId: string | undefined;
			label: string | undefined;
			commentingRangesInfo: languages.CommentingRanges | undefined;
		}[]
	) {
		...
		if (this._commentWidgets) {
			this._commentWidgets.forEach((zone) => {
				...
				if (Object.keys(pendingEdits).length > 0) {
					...
					delete providerEditsCacheStore[
						zone.commentThread.threadId!
					];
				}
