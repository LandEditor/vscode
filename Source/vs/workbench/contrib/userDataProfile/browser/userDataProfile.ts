	...
	implements IWorkbenchContribution
{
	...
	private registerEditCurrentProfileAction(): IDisposable {
		const that = this;
		return registerAction2(
			class RenameCurrentProfileAction extends Action2 {
				constructor() {
	...
	private registerShowCurrentProfileContentsAction(): IDisposable {
		...
		return registerAction2(
			class ShowProfileContentsAction extends Action2 {
				constructor() {
					super({
						id,
						title: {
				...
				class ExportProfileAction extends Action2 {
					constructor() {
						super({
							...
							category: PROFILES_CATEGORY,
							precondition:
								IS_PROFILE_EXPORT_IN_PROGRESS_CONTEXT.toNegated(),
							menu: [
								{
									id: ProfilesMenu,
				...
				class ImportProfileAction extends Action2 {
					constructor() {
						super({
							...
							category: PROFILES_CATEGORY,
							precondition:
								IS_PROFILE_IMPORT_IN_PROGRESS_CONTEXT.toNegated(),
							menu: [
								{
									id: ProfilesMenu,
									group: "4_import_export_profiles",
							...
							quickPick.onDidAccept(async () => {
								...
								try {
									if (
										...
										return await that.userDataProfileImportExportService.createProfile(
											URI.parse(
												(<
													IProfileTemplateQuickPickItem
												>selectedItem).url,
	...
	private async getProfileTemplatesQuickPickItems(): Promise<
		IProfileTemplateQuickPickItem[]
	> {
