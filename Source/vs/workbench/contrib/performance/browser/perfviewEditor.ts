...
import type * as perf from "vs/base/common/performance";
...
class PerfModelContentProvider implements ITextModelContentProvider {
	...
	private _addSummary(md: MarkdownBuilder): void {
		...
		if (metrics.meminfo) {
			md.li(
				`Memory(Process): ${(
					metrics.meminfo.workingSetSize / ByteSize.KB
				).toFixed(2)} MB working set(${(
	...
	private _addSummaryTable(md: MarkdownBuilder, stats?: LoaderStats): void {
		...
		table.push([
			...
			"[main]",
			`initial startup: ${metrics.initialStartup}, ${
				metrics.initialStartup
					? `state: ${metrics.timers.ellapsedWindowRestoreState}ms, widget: ${metrics.timers.ellapsedBrowserWindowCreate}ms, show: ${metrics.timers.ellapsedWindowMaximize}ms`
					: ""
			}`,
		]);
		table.push([
		...
		table.push([
			"require(workbench.desktop.main.js)",
			metrics.timers.ellapsedRequire,
			"[renderer]",
			`cached data: ${metrics.didUseCachedData ? "YES" : "NO"}${
				stats ? `, node_modules took ${stats.nodeRequireTotal}ms` : ""
			}`,
		]);
		table.push([
		...
		table.push([
			...
			"[renderer]",
			`${
				(contribTimings.get(LifecyclePhase.Starting)?.length ?? 0) +
				(contribTimings.get(LifecyclePhase.Starting)?.length ?? 0)
			} blocking startup`,
		]);
		table.push([
