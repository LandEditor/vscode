...
suite("Workbench - TerminalLocalLinkDetector", () => {
	...
	setup(async () => {
		...
		instantiationService.stub(IFileService, {
			async stat(resource) {
				if (
					!validResources.map((e) => e.path).includes(resource.path)
				) {
	...
	if (isWindows) {
		suite("Windows", () => {
						...
						backend: {
							async getWslPath(
								...
								if (direction === "unix-to-win") {
									return (
										wslUnixToWindowsPathMap.get(original) ??
										original
									);
								}
								return original;
			...
			for (const l of windowsLinks) {
				const baseLink = typeof l === "string" ? l : l.link;
				const resource =
					typeof l === "string" ? URI.file(l) : l.resource;
				suite(`Link "${baseLink}"`, () => {
					for (let i = 0; i < supportedLinkFormats.length; i++) {
						const linkFormat = supportedLinkFormats[i];
						const formattedLink = format(
			...
			for (const l of windowsFallbackLinks) {
				...
				suite(`Fallback link "${baseLink}"`, () => {
					for (
						let i = 0;
						i < supportedFallbackLinkFormats.length;
						i++
					) {
						...
						const linkCellStartOffset =
							linkFormat.linkCellStartOffset ?? 0;
						const linkCellEndOffset =
							linkFormat.linkCellEndOffset ?? 0;
						test(`should detect in "${formattedLink}"`, async () => {
							validResources = [resource];
							await assertLinks(
								TerminalBuiltinLinkType.LocalFile,
