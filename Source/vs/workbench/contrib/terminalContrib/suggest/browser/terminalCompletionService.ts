	...
	implements ITerminalCompletionService
{
	...
	private async _collectCompletions(
		...
		const completionPromises = providers.map(async (provider) => {
			...
			const itemsWithModifiedLabels = completionItems.map(
				(completion) => {
					if (
						devModeEnabled &&
	...
	private async _resolveResources(
		...
		const parentDirPath = cwd.fsPath
		...
		const lastWord =
			promptValue.substring(0, cursorPosition).split(" ").pop() ?? "";

		for (const [dir, prefix] of dirToPrefixMap) {
			const fileStat = await this._fileService.resolve(dir, {
