	...
	implements ITerminalCompletionService
{
	...
	private async _collectCompletions(
		...
		const completionPromises = providers.map(async (provider) => {
			...
			const completions:
				| ITerminalCompletion[]
				| TerminalCompletionList<ITerminalCompletion>
				| undefined = await provider.provideCompletions(
				promptValue,
				cursorPosition,
				token,
			);
			...
			const completionItems = Array.isArray(completions)
				? completions
				: (completions.items ?? []);
			const itemsWithModifiedLabels = this._addDevModeLabel(
				completionItems,
				provider.id,
			);
			...
			if (completions.resourceRequestConfig) {
				...
				if (resourceCompletions) {
					itemsWithModifiedLabels.push(
						...this._addDevModeLabel(
							resourceCompletions,
							provider.id,
						),
					);
	...
	async resolveResources(
		...
		const fileStat = await this._fileService.resolve(cwd, {
		...
		for (const stat of fileStat.children) {
			...
			if (
				filesRequested &&
				!stat.isDirectory &&
				(stat.isFile || stat.resource.scheme === Schemas.file)
			) {
			...
			if (
				...
				if (
					lastWord.length &&
					lastWord.at(-1) !== resourceRequestConfig.pathSeparator &&
					lastWord.at(-1) !== "."
				) {
					label = `.${resourceRequestConfig.pathSeparator}${fileName}`;
				}
			...
			if (
				isDirectory &&
				!label.endsWith(resourceRequestConfig.pathSeparator)
			) {
			...
			resourceCompletions.push({
				...
				replacementIndex:
					cursorPosition - lastWord.length > 0
						? cursorPosition - lastWord.length
						: cursorPosition,
				replacementLength:
					lastWord.length > 0 ? lastWord.length : label.length,
			});
		}
