	...
	implements ITerminalAddon, ISuggestController
{
	...
	private async _handleCompletionProviders(
		...
		if (
			this._lastUserDataTimestamp <
			SuggestAddon.lastAcceptedCompletionTimestamp
		) {
			doNotRequestExtensionCompletions = true;
		}

		const enableExtensionCompletions =
		...
		const replacementIndices = [
			...new Set(providedCompletions.map((c) => c.replacementIndex)),
		];

		const replacementIndex =
			replacementIndices.length === 1 ? replacementIndices[0] : 0;
		...
		if (
			...
			completions.every((e) => e.isDirectory)
		) {
		...
		if (this._isFilteringDirectories) {
			...
			this._pathSeparator =
				firstDir?.label.match(/(?<sep>[\\\/])/)?.groups?.sep ?? sep;
		...
		const model = new SimpleCompletionModel(
			completions
				.filter((c) => !!c.label)
				.map((c) => new SimpleCompletionItem(c)),
			lineContext,
		);
	...
	private _sync(promptInputState: IPromptInputModelState): void {
		...
		if (
			...
			if (config.suggestOnTriggerCharacters && !sent) {
				...
				if (!sent) {
					for (const provider of this._terminalCompletionService
						.providers) {
		...
		if (this._terminalSuggestWidgetVisibleContextKey.get()) {
			...
			let normalizedLeadingLineContent =
				this._currentPromptInputState.value.substring(
					this._providerReplacementIndex,
	...
	acceptSelectedSuggestion(
		...
		if (
			(completion.isDirectory || completion.isFile) &&
