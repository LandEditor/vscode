...
export class TerminalStickyScrollOverlay extends Disposable {
			...
			Event.runAndSubscribe(
				...
				(e) => {
					if (
						...
						this._maxLineCount = configurationService.getValue(
							TerminalSettingId.StickyScrollMaxLineCount
						);
					}
				}
	...
	private _updateContent(
		...
		const stickyScrollLineCount =
			Math.min(promptRowCount + commandRowCount - 1, this._maxLineCount) -
			rowOffset;
		...
		if (content) {
			...
			if (this._element) {
				...
				this._element.style.bottom = `${
					termBox.height - overlayHeight + 1
				}px`;
			}
		} else {
			this._setVisible(false);
		...
		if (this._xterm.isGpuAccelerated) {
			if (!this._canvasAddon.value && !this._pendingCanvasAddon) {
				this._pendingCanvasAddon = createCancelablePromise(
					async (token) => {
						const CanvasAddon =
							await this._getCanvasAddonConstructor();
	...
	> {
		const m = await importAMDNodeModule<
			typeof import("@xterm/addon-canvas")
		>("@xterm/addon-canvas", "lib/xterm-addon-canvas.js");
