...
import {
	...
	type InlineChatProviderChangeEvent,
...
export class InitialHintAddon extends Disposable implements ITerminalAddon {
	...
	activate(terminal: RawXtermTerminal): void {
		...
		this._disposables.value?.add(
			Event.once(this._onDidChangeProviders)(() =>
				this._onDidRequestCreateHint.fire(),
			),
		);
	}
	...
	implements ITerminalContribution
{
	...
	private _createHint(): void {
		...
		if (
			...
			commandDetectionCapability?.hasInput ||
			instance.reconnectionProperties
		) {
			return;
		}
	...
	getDomNode(providers: IInlineChatSessionProvider[]): HTMLElement {
		if (!this.domNode) {
			...
			const { hintElement, ariaLabel } =
				this._getHintInlineChat(providers);
