...
import {
	...
	toLineRange,
	toRangeMapping,
} from "vs/workbench/contrib/mergeEditor/browser/model/diffComputer";
		...
		applyRanges(
			...
			baseRanges.map<LabeledRange>((r, idx) => ({
				range: this.mergeModel
					...
					.toRange(),
				label: `{${this.mergeModel
					.getState(r)
					.get()}}${toSmallNumbersDec(idx)}`,
			}))
		);

		const result = {
			base: baseTextModel.getValue(EndOfLinePreference.LF).split("\n"),
			input1: input1TextModel
				.getValue(EndOfLinePreference.LF)
				.split("\n"),
			input2: input2TextModel
				.getValue(EndOfLinePreference.LF)
				.split("\n"),
