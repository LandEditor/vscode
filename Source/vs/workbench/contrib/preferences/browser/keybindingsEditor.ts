...
import type { IActionViewItemOptions } from "vs/base/browser/ui/actionbar/actionViewItems";
...
} from "vs/workbench/contrib/preferences/browser/keybindingWidgets";
import {
	keybindingsAddIcon,
	keybindingsEditIcon,
...
import type { KeybindingsEditorInput } from "vs/workbench/services/preferences/browser/keybindingsEditorInput";
import {
	KEYBINDING_ENTRY_TEMPLATE_ID,
	type KeybindingsEditorModel,
} from "vs/workbench/services/preferences/browser/keybindingsEditorModel";
import type {
	...
	implements IKeybindingsEditorPane
{
	...
	private createRecordingBadge(container: HTMLElement): HTMLElement {
		...
		recordingBadge.style.border = `1px solid ${asCssVariable(
			contrastBorder,
		)}`;
	...
	private layoutSearchWidget(dimension: DOM.Dimension): void {
		...
		this.searchWidget.inputBox.inputElement.style.paddingRight = `${
			DOM.getTotalWidth(this.actionsContainer) + 12
		}px`;
	}
	...
	implements ITableRenderer<IKeybindingItemEntry, ICommandColumnTemplateData>
{
	...
	renderTemplate(container: HTMLElement): ICommandColumnTemplateData {
		...
		const commandColumnHover = this._hoverService.setupUpdatableHover(
	...
	implements ITableRenderer<IKeybindingItemEntry, ISourceColumnTemplateData>
{
	...
	renderTemplate(container: HTMLElement): ISourceColumnTemplateData {
		...
		const sourceColumnHover = this.hoverService.setupUpdatableHover(
			getDefaultHoverDelegate("mouse"),
			sourceColumn,
			"",
		);
...
registerColor(
	...
	{
		dark: tableOddRowsBackgroundColor,
		light: tableOddRowsBackgroundColor,
		hcDark: tableOddRowsBackgroundColor,
		hcLight: tableOddRowsBackgroundColor,
	},
