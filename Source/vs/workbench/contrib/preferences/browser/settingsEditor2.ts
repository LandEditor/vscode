...
import type { ISettingOverrideClickEvent } from "vs/workbench/contrib/preferences/browser/settingsEditorSettingIndicators";
import {
	type ITOCEntry,
	getCommonlyUsedData,
	tocData,
} from "vs/workbench/contrib/preferences/browser/settingsLayout";
...
import {
	...
	resolveSettingsTree,
} from "vs/workbench/contrib/preferences/browser/settingsTree";
import {
...
import {
	...
	TOCTreeModel,
	createTOCIterator,
} from "vs/workbench/contrib/preferences/browser/tocTree";
import {
...
import type { SettingsEditor2Input } from "vs/workbench/services/preferences/common/preferencesEditorInput";
import {
...
export class SettingsEditor2 extends EditorPane {
	...
	private static NARROW_TOTAL_WIDTH: number =
		SettingsEditor2.TOC_RESET_WIDTH + SettingsEditor2.EDITOR_MIN_WIDTH;
	...
	private applyFilter(filter: string) {
		if (
			...
			const newQuery = `${filter} ${this.searchWidget
				.getValue()
				.trimStart()}`;
	...
	private addOrRemoveManageExtensionSetting(
		setting: ISetting,
		extension: IGalleryExtension,
		groups: ISettingsGroup[],
	): ISettingsGroup | undefined {
		const matchingGroups = groups.filter((g) => {
			const lowerCaseId = g.extensionInfo?.id.toLowerCase();
			return (
				lowerCaseId === setting.stableExtensionId!.toLowerCase() ||
		...
		if (!matchingGroups.length && !extensionInstalled) {
			...
			const matchingGroupIndex = matchingGroups.findIndex(
				(group) =>
					...
					group.sections[0].settings.length === 1 &&
					group.sections[0].settings[0].displayExtensionId,
			);
	...
	private async onConfigUpdate(
		...
		if (toggleData && groups.filter((g) => g.extensionInfo).length) {
			for (const key in toggleData.settingsEditorRecommendedExtensions) {
				...
				const additionalGroup = this.addOrRemoveManageExtensionSetting(
					setting,
					extension,
					groups,
				);
	...
	private remoteSearchPreferences(
		...
		const newExtSearchProvider =
			this.preferencesSearchService.getRemoteSearchProvider(query, true);
