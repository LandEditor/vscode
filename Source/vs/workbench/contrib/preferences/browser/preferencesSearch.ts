	...
	implements IPreferencesSearchService
{
	...
	getRemoteSearchProvider(
		filter: string,
		newExtensionsOnly = false
	): IRemoteSearchProvider | undefined {
...
export class LocalSearchProvider implements ISearchProvider {
	...
	searchModel(
		...
		if (
			filterMatches[0] &&
			filterMatches[0].score === LocalSearchProvider.EXACT_MATCH_SCORE
		) {
...
export class SettingMatches {
	...
	private toDescriptionRange(
		...
		return {
			startLineNumber:
				setting.descriptionRanges[lineIndex].startLineNumber,
			startColumn:
			...
			endColumn:
				setting.descriptionRanges[lineIndex].startColumn + match.end,
		};
	}
...
class AiRelatedInformationSearchProvider implements IRemoteSearchProvider {
	...
	private async getAiRelatedInformationItems(
		...
		for (const info of relatedInformation) {
			if (
				info.weight <
					AiRelatedInformationSearchProvider.AI_RELATED_INFORMATION_THRESHOLD ||
