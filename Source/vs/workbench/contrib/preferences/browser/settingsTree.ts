...
import type { IObjectTreeOptions } from "vs/base/browser/ui/tree/objectTree";
...
import { Action, type IAction, Separator } from "vs/base/common/actions";
...
import type { SettingsTarget } from "vs/workbench/contrib/preferences/browser/preferencesWidgets";
import {
	type ISettingOverrideClickEvent,
	SettingsTreeIndicatorsLabel,
	getIndicatorsLabelAriaLabel,
} from "vs/workbench/contrib/preferences/browser/settingsEditorSettingIndicators";
import type { ITOCEntry } from "vs/workbench/contrib/preferences/browser/settingsLayout";
import {
		...
		>
{
	...
	private computeNewList(
		...
		e: ISettingListChangeEvent<IListDataItem>,
	): string[] | undefined {
		if (template.context) {
			let newValue: string[] = [];
		...
		>
{
	...
	protected onDidChangeObject(
		...
		e: ISettingListChangeEvent<IObjectDataItem>,
	): void {
		const widget = (template.objectCheckboxWidget ??
			template.objectDropdownWidget)!;
		if (template.context) {
			...
			widget.items.forEach((item, idx) => {
				...
				// All remaining items, but skip the one that we just updated
					...
					isUndefinedOrNull(e.item) ||
					e.item.key.data !== item.key.data
				) {
			...
			if (isUndefinedOrNull(e.item)) {
				...
				const itemToDelete = newItems.findIndex(
					(item) => item.key.data === e.originalItem.key.data,
				);
				const defaultItemValue = defaultValue[
					e.originalItem.key.data
				] as string | boolean;

				// Item does not have a default
				if (
					isUndefinedOrNull(defaultValue[e.originalItem.key.data]) &&
					itemToDelete > -1
				) {
			...
			// New item was added
				...
				widget.isItemNew(e.originalItem) &&
				e.item.key.data !== ""
			) {
				newValue[e.item.key.data] = e.item.value.data;
				newItems.push(e.item);
		...
		>
{
	...
	protected override onDidChangeObject(
		template: ISettingObjectItemTemplate,
		e: ISettingListChangeEvent<IObjectDataItem>,
	): void {
		if (template.context) {
		...
		if (template.context) {
			...
			if (e.item?.value) {
				if (
					...
					newValue[e.item.value.data.toString()] = e.item.sibling
						? { when: e.item.sibling }
						: true;
				}
			}
...
export class SettingEnumRenderer
	extends AbstractSettingRenderer
	implements
		ITreeRenderer<
			SettingsTreeSettingElement,
...
export class SettingNumberRenderer
	extends AbstractSettingRenderer
	implements
		ITreeRenderer<
			SettingsTreeSettingElement,
...
export class SettingsExtensionToggleRenderer
	extends AbstractSettingRenderer
	implements
		ITreeRenderer<
			SettingsTreeSettingElement,
...
export class SettingsTree extends WorkbenchObjectTree<SettingsTreeElement> {
	...
	protected override createModel(
		user: string,
		view: IList<ITreeNode<SettingsTreeGroupChild>>,
...
class CopySettingAsJSONAction extends Action {
	...
	override async run(context: SettingsTreeSettingElement): Promise<void> {
		if (context) {
			const jsonResult = `"${context.setting.key}": ${JSON.stringify(
				context.value,
				undefined,
				"  ",
			)}`;
