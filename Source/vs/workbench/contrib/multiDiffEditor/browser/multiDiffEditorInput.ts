...
import {
	ConstLazyPromise,
	type IDocumentDiffItem,
	type IMultiDiffEditorModel,
	type LazyPromise,
} from "vs/editor/browser/widget/multiDiffEditor/model";
import { MultiDiffEditorViewModel } from "vs/editor/browser/widget/multiDiffEditor/multiDiffEditorViewModel";
import type { IDiffEditorOptions } from "vs/editor/common/config/editorOptions";
import {
...
} from "vs/workbench/common/editor/editorInput";
...
import {
	...
	MultiDiffEditorItem,
} from "vs/workbench/contrib/multiDiffEditor/browser/multiDiffSourceResolverService";
import {
	...
	implements ILanguageSupport
{
	...
	private readonly _isDirtyObservables = mapObservableArrayCached(
		...
		(res) => {
			const isModifiedDirty = res.modified
				? isUriDirty(this._textFileService, res.modified)
				: constObservable(false);
			const isOriginalDirty = res.original
				? isUriDirty(this._textFileService, res.original)
				: constObservable(false);
			return derived(
				(reader) =>
...
function isUriDirty(textFileService: ITextFileService, uri: URI) {
	return observableFromEvent(
		Event.filter(
			textFileService.files.onDidChangeDirty,
			(e) => e.resource.toString() === uri.toString(),
		),
