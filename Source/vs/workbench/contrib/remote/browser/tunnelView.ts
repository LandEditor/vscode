...
import {
	...
	privatePortIcon,
	stopForwardIcon,
} from "vs/workbench/contrib/remote/browser/remoteIcons";
...
import {
	...
	parseAddress,
} from "vs/workbench/services/remote/common/tunnelModel";
...
class OriginColumn implements ITableColumn<ITunnelItem, ActionBarCell> {
	...
	project(row: ITunnelItem): ActionBarCell {
		...
		const tooltip = `${
			row instanceof TunnelItem ? row.originTooltip : ""
		}. ${row instanceof TunnelItem ? row.tooltipPostfix : ""}`;
		return {
			label,
	...
	implements ITableRenderer<ActionBarCell, IActionBarTemplateData>
{
	...
	renderActionBarItem(
		...
		if (element.icon) {
			templateData.icon.className = `ports-view-actionbar-cell-icon ${ThemeIcon.asClassName(
				element.icon,
			)}`;
...
export class TunnelPanel extends ViewPane {
	...
	private tableDisposables: DisposableStore = this._register(
		new DisposableStore(),
	);
					...
					getAriaLabel: (item: ITunnelItem) => {
						if (item instanceof TunnelItem) {
							return `${item.tooltipPostfix} ${
								item.portTooltip
							} ${item.iconTooltip} ${item.processTooltip} ${
								item.originTooltip
							} ${
...
export namespace ForwardPortAction {
	...
	export const LABEL: ILocalizedString = {
		value: nls.localize("remote.tunnel.forward", "Forward a Port"),
...
namespace ClosePortAction {
	...
	export const LABEL: ILocalizedString = {
		value: nls.localize("remote.tunnel.close", "Stop Forwarding Port"),
