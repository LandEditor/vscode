...
export class OpenerValidatorContributions implements IWorkbenchContribution {
	private _readWorkspaceTrustedDomainsResult: WindowIdleValue<
		Promise<string[]>
	>;
	private _readAuthenticationTrustedDomainsResult: WindowIdleValue<
		Promise<string[]>
	>;
	...
	async validateLink(
		...
		const [workspaceDomains, userDomains] = await Promise.all([
			this._readWorkspaceTrustedDomainsResult.value,
			this._readAuthenticationTrustedDomainsResult.value,
		]);
