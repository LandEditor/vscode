...
} from "vs/base/browser/ui/dropdown/dropdownActionViewItem";
import {
	Action,
	type IAction,
	Separator,
...
} from "vs/workbench/services/extensionManagement/common/extensionFeatures";
import {
	EnablementState,
...
export class PromptExtensionInstallFailureAction extends Action {
	...
	private async getDownloadUrl(): Promise<URI | undefined> {
		...
		return URI.parse(
			`${this.productService.extensionsGallery.serviceUrl}/publishers/${
				this.extension.publisher
			}/vsextensions/${this.extension.name}/${this.version}/vspackage${
				targetPlatform !== TargetPlatform.UNDEFINED
...
export class ActionWithDropDownAction extends ExtensionAction {
	...
	update(donotUpdateActions?: boolean): void {
		...
		const enabledActionsGroups = this.actionsGroups.map((actionsGroup) =>
			actionsGroup.filter((a) => a.enabled),
		);
...
export class InstallAction extends ExtensionAction {
	static readonly Class =
		`${ExtensionAction.LABEL_ACTION_CLASS} prominent install`;
	...
	protected async computeAndUpdateEnablement(): Promise<void> {
		...
		if (
			...
			this.enabled = this.options.installPreReleaseVersion
...
abstract class AbstractUpdateAction extends ExtensionAction {
	private static readonly EnabledClass =
		`${ExtensionAction.LABEL_ACTION_CLASS} prominent update`;
	private static readonly DisabledClass =
		`${AbstractUpdateAction.EnabledClass} disabled`;
	...
	private async computeAndUpdateEnablement(): Promise<void> {
		...
		this.class = this.enabled
			? AbstractUpdateAction.EnabledClass
			: AbstractUpdateAction.DisabledClass;
	}
}
...
export class ToggleAutoUpdateForExtensionAction extends ExtensionAction {
	...
	private static readonly DisabledClass =
		`${ToggleAutoUpdateForExtensionAction.EnabledClass} hide`;
	...
	override update() {
		...
		if (
			!this.enableWhenAutoUpdateValue.includes(
...
export class MigrateDeprecatedExtensionAction extends ExtensionAction {
	...
	private static readonly DisabledClass =
		`${MigrateDeprecatedExtensionAction.EnabledClass} disabled`;
...
export class ExtensionActionWithDropdownActionViewItem extends ActionWithDropdownActionViewItem {
	...
	protected override updateClass(): void {
		...
		if (
			this.element &&
			this.dropdownMenuActionViewItem &&
			this.dropdownMenuActionViewItem.element
		) {
			this.element.classList.toggle(
				"empty",
				(<ActionWithDropDownAction>this._action).menuActions.length ===
					0,
			);
...
export class ManageExtensionAction extends ExtensionDropDownAction {
	...
	private static readonly HideManageExtensionClass =
		`${ManageExtensionAction.Class} hide`;
...
export class ExtensionEditorManageExtensionAction extends ExtensionDropDownAction {
	constructor(
		...
		super(
			...
			} manage ${ThemeIcon.asClassName(manageExtensionIcon)}`,
			true,
			instantiationService,
		);
		this.tooltip = localize("manage", "Manage");
...
export class TogglePreReleaseExtensionAction extends ExtensionAction {
	...
	private static readonly DisabledClass =
		`${TogglePreReleaseExtensionAction.EnabledClass} hide`;
	...
	override async run(): Promise<any> {
		...
		if (pick) {
			...
			try {
				if (pick.latest) {
					const [extension] =
						pick.id !== this.extension?.version
							? await this.extensionsWorkbenchService.getExtensions(
									[
										{
...
export class ExtensionRuntimeStateAction extends ExtensionAction {
	...
	private static readonly DisabledClass =
		`${ExtensionRuntimeStateAction.EnabledClass} disabled`;
...
export class SetColorThemeAction extends ExtensionAction {
	...
	private static readonly DisabledClass =
		`${SetColorThemeAction.EnabledClass} disabled`;
...
export class SetFileIconThemeAction extends ExtensionAction {
	...
	private static readonly EnabledClass =
		`${ExtensionAction.LABEL_ACTION_CLASS} theme`;
	private static readonly DisabledClass =
		`${SetFileIconThemeAction.EnabledClass} disabled`;
...
export class SetProductIconThemeAction extends ExtensionAction {
	...
	private static readonly EnabledClass =
		`${ExtensionAction.LABEL_ACTION_CLASS} theme`;
	private static readonly DisabledClass =
		`${SetProductIconThemeAction.EnabledClass} disabled`;
...
export class SetLanguageAction extends ExtensionAction {
	...
	private static readonly DisabledClass =
		`${SetLanguageAction.EnabledClass} disabled`;
...
export class ClearLanguageAction extends ExtensionAction {
	...
	private static readonly EnabledClass =
		`${ExtensionAction.LABEL_ACTION_CLASS} language`;
	private static readonly DisabledClass =
		`${ClearLanguageAction.EnabledClass} disabled`;
...
export class ShowRecommendedExtensionAction extends Action {
	...
	override async run(): Promise<any> {
		...
		const paneContainer =
			paneComposite?.getViewPaneContainer() as IExtensionsViewPaneContainer;
	...
	implements IExtensionContainer
{
	...
	private static readonly DISABLED_CLASS =
		`${ExtensionStatusLabelAction.ENABLED_CLASS} hide`;
			...
			if (
				...
				return canAddExtension()
					? this.initialStatus === ExtensionState.Installed &&
						...
						? localize("updated", "Updated")
						: localize("installed", "Installed")
					: null;
			}
			if (
...
export class ToggleSyncExtensionAction extends ExtensionDropDownAction {
	...
	private static readonly SYNC_CLASS = `${
		ToggleSyncExtensionAction.ICON_ACTION_CLASS
...
export class ExtensionStatusAction extends ExtensionAction {
			...
			if (this.extension.deprecationInfo.extension) {
				const link = `[${
					...
					`command:extension.open?${encodeURIComponent(
						JSON.stringify([
							this.extension.deprecationInfo.extension.id,
						]),
					)}`,
				)})`;
				this.updateStatus(
	...
	private updateStatus(
		...
		if (
			...
			this._status.icon?.id === status.icon?.id
		) {
			return;
		}
		this._status = status;
		if (updateClass) {
			if (this._status?.icon === errorIcon) {
				...
				this.class = `${
					ExtensionStatusAction.CLASS
				} extension-status-warning ${ThemeIcon.asClassName(
					warningIcon,
				)}`;
				...
				this.class = `${
					ExtensionStatusAction.CLASS
				} ${ThemeIcon.asClassName(trustIcon)}`;
			} else {
...
export class ReinstallAction extends Action {
	...
	private reinstallExtension(extension: IExtension): Promise<void> {
		return this.instantiationService
			.createInstance(SearchExtensionsAction, "@installed ")
			.run()
			.then(() => {
