			...
			if (status.runtimeErrors.length) {
				data.appendMarkdown(
					`\n ### ${localize(
						"uncaught errors",
						"Uncaught Errors ({0})",
						status.runtimeErrors.length,
					)}\n`,
				);
				for (const error of status.runtimeErrors) {
					data.appendMarkdown(
						`$(${Codicon.error.id})&nbsp;${getErrorMessage(
							error,
						)}\n\n`,
					);
				}
	...
	renderElement(
		...
		const updateStatus = () => {
			...
			if (accessData?.current?.status) {
				...
				templateData.statusElement.className = `${statusElementClassName} ${SeverityIcon.className(
					accessData.current.status.severity,
				)}`;
			} else {
