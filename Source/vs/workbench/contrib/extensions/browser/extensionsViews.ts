...
const FORCE_FEATURE_EXTENSIONS = [
	"vscode.git",
	"vscode.git-base",
...
export class ExtensionsListView extends ViewPane {
		...
		this.list = this.instantiationService.createInstance(
			...
			[renderer],
			{
				...
				accessibilityProvider: <
					IListAccessibilityProvider<IExtension | null>
				>{
		...
		if (options.sortBy !== undefined) {
			...
			result = local.filter(
				(e) =>
					(!e.isBuiltin ||
						e.outdated ||
						e.reloadRequiredStatus !== undefined) &&
					matchingText(e)
			);
			...
			result = [
				...
				...reloadRequired.sort(defaultSort),
	...
	private async getWorkspaceRecommendationsModel(
		...
		const result: IExtension[] = coalesce(
	...
	private async getAllRecommendationsModel(
		...
		const allRecommendations = distinct(
			flatten(
				...
				(extensionId) => !local.includes(extensionId.toLowerCase())
			),
			(extensionId) => extensionId.toLowerCase()
		);
...
export class BuiltInFeatureExtensionsView extends ExtensionsListView {
	override async show(query: string): Promise<IPagedModel<IExtension>> {
		return query && query.trim() !== "@builtin"
			? this.showEmptyModel()
			: super.show("@builtin:features");
	}
}
	...
	implements IWorkspaceRecommendedExtensionsView
{
	...
	private async getInstallableWorkspaceRecommendations() {
		...
		const recommendations = (
			...
			installed.every(
				(local) =>
					!areSameExtensions({ id: extensionId }, local.identifier)
			)
		);
	...
	async installWorkspaceRecommendations(): Promise<void> {
		...
		if (installableRecommendations.length) {
			await this.extensionManagementService.installGalleryExtensions(
				installableRecommendations.map((i) => ({
					extension: i.gallery!,
					options: {},
				}))
			);
...
export function getAriaLabelForExtension(extension: IExtension | null): string {
	...
	return `${extension.displayName}, ${deprecated ? `${deprecated}, ` : ""}${
		extension.version
	}, ${publisher}, ${extension.description} ${rating ? `, ${rating}` : ""}`;
}
