...
import { Delayer, Promises, timeout } from "vs/base/common/async";
...
import type { ViewPane } from "vs/workbench/browser/parts/views/viewPane";
...
import type { MementoObject } from "vs/workbench/common/memento";
import type { IPaneComposite } from "vs/workbench/common/panecomposite";
	...
	implements IWorkbenchContribution
{
	...
	private createBuiltinExtensionsViewDescriptors(): IViewDescriptor[] {
		...
		const otherCategoriesQuery = `${otherCategories
			.map((c) => `category:"${c}"`)
			.join(" ")} ${configuredCategories
			.map((c) => `category:"-${c}"`)
			.join(" ")}`;
		viewDescriptors.push({
	...
	implements IWorkbenchContribution
{
	...
	private onServiceChange(): void {
		...
		const actionRequired = this.extensionsWorkbenchService.installed.filter(
