...
class Extensions extends Disposable {
	...
	private async fetchInstalledExtensions(): Promise<void> {
		...
		const all = await this.migrateIgnoredAutoUpdateExtensions(
			await this.server.extensionManagementService.getInstalled()
		);
	...
	private async migrateIgnoredAutoUpdateExtensions(
		extensions: ILocalExtension[]
	): Promise<ILocalExtension[]> {
		const ignoredAutoUpdateExtensions = JSON.parse(
		...
		const result = await Promise.all(
			extensions.map((extension) => {
				if (
					ignoredAutoUpdateExtensions.indexOf(
						new ExtensionKey(
							extension.identifier,
	...
	implements IExtensionsWorkbenchService, IURLHandler
{
	...
	private async installFromVSIX(
		...
		if (
			existingExtension &&
			existingExtension.latestVersion !== manifest.version
		) {
			installOptions = installOptions || {};
			installOptions.installGivenVersion = true;
		}
