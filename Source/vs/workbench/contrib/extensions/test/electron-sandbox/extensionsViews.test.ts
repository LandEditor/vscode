...
suite("ExtensionsViews Tests", () => {
	...
	setup(async () => {
		...
		instantiationService.stub(IExtensionManagementService, <
			Partial<IExtensionManagementService>
		>{
		...
		instantiationService.stub(IExtensionManagementServerService, <
			Partial<IExtensionManagementServerService>
		>{
		...
		instantiationService.stub(IExtensionRecommendationsService, <
			Partial<IExtensionRecommendationsService>
		>{
	...
	test("Test installed query results", async () => {
		...
		await testableView.show("@builtin:themes").then((result) => {
			assert.strictEqual(
				result.length,
				1,
				"Unexpected number of results for @builtin:themes query"
			);
			assert.strictEqual(
				result.get(0).name,
	...
	test("Test installed query with category", async () => {
		...
		await testableView
			.show('@installed category:"themes"')
			.then((result) => {
				assert.strictEqual(
					result.length,
					2,
	...
	test("Test local query with sorting order", async () => {
		...
		await testableView
			.show("@installed @sort:updateDate")
			.then((result) => {
				assert.strictEqual(
					result.length,
					5,
	...
	test("Test preferred search experiment", () => {
		...
		const experimentTarget = <SinonStub>instantiationService.stubPromise(
			IWorkbenchExtensionManagementService,
	...
	test("Skip preferred search experiment when user defines sort order", () => {
		...
		testableView.dispose();
		disposableStore.add(
			(testableView = instantiationService.createInstance(
				ExtensionsListView,
				{},
				{ id: "", title: "" }
			))
