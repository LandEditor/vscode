...
} from "vs/workbench/services/extensionManagement/test/browser/extensionEnablementService.test";
...
suite("ExtensionsWorkbenchServiceTest", () => {
	...
	setup(async () => {
		...
		instantiationService.stub(IConfigurationService, <
			Partial<IConfigurationService>
		>{
			onDidChangeConfiguration: () => {
				return undefined!;
			},
			getValue: (key?: string) => {
				return key === AutoCheckUpdatesConfigurationKey ||
	...
	test("test for installed extensions", async () => {
		...
		assert.ok(
			actual.iconUrl === "file:///localPath1/localIcon1" ||
				actual.iconUrl ===
					"vscode-file://vscode-app/localPath1/localIcon1"
		);
		assert.ok(
			actual.iconUrlFallback === "file:///localPath1/localIcon1" ||
				actual.iconUrlFallback ===
					"vscode-file://vscode-app/localPath1/localIcon1"
		);
	...
	async function aWorkbenchService(): Promise<ExtensionsWorkbenchService> {
		const workbenchService: ExtensionsWorkbenchService =
			disposableStore.add(
