...
export class ChatRequestParser {
		...
		for (let i = 0; i < message.length; i++) {
			...
			if (previousChar.match(/\s/) || i === 0) {
				if (char === chatVariableLeader) {
					newPart =
						this.tryToParseVariable(
							message.slice(i),
							i,
							new Position(lineNumber, column),
							parts
						) ||
						(await this.tryToParseDynamicVariable(
							message.slice(i),
							i,
							new Position(lineNumber, column),
							references
					...
					newPart = await this.tryToParseSlashCommand(
						sessionId,
						message.slice(i),
						message,
						i,
	...
	> {
		...
		if (usedAgent) {
			...
			const subCommands = await usedAgent.agent.provideSlashCommands(
			...
			const slashCommands = await this.chatService.getSlashCommands(
				sessionId,
				CancellationToken.None
			);
	...
	private async tryToParseDynamicVariable(
		message: string,
		offset: number,
		position: IPosition,
		references: ReadonlyArray<IDynamicReference>
	): Promise<ChatRequestDynamicReferencePart | undefined> {
