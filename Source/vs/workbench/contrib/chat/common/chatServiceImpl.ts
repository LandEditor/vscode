...
import {
	...
	isCompleteInteractiveProgressTreeData,
...
import {
	...
	IChatSlashFragment,
} from "vs/workbench/contrib/chat/common/chatSlashCommands";
...
type ChatProviderInvokedClassification = {
	providerId: {
		...
		comment: "The identifier of the provider that was invoked.";
	};
	timeToFirstProgress: {
...
type ChatVoteClassification = {
	providerId: {
		classification: "PublicNonPersonalData";
		purpose: "FeatureInsight";
		comment: "The identifier of the provider that this response came from.";
	};
	direction: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
...
type ChatCopyClassification = {
	providerId: {
		classification: "PublicNonPersonalData";
		purpose: "FeatureInsight";
		comment: "The identifier of the provider that this codeblock response came from.";
	};
	copyKind: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
...
export class ChatService extends Disposable implements IChatService {
	...
	private readonly _onDidSubmitSlashCommand = this._register(
		new Emitter<
			| { slashCommand: string; sessionId: string }
			| {
	...
	public readonly onDidSubmitSlashCommand =
		this._onDidSubmitSlashCommand.event;
	...
	private readonly _onDidRegisterProvider = this._register(
		new Emitter<{ providerId: string }>()
	);
	...
	constructor(
		...
		this._hasProvider = CONTEXT_PROVIDER_EXISTS.bindTo(
		...
		if (action.action.kind === "vote") {
			...
			>("interactiveSessionVote", {
				...
				direction:
					...
					InteractiveSessionVoteDirection.Up
						? "up"
						: "down",
			});
			...
			>("interactiveSessionCopy", {
				providerId: action.providerId,
				copyKind:
					action.action.copyType === InteractiveSessionCopyKind.Action
						? "action"
						: "toolbar",
			});
			...
			const command = CommandsRegistry.getCommand(
				action.action.command.commandId
			);
	...
	private deserializeChats(sessionData: string): ISerializableChatsData {
		try {
			...
			this.error(
				"deserializeChats",
				`Malformed session data: ${err}. [${sessionData.substring(
					0,
					20
	...
	private _startSession(
		providerId: string,
		someSessionHistory: ISerializableChatData | undefined,
	...
	private async initializeSession(
		...
		try {
			...
			await this.extensionService.activateByEvent(
				`onInteractiveSession:${model.providerId}`
			);
			...
			try {
				...
				this.trace(
					"initializeSession",
					`Provider initializeSession threw: ${err}`
				);
			}
			...
			const welcomeMessage = model.welcomeMessage
				? undefined
				: (await provider.provideWelcomeMessage?.(token)) ?? undefined;
			const welcomeModel =
				welcomeMessage &&
				new ChatWelcomeMessageModel(
					model,
	...
	getSessionId(sessionProviderId: number): string | undefined {
		return Iterable.find(
			this._sessionModels.values(),
			(model) => model.session?.id === sessionProviderId
		)?.sessionId;
	}
	...
	async sendRequest(
		sessionId: string,
		request: string,
		usedSlashCommand?: ISlashCommand
	...
	private async _sendRequestAsync(
		...
		const parsedRequest = await this.instantiationService
		...
		const rawResponsePromise = createCancelablePromise<void>(
			async (token) => {
				const progressCallback = (progress: IChatProgress) => {
					...
					if ("content" in progress) {
						this.trace(
							"sendRequest",
							`Provider returned progress for session ${
								model.sessionId
							}, ${
								typeof progress.content === "string"
									? progress.content.length
									: progress.content.value.length
							} chars`
						);
					} else if ("placeholder" in progress) {
						this.trace(
							"sendRequest",
							`Provider returned placeholder for session ${model.sessionId}, ${progress.placeholder}`
						);
					} else if (
						isCompleteInteractiveProgressTreeData(progress)
					) {
						// This isn't exposed in API
						this.trace(
							"sendRequest",
							`Provider returned tree data for session ${model.sessionId}, ${progress.treeData.label}`
						);
					} else if ("documents" in progress) {
						this.trace(
							"sendRequest",
							`Provider returned documents for session ${
								model.sessionId
							}:\n ${JSON.stringify(
								progress.documents,
								null,
						...
						this.trace(
							"sendRequest",
							`Provider returned a reference for session ${
								model.sessionId
							}:\n ${JSON.stringify(
								progress.reference,
								null,
						...
						this.trace(
							"sendRequest",
							`Provider returned an inline reference for session ${
								model.sessionId
							}:\n ${JSON.stringify(
						...
						this.trace(
							"sendRequest",
							`Provider returned an agent detection for session ${
								model.sessionId
						...
						this.trace(
							"sendRequest",
							`Provider returned id for session ${model.sessionId}, ${progress.requestId}`
						);
					}
				...
				try {
					...
					if (agentPart || (defaultAgent && !commandPart)) {
						...
						const agentResult =
							await this.chatAgentService.invokeAgent(
								agent.id,
								requestProps,
								progressCallback,
								history,
						...
						agentOrCommandFollowups = agentResult?.followUp
							? Promise.resolve(agentResult.followUp)
						...
						const commandResult =
							await this.chatSlashCommandService.executeCommand(
								...
								message
									...
									.trimStart(),
								new Progress<IChatSlashFragment>((p) => {
									const { content } = p;
									const data =
										isCompleteInteractiveProgressTreeData(
											content
						...
						rawResponse = await provider.provideReply(
							requestProps,
							progressCallback,
							token
						);
					...
					if (token.isCancellationRequested) {
						...
						const result = rawResponse.errorDetails
							?.responseIsFiltered
							? "filtered"
							: rawResponse.errorDetails && gotProgress
							? "errorWithOutput"
						...
						>("interactiveSessionProviderInvoked", {
							providerId: provider.id,
							timeToFirstProgress:
								rawResponse.timings?.firstProgress,
						...
						if (agentOrCommandFollowups) {
							...
							Promise.resolve(
								provider.provideFollowups(
									model.session!,
									CancellationToken.None
								)
							).then((providerFollowups) => {
								model.setFollowups(
									request,
									providerFollowups ?? undefined
								);
								model.completeResponse(request);
							});
						} else {
	...
	async getSlashCommands(
		...
		const serviceResults = this.chatSlashCommandService
			.getCommands()
			.map((data) => {
		...
		const mainProviderRequest = provider.provideSlashCommands?.(
			model.session!,
			token
		);
	...
	async addCompleteRequest(
		...
		if (typeof response.message === "string") {
			...
			for (const part of response.message) {
				const progress =
					"inlineReference" in part
						? part
						: isMarkdownString(part)
						? { content: part.value }
						: { treeData: part };
	...
	public hasSessions(providerId: string): boolean {
		return !!Object.values(this._persistedSessions).find(
