...
suite("ChatRequestParser", () => {
	...
	test("slash command", async () => {
		...
		chatService.getSlashCommands.returns(
			Promise.resolve([{ command: "fix" }])
		);
	...
	test("agent with subcommand after text", async () => {
		...
		agentsService.getAgent.returns(<Partial<IChatAgent>>{
			id: "agent",
			metadata: { description: "" },
			provideSlashCommands: async () => {
				return [{ name: "subCommand", description: "" }];
			},
		});
		instantiationService.stub(IChatAgentService, agentsService as any);

		parser = instantiationService.createInstance(ChatRequestParser);
		const result = await parser.parseChatRequest(
			"1",
