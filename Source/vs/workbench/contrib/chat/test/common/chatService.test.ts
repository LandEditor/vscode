...
class SimpleTestProvider extends Disposable implements IChatProvider {
	...
	): Promise<{ session: IChat; followups: never[] }> {
		return { session: request.session, followups: [] };
	}
}

/** Chat provider for testing that returns used context */
class ChatProviderWithUsedContext
	extends SimpleTestProvider
	implements IChatProvider
{
	override provideReply(
		request: IChatRequest,
		progress: (progress: IChatProgress) => void
	): Promise<{ session: IChat; followups: never[] }> {
		progress({
...
suite("Chat", () => {
	...
	test("retrieveSession", async () => {
		...
		const provider1 = testDisposables.add(
			new SimpleTestProvider("provider1")
		);
		const provider2 = testDisposables.add(
			new SimpleTestProvider("provider2")
		);
		...
		const session1 = testDisposables.add(
			testService.startSession("provider1", CancellationToken.None)
		);
		await session1.waitForInitialization();
		session1!.addRequest({ parts: [], text: "request 1" });

		const session2 = testDisposables.add(
			testService.startSession("provider2", CancellationToken.None)
		);
		...
		assert.deepStrictEqual(provider1.lastInitialState, {
			state: "provider1_state",
		});
		assert.deepStrictEqual(provider2.lastInitialState, {
			state: "provider2_state",
		});
	});
		...
		testDisposables.add(
			testService.registerProvider({
				...
				prepareSession: function (
					initialState: IPersistedChatState | undefined,
					token: CancellationToken
				): ProviderResult<IChat | undefined> {
					throw new Error("Function not implemented.");
				},
				provideReply: function (
					request: IChatRequest,
					progress: (progress: IChatProgress) => void,
					token: CancellationToken
				): ProviderResult<IChatResponse> {
					throw new Error("Function not implemented.");
				},
			})
		);
	...
	test("getSlashCommands", async () => {
		...
		const provider = testDisposables.add(
			new (class extends SimpleTestProvider {
				constructor() {
					super("testProvider");
				}
		...
		const model = testDisposables.add(
			testService.startSession("testProvider", CancellationToken.None)
		);
		const commands = await testService.getSlashCommands(
			model.sessionId,
	...
	test("sendRequestToProvider", async () => {
		...
		const response = await testService.sendRequestToProvider(
			model.sessionId,
	...
	test("addCompleteRequest", async () => {
		...
		await testService.addCompleteRequest(model.sessionId, "test request", {
			message: "test response",
		});
		assert.strictEqual(model.getRequests().length, 1);
		assert.ok(model.getRequests()[0].response);
		assert.strictEqual(
			model.getRequests()[0].response?.response.asString(),
			"test response"
		);
	});
	...
	test("can serialize", async () => {
		...
		testDisposables.add(
			testService.registerProvider(
				testDisposables.add(new ChatProviderWithUsedContext(providerId))
			)
		);

		const model = testDisposables.add(
			testService.startSession(providerId, CancellationToken.None)
		);
		assert.strictEqual(model.getRequests().length, 0);
	...
	test("can deserialize", async () => {
		...
		// create the first service, send request, get response, and serialize the state
		{
			...
			testDisposables.add(
				testService.registerProvider(
					testDisposables.add(
						new ChatProviderWithUsedContext(providerId)
					)
				)
			);
		...
		testDisposables.add(
			testService2.registerProvider(
				testDisposables.add(new ChatProviderWithUsedContext(providerId))
			)
		);

		const chatModel2 =
			testService2.loadSessionFromContent(serializedChatData);
		assert(chatModel2);

		// should `loadSessionFromContent` return `ChatModel` that's disposable instead of `IChatModel`?
		testDisposables.add({
			dispose: () =>
				testService2.clearSession(serializedChatData.sessionId),
		});
