...
suite("ChatVariables", function () {
	...
	test("ChatVariables - resolveVariables", async function () {
		const v1 = service.registerVariable(
			...
			async () => [{ level: "full", value: "farboo" }]
		);
		const v2 = service.registerVariable(
			{ name: "far", description: "boo" },
			async () => [{ level: "full", value: "farboo" }]
		);
		...
		}
		{
			...
			assert.deepEqual(Object.keys(data.variables).sort(), [
				"far",
				"foo",
			]);
		}
		{
			...
			assert.strictEqual(
				data.prompt,
				"Hello [#foo](values:foo) and [#far](values:far) [#foo](values:foo) #unknown"
			);
		}
