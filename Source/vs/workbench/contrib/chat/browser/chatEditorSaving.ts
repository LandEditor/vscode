...
export class ChatEditorAutoSaveDisabler
	extends Disposable
	implements IWorkbenchContribution
{
	...
	constructor(
		...
		this._store.add(
			lifecycleService.onWillShutdown((e) => {
				storageService.store(
					STORAGE_KEY_AUTOSAVE_DISABLED,
					this._autosaveDisabledUris,
		...
		const alwaysSaveConfig = observableConfigValue<boolean>(
		...
		if (!alwaysSaveConfig.get()) {
			const autoSaveDisabled = storageService.getObject<string[]>(
				STORAGE_KEY_AUTOSAVE_DISABLED,
				StorageScope.WORKSPACE,
				[],
			);
			if (
				Array.isArray(autoSaveDisabled) &&
				autoSaveDisabled.length > 0
			) {
				const initializingStore = new DisposableStore();
				for (const uriString of autoSaveDisabled) {
					initializingStore.add(
						fileConfigService.disableAutoSave(URI.parse(uriString)),
					);
				}
		...
		this._store.add(
			autorun((reader) => {
				...
				try {
					...
					const session =
						chatEditingService.currentEditingSessionObs.read(
							reader,
						);
					if (session) {
						...
						for (const entry of entries) {
							if (
								...
								autoSaveDisabled.push(
									entry.modifiedURI.toString(),
								);
								store.add(
									fileConfigService.disableAutoSave(
										entry.modifiedURI,
									),
								);
							}
				...
				private async _prompt() {
					...
					const message =
						this._uris.size === 1
							...
							: localize(
									"message.2",
									"Do you want to save the changes {0} made to {1} files?",
									agentName,
									this._uris.size,
								);
			...
			store.add(
				textFileService.files.addSaveParticipant({
					participate: async (
						...
						const session =
							await chatEditingService.getOrRestoreEditingSession();
						if (!session) {
							return;
						}
				...
				},
				{
					...
					when: ContextKeyExpr.and(
						...
						ContextKeyExpr.equals(
							`config.${ChatEditorSaving._config}`,
							false,
						),
		...
		if (!alwaysSave) {
			...
			if (editors.length === 1) {
				...
				if (resource) {
					...
					message = agentName
						? localize(
								"message.batched.oneFile.3",
								"Do you want to save the changes {0} made in 1 file?",
								agentName,
							)
						: localize(
								"message.batched.oneFile.4",
								"Do you want to save the changes chat made in 1 file?",
							);
				}
			} else {
				message = agentName
					? localize(
							"message.batched.multiFile.1",
							"Do you want to save the changes {0} made in {1} files?",
							agentName,
							editors.length,
						)
					: localize(
							"message.batched.multiFile.2",
							"Do you want to save the changes chat made in {0} files?",
							editors.length,
						);
			}
