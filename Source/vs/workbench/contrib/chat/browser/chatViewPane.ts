...
export class ChatViewPane extends ViewPane implements IChatViewPane {
	...
	constructor(
		...
		this._register(
			this.chatService.onDidRegisterProvider(({ providerId }) => {
				if (
					providerId === this.chatViewOptions.providerId &&
			...
			const disposeListener = sessionId
				? this._register(
						this.chatService.onDidDisposeSession((e) => {
							if (
								e.reason === "initializationFailed" &&
								e.providerId === this.chatViewOptions.providerId
							) {
