...
export const IChatQuotasService =
	createDecorator<IChatQuotasService>("chatQuotasService");
...
export const OPEN_CHAT_QUOTA_EXCEEDED_DIALOG =
	"workbench.action.chat.openQuotaExceededDialog";

export class ChatQuotasService
	extends Disposable
	implements IChatQuotasService
{
	...
	private readonly chatQuotaExceededContextKey =
		ChatContextKeys.chatQuotaExceeded.bindTo(this.contextKeyService);
	private readonly completionsQuotaExceededContextKey =
		ChatContextKeys.completionsQuotaExceeded.bindTo(this.contextKeyService);

	private ExtensionQuotaContextKeys = {
		// TODO@bpasero move into product.json or turn into core keys
	...
	private registerListeners(): void {
		const chatQuotaExceededSet = new Set([
		...
		this._register(
			this.contextKeyService.onDidChangeContext((e) => {
				...
				if (e.affectsSome(chatQuotaExceededSet)) {
					const newChatQuotaExceeded =
						this.contextKeyService.getContextKeyValue<boolean>(
							this.ExtensionQuotaContextKeys.chatQuotaExceeded,
						);
					if (
						typeof newChatQuotaExceeded === "boolean" &&
				...
				if (e.affectsSome(completionsQuotaExceededSet)) {
					const newCompletionsQuotaExceeded =
						this.contextKeyService.getContextKeyValue<boolean>(
							this.ExtensionQuotaContextKeys
								.completionsQuotaExceeded,
						);
					if (
						typeof newCompletionsQuotaExceeded === "boolean" &&
						newCompletionsQuotaExceeded !==
	...
	clearQuotas(): void {
		if (
			this.quotas.chatQuotaExceeded ||
			this.quotas.completionsQuotaExceeded
		) {
			this.acceptQuotas({
				chatQuotaExceeded: false,
				completionsQuotaExceeded: false,
...
export class ChatQuotasStatusBarEntry
	extends Disposable
	implements IWorkbenchContribution
{
	...
	constructor(
		...
		this._register(
			Event.runAndSubscribe(
				this.chatQuotasService.onDidChangeQuotas,
				() => this.updateStatusbarEntry(),
			),
		);
		this._register(
			this.statusbarService.onDidChangeEntryVisibility((e) => {
	...
	private updateStatusbarEntry(): void {
		...
		if (chatQuotaExceeded || completionsQuotaExceeded) {
			...
			const isCopilotStatusVisible = this.statusbarService.isEntryVisible(
				ChatQuotasStatusBarEntry.COPILOT_STATUS_ID,
			);
				...
				this.statusbarService.addEntry(
					{
						...
						kind: "prominent",
						tooltip: quotaToButtonMessage({
							chatQuotaExceeded,
							completionsQuotaExceeded,
						}),
					},
					ChatQuotasStatusBarEntry.ID,
			...
			this.entry.add(
				this.statusbarService.overrideEntry(
					ChatQuotasStatusBarEntry.COPILOT_STATUS_ID,
					{
...
export function quotaToButtonMessage({
	chatQuotaExceeded,
	completionsQuotaExceeded,
}: {
	chatQuotaExceeded: boolean;
