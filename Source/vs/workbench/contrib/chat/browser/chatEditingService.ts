	...
	implements IChatEditingService
{
	...
	private readonly _currentSessionDisposeListener = this._register(
			...
			async (builder, token) => {
				const codeMapperResponse: ICodeMapperResponse = {
					textEdit: (resource, edits) =>
						builder.textEdits(resource, edits),
				};
...
class ModifiedFileEntry extends Disposable implements IModifiedFileEntry {
	...
	constructor(
		...
		(async () => {
			this._register(
				await textModelService.createModelReference(docSnapshot.uri),
			);
		})();
