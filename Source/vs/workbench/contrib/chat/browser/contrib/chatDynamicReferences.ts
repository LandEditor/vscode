...
export class ChatDynamicReferenceModel
	extends Disposable
				...
				e.changes.forEach((c) => {
					this._references.forEach((ref, i) => {
						if (Range.areIntersecting(ref.range, c.range)) {
							...
							Range.compareRangesUsingStarts(ref.range, c.range) >
							0
						) {
...
export class SelectAndInsertFileAction extends Action2 {
	...
	async run(accessor: ServicesAccessor, ...args: any[]) {
		...
		context.widget
			.getContrib<ChatDynamicReferenceModel>(ChatDynamicReferenceModel.ID)
			?.addReference({
				range: {
