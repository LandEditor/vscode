...
import {
	...
	dynamicReferenceDecorationType,
} from "vs/workbench/contrib/chat/browser/contrib/chatDynamicReferences";
import {
	IChatAgentCommand,
	IChatAgentData,
	IChatAgentService,
...
class InputEditorDecorations extends Disposable {
	...
	constructor(
		...
		this._register(
			this.chatService.onDidSubmitSlashCommand((e) => {
				if (e.sessionId === this.widget.viewModel?.sessionId) {
					if ("agent" in e) {
						this.previouslyUsedSlashCommands.add(
	...
	private async updateInputEditorDecorations() {
		...
		if (!inputValue) {
			const viewModelPlaceholder =
				this.widget.viewModel?.inputPlaceholder;
		...
		const parsedRequest = (
			await this.instantiationService
				...
				.parseChatRequest(viewModel.sessionId, inputValue)
		).parts;
				...
				placeholderDecoration = [
					{
						...
						renderOptions: {
							after: {
								contentText:
									agentPart.agent.metadata.description,
								color: this.getPlaceholderColor(),
							},
						},
					},
		...
		if (onlyAgentCommandAndWhitespace) {
			...
			const isFollowupSlashCommand = this.previouslyUsedSlashCommands.has(
				agentAndCommandToKey(
					agentPart.agent.id,
		...
		const onlySlashCommandAndWhitespace =
			slashCommandPart &&
			parsedRequest.every(
				(p) =>
					(p instanceof ChatRequestTextPart &&
		...
		if (onlySlashCommandAndWhitespace) {
			...
			const isFollowupSlashCommand = this.previouslyUsedSlashCommands.has(
				slashCommandPart.slashCommand.command
			);
			const shouldRenderFollowupPlaceholder =
				isFollowupSlashCommand &&
				slashCommandPart.slashCommand.followupPlaceholder;
			if (
				shouldRenderFollowupPlaceholder ||
				slashCommandPart.slashCommand.detail
			) {
				placeholderDecoration = [
					{
						range: {
...
class InputEditorSlashCommandMode extends Disposable {
	...
	private async repopulateSlashCommand(slashCommand: string) {
		...
		if (
			slashCommands?.find((c) => c.command === slashCommand)
				?.shouldRepopulate
		) {
				...
				},
				{
					...
					provideCompletionItems: async (
						...
						return <CompletionList>{
							suggestions:
								sortSlashCommandsByYieldTo<ISlashCommand>(
									slashCommands
								).map((c, i) => {
					...
					provideCompletionItems: async (
						...
						if (
							!model
								.getValue()
								.trim()
								.match(new RegExp(`^${chatAgentLeader}\\w*$`))
						) {
			...
			this.languageFeaturesService.completionProvider.register(
				...
				},
				{
					...
					provideCompletionItems: async (
						...
						const commands =
							await usedAgent.agent.provideSlashCommands(token);
					...
					provideCompletionItems: async (
						...
						if (
							!model
								...
								.trim()
								.match(
									new RegExp(`^${chatSubcommandLeader}\\w*$`)
								)
						) {
						...
						const all = agents.map((agent) =>
							agent.provideSlashCommands(token)
						);
						const commands = await raceCancellation(
							Promise.all(all),
							token
						);
						...
						return <CompletionList>{
							suggestions: agents.flatMap((agent, i) =>
								commands[i].map((c, i) => {
			...
			this.languageFeaturesService.completionProvider.register(
				...
				},
				{
					...
					provideCompletionItems: async (
						...
						const fileVariablesEnabled =
							this.configurationService.getValue(
								"chat.experimental.fileVariables"
			...
			this.languageFeaturesService.completionProvider.register(
				...
				},
				{
					...
					provideCompletionItems: async (
						...
						const historyVariablesEnabled =
							this.configurationService.getValue(
								"chat.experimental.historyVariables"
							);
						const historyItems = historyVariablesEnabled
							? history.map(
									(h, i): CompletionItem => ({
										label: `${chatVariableLeader}response:${
											i + 1
										}`,
										detail: h.response.asString(),
										insertText: `${chatVariableLeader}response:${String(
											i + 1
										).padStart(
											String(history.length).length,
											"0"
										)} `,
										kind: CompletionItemKind.Text,
										range,
			...
			if (!change.text) {
				parser
					...
					.then((previousParsedValue) => {
						const deletableTokens =
							previousParsedValue.parts.filter(
								(p) =>
									p instanceof ChatRequestAgentPart ||
									p instanceof
										ChatRequestAgentSubcommandPart ||
