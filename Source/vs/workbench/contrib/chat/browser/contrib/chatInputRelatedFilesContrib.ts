...
export class ChatRelatedFilesContribution
	extends Disposable
	implements IWorkbenchContribution
{
	...
	private _updateRelatedFileSuggestions() {
		...
		if (currentEditingSession) {
			...
			this._currentRelatedFilesRetrievalOperation =
				this.chatEditingService
					.getRelatedFiles(
						currentEditingSession.chatSessionId,
						widget.getInput(),
					...
					.then((files) => {
						...
						if (
							...
							currentEditingSession.entries.get().length
						) {
						...
						const maximumRelatedFiles = Math.min(
							2,
							this.chatEditingService.editingSessionFileLimit -
								widget.input.chatEditWorkingSetFiles.length,
						);
						...
						for (const group of files) {
							for (const file of group.files) {
								if (
									newSuggestions.size >= maximumRelatedFiles
								) {
									break;
								}
						...
						for (const file of newSuggestions) {
							currentEditingSession.addFileToWorkingSet(
								file,
								localize("relatedFile", "Suggested File"),
								WorkingSetEntryState.Suggested,
							);
						}
					})
	...
	private _handleNewEditingSession() {
		...
		if (
			!widget ||
			widget.viewModel?.sessionId !== currentEditingSession.chatSessionId
		) {
			return;
		}
