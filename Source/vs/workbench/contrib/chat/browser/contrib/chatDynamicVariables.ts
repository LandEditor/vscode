	...
	implements IChatWidgetContrib
{
	...
	private getHoverForReference(
		...
		if (URI.isUri(value)) {
			...
			return new MarkdownString(
				prefix +
					this.labelService.getUriLabel(value.uri, {
						relative: true,
					}) +
					rangeString,
			);
...
function isSelectAndInsertActionContext(
	context: any,
): context is SelectAndInsertActionContext {
...
export class SelectAndInsertSymAction extends Action2 {
	...
	async run(accessor: ServicesAccessor, ...args: any[]) {
		...
		if (
			!symbol ||
			!textModelService.canHandleResource(symbol.location.uri)
		) {
			logService.trace(
