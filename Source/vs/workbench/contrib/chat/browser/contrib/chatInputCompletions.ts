			...
			this.languageFeaturesService.completionProvider.register(
				...
				},
				{
					...
					provideCompletionItems: async (
						...
						return {
							suggestions: agents.flatMap((agent) =>
								agent.slashCommands.map((c, i) => {
									const { label: agentLabel, isDupe } =
										this.getAgentCompletionDetails(agent);
			...
			this.languageFeaturesService.completionProvider.register(
				...
				},
				{
					...
					provideCompletionItems: async (
						...
						if (
							!model.getLineContent(1).startsWith(chatAgentLeader)
						) {
							return;
						}
...
class BuiltinDynamicCompletions extends Disposable {
	...
	constructor(
		...
		private readonly outlineService: IOutlineModelService,
	) {
		super();
	...
	private async addFileEntries(
					...
					arguments: [
						new ReferenceArgument(widget, {
							...
							range: {
								...
								endColumn:
									info.replace.startColumn + text.length,
							},
							data: resource,
						}),
					],
		...
		if (
			...
			widget.viewModel &&
			this._chatEditingService.currentEditingSessionObs.get()
				?.chatSessionId === widget.viewModel?.sessionId
		) {
			const relatedFiles =
				(await raceTimeout(
...
function isEmptyUpToCompletionWord(
	model: ITextModel,
