	...
	renderElement(
		...
		if (typeof reference === "object" && "variableName" in reference) {
			...
			if (
				...
				if (
					data.state === WorkingSetEntryState.Transient ||
					data.state === WorkingSetEntryState.Suggested
				) {
					templateData.label.setResource(
						{
							resource: uri,
							...
							description:
								data.description ??
								localize("chat.openEditor", "Open Editor"),
							range:
								"range" in reference
									? reference.range
									: undefined,
						},
						{
							icon,
							title:
								data.options?.status?.description ?? data.title,
							italic:
								data.state === WorkingSetEntryState.Suggested,
							strikethrough: data.excluded,
							extraClasses,
					...
					templateData.label.setFile(uri, {
						...
						range:
							"range" in reference ? reference.range : undefined,
						title: data.options?.status?.description ?? data.title,
						strikethrough: data.excluded,
						extraClasses,
					});
		...
		if (data.state !== undefined) {
			if (templateData.actionBarContainer) {
				if (
					data.state === WorkingSetEntryState.Modified &&
					!templateData.actionBarContainer.classList.contains(
						"modified",
					)
				) {
