...
abstract class NavigateAction extends Action2 {
		...
		super({
			...
			keybinding: {
				...
				when: ContextKeyExpr.and(
					ContextKeyExpr.or(
						ctxHasEditorModification,
						ctxNotebookHasEditorModification,
					),
	...
	override run(accessor: ServicesAccessor) {
		...
		const idx = entries.findIndex((e) =>
			isEqual(e.modifiedURI, editor.getModel().uri),
		);
		...
		const newIdx =
			(idx + (this.next ? 1 : -1) + entries.length) % entries.length;
...
abstract class AcceptDiscardAction extends Action2 {
	...
	override run(accessor: ServicesAccessor) {
		...
		let uri = getNotebookEditorFromEditorPane(
		...
		if (!uri) {
			const editor = editorService.activeTextEditorControl;
			uri =
				isCodeEditor(editor) && editor.hasModel()
					? editor.getModel().uri
					: undefined;
		}
		if (!uri) {
