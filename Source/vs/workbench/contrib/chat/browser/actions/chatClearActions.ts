...
export function registerNewChatActions() {
	...
	registerAction2(
		class NewEditSessionAction extends Action2 {
			constructor() {
				super({
			 ...
			 */
			private async _handleCurrentEditingSession(
				...
				if (currentEditingSession && currentEditCount) {
					const undecidedEdits = currentEdits.filter(
						(edit) =>
							edit.state.get() === WorkingSetEntryState.Modified,
					);
			...
			async run(accessor: ServicesAccessor, ...args: any[]) {
				...
				if (isChatViewTitleActionContext(context)) {
					...
					const widget = widgetService.getWidgetBySessionId(
						context.sessionId,
					);
	...
	registerAction2(
		class GlobalEditsDoneAction extends Action2 {
			constructor() {
				super({
	...
	registerAction2(
		class UndoChatEditInteractionAction extends Action2 {
			constructor() {
				super({
			...
			async run(accessor: ServicesAccessor, ...args: any[]) {
				...
				widget?.viewModel?.model.disableRequests(
					currentEditingSession.hiddenRequestIds.get(),
				);
			}
		},
	);
	...
	registerAction2(
		class RedoChatEditInteractionAction extends Action2 {
			constructor() {
				super({
