...
export function registerChatCodeBlockActions() {
	...
	registerAction2(
		class ApplyAllAction extends Action2 {
			constructor() {
				super({
			...
			override async run(accessor: ServicesAccessor, ...args: any[]) {
				...
				const request = chatModel
					...
					.find(
						(request) =>
							request.response?.result?.metadata?.applyEditsId ===
							applyEditsId,
					);
				if (request && request.response) {
					await chatEditingService.startOrContinueEditingSession(
						widget.viewModel.sessionId,
						{ silent: true },
					); // make sure we have an editing session
					await chatEditingService.triggerEditComputation(
						request.response,
					);
				}
			}
