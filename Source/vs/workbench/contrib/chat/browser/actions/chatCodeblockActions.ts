...
function getUsedDocuments(
	...
	return isResponseVM(context.element)
		? context.element.response.usedContext?.documents
		: undefined;
}
		...
		class InsertCodeBlockAction extends ChatCodeBlockAction {
			...
			override async runWithContext(
				...
				const activeTextModel =
					textFileService.files.get(activeModel.uri) ??
					textFileService.untitled.get(activeModel.uri);
			...
			private async handleTextEditor(
				...
				const mappedEditsProviders = accessor
					.get(ILanguageFeaturesService)
					.mappedEditsProvider.ordered(activeModel);
	...
	registerAction2(
		class InsertIntoNewFileAction extends ChatCodeBlockAction {
			...
			override async runWithContext(
				...
				editorService.openEditor(<IUntitledTextResourceEditorInput>{
