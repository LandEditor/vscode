...
type IAttachmentQuickPickItem =
	...
	| IScreenShotQuickPickItem
	| IRelatedFilesQuickPickItem;
...
function isScreenshotQuickPickItem(
	obj: unknown,
): obj is IScreenShotQuickPickItem {
	return (
		typeof obj === "object" &&
...
function isRelatedFileQuickPickItem(
	obj: unknown,
): obj is IRelatedFilesQuickPickItem {
	return (
		typeof obj === "object" &&
	...
	override async run(
		...
		if (matches && matches.length > 0) {
			...
			if (
				...
				if (selection) {
					(
						await showChatView(accessor.get(IViewsService))
					)?.focusInput();
					variablesService.attachContext(
						"file",
...
class AttachSelectionToEditingSessionAction extends Action2 {
	...
	override async run(
		...
		if (
			...
			if (selection) {
				(
					await showEditsView(accessor.get(IViewsService))
				)?.focusInput();
				variablesService.attachContext(
					"file",
					{ uri: activeUri, range: selection },
...
export class AttachContextAction extends Action2 {
				...
				},
				{
					when: ContextKeyExpr.and(
						ChatContextKeys.location
							.isEqualTo(ChatAgentLocation.Panel)
							.negate(),
						AttachContextAction._cdt,
					),
			...
			if (isISymbolQuickPickItem(pick) && pick.symbol) {
				...
				if (
					...
					if (chatEditingService) {
						chatEditingService.currentEditingSessionObs
							.get()
							?.addFileToWorkingSet(pick.resource);
					} else {
						toAttach.push({
				...
				for (const result of searchView.model.searchResult.matches()) {
					if (chatEditingService) {
						chatEditingService.currentEditingSessionObs
							.get()
							?.addFileToWorkingSet(result.resource);
					} else {
						toAttach.push({
							id: this._getFileContextId({
				...
				for (const file of selectedFiles ?? []) {
					chatEditingService?.currentEditingSessionObs
						.get()
						?.addFileToWorkingSet(file.value);
				}
	...
	override async run(
		...
		if (!context || !context.showFilesOnly) {
			...
			if (
				extensionService.extensions.some((ext) =>
			...
			if (
				chatEditingService?.hasRelatedFilesProviders() &&
				(widget.getInput() ||
	...
	private _show(
		...
		const providerOptions: AnythingQuickAccessProviderRunOptions = {
			handleAccept: (
				item: IChatContextQuickPickItem,
			...
			filter: (item: IChatContextQuickPickItem | IQuickPickSeparator) => {
				...
				if (chatEditingService) {
					for (const [
						file,
						state,
					] of chatEditingService.currentEditingSessionObs
						.get()
						?.workingSet.entries() ?? []) {
						if (state.state !== WorkingSetEntryState.Suggested) {
							attachedContext.add(
								this._getFileContextId({ resource: file }),
							);
						}
					}
				}
	...
	class AttachFilesAction extends AttachContextAction {
		constructor() {
			super({
				...
				title: localize2(
					"workbench.action.chat.editing.attachFiles.label",
