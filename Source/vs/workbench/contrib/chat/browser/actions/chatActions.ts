...
export function registerChatActions() {
	...
	registerAction2(
		class ClearChatHistoryAction extends Action2 {
			...
			async run(accessor: ServicesAccessor, ...args: any[]) {
				...
				const chatView = viewsService.getViewWithId(ChatViewId) as
					| ChatViewPane
					| undefined;
	...
	function registerOpenLinkAction(
		id: string,
	...
	registerOpenLinkAction(
		...
		localize2("managePlan", "Manage Copilot Plan"),
		defaultChat.managePlanUrl,
		1,
	);
	registerOpenLinkAction(
		"workbench.action.chat.manageSettings",
		localize2("manageSettings", "Manage Copilot Settings"),
		defaultChat.manageSettingsUrl,
		2,
	);
	registerOpenLinkAction(
		"workbench.action.chat.learnMore",
		localize2("learnMore", "Learn More"),
		defaultChat.documentationUrl,
		3,
	);

	registerAction2(
		class ShowExtensionsUsingCopilit extends Action2 {
			constructor() {
				super({
...
export class ChatCommandCenterRendering
	extends Disposable
	...
	constructor(
		...
		actionViewItemService.register(
			...
			(action, options) => {
				...
				const chatExtensionInstalled = agentService
					.getAgents()
					.some((agent) => agent.isDefault);
				const { chatQuotaExceeded, completionsQuotaExceeded } =
					chatQuotasService.quotas;
				const signedOut =
				...
				if (!chatExtensionInstalled) {
					...
					if (signedOut) {
						...
						primaryActionTitle = quotaToButtonMessage({
							chatQuotaExceeded,
