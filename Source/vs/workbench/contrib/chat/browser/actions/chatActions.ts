...
registerAction2(
	class ToggleChatControl extends ToggleTitleBarConfigAction {
		constructor() {
			super(
		...
		actionViewItemService.register(
			...
			(action, options) => {
				...
				const primaryAction = instantiationService.createInstance(
					MenuItemAction,
					{
						id: chatExtensionInstalled
							? CHAT_OPEN_ACTION_ID
							: "workbench.action.chat.triggerSetup", // TODO@bpasero revisit layering of this action
						title: OpenChatGlobalAction.TITLE,
						icon: defaultChat.icon,
					},
					undefined,
