...
const defaultChat = {
	...
	icon: Codicon[
		(product.defaultChatAgent?.icon as keyof typeof Codicon) ??
			"commentDiscussion"
	],
	chatWelcomeTitle: product.defaultChatAgent?.chatWelcomeTitle ?? "",
	documentationUrl: product.defaultChatAgent?.documentationUrl ?? "",
	privacyStatementUrl: product.defaultChatAgent?.privacyStatementUrl ?? "",
	collectionDocumentationUrl:
		product.defaultChatAgent?.collectionDocumentationUrl ?? "",
	skusDocumentationUrl: product.defaultChatAgent?.skusDocumentationUrl ?? "",
	providerId: product.defaultChatAgent?.providerId ?? "",
	providerName: product.defaultChatAgent?.providerName ?? "",
	providerScopes: product.defaultChatAgent?.providerScopes ?? [[]],
	entitlementUrl: product.defaultChatAgent?.entitlementUrl ?? "",
	entitlementChatEnabled:
		product.defaultChatAgent?.entitlementChatEnabled ?? "",
	entitlementSkuLimitedUrl:
		product.defaultChatAgent?.entitlementSkuLimitedUrl ?? "",
	entitlementSkuLimitedEnabled:
		product.defaultChatAgent?.entitlementSkuLimitedEnabled ?? "",
};
...
class ChatSetupContribution
	extends Disposable
	implements IWorkbenchContribution
{
	private readonly chatSetupState =
		this.instantiationService.createInstance(ChatSetupState);
	private readonly entitlementsResolver = this._register(
		this.instantiationService.createInstance(ChatSetupEntitlementResolver),
	);
	...
	private registerChatWelcome(): void {
		Registry.as<IChatViewsWelcomeContributionRegistry>(
			...
			content: () =>
				ChatSetupWelcomeContent.getInstance(
					this.instantiationService,
					this.entitlementsResolver,
				).element,
		});
	}
	...
	private async checkExtensionInstallation(): Promise<void> {
		...
		const chatInstalled = !!extensions.find((value) =>
			ExtensionIdentifier.equals(
				value.identifier.id,
				defaultChat.extensionId,
			),
...
type ChatSetupEntitlementClassification = {
	entitled: {
		...
		comment: "Flag indicating if the user is chat setup entitled";
	};
	entitlement: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
...
class ChatSetupEntitlementResolver extends Disposable {
	...
	private readonly _onDidChangeEntitlement = this._register(
		new Emitter<ChatEntitlement>(),
	);
	readonly onDidChangeEntitlement = this._onDidChangeEntitlement.event;

	private readonly chatSetupEntitledContextKey =
		ChatContextKeys.Setup.entitled.bindTo(this.contextKeyService);
			...
			this.authenticationService.onDidChangeSessions(
				async ({ providerId }) => {
					if (providerId === defaultChat.providerId) {
						this.update(
							this.toEntitlement(
								await this.hasProviderSessions(),
							),
						);
					}
				},
	...
	private async handleDeclaredAuthProviders(): Promise<void> {
		if (
			this.authenticationService.declaredProviders.find(
				(provider) => provider.id === defaultChat.providerId,
			)
		) {
	...
	private async doResolveEntitlement(
		...
		const response = await this.instantiationService.invokeFunction(
			(accessor) =>
				ChatSetupRequestHelper.request(
					accessor,
		...
		if (response.res.statusCode && response.res.statusCode !== 200) {
			this.logService.trace(
				`[chat setup] entitlement: unexpected status code ${response.res.statusCode}`,
			);
			return ChatEntitlement.Unresolved;
		}
		...
		if (!result) {
			this.logService.trace(
				"[chat setup] entitlement: response has no content",
			);
			return ChatEntitlement.Unresolved;
		}
		...
		try {
			...
			this.logService.trace(
				`[chat setup] entitlement: parsed result is ${JSON.stringify(parsedResult)}`,
			);
		} catch (err) {
			this.logService.trace(
				`[chat setup] entitlement: error parsing response (${err})`,
			);
			return ChatEntitlement.Unresolved;
		}

		const entitled = Boolean(
			parsedResult[defaultChat.entitlementChatEnabled],
		);
		this.chatSetupEntitledContextKey.set(entitled);

		const skuLimitedAvailable = Boolean(
			parsedResult[defaultChat.entitlementSkuLimitedEnabled],
		);
		this.resolvedEntitlement = this.toEntitlement(
			entitled,
			skuLimitedAvailable,
		);

		this.logService.trace(
			`[chat setup] entitlement: resolved to ${this.resolvedEntitlement}`,
		);

		this.telemetryService.publicLog2<
			ChatSetupEntitlementEvent,
			ChatSetupEntitlementClassification
...
type InstallChatClassification = {
	...
	signedIn: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		comment: "Whether the user did sign in prior to installing the extension.";
	};
};
...
class ChatSetupWelcomeContent extends Disposable {
	...
	static getInstance(
		...
		options: IChatSetupWelcomeContentOptions,
	): ChatSetupWelcomeContent {
		if (!ChatSetupWelcomeContent.INSTANCE) {
			ChatSetupWelcomeContent.INSTANCE =
	...
	private create(): void {
		const markdown = this._register(
			this.instantiationService.createInstance(MarkdownRenderer, {}),
		);

		// Header
		this.element.appendChild($("p")).textContent = localize(
			"setupHeader",
			"{0} is your AI pair programmer.",
			defaultChat.name,
		);

		// SKU Limited Sign-up
		const skuHeader = localize(
			{ key: "skuHeader", comment: ['{Locked="]({0})"}'] },
			"Setup will sign you up to {0} [limited access]({1}).",
			defaultChat.name,
			defaultChat.skusDocumentationUrl,
		);
		const skuHeaderElement = this.element
			.appendChild($("p"))
			.appendChild(
				this._register(
					markdown.render(
						new MarkdownString(skuHeader, { isTrusted: true }),
					),
				).element,
			);

		const telemetryLabel = localize(
			"telemetryLabel",
			"Allow {0} to use my data, including Prompts, Suggestions, and Code Snippets, for product improvements",
			defaultChat.providerName,
		);
		const { container: telemetryContainer, checkbox: telemetryCheckbox } =
			this.createCheckBox(
				telemetryLabel,
		...
		const detectionLabel = localize(
			"detectionLabel",
			"Allow suggestions matching public code",
		);
		const { container: detectionContainer, checkbox: detectionCheckbox } =
			this.createCheckBox(detectionLabel, true);
		...
		this.updateControls(
			button,
			[telemetryCheckbox, detectionCheckbox],
			false,
		);
		...
		const footer = localize(
			{ key: "privacyFooter", comment: ['{Locked="]({0})"}'] },
			"By proceeding you agree to our [privacy statement]({0}). You can [learn more]({1}) about {2}.",
		...
		this._register(
			this.options.onDidChangeEntitlement(() => {
				if (setupRunning) {
					return; // do not change when setup running
				}
				setVisibility(
					this.options.entitlement !== ChatEntitlement.Unavailable,
					skuHeaderElement,
					telemetryContainer,
					detectionContainer,
				);
	...
	private createCheckBox(
		label: string,
		checked: boolean,
	): { container: HTMLElement; checkbox: Checkbox } {
		const container = this.element.appendChild($("p"));
		const checkbox = this._register(
			new Checkbox(label, checked, defaultCheckboxStyles),
		);
		...
		this._register(
			addDisposableListener(
				telemetryCheckboxLabelContainer,
				EventType.CLICK,
				() => {
					if (checkbox?.enabled) {
	...
	private updateControls(
		...
		if (setupRunning) {
			...
			button.enabled = true;
			button.label =
				this.options.entitlement === ChatEntitlement.Unknown
	...
	private async setup(
		...
		enableDetection: boolean | undefined,
	): Promise<boolean> {
	...
	private async install(
		...
		try {
			...
			if (this.options.entitlement !== ChatEntitlement.Unavailable) {
				const body = {
					public_code_suggestions: enableDetection
						? "enabled"
						: "disabled",
					restricted_telemetry: enableTelemetry
						? "enabled"
						: "disabled",
				};
				this.logService.trace(
					`[chat setup] install: signing up to limited SKU with ${JSON.stringify(body)}`,
				);
				...
				const response = await this.instantiationService.invokeFunction(
					(accessor) =>
						ChatSetupRequestHelper.request(
							accessor,
							defaultChat.entitlementSkuLimitedUrl,
							"POST",
							body,
							session,
				...
				if (response && this.logService.getLevel() === LogLevel.Trace) {
					this.logService.trace(
						`[chat setup] install: response from signing up to limited SKU ${JSON.stringify(await asText(response))}`,
					);
				}
			} else {
				this.logService.trace(
					"[chat setup] install: not signing up to limited SKU",
				);
			}
		...
		this.telemetryService.publicLog2<
			InstallChatEvent,
			InstallChatClassification
		>("commandCenter.chatInstall", { installResult, signedIn });
...
class ChatSetupRequestHelper {
	...
	static async request(
		...
		try {
			if (!session) {
				session = (
					await authenticationService.getSessions(
						defaultChat.providerId,
					)
				).at(0);
			}
			...
			if (!session) {
				throw new Error(
					"ChatSetupRequestHelper: No session found for provider",
				);
			}
...
class ChatSetupState {
	...
	private static readonly CHAT_EXTENSION_INSTALLED =
		"chat.extensionInstalled";

	private readonly chatSetupTriggeredContext =
		ChatContextKeys.Setup.triggered.bindTo(this.contextKeyService);
	private readonly chatSetupInstalledContext =
		ChatContextKeys.Setup.installed.bindTo(this.contextKeyService);
	...
	update(context: { triggered?: boolean; chatInstalled?: boolean }): void {
		if (typeof context.chatInstalled === "boolean") {
			this.storageService.store(
				ChatSetupState.CHAT_EXTENSION_INSTALLED,
				context.chatInstalled,
				StorageScope.PROFILE,
				StorageTarget.MACHINE,
			);
			this.storageService.store(
				ChatSetupState.CHAT_SETUP_TRIGGERD,
				true,
				StorageScope.PROFILE,
				StorageTarget.MACHINE,
			); // allows to fallback to setup view if the extension is uninstalled
		}
	...
	private updateContext(): void {
		const chatSetupTriggered = this.storageService.getBoolean(
			ChatSetupState.CHAT_SETUP_TRIGGERD,
			StorageScope.PROFILE,
			false,
		);
		const chatInstalled = this.storageService.getBoolean(
			ChatSetupState.CHAT_EXTENSION_INSTALLED,
			StorageScope.PROFILE,
			false,
		);
		...
		if (showChatSetup) {
			...
			this.storageService.remove(
				"chat.welcomeMessageContent.panel",
	...
	override async run(accessor: ServicesAccessor): Promise<void> {
		...
		const { confirmed } = await dialogService.confirm({
			message: localize(
				...
				"Are you sure you want to hide {0}?",
				defaultChat.name,
			),
			detail: localize(
				"hideChatSetupDetail",
				"You can restore chat controls from the 'chat.commandCenter.enabled' setting.",
			),
