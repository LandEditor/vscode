	...
	implements IHistoryNavigationWidget
{
	...
	render(container: HTMLElement, initialValue: string, widget: IChatWidget) {
		...
		this._register(
			this._inputEditor.onDidChangeCursorPosition((e) => {
				...
				const atTop =
					e.position.column === 1 && e.position.lineNumber === 1;
		...
		this.inputModel =
			this.modelService.getModel(this.inputUri) ||
	...
	private _layout(
		...
		const sideToolbarWidth =
			this.options.renderStyle === "compact" ? 20 : 0;
	...
	saveState(): void {
		...
		const historyEntries = inputHistory.map((entry) => ({
			text: entry,
			state: this.historyStates.get(entry),
		}));
...
class SubmitButtonActionViewItem extends ActionViewItem {
	...
	constructor(
		...
		const primaryKeybinding = keybindingService
			.lookupKeybinding(ChatSubmitEditorAction.ID)
			?.getLabel();
		...
		if (secondaryAgent) {
			const secondaryKeybinding = keybindingService
				.lookupKeybinding(ChatSubmitSecondaryAgentEditorAction.ID)
				?.getLabel();
