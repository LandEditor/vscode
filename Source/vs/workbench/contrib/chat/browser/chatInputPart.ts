	...
	implements IHistoryNavigationWidget
{
	...
	private _onDidChangeCurrentLanguageModel = this._register(
		new Emitter<string>(),
	);
	...
	private readonly _chatEditsFileLimitHover = this._register(
		new MutableDisposable<IDisposable>(),
	);
	...
	constructor(
		...
		if (this.location === ChatAgentLocation.EditingSession) {
			this._attachmentModel = this._register(
				this.instantiationService.createInstance(EditsAttachmentModel),
			);
	...
	render(container: HTMLElement, initialValue: string, widget: IChatWidget) {
		...
		ChatContextKeys.inChatInput
			.bindTo(inputScopedContextKeyService)
			.set(true);
			...
			this.instantiationService.createInstance(
				...
				this.options.menus.executeToolbar,
				{
					...
					actionViewItemProvider: (action, options) => {
						if (
							this.location === ChatAgentLocation.Panel ||
							this.location === ChatAgentLocation.Editor
						) {
							if (
								(action.id === ChatSubmitAction.ID ||
		...
		const attachments =
			this.location === ChatAgentLocation.EditingSession
				? // Render as attachments anything that isn't a file, but still render specific ranges in a file
					[...this.attachmentModel.attachments.entries()].filter(
						([_, attachment]) =>
							!attachment.isFile ||
							(attachment.isFile &&
		...
		for (const [index, attachment] of attachments) {
			...
			if (resource && (attachment.isFile || attachment.isDirectory)) {
				...
				label.setFile(
					...
					attachment.isFile
						...
						: {
								...
								icon: !this.themeService.getFileIconTheme()
									.hasFolderIcons
									? FolderThemeIcon
									: undefined,
							},
				...
				if (attachment.copiedFrom) {
					...
					label.setLabel(filename, undefined, {
						extraClasses: classNames,
					});
				} else {
					label.setLabel(attachment.fileName, undefined, {
						extraClasses: classNames,
					});
				}
				widget.appendChild(
	...
	async renderChatEditingSessionState(
		...
		let entries: IChatCollapsibleListItem[] =
		...
		for (const attachment of (this.attachmentModel as EditsAttachmentModel)
			.fileAttachments) {
			if (
				URI.isUri(attachment.value) &&
				!seenEntries.has(attachment.value)
			) {
				entries.unshift({
		...
		for (const [
			file,
			metadata,
		] of chatEditingSession.workingSet.entries()) {
			if (
				!seenEntries.has(file) &&
				metadata.state !== WorkingSetEntryState.Suggested
			) {
				entries.unshift({
					reference: file,
		...
		for (const part of chatWidget?.parsedInput.parts ?? []) {
			if (
				...
				part.isFile &&
				((URI.isUri(part.data) && !seenEntries.has(part.data)) ||
					(isLocation(part.data) && !seenEntries.has(part.data.uri)))
			) {
				entries.unshift({
					reference: part.data,
					state: WorkingSetEntryState.Attached,
		...
		for (const excludedAttachment of (
			this.attachmentModel as EditsAttachmentModel
		).excludedFileAttachments) {
			if (
				excludedAttachment.isFile &&
				URI.isUri(excludedAttachment.value) &&
				!seenEntries.has(excludedAttachment.value)
			) {
				excludedEntries.push({
					...
					excluded: true,
					title: localize(
						"chatEditingSession.excludedFile",
		...
		let remainingFileEntriesBudget =
		...
		const overviewWorkingSet =
			overviewTitle.querySelector("span") ??
			dom.append(overviewTitle, $("span"));

		const overviewFileCount =
			overviewTitle.querySelector("span.working-set-count") ??
			dom.append(overviewTitle, $("span.working-set-count"));

		overviewWorkingSet.textContent = localize(
			"chatEditingSession.workingSet",
			"Working Set",
		);

		// Record the number of entries that the user wanted to add to the working set
		this._attemptedWorkingSetEntriesCount =
			entries.length + excludedEntries.length;
		...
		if (entries.length === 1) {
			...
			suggestedFilesInWorkingSetCount =
				entries[0].kind === "reference" &&
				entries[0].state === WorkingSetEntryState.Suggested
					? 1
					: 0;
			...
			for (let i = 0; i < entries.length; i += 1) {
				...
				if (entry.state === WorkingSetEntryState.Suggested) {
					...
					this._combinedChatEditWorkingSetEntries.find(
						(e) => e.toString() === currentEntryUri?.toString(),
					)
				) {
			...
			const newEntriesThatFit =
				remainingFileEntriesBudget > 0
					? newEntries.slice(0, remainingFileEntriesBudget)
					: [];

			remainingFileEntriesBudget -= newEntriesThatFit.length;

			const suggestedFilesThatFit =
				remainingFileEntriesBudget > 0
					? suggestedFiles.slice(0, remainingFileEntriesBudget)
					: [];
			...
			entries = [
				...entriesToPreserve,
				...newEntriesThatFit,
				...suggestedFilesThatFit,
			];

			suggestedFilesInWorkingSetCount = suggestedFilesThatFit.length;
		} else {
			suggestedFilesInWorkingSetCount = entries.filter(
				(e) =>
					e.kind === "reference" &&
					e.state === WorkingSetEntryState.Suggested,
			).length;
		}
		...
		if (excludedEntries.length > 0) {
			overviewFileCount.textContent =
				" " +
				localize(
					"chatEditingSession.excludedFiles",
					"({0}/{1} files)",
					this.chatEditingService.editingSessionFileLimit +
		...
		overviewFileCount.classList.toggle(
			"file-limit-reached",
			fileLimitReached,
		);
		...
		if (fileLimitReached) {
			let title = localize(
				"chatEditingSession.fileLimitReached",
				"You have reached the maximum number of files that can be added to the working set.",
			);

			title +=
				excludedEntries.length === 1
					? " " +
						localize(
							"chatEditingSession.excludedOneFile",
							"1 file is excluded from the Working Set.",
						)
					: "";
			...
			title +=
				excludedEntries.length > 1
					? " " +
						localize(
							"chatEditingSession.excludedSomeFiles",
							"{0} files are excluded from the Working Set.",
							excludedEntries.length,
						)
					: "";

			this._chatEditsFileLimitHover.value =
				getBaseLayerHoverDelegate().setupDelayedHover(
					overviewFileCount as HTMLElement,
					{
						content: title,
				...
				MenuId.ChatEditingWidgetToolbar,
				{
					...
					buttonConfigProvider: (action) => {
						if (
							...
							action.id === ChatEditingSaveAllAction.ID ||
			...
			this._chatEditsDisposables.add(
				list.onDidOpen((e) => {
					if (
						...
						const entry =
							chatEditingSession.getEntry(modifiedFileUri);
						const diffInfo = entry?.diffInfo.get();

						const range = diffInfo?.changes
							.at(0)
		...
		this._combinedChatEditWorkingSetEntries = coalesce(
			entries.map((e) =>
				e.kind === "reference" && URI.isUri(e.reference)
					? e.reference
					: undefined,
			),
		);
		...
		button.setTitle(
			button.enabled
				? localize("addFiles.label", "Add files to your working set")
				: localize(
						"chatEditingSession.fileLimitReached",
						"You have reached the maximum number of files that can be added to the working set.",
					),
		...
		for (const [uri, metadata] of chatEditingSession.workingSet) {
			...
			const addBtn = this._chatEditsActionsDisposables.add(
				new Button(addFilesElement, {
					supportIcons: true,
					secondary: true,
					hoverDelegate,
			...
			const rmBtn = this._chatEditsActionsDisposables.add(
				new Button(addFilesElement, {
					supportIcons: false,
					secondary: true,
					hoverDelegate,
			...
			rmBtn.setTitle(
				localize(
					"chatEditingSession.removeSuggested",
					"Remove suggestion",
				),
			);
			this._chatEditsActionsDisposables.add(
	...
	protected override updateLabel(): void {
		if (this.label) {
			...
			if (model) {
				dom.reset(
					this.label,
					dom.$("span.chat-model-label", undefined, model.name),
