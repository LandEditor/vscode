	...
	implements IHistoryNavigationWidget
{
	...
	async renderChatEditingSessionState(
		...
		dom.setVisibility(
			Boolean(chatEditingSession) || Boolean(initialState),
			this.chatEditingSessionWidgetContainer,
		);
		...
		if (
			this._chatEditList &&
			chatEditingSession?.state.get() === ChatEditingSessionState.Idle
		) {
		...
		const clearButton = this._chatEditsActionsDisposables.add(
			new Button(actionsContainer, { supportIcons: true }),
		);
		clearButton.icon = Codicon.close;
		this._chatEditsActionsDisposables.add(
			clearButton.onDidClick((e) => {
				this.commandService.executeCommand(
		...
		if (
			!this._chatEditsProgress &&
			(chatEditingSession.state.get() ===
