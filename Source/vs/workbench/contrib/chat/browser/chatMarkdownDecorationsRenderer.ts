...
export function reduceInlineContentReferences(
	...
	for (const item of response) {
		...
		if ("inlineReference" in item) {
			...
			const markdownText = `[${
				item.name || basename(location.uri)
			}](${printUri.toString()})`;
			if (isMarkdownString(previousItem)) {
				result[result.length - 1] = new MarkdownString(
					previousItem.value + markdownText,
					{ isTrusted: previousItem.isTrusted }
				);
			} else {
