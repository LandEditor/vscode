...
registerAction2(
	class AddFileToWorkingSet extends WorkingSetAction {
		constructor() {
			super({
				...
				menu: [
					{
						...
						when: ContextKeyExpr.or(
							...
							ContextKeyExpr.equals(
								chatEditingWidgetFileStateContextKey.key,
								WorkingSetEntryState.Suggested,
							),
						),
						order: 0,
						group: "navigation",
...
registerAction2(
	class RemoveFileFromWorkingSet extends WorkingSetAction {
		...
		async runWorkingSetAction(
			...
			const suggestedFiles = entries.filter(
				([_, state]) => state.state === WorkingSetEntryState.Suggested,
			);
			if (
				suggestedFiles.length === entries.length &&
				!chatWidget.attachmentModel.attachments.find(
					(v) => v.isFile && URI.isUri(v.value),
				)
			) {
				currentEditingSession.remove(
...
export class ChatEditingRemoveAllFilesAction extends Action2 {
			...
			menu: [
				{
					...
					order: 5,
					when: ContextKeyExpr.and(
						hasAppliedChatEditsContextKey.negate(),
						ChatContextKeys.location.isEqualTo(
							ChatAgentLocation.EditingSession,
						),
					),
	...
	async run(accessor: ServicesAccessor, ...args: any[]): Promise<void> {
		...
		const chatWidget = accessor
			.get(IChatWidgetService)
			.getWidgetBySessionId(currentEditingSession.chatSessionId);
		...
		const attachmentModel = chatWidget?.attachmentModel as
			| EditsAttachmentModel
			| undefined;

		const fileAttachments = attachmentModel
			? [
					...attachmentModel.excludedFileAttachments,
					...attachmentModel.fileAttachments,
				]
			: [];

		const attachmentIdsToRemove = fileAttachments.map((attachment) =>
			(attachment.value as URI).toString(),
		);
	...
	class AddFilesToWorkingSetAction extends Action2 {
		...
		override async run(
			...
			for (const file of uris) {
				chatEditingService?.currentEditingSessionObs
					.get()
					?.addFileToWorkingSet(file);
			}
		}
	},
);
...
registerAction2(
	class RemoveAction extends Action2 {
		...
		async run(accessor: ServicesAccessor, ...args: any[]) {
			...
			if (
				chatModel?.initialLocation !== ChatAgentLocation.EditingSession
			) {
				return;
			}
				...
				if (editsToUndo === 1) {
					if (entriesModifiedInRequestsToRemove.length === 1) {
						message = localize(
							...
							basename(
								entriesModifiedInRequestsToRemove[0]
									.modifiedURI,
							),
						);
					} else {
	...
	class OpenWorkingSetHistoryAction extends Action2 {
		...
		constructor() {
			super({
				...
				precondition: ContextKeyExpr.notIn(
		...
		override async run(
			...
			const context:
				| { sessionId: string; requestId: string; uri: URI }
				| undefined = args[0];
			...
			const snapshotRequestIndex = requests?.findIndex(
				(v, i) => i > 0 && requests[i - 1]?.id === context.requestId,
			);
			...
			if (snapshotRequestId) {
				const snapshot =
					chatEditingService.currentEditingSession?.getSnapshotUri(
						snapshotRequestId,
						context.uri,
					);
				if (snapshot) {
...
registerAction2(
	class ResolveSymbolsContextAction extends Action2 {
		constructor() {
			super({
		...
		override async run(
			...
			const [widget] = widgetService.getWidgetsByLocations(
				ChatAgentLocation.EditingSession,
			);
			...
			const [references, definitions, implementations] =
				await Promise.all([
					this.getReferences(
						position,
						textModel,
			...
			for (const reference of [
				...
				attachments.push(
					widget.attachmentModel.asVariableEntry(reference.uri),
				);
			}
