	...
	implements IChatEditingService
{
	...
	private _applyingChatEditsFailedContextKey: IContextKey<
		boolean | undefined
	>;
	...
	constructor(
		...
		super();
		this._applyingChatEditsFailedContextKey =
			applyingChatEditsFailedContextKey.bindTo(contextKeyService);
		...
		this._register(
			bindContextKey(
				...
				(r) => {
					return (
						this._currentSessionObs.read(r)?.canUndo.read(r) ||
						false
					);
				},
			),
		...
		this._register(
			bindContextKey(
				...
				(r) => {
					return (
						this._currentSessionObs.read(r)?.canRedo.read(r) ||
						false
					);
				},
			),
			...
			this.lifecycleService.onWillShutdown((e) => {
				...
				if (session) {
					storageService.store(
						STORAGE_KEY_EDITING_SESSION,
						session.chatSessionId,
						StorageScope.WORKSPACE,
						StorageTarget.MACHINE,
					);
					e.join(session.storeState(), {
		...
		this._editingSessionFileLimitPromise = this._workbenchAssignmentService
			.getTreatment<number>(chatEditingMaxFileAssignmentName)
			.then((value) => {
				this._editingSessionFileLimit =
					value ?? defaultChatEditingMaxFileLimit;
				return this._editingSessionFileLimit;
			});
		void this._editingSessionFileLimitPromise;

		const sessionIdToRestore = storageService.get(
			STORAGE_KEY_EDITING_SESSION,
			StorageScope.WORKSPACE,
		);
		if (isString(sessionIdToRestore)) {
			if (this._chatService.getOrRestoreSession(sessionIdToRestore)) {
				...
				logService.error(
					`Edit session session to restore is a non-existing chat session: ${sessionIdToRestore}`,
				);
			}
	...
	private async _createEditingSession(
		...
		const session = this._instantiationService.createInstance(
			ChatEditingSession,
			chatSessionId,
			this._editingSessionFileLimitPromise,
		);
		await session.init();
		...
		this._currentSessionDisposables.add(
			this.installAutoApplyObserver(session),
		);
	...
	private installAutoApplyObserver(session: ChatEditingSession): IDisposable {
		const chatModel = this._chatService.getOrRestoreSession(
			session.chatSessionId,
		);
		if (!chatModel) {
			throw new Error(
		...
		const onResponseComplete = (responseModel: IChatResponseModel) => {
			if (
				responseModel.result?.errorDetails &&
				...
				if (
					...
					if (!editedFilesExist.get(part.uri)) {
						editedFilesExist.set(
							part.uri,
							this._fileService.exists(part.uri).then((e) => {
								if (e) {
					...
					const allEdits: TextEdit[][] =
						part.kind === "textEditGroup" ? part.edits : [];
					...
					if (newEdits.length > 0 || entry.seen === 0) {
						...
						editsSource.emitOne({
							...
							edits: newEdits,
							kind: "textEditGroup",
							done: part.kind === "textEditGroup" && part.done,
						});
					}
					...
					if (first) {
						...
						editsPromise = this._continueEditingSession(
							session,
							async (builder, token) => {
								for await (const item of editsSource!
									.asyncIterable) {
									...
									for (
										...
										const isLastGroup =
											i === item.edits.length - 1;
										builder.textEdits(
											item.uri,
											group,
											isLastGroup && (item.done ?? false),
											responseModel,
										);
									}
								}
			...
			providers.map(async (provider) => {
				try {
					const relatedFiles = await provider.provideRelatedFiles(
						{ prompt, files: userAddedWorkingSetEntries },
						token,
					);
	...
	implements IDecorationsProvider
{
	...
	private readonly _currentlyEditingUris = derived<URI[]>(this, (r) => {
		...
		return uri
			.filter((entry) => entry.isCurrentlyBeingModified.read(r))
	...
	provideDecorations(
		...
		const isModified = this._modifiedUris
			.get()
			.some((e) => e.toString() === uri.toString());
		if (isModified) {
...
export class ChatEditingMultiDiffSourceResolver
