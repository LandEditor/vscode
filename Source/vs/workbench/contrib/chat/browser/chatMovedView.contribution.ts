...
export class MoveChatViewContribution
	extends Disposable
	implements IWorkbenchContribution
{
	...
	private async hideViewIfCopilotIsNotInstalled(): Promise<void> {
		...
		const installed = extensions.find((value) =>
	...
	private registerMovedChatWelcomeView(): IDisposable {
		...
		const title = localize2(
			"chat.viewContainer.movedChat.label",
			"Chat (Old Location)",
		);
		...
		const viewDescriptor: IViewDescriptor = {
			...
			when: ContextKeyExpr.and(
				ChatContextKeys.shouldShowMovedViewWelcome,
				ContextKeyExpr.or(
					ChatContextKeys.panelParticipantRegistered,
					ChatContextKeys.extensionInvalid,
				),
			),
			ctorDescriptor: new SyncDescriptor(MovedChatViewPane, [
				{ id: viewId },
			]),
		};
		...
		const secondarySideBarLeft =
			this.configurationService.getValue("workbench.sideBar.location") !==
			"left";

		let welcomeViewMainMessage = secondarySideBarLeft
			? localize(
					"chatMovedMainMessage1Left",
					"Chat has been moved to the Secondary Side Bar on the left for a more integrated AI experience in your editor.",
				)
			: localize(
					"chatMovedMainMessage1Right",
					"Chat has been moved to the Secondary Side Bar on the right for a more integrated AI experience in your editor.",
				);

		const chatViewKeybinding = this.keybindingService
			.lookupKeybinding(CHAT_SIDEBAR_PANEL_ID)
			?.getLabel();
		...
		if (this.hasCommandCenterChat() && chatViewKeybinding) {
			quicklyAccessMessage = localize(
				"chatMovedCommandCenterAndKeybind",
				"You can quickly access Chat via the new Copilot icon ({0}) in the editor title bar or with the keyboard shortcut {1}.",
				copilotIcon,
				chatViewKeybinding,
			);
		} else if (this.hasCommandCenterChat()) {
			quicklyAccessMessage = localize(
				"chatMovedCommandCenter",
				"You can quickly access Chat via the new Copilot icon ({0}) in the editor title bar.",
				copilotIcon,
			);
		} else if (chatViewKeybinding) {
			quicklyAccessMessage = localize(
				"chatMovedKeybind",
				"You can quickly access Chat with the keyboard shortcut {0}.",
				chatViewKeybinding,
			);
		}
		...
		const welcomeViewFooterMessage = localize(
			"chatMovedFooterMessage",
			"[Learn more](command:_chatMovedViewWelcomeView.learnMore) about the Secondary Side Bar.",
		);
