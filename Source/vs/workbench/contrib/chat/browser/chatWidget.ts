...
export class ChatWidget extends Disposable implements IChatWidget {
	...
	constructor(
		...
		this.agentInInput =
			ChatContextKeys.inputHasAgent.bindTo(contextKeyService);
		...
		this._register(
			autorun((r) => {
				const session =
					this.chatEditingService.currentEditingSessionObs.read(r);
				if (!session) {
					return;
				}
	...
	private renderWelcomeViewContentIfNeeded() {
		...
		if (
			...
			const welcomePart = this._register(
				this.instantiationService.createInstance(
					ChatViewWelcomePart,
					{ ...welcomeContent, tips },
			...
			this.instantiationService.createInstance(
				...
				this.location,
				{
					...
					enableImplicitContext:
						this.viewOptions.enableImplicitContext,
				},
				this.styles,
	...
	private async _acceptInput(
		query: { query: string } | { prefix: string } | undefined,
		options?: IChatAcceptInputOptions,
	): Promise<IChatResponseModel | undefined> {
		if (this.viewModel) {
			...
			if (this.location === ChatAgentLocation.EditingSession) {
				...
				for (const v of this.inputPart.chatEditWorkingSetFiles) {
					...
					if (
						currentEditingSession?.workingSet.get(v)?.state ===
						...
						editingSessionAttachedContext.push(
							this.attachmentModel.asVariableEntry(v),
						);
					}
				}

				let maximumFileEntries =
					this.chatEditingService.editingSessionFileLimit -
					editingSessionAttachedContext.length;
				...
				>("chatEditing/workingSetSize", {
					originalSize:
						this.inputPart.attemptedWorkingSetEntriesCount,
	...
	layout(height: number, width: number): void {
		...
		const followupsOffset = this.viewOptions.renderFollowups
			? Math.max(100 - this.inputPart.followupsHeight, 0)
			: 0;
		...
		if (
			lastElementVisible &&
			(!lastResponseIsRendering || this.viewOptions.autoScroll)
		) {
			this.scrollToEnd();
		}
	...
	implements IChatWidgetService
{
	...
	private readonly _onDidAddWidget = this._register(
		new Emitter<ChatWidget>(),
	);
