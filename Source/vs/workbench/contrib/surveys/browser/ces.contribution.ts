...
class CESContribution extends Disposable implements IWorkbenchContribution {
	...
	private async promptUser() {
		const isCandidate =
			await this.tasExperimentService?.getTreatment<boolean>("CESSurvey");
		if (!isCandidate) {
			this.skipSurvey();
			return;
		}
		...
		const sendTelemetry = (
			...
			*/
			this.telemetryService.publicLog("cesSurvey:popup", {
				userReaction,
			});
		};
	...
	private async schedulePrompt(): Promise<void> {
		...
		if (remindLaterDate) {
			const timeToRemind =
				new Date(remindLaterDate).getTime() +
				REMIND_LATER_DELAY -
				Date.now();
			if (timeToRemind > 0) {
