...
class QuickDiffWidget extends PeekViewWidget {
	...
	protected override _fillHead(container: HTMLElement): void {
		...
		this.dropdown = this.instantiationService.createInstance(
			...
			new QuickDiffPickerBaseAction((event?: IQuickDiffSelectItem) =>
	...
	protected override _getActionBarOptions(): IActionBarOptions {
		...
		actionRunner.onDidRun((e) => {
			if (
				!(e.action instanceof QuickDiffWidgetEditorAction) &&
				!e.error
			) {
				this.dispose();
			}
	...
	implements IEditorContribution
{
	...
	constructor(
		...
		if (this.enabled) {
			this.isQuickDiffVisible =
				isQuickDiffVisible.bindTo(contextKeyService);
	...
	next(lineNumber?: number): void {
		...
		if (
			...
			const providerChanges: number[] =
				this.model.quickDiffChanges.get(this.widget.provider) ??
