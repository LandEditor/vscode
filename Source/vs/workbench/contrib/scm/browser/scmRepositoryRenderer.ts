		...
		IListRenderer<ISCMRepository, RepositoryTemplate>
{
	...
	renderTemplate(container: HTMLElement): RepositoryTemplate {
		...
		const toolBar = new ToolBar(actions, this.contextMenuService, {
	...
	renderElement(
		...
		if (repository.provider.rootUri) {
			...
			if (
				folder?.uri.toString() ===
				repository.provider.rootUri.toString()
			) {
				templateData.name.textContent = folder.name;
			} else {
				templateData.name.textContent = basename(
					repository.provider.rootUri
				);
			}
		...
		templateData.elementDisposables.add(
			connectPrimaryMenu(menus.titleMenu.menu, (primary, secondary) => {
