				...
				this._visibilityDisposables.add(
					autorunWithStore((reader, store) => {
						...
						const historyItemRefId = derived((reader) => {
							return historyProvider.historyItemRef.read(reader)
								?.id;
						});
						store.add(
							runOnChange(
	...
	private _onContextMenu(e: ITreeContextMenuEvent<TreeElement | null>): void {
		...
		for (const ref of element.historyItemViewModel.historyItem.references ??
			...
			const contextKeyService =
				this.scopedContextKeyService.createOverlay([
					["scmHistoryItemRef", ref.id],
				]);

			const historyItemRefMenuActions = this._menuService.getMenuActions(
				MenuId.SCMHistoryItemRefContext,
				contextKeyService,
			);
			const historyItemRefSubMenuActions = getFlatContextMenuActions(
				historyItemRefMenuActions,
			).map(
				(action) =>
					new (class extends MenuItemAction {
						constructor() {
							super(
								...
								undefined,
								contextKeyService,
								that._commandService,
							);
						}

						override run(): Promise<void> {
							return super.run(
								element.historyItemViewModel.historyItem,
								ref.id,
							);
						}
					})(),
			...
			if (historyItemRefSubMenuActions.length > 0) {
				actions.push(
					new SubmenuAction(
						`scm.historyItemRef.${ref.id}`,
						ref.name,
						historyItemRefSubMenuActions,
					),
				);
			}
		}
