...
class SCMHistoryViewModel extends Disposable {
	...
	readonly historyItemsFilter = observableValue<HistoryItemRefsFilter>(
		this,
		"auto",
	);
			...
			this._visibilityDisposables.add(
				autorunWithStore((reader, store) => {
					...
					const historyItemRemoteRefRevision = derived((reader) => {
						return historyProvider.historyItemRemoteRef.read(reader)
							?.revision;
					});
					...
					store.add(
						autorunWithStoreHandleChanges<{
							...
							(reader, changeSummary) => {
								...
								const historyItemRefValue =
									historyProvider.historyItemRef.read(reader);
								const historyItemRemoteRefRevisionValue =
									historyItemRemoteRefRevision.read(reader);
								...
								if (changeSummary.refresh === "ifScrollTop") {
									...
									if (
										historyItemRefValue?.revision ===
										historyItemRemoteRefRevisionValue
									) {
										this.refresh();
										return;
									}
