...
function toDiffEditorArguments(
	...
	const modifiedQuery = JSON.parse(modifiedUri.query) as {
		path: string;
		ref: string;
	};

	const originalShortRef = originalQuery.ref
		.substring(0, 8)
		.concat(originalQuery.ref.endsWith("^") ? "^" : "");
	const modifiedShortRef = modifiedQuery.ref
		.substring(0, 8)
		.concat(modifiedQuery.ref.endsWith("^") ? "^" : "");

	return [
		originalUri,
		modifiedUri,
		`${basename} (${originalShortRef}) ↔ ${basename} (${modifiedShortRef})`,
		null,
	];
}
...
class HistoryItemRenderer
	implements
		ITreeRenderer<SCMHistoryItemTreeElement, void, HistoryItemTemplate>
{
		...
		>
{
	...
	renderElement(
		...
		const fileKind =
			...
			historyItemChangeOrFolder.childrenCount > 0
				? FileKind.FOLDER
				: FileKind.FILE;
...
class SCMSyncViewPaneTreeSorter implements ITreeSorter<TreeElement> {
	compare(element: TreeElement, otherElement: TreeElement): number {
		...
		// History item change
		const elementPath = (element as SCMHistoryItemChangeTreeElement).uri
			.fsPath;
		const otherElementPath = (
			otherElement as SCMHistoryItemChangeTreeElement
		).uri.fsPath;
...
export class SCMSyncViewPane extends ViewPane {
	...
	get viewModel(): SCMSyncPaneViewModel {
	...
	private async onDidOpen(
		...
		if (isSCMHistoryItemChangeTreeElement(e.element)) {
			...
			ResourceTree.isResourceNode(e.element) &&
			e.element.childrenCount === 0
		) {
			if (
...
class SCMSyncPaneViewModel {
	...
	constructor(
		private readonly tree: WorkbenchCompressibleAsyncDataTree<
			TreeElement,
			TreeElement
		>,
		@IContextKeyService contextKeyService: IContextKeyService,
		...
		this._onDidChangeVisibleRepositories({
			added: scmViewService.visibleRepositories,
			removed: [],
		});
	...
	async refresh(repository?: ISCMRepository): Promise<void> {
		...
		if (repository) {
			...
			await this.tree.setInput([...this.repositories.keys()], {
				expanded,
			});
		}
	}
