...
import { type IMenu, MenuItemAction } from "vs/platform/actions/common/actions";
...
export function isSCMHistoryItemTreeElement(
	...
	return (
		(element as SCMHistoryItemTreeElement).type === "allChanges" ||
...
> {
	return (
		ResourceTree.isResourceNode(element) &&
		isSCMHistoryItemTreeElement(element.context)
	);
}
...
export function getProviderKey(provider: ISCMProvider): string {
	return `${provider.contextValue}:${provider.label}${
		provider.rootUri ? `:${provider.rootUri.toString()}` : ""
	}`;
}
