...
export class SCMRepositoryMenus implements ISCMRepositoryMenus, IDisposable {
	...
	get repositoryMenu(): IMenu {
		if (!this._repositoryMenu) {
			this._repositoryMenu = this.menuService.createMenu(
	...
	constructor(
		...
		provider.groups.onDidSplice(
			this.onDidSpliceGroups,
			this,
			this.disposables
		);
		this.onDidSpliceGroups({
			start: 0,
			deleteCount: 0,
			toInsert: provider.groups.elements,
		});
	}
	...
	private onDidSpliceGroups({
		...
		deleteCount,
		toInsert,
	}: ISplice<ISCMResourceGroup>): void {
		const deleted = this.resourceGroups.splice(
			start,
			deleteCount,
			...toInsert
		);
