...
import type {
	IUpdatableHover,
	IUpdatableHoverTooltipMarkdownString,
...
} from "vs/base/browser/ui/list/listView";
import type { IListAccessibilityProvider } from "vs/base/browser/ui/list/listWidget";
import type { LabelFuzzyScore } from "vs/base/browser/ui/tree/abstractTree";
import type {
...
import { type IResourceNode, ResourceTree } from "vs/base/common/resourceTree";
...
import {
	...
	isSCMHistoryItemChangeTreeElement,
	isSCMHistoryItemGroupTreeElement,
	isSCMHistoryItemTreeElement,
	isSCMInput,
	isSCMRepository,
	isSCMResource,
	isSCMResourceGroup,
	...
	private renderIcon(
		...
		if (icon) {
			if (ThemeIcon.isThemeIcon(icon)) {
				template.decorationIcon.className = `decoration-icon ${ThemeIcon.asClassName(
					icon,
				)}`;
				if (icon.color) {
...
class HistoryItemGroupActionRunner extends ActionRunner {
	protected override runAction(
		action: IAction,
		context: SCMHistoryItemGroupTreeElement,
	): Promise<void> {
		if (!(action instanceof MenuItemAction)) {
			return super.runAction(action, context);
		}
		...
		>
{
	...
	renderElement(
		...
		if (
			historyItemGroup.icon &&
			ThemeIcon.isThemeIcon(historyItemGroup.icon)
		) {
			templateData.iconContainer.classList.add(
				...ThemeIcon.asClassNameArray(historyItemGroup.icon),
			);
		}
		...
		templateData.label.setLabel(
			...
			{ title: historyItemGroup.ariaLabel },
		);
		templateData.count.setCount(historyItemGroup.count ?? 0);

		const repositoryMenus = this.scmViewService.menus.getRepositoryMenus(
			historyItemGroup.repository.provider,
		);
		const historyProviderMenu = repositoryMenus.historyProviderMenu;

		if (historyProviderMenu) {
			const menu =
				historyProviderMenu.getHistoryItemGroupMenu(historyItemGroup);
			const resetMenuId =
				historyItemGroup.direction === "incoming"
					? MenuId.SCMIncomingChanges
					: MenuId.SCMOutgoingChanges;
	...
	renderCompressedElements(
		node: ITreeNode<
			ICompressedTreeNode<SCMHistoryItemGroupTreeElement>,
			void
		>,
		index: number,
		templateData: HistoryItemGroupTemplate,
...
class HistoryItemActionRunner extends ActionRunner {
	protected override async runAction(
		action: IAction,
		context: SCMHistoryItemTreeElement,
	): Promise<any> {
		if (!(action instanceof MenuItemAction)) {
			return super.runAction(action, context);
		}
		...
		>
{
	...
	renderTemplate(container: HTMLElement): HistoryItemTemplate {
		...
		const statsCustomHover = this.hoverService.setupUpdatableHover(
	...
	renderElement(
		node: ITreeNode<SCMHistoryItemTreeElement, LabelFuzzyScore>,
		index: number,
		...
		templateData.label.setLabel(historyItem.message, historyItem.author, {
			matches,
			descriptionMatches,
			title,
		});
		...
		const menus = this.scmViewService.menus.getRepositoryMenus(
			historyItem.historyItemGroup.repository.provider,
		);
		if (menus.historyProviderMenu) {
			const historyItemMenu =
				menus.historyProviderMenu.getHistoryItemMenu(historyItem);
			templateData.elementDisposables.add(
				connectPrimaryMenuToInlineActionBar(
					historyItemMenu,
					templateData.actionBar,
				),
			);
		}
	...
	renderCompressedElements(
		node: ITreeNode<
			ICompressedTreeNode<SCMHistoryItemTreeElement>,
			LabelFuzzyScore
		>,
		index: number,
		templateData: HistoryItemTemplate,
	...
	private getTooltip(
		historyItem: SCMHistoryItemTreeElement,
	): IUpdatableHoverTooltipMarkdownString {
		...
		if (historyItem.timestamp) {
			...
			markdown.appendMarkdown(
				`$(history) ${dateFormatter.format(historyItem.timestamp)}\n\n`,
			);
		}
	...
	private processMatches(
		...
		return [
			historyItem.message === filterData.label
				? createMatches(filterData.score)
				: undefined,
			historyItem.author === filterData.label
				? createMatches(filterData.score)
				: undefined,
		];
	}
	...
	private renderStatistics(
		...
		if (historyItem.statistics) {
			...
			templateData.filesLabel.textContent =
				historyItem.statistics.files.toString();

			templateData.insertionsLabel.textContent =
				historyItem.statistics.insertions > 0
					? `+${historyItem.statistics.insertions}`
					: "";
			templateData.insertionsLabel.classList.toggle(
				"hidden",
				historyItem.statistics.insertions === 0,
			);

			templateData.deletionsLabel.textContent =
				historyItem.statistics.deletions > 0
					? `-${historyItem.statistics.deletions}`
					: "";
			templateData.deletionsLabel.classList.toggle(
				"hidden",
				historyItem.statistics.deletions === 0,
			);
		}

		templateData.statsContainer.classList.toggle(
			"hidden",
			historyItem.statistics === undefined,
		);
	}

	disposeElement(
		element: ITreeNode<SCMHistoryItemTreeElement, LabelFuzzyScore>,
		index: number,
		templateData: HistoryItemTemplate,
		height: number | undefined,
	): void {
		...
		>
{
	...
	renderElement(
		...
		const uri = ResourceTree.isResourceNode(historyItemChangeOrFolder)
			...
			: historyItemChangeOrFolder.uri;
		const fileKind = ResourceTree.isResourceNode(historyItemChangeOrFolder)
			? FileKind.FOLDER
			: FileKind.FILE;
		const hidePath = this.viewMode() === ViewMode.Tree;
	...
	renderCompressedElements(
		...
		templateData.fileLabel.setResource(
			{ resource: folder.uri, name: label },
			{
				fileDecorations: { colors: false, badges: true },
		...
		>
{
	...
	renderElement(
		element: ITreeNode<SCMViewSeparatorElement, void>,
		index: number,
		templateData: SeparatorTemplate,
		height: number | undefined,
	): void {
		templateData.label.setLabel(element.element.label, undefined, {
			title: element.element.ariaLabel,
		});
	}

	renderCompressedElements(
		node: ITreeNode<ICompressedTreeNode<SCMViewSeparatorElement>, void>,
		index: number,
		templateData: SeparatorTemplate,
		height: number | undefined,
	): void {
...
class ListDelegate implements IListVirtualDelegate<TreeElement> {
	...
	getTemplateId(element: TreeElement) {
		if (isSCMRepository(element)) {
			...
			isSCMHistoryItemChangeTreeElement(element) ||
...
export class SCMTreeSorter implements ITreeSorter<TreeElement> {
	...
	compare(one: TreeElement, other: TreeElement): number {
		...
		if (
			isSCMHistoryItemChangeTreeElement(one) ||
			isSCMHistoryItemChangeNode(one)
		) {
			// List
			...
			const oneName = isSCMHistoryItemChangeNode(one)
				? one.name
				: basename(one.uri);
			const otherName = isSCMHistoryItemChangeNode(other)
				? other.name
				: basename(other.uri);
	...
	implements ICompressibleKeyboardNavigationLabelProvider<TreeElement>
{
	...
	getKeyboardNavigationLabel(
		...
		if (ResourceTree.isResourceNode(element)) {
			...
			const uri = isSCMResource(element)
				? element.sourceUri
				: element.uri;
			return [
				basename(uri),
			...
			return basename(
				isSCMResource(element) ? element.sourceUri : element.uri,
			);
		}
	}
...
function getSCMResourceId(element: TreeElement): string {
	if (isSCMRepository(element)) {
		...
		return `resource:${provider.id}/${
			group.id
		}/${element.sourceUri.toString()}`;
	} else if (isSCMResourceNode(element)) {
		const group = element.context;
		return `folder:${group.provider.id}/${
			group.id
		...
		return `historyItem:${provider.id}/${historyItemGroup.id}/${
			element.id
		}/${element.parentIds.join(",")}`;
		...
		const provider = historyItemGroup.repository.provider;
		return `historyItemChange:${provider.id}/${historyItemGroup.id}/${
			historyItem.id
		...
		const historyItemGroup = historyItem.historyItemGroup;
		const provider = historyItemGroup.repository.provider;
		return `folder:${provider.id}/${historyItemGroup.id}/${
			historyItem.id
		}/$FOLDER/${element.uri.toString()}`;
	} else if (isSCMViewSeparator(element)) {
		const provider = element.repository.provider;
	...
	implements IListAccessibilityProvider<TreeElement>
{
	...
	getAriaLabel(element: TreeElement): string {
		if (ResourceTree.isResourceNode(element)) {
			...
			return `${stripIcons(element.message).trim()}${
				element.author ? `, ${element.author}` : ""
			}`;
		} else if (isSCMHistoryItemChangeTreeElement(element)) {
			const result = [basename(element.uri)];
			const path = this.labelService.getUriLabel(dirname(element.uri), {
				relative: true,
				noPrefix: true,
			});
...
registerAction2(
	class extends Action2 {
		...
		override run(accessor: ServicesAccessor) {
			const configurationService = accessor.get(IConfigurationService);
			const configValue =
				configurationService.getValue("scm.showChangesSummary") ===
				true;
...
class SCMInputWidgetToolbar extends WorkbenchToolBar {
	...
	public setInput(input: ISCMInput): void {
		...
		const menu = this.repositoryDisposables.add(
...
class SCMInputWidget {
	...
	set input(input: ISCMInput | undefined) {
		...
		const updatePlaceholderVisibility = () =>
			this.placeholderTextContainer.classList.toggle(
				"hidden",
				textModel.getValueLength() > 0,
			);
		this.repositoryDisposables.add(
		...
		const updateTemplate = () => {
			if (
				typeof input.repository.provider.commitTemplate ===
					"undefined" ||
				!input.visible
			) {
				return;
		...
		this.repositoryDisposables.add(
			input.repository.provider.onDidChangeCommitTemplate(
				updateTemplate,
				this,
			),
		);
...
export class SCMViewPane extends ViewPane {
	...
	private createTree(
		...
		historyItemGroupActionRunner.onWillRun(
			() => this.tree.domFocus(),
			this,
			this.disposables,
		);
		...
		historyItemActionRunner.onWillRun(
			() => this.tree.domFocus(),
			this,
			this.disposables,
		);
		...
		this.tree = this.instantiationService.createInstance(
			...
			[
				...
				this.instantiationService.createInstance(
					...
					historyItemGroupActionRunner,
				),
				this.instantiationService.createInstance(
					HistoryItemRenderer,
					historyItemActionRunner,
					getActionViewItemProvider(this.instantiationService),
				),
				this.instantiationService.createInstance(
	...
	private async open(e: IOpenEvent<TreeElement | undefined>): Promise<void> {
		if (!e.element) {
			...
			this.scmViewService.focus(
				e.element.historyItem.historyItemGroup.repository,
			);
			return;
		} else if (isSCMHistoryItemChangeNode(e.element)) {
			this.scmViewService.focus(
				e.element.context.historyItemGroup.repository,
			);
			return;
		}
	}
					...
					() => {
						if (!repository.provider.historyProvider) {
							this.logService.debug(
								"SCMViewPane:onDidChangeVisibleRepositories - no history provider present",
							);
							return;
						}
						...
						repositoryDisposables.add(
							repository.provider.historyProvider.onDidChangeCurrentHistoryItemGroup(
								() => {
									this.updateChildren(repository);
									this.logService.debug(
										"SCMViewPane:onDidChangeCurrentHistoryItemGroup - update children",
									);
								},
							),
						);
						...
						this.logService.debug(
							"SCMViewPane:onDidChangeVisibleRepositories - onDidChangeCurrentHistoryItemGroup listener added",
						);
					},
				),
			);
	...
	private onListContextMenu(
		...
		if (isSCMRepository(element)) {
			...
			const menus = this.scmViewService.menus.getRepositoryMenus(
				element.repository.provider,
			);
			const menu =
				menus.historyProviderMenu?.getHistoryItemGroupContextMenu(
					element,
				);
			...
			const menus = this.scmViewService.menus.getRepositoryMenus(
				element.historyItemGroup.repository.provider,
			);
...
{
	private readonly historyProviderCache = new Map<
		ISCMRepository,
		ISCMHistoryProviderCacheEntry
	>();
	private readonly repositoryDisposables = new DisposableMap<
		ISCMRepository,
		IDisposable
	>();
	private readonly disposables = new DisposableStore();
	...
	async getChildren(
		...
		const { alwaysShowRepositories, showActionButton } =
		...
		if (
			...
			// History item groups
			const historyItemGroups =
				await this.getHistoryItemGroups(inputOrElement);
			...
			if (historyItemGroups.length > 0) {
				...
				let ariaLabel = localize(
					"syncSeparatorHeaderAriaLabel",
					"Incoming and outgoing changes",
				);

				const incomingHistoryItems = historyItemGroups.find(
					(g) => g.direction === "incoming",
				);
				const outgoingHistoryItems = historyItemGroups.find(
					(g) => g.direction === "outgoing",
				);
			...
			isSCMResourceNode(inputOrElement) ||
	...
	private async getHistoryItemGroups(
		...
		const { showIncomingChanges, showOutgoingChanges } =
			this.getConfiguration();
		...
		if (
			...
			(showIncomingChanges === "never" && showOutgoingChanges === "never")
		) {
			return [];
		}

		const children: SCMHistoryItemGroupTreeElement[] = [];
		const historyProviderCacheEntry =
			this.getHistoryProviderCacheEntry(element);

		let incomingHistoryItemGroup =
			historyProviderCacheEntry?.incomingHistoryItemGroup;
		let outgoingHistoryItemGroup =
			historyProviderCacheEntry?.outgoingHistoryItemGroup;
		...
		if (
			...
			(showIncomingChanges === "always" ||
				(showIncomingChanges === "auto" &&
					(incomingHistoryItemGroup.count ?? 0) > 0))
		) {
		...
		if (
			...
			(showOutgoingChanges === "always" ||
				(showOutgoingChanges === "auto" &&
					(outgoingHistoryItemGroup.count ?? 0) > 0))
		) {
	...
	private async getHistoryItems(
		...
		const historyProviderCacheEntry =
			this.getHistoryProviderCacheEntry(repository);
		const historyItemsMap = historyProviderCacheEntry.historyItems;
		let historyItemsElement = historyProviderCacheEntry.historyItems.get(
			element.id,
		);

		if (!historyItemsElement) {
			const historyItems =
				(await historyProvider.provideHistoryItems(element.id, {
					limit: { id: element.ancestor },
				})) ?? [];

			// All Changes
			const { showChangesSummary } = this.getConfiguration();
			const allChanges =
				showChangesSummary && historyItems.length >= 2
					? await historyProvider.provideHistoryItemSummary(
							historyItems[0].id,
							element.ancestor,
						)
					: undefined;
			...
			this.historyProviderCache.set(repository, {
				...historyProviderCacheEntry,
				historyItems: historyItemsMap.set(
					element.id,
					historyItemsElement,
				),
			});
	...
	> {
		...
		const historyProviderCacheEntry =
			this.getHistoryProviderCacheEntry(repository);
		const historyItemChangesMap =
			historyProviderCacheEntry.historyItemChanges;

		const historyItemParentId =
			element.parentIds.length > 0 ? element.parentIds[0] : undefined;
		let historyItemChanges = historyItemChangesMap.get(
			`${element.id}/${historyItemParentId}`,
		);
		...
		if (!historyItemChanges) {
			...
			this.historyProviderCache.set(repository, {
				...historyProviderCacheEntry,
				historyItemChanges: historyItemChangesMap.set(
					`${element.id}/${historyItemParentId}`,
					historyItemChanges,
				),
			});
	...
	private getConfiguration(): {
		...
		return {
			alwaysShowRepositories: this.configurationService.getValue<boolean>(
				"scm.alwaysShowRepositories",
			),
			showActionButton: this.configurationService.getValue<boolean>(
				"scm.showActionButton",
			),
			showChangesSummary: this.configurationService.getValue<boolean>(
				"scm.showChangesSummary",
			),
			showIncomingChanges:
				this.configurationService.getValue<ShowChangesSetting>(
					"scm.showIncomingChanges",
				),
			showOutgoingChanges:
				this.configurationService.getValue<ShowChangesSetting>(
			...
			repositoryDisposables.add(
				Event.runAndSubscribe(
					...
					() => {
						if (!repository.provider.historyProvider) {
							this.logService.debug(
								"SCMTreeDataSource:onDidChangeVisibleRepositories - no history provider present",
							);
							return;
						}
						...
						repositoryDisposables.add(
							repository.provider.historyProvider.onDidChangeCurrentHistoryItemGroup(
								() => {
									this.historyProviderCache.delete(
										repository,
									);
									this.logService.debug(
										"SCMTreeDataSource:onDidChangeCurrentHistoryItemGroup - cache cleared",
									);
								},
							),
						);
						...
						this.logService.debug(
							"SCMTreeDataSource:onDidChangeVisibleRepositories - onDidChangeCurrentHistoryItemGroup listener added",
						);
					},
				),
			);
	...
	private getHistoryProviderCacheEntry(
		repository: ISCMRepository,
	): ISCMHistoryProviderCacheEntry {
		return (
			this.historyProviderCache.get(repository) ?? {
				incomingHistoryItemGroup: undefined,
				outgoingHistoryItemGroup: undefined,
				historyItems: new Map<
					string,
					[ISCMHistoryItem | undefined, ISCMHistoryItem[]]
				>(),
