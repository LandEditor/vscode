...
const CTX_QUICK_CHAT_IN_PROGRESS = new RawContextKey<boolean>(
	"inlineChat.quickChatInProgress",
	false,
);
	...
	constructor() {
		super({
			...
			precondition: ContextKeyExpr.and(
				HasSpeechProvider,
				CTX_QUICK_CHAT_IN_PROGRESS.toNegated(),
...
class QuickVoiceWidget implements IContentWidget {
	...
	beforeRender(): IDimension | null {
		...
		this._elements.main.style.setProperty(
...
export class InlineChatQuickVoice implements IEditorContribution {
	...
	async start() {
		...
		const session = await this._voiceChatService.createVoiceChatSession(
			cts.token,
			{ usesAgents: false },
		);
		const listener = session.onDidChange((e) => {
			...
			switch (e.status) {
				...
				case SpeechToTextStatus.Recognized:
					message = message ? `${message} ${e.text}` : e.text;
					preview = "";
					this._widget.updateInput({ message, preview });
					break;
			}
