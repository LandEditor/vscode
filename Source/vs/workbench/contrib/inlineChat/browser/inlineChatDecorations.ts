...
const GUTTER_INLINE_CHAT_ICON = registerIcon(
	"inline-chat",
	Codicon.sparkle,
	localize(
		"startInlineChatIcon",
		"Icon which spawns the inline chat from the gutter"
	)
);
	...
	implements IEditorContribution
{
	...
	constructor(
		...
		super();
		const keybindings = _keybindingService
			.lookupKeybinding("inlineChat.start")
			?.getLabel();
	...
	private _onEnablementOrModelChanged(): void {
		...
		const onInlineChatSessionChanged = (e: ICodeEditor) =>
			e === editor && decorationUpdateScheduler.schedule();
		this._localToDispose.add(
			this._inlineChatSessionService.onWillStartSession(
				onInlineChatSessionChanged
			)
		);
		this._localToDispose.add(
			...
			this._editor.onMouseDown(async (e: IEditorMouseEvent) => {
				if (
					!e.target.element?.classList.contains(
						InlineChatDecorationsContribution.GUTTER_ICON_CLASSNAME
					)
				) {
					return;
				}
	...
	private _onSelectionOrContentChanged(editor: IActiveCodeEditor): void {
		...
		const isInlineChatVisible = this._inlineChatSessionService.getSession(
			editor,
			editor.getModel().uri
		);
		const isEnabled =
			selection.isEmpty() &&
			/^\s*$/g.test(
				editor.getModel().getLineContent(selection.startLineNumber)
			) &&
			!isInlineChatVisible;
		if (isEnabled) {
			if (this._gutterDecorationID === undefined) {
				...
				const decorationRange = editor
					.getModel()
					.getDecorationRange(this._gutterDecorationID);
				if (
