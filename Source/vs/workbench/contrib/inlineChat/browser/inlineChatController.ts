...
export abstract class InlineChatRunOptions {
	...
	static isInteractiveEditorOptions(
		...
		const { initialSelection, initialRange, message, autoSend, position } =
			options;
		if (
...
export class InlineChatController implements IEditorContribution {
	...
	private readonly _ctxLastResponseType: IContextKey<
		undefined | InlineChatResponseType
	>;
	private readonly _ctxResponseTypes: IContextKey<
		undefined | InlineChateResponseTypes
	>;
	private readonly _ctxDidEdit: IContextKey<boolean>;
	private readonly _ctxUserDidEdit: IContextKey<boolean>;
	private readonly _ctxLastFeedbackKind: IContextKey<
		"helpful" | "unhelpful" | ""
	>;
	...
	readonly onDidCancelInput = Event.filter(
		this._messages.event,
		(m) => m === Message.CANCEL_INPUT || m === Message.CANCEL_SESSION,
		this._store
	);

	private readonly _sessionStore: DisposableStore = this._store.add(
		new DisposableStore()
	);
	private readonly _stashedSession: MutableDisposable<StashedSession> =
	...
	constructor(
		...
		this._ctxResponseTypes =
			CTX_INLINE_CHAT_RESPONSE_TYPES.bindTo(contextKeyService);
		this._ctxLastResponseType =
			CTX_INLINE_CHAT_LAST_RESPONSE_TYPE.bindTo(contextKeyService);
		this._ctxLastFeedbackKind =
			CTX_INLINE_CHAT_LAST_FEEDBACK.bindTo(contextKeyService);
		this._ctxSupportIssueReporting =
			CTX_INLINE_CHAT_SUPPORT_ISSUE_REPORTING.bindTo(contextKeyService);
		this._zone = new Lazy(() =>
			this._store.add(
				_instaService.createInstance(InlineChatZoneWidget, this._editor)
			)
		);
	...
	private _getMode(): EditMode {
		const editMode =
			this._configurationService.inspect<EditMode>("inlineChat.mode");
		let editModeValue = editMode.value;
		if (
			this._accessibilityService.isScreenReaderOptimized() &&
			editModeValue === editMode.defaultValue
		) {
	...
	async run(options: InlineChatRunOptions | undefined = {}): Promise<void> {
		try {
			...
			if (this._activeSession) {
				this._inlineChatSessionService.releaseSession(
					this._activeSession
				);
			}
			this[State.PAUSE]();
	...
	private _showWidget(initialRender: boolean = false, position?: Position) {
		...
		if (
			this._activeSession &&
			(this._activeSession.hasChangedText ||
				this._activeSession.lastExchange)
		) {
			widgetPosition = this._activeSession.wholeRange.value
				.getStartPosition()
				.delta(-1);
		}
		if (this._activeSession) {
			this._zone.value.updateBackgroundColor(
				widgetPosition,
				this._activeSession.wholeRange.value
			);
		}
		if (this._strategy) {
	...
	private async [State.CREATE_SESSION](
		options: InlineChatRunOptions
	): Promise<State.CANCEL | State.INIT_UI | State.PAUSE> {
		...
		if (!session) {
			this._dialogService.info(
				localize("create.fail", "Failed to start editor chat"),
				localize(
					"create.fail.detail",
	...
	private async [State.INIT_UI](
		...
		State.WAIT_FOR_INPUT | State.SHOW_RESPONSE | State.APPLY_RESPONSE
	> {
		...
		this._sessionStore.add(
			toDisposable(() => wholeRangeDecoration.clear())
		);
		this._sessionStore.add(
			this._activeSession.wholeRange.onDidChange(
				updateWholeRangeDecoration
			)
		);
		updateWholeRangeDecoration();

		this._zone.value.widget.updateSlashCommands(
			this._activeSession.session.slashCommands ?? []
		);
		this._updatePlaceholder();
		this._zone.value.widget.updateInfo(
		...
		this._zone.value.widget.preferredExpansionState =
			this._activeSession.lastExpansionState;
		this._zone.value.widget.value =
			this._activeSession.session.input ??
			this._activeSession.lastInput?.value ??
			this._zone.value.widget.value;
		...
		// Update context key
		this._ctxSupportIssueReporting.set(
			this._activeSession.provider.supportIssueReporting ?? false
		);
	...
	private _getPlaceholderText(): string {
		let result =
			this._forcedPlaceholder ??
			this._activeSession?.session.placeholder ??
			localize("default.placeholder", "Ask a question");
		if (
			typeof this._forcedPlaceholder === "undefined" &&
			InlineChatController._promptHistory.length > 0
		) {
			const kb1 = this._keybindingService
				.lookupKeybinding("inlineChat.previousFromHistory")
				?.getLabel();
			const kb2 = this._keybindingService
				.lookupKeybinding("inlineChat.nextFromHistory")
				?.getLabel();
		...
		if (message & Message.RERUN_INPUT && this._activeSession.lastExchange) {
			...
			if (lastExchange.response instanceof ReplyResponse) {
				try {
					...
					await this._strategy.undoChanges(
						lastExchange.response.modelAltVersionId
					);
				} finally {
		...
		if (refer) {
			...
			if (input.startsWith(chatSubcommandLeader)) {
				...
				for (const agent of this._chatAgentService.getAgents()) {
					const commands = await agent.provideSlashCommands(
						cts.token
					);
					if (
						commands.find((command) =>
							withoutSubCommandLeader.startsWith(command.name)
						)
					) {
	...
	private async [State.MAKE_REQUEST](): Promise<
		State.APPLY_RESPONSE | State.PAUSE | State.CANCEL | State.ACCEPT
	> {
		assertType(this._editor.hasModel());
		assertType(this._activeSession);
		assertType(this._strategy);
		...
		const progress = new Progress<IInlineChatProgressItem>((data) => {
			...
			if (data.slashCommand) {
				...
				if (!valueNow.startsWith("/")) {
					this._zone.value.widget.updateSlashCommandUsed(
						data.slashCommand
					);
				}
			}
		...
		try {
			...
			if (!reply) {
				...
				for (
					...
					i < replyResponse.allLocalEdits.length;
					i++
				) {
					await this._makeChanges(
				...
				const a11yMessageResponse = renderMarkdownAsPlaintext(
					replyResponse.mdContent
				);
	...
	private async [State.APPLY_RESPONSE](): Promise<
		State.SHOW_RESPONSE | State.CANCEL
	> {
		assertType(this._activeSession);
		assertType(this._strategy);
		...
		if (response instanceof ReplyResponse && response.workspaceEdit) {
			...
			this._bulkEditService.apply(response.workspaceEdit, {
				showPreview: true,
			});
			return State.CANCEL;
		}
	...
	private async [State.SHOW_RESPONSE](): Promise<
		State.WAIT_FOR_INPUT | State.CANCEL
	> {
		assertType(this._activeSession);
		assertType(this._strategy);
	...
	private async [State.CANCEL]() {
		...
		if (!mySession.isUnstashed && mySession.lastExchange) {
			...
			this._stashedSession.value = this._instaService.createInstance(
				StashedSession,
				this._editor,
				mySession
			);
	...
	viewInChat() {
		if (
			this._activeSession?.lastExchange?.response instanceof ReplyResponse
		) {
			this._instaService.invokeFunction(
				showMessageResponse,
				this._activeSession.lastExchange.prompt.value,
				this._activeSession.lastExchange.response.mdContent.value
			);
		}
	}
	...
	updateExpansionState(expand: boolean) {
		if (this._activeSession) {
			const expansionState = expand
				? ExpansionState.EXPANDED
				: ExpansionState.CROPPED;
			this._zone.value.widget.updateMarkdownMessageExpansionState(
				expansionState
			);
	...
	createSnapshot(): void {
		if (
			this._activeSession &&
			!this._activeSession.textModel0.equalsTextBuffer(
				this._activeSession.textModelN.getTextBuffer()
			)
		) {
	...
	acceptSession(): void {
		if (
			...
			this._activeSession.provider.handleInlineChatResponseFeedback?.(
				this._activeSession.session,
				this._activeSession.lastExchange.response.raw,
				InlineChatResponseFeedbackKind.Accepted
			);
		}
	...
	async cancelSession() {
		...
		if (this._activeSession) {
			const diff = await this._editorWorkerService.computeDiff(
				this._activeSession.textModel0.uri,
				this._activeSession.textModelN.uri,
				{
			...
			if (
				this._activeSession.lastExchange &&
				this._activeSession.lastExchange.response instanceof
					ReplyResponse
			) {
				this._activeSession.provider.handleInlineChatResponseFeedback?.(
					this._activeSession.session,
					this._activeSession.lastExchange.response.raw,
					InlineChatResponseFeedbackKind.Undone
				);
			}
		}
...
async function showMessageResponse(
