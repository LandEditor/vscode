...
export class InlineChatController implements IEditorContribution {
	...
	> {
		...
		const progress = new AsyncProgress<IInlineChatProgressItem>(
			async (data) => {
				...
				if (data.markdownFragment) {
					...
					this._zone.value.widget.updateMarkdownMessage(
						markdownContents
					);
				}
			}
		...
		try {
			...
			if (!reply) {
				...
				const replyResponse = (response = new ReplyResponse(
					reply,
					markdownContents,
				...
				a11yResponse =
					this._strategy.checkChanges(replyResponse) &&
					a11yVerboseInlineChat
						? a11yMessageResponse
							? localize(
	...
	feedbackLast(helpful: boolean) {
		if (
			...
			const kind = helpful
				? InlineChatResponseFeedbackKind.Helpful
				: InlineChatResponseFeedbackKind.Unhelpful;
