...
export class InlineChatController implements IEditorContribution {
	...
	> {
		...
		if (
			...
			!response.result.errorDetails.responseIsFiltered
		) {
	...
	private _showWidget(
		...
		if (
			initialRender &&
			this._editor.getOption(EditorOption.stickyScroll).enabled
		) {
	...
	private _updatePlaceholder(): void {
		this._ui.value.widget.placeholder =
			this._session?.agent.description ?? "";
	}
