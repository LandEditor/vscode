...
import { type ISelection, Selection } from "vs/editor/common/core/selection";
import type {
	IEditorContribution,
...
import type { StashedSession } from "./inlineChatSession";
...
export class InlineChatController implements IEditorContribution {
	...
	private async [State.INIT_UI](
		options: InlineChatRunOptions,
	): Promise<State.WAIT_FOR_INPUT | State.SHOW_RESPONSE> {
		...
		this._sessionStore.add(
			this._session.wholeRange.onDidChange(updateWholeRangeDecoration),
		);
		updateWholeRangeDecoration();

		this._sessionStore.add(
			this._input.value.onDidBlur(() => this.cancelSession()),
		);
			...
			this._session.chatModel.onDidChange(async (e) => {
				if (e.kind === "addRequest" && e.request.response) {
					...
					const listener = e.request.response.onDidChange(() => {
						if (
							e.request.response?.isCanceled ||
							e.request.response?.isComplete
						) {
					...
					if (
						this._session!.lastExchange?.response instanceof
						ReplyResponse
					) {
						try {
							this._session!.hunkData.ignoreTextModelNChanges =
		...
		this._ctxSupportIssueReporting.set(
			this._session.provider.supportIssueReporting ?? false,
		);
		...
		this._sessionStore.add(
			this._languageFeatureService.completionProvider.register(
				{
				...
				},
				{
					...
					provideCompletionItems: (
						...
						if (
							!this._session ||
							!this._session.session.slashCommands
						) {
							return undefined;
						}
						const widget =
						...
						if (
							widget !== this._zone.value.widget.chatWidget &&
							widget !== this._input.value.chatWidget
						) {
							return undefined;
						}
						...
						for (const command of this._session.session
							.slashCommands) {
		...
		this._sessionStore.add(
			inputInputEditor.onDidChangeModelContent(() =>
				updateSlashDecorations(
					inputDecorations,
					inputInputEditor.getModel()!,
				),
			),
		);
		this._sessionStore.add(
			zoneInputEditor.onDidChangeModelContent(() =>
				updateSlashDecorations(
					zoneDecorations,
					zoneInputEditor.getModel()!,
				),
			),
		);
		this._sessionStore.add(
	...
	> {
		...
		let slashCommandLike = request.message.parts.find(
			(part) =>
		...
		if (refer && slashCommandLike && !this._session.lastExchange) {
			this._log(
				...
				this._session.provider.extensionId,
			);
	...
	private async [State.SHOW_REQUEST](): Promise<
		State.SHOW_RESPONSE | State.CANCEL | State.PAUSE | State.ACCEPT
	> {
		assertType(this._session);
		assertType(this._session.chatModel.requestInProgress);

		const request: IChatRequestModel | undefined = tail(
			this._session.chatModel.getRequests(),
		);
		...
		store.add(
			this._zone.value.widget.chatWidget.inputEditor.onDidChangeModelContent(
				() => {
					this._chatService.cancelCurrentRequestForSession(
						request.session.sessionId,
					);
		...
		store.add(
			response.onDidChange(() => {
				...
				const edits = response.response.value.flatMap((part) => {
					if (
	...
	private async [State.SHOW_RESPONSE](): Promise<State.WAIT_FOR_INPUT> {
		...
		for (const request of this._session.chatModel.getRequests()) {
			...
			const thisType = asInlineChatResponseType(
		...
		if (response instanceof EmptyResponse) {
			...
			if (this._session.provider.provideFollowups) {
				...
				const followupTask = this._session.provider.provideFollowups(
					this._session.session,
					response.raw,
					followupCts.token,
				);
	...
	private _showWidget(initialRender = false, position?: Position) {
		...
		if (
			this._session &&
			!position &&
			(this._session.hasChangedText || this._session.lastExchange)
		) {
			widgetPosition = this._session.wholeRange.value
				.getStartPosition()
				.delta(-1);
		}
		...
		if (this._session && this._zone.rawValue) {
			this._zone.rawValue.updateBackgroundColor(
				widgetPosition,
				this._session.wholeRange.value,
			);
		}
	...
	private _getPlaceholderText(): string {
		return (
			this._forcedPlaceholder ?? this._session?.session.placeholder ?? ""
		);
	}
	...
	updateInput(text: string, selectAll = true): void {
		...
		if (selectAll) {
			...
			this._zone.value.widget.chatWidget.inputEditor.setSelection(
				newSelection,
			);
		}
	}
	...
	viewInChat() {
		if (this._session?.lastExchange?.response instanceof ReplyResponse) {
			this._instaService.invokeFunction(
				showMessageResponse,
				this._session.lastExchange.prompt.value,
				this._session.lastExchange.response.mdContent.value,
			);
		}
	}
	...
	acceptSession(): void {
		if (
			this._session?.lastExchange?.response instanceof ReplyResponse &&
			this._session?.lastExchange?.response.chatResponse
		) {
...
async function showMessageResponse(
	...
	const agent = chatAgentService
		...
		.find(
			(agent) =>
				agent.locations.includes(ChatAgentLocation.Panel) &&
				agent.isDefault,
		);
	if (!agent) {
		return;
...
function asInlineChatResponseType(
	response: IResponse,
): InlineChatResponseTypes {
