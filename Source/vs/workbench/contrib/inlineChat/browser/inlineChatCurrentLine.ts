...
export const CTX_INLINE_CHAT_SHOWING_HINT = new RawContextKey<boolean>(
...
export class ShowInlineChatHintAction extends EditorAction2 {
	...
	override async runEditorCommand(
		_accessor: ServicesAccessor,
		editor: ICodeEditor,
		...args: [uri: URI, position: IPosition, ...rest: any[]]
	) {
		if (!editor.hasModel()) {
			return;
		}
...
export class InlineChatHintsController
	extends Disposable
	implements IEditorContribution
{
	...
	static get(editor: ICodeEditor): InlineChatHintsController | null {
		return editor.getContribution<InlineChatHintsController>(
	...
	constructor(
		...
		markerDecorationService: IMarkerDecorationsService,
		@IContextMenuService
		...
		this._ctxShowingHint =
			CTX_INLINE_CHAT_SHOWING_HINT.bindTo(contextKeyService);
		...
		this._store.add(
			commandService.onWillExecuteCommand((e) => {
				if (
					e.commandId === _inlineChatActionId ||
					e.commandId === ACTION_START
				) {
					this.hide();
				}
			}),
			...
			this._editor.onMouseDown((e) => {
				...
				if (e.event.leftButton) {
					...
					this._showContextMenu(
						e.event,
						e.target.element?.classList.contains("whitespace")
		...
		const showDataObs = derived((r) => {
			...
			if (
				model.getLanguageId() === PLAINTEXT_LANGUAGE_ID ||
			...
			autorun((r) => {
				...
				const agentName =
					chatAgentService.getDefaultAgent(ChatAgentLocation.Editor)
						?.name ?? localize("defaultTitle", "Chat");
				const { position, isEol, isWhitespace, kb, model } = showData;

				const inlineClassName: string[] = [
				...
				markerSuppression.value =
					markerDecorationService.addMarkerSuppression(
						model.uri,
						model.validateRange(
							new Range(
								position.lineNumber,
