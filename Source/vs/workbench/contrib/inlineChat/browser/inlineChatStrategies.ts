...
export class PreviewStrategy extends EditModeStrategy {
	...
	async apply() {
		...
		if (
			untitledTextModel &&
			!untitledTextModel.isDisposed() &&
			untitledTextModel.isDirty()
		) {
...
export class LivePreviewStrategy extends LiveStrategy {
	...
	override async renderChanges(response: ReplyResponse): Promise<undefined> {
		...
		if (
			...
			!response.untitledTextModel.isDisposed()
		) {
...
export class LiveStrategy3 extends EditModeStrategy {
	...
	private readonly _decoModifiedInteractedWith =
	...
	private async _showDiff(
		...
		const mightContainNonBasicASCII =
			this._session.textModel0.mightContainNonBasicASCII() ?? false;
		const mightContainRTL =
			this._session.textModel0.mightContainRTL() ?? false;
		...
		for (let i = 0; i < diff.changes.length; i++) {
			...
			const hasBeenInteractedWith =
				this._modifiedRangesThatHaveBeenInteractedWith.some((id) => {
					const range =
						this._session.textModelN.getDecorationRange(id);
			...
			if (isFinalChanges) {
				const [id] = this._modifiedRangesDecorations.append([
					{
						range: modifiedRange,
		...
		const decorations =
			this._editor.createDecorationsCollection(newDecorations);
