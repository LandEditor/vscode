...
export class PreviewStrategy extends EditModeStrategy {
	...
	checkChanges(response: ReplyResponse): boolean {
		...
		this._bulkEditService.apply(response.workspaceEdits, {
			showPreview: true,
		});
		return false;
	}
	...
	async apply() {
		...
		if (editResponse.workspaceEdits) {
			...
			if (
				modelN.equalsTextBuffer(
					this._session.textModel0.getTextBuffer()
				)
			) {
	...
	override async renderChanges(response: ReplyResponse): Promise<void> {
		if (response.allLocalEdits.length > 0) {
			...
			await this._widget.showEditsPreview(
		...
		if (response.singleCreateFileEdit) {
			this._widget.showCreatePreview(
				response.singleCreateFileEdit.uri,
				await Promise.all(response.singleCreateFileEdit.edits)
			);
		} else {
...
class InlineDiffDecorations {
	...
	private static _asDecorationData(edit: IValidEditOperation): {
		...
		const decorating: IModelDecorationOptions = {
			description: "inline-chat-inline-diff",
			className: !edit.range.isEmpty()
				? "inline-chat-lines-inserted-range"
				: undefined,
...
export class LiveStrategy extends EditModeStrategy {
	...
	constructor(
		...
		super();
		this._diffEnabled = configService.getValue<boolean>(
			"inlineChat.showDiff"
		);

		this._inlineDiffDecorations = new InlineDiffDecorations(
			this._editor,
			this._diffEnabled
		);
	...
	async apply() {
		...
		if (this._lastResponse?.workspaceEdits) {
			await this._bulkEditService.apply(
				this._lastResponse.workspaceEdits
			);
			this._instaService.invokeFunction(
				showSingleCreateFile,
				this._lastResponse
	...
	async cancel() {
		...
		const targetAltVersion =
			textModelNSnapshotAltVersion ?? textModelNAltVersion;
	...
	private static _undoModelUntil(
		model: ITextModel,
...
export class LivePreviewStrategy extends LiveStrategy {
	...
	private async _updateDiffZones() {
		...
		const originalStartLineNumber =
			this._session.session.wholeRange?.startLineNumber ?? 1;
		...
		for (let i = 0; i < diff.changes.length; i++) {
			...
			if (
				change.original.startLineNumber >= originalStartLineNumber ||
				"true"
			) {
				// TODO@jrieken be smarter and fix this
		...
		const beforeAndNowAreEqual = equals(
			this._currentLineRangeGroups,
			groups,
			(groupA, groupB) => {
				return equals(groupA, groupB, (mappingA, mappingB) => {
					return (
						mappingA.original.equals(mappingB.original) &&
	...
	override async renderChanges(response: ReplyResponse) {
		...
		if (response.singleCreateFileEdit) {
			this._previewZone.value.showCreation(
				this._session.wholeRange.value.collapseToStart(),
				response.singleCreateFileEdit.uri,
				await Promise.all(response.singleCreateFileEdit.edits)
			);
		} else {
...
function showSingleCreateFile(accessor: ServicesAccessor, edit: ReplyResponse) {
	...
	if (edit.singleCreateFileEdit) {
		editorService.openEditor(
			{ resource: edit.singleCreateFileEdit.uri },
			SIDE_GROUP
		);
	}
}
