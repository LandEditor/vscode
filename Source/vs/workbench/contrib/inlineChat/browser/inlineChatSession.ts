...
import type { LineRange } from "vs/editor/common/core/lineRange";
...
import type { IModelContentChangedEvent } from "vs/editor/common/textModelEvents";
import {
	type IContextKey,
	IContextKeyService,
		...
		for (const change of event.changes) {
			...
			for (const { rangeN, range0 } of hunkRanges) {
				if (
					...
					// range into account.
					pendingChangesLen +=
						this._textModelN.getValueLengthInRange(rangeN);
					pendingChangesLen -=
						this._textModel0.getValueLengthInRange(range0);
				} else if (
					Range.areIntersectingOrTouching(rangeN, change.range)
				) {
					isOverlapping = true;
					break;
				} else {
