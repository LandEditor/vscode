...
} from "vs/workbench/contrib/inlineChat/browser/inlineChatController";
import {
...
export const LOCALIZED_START_INLINE_CHAT_STRING = localize(
	"run",
	"Start Inline Chat"
);
...
export class StartSessionAction extends EditorAction2 {
	constructor() {
		super({
			id: "inlineChat.start",
			title: {
				value: LOCALIZED_START_INLINE_CHAT_STRING,
				original: "Start Inline Chat",
			},
...
export class UnstashSessionAction extends EditorAction2 {
	constructor() {
		super({
			...
			title: {
				value: localize("unstash", "Resume Last Dismissed Inline Chat"),
...
abstract class AbstractInlineChatAction extends EditorAction2 {
	static readonly category = {
		value: localize("cat", "Inline Chat"),
...
export class FocusInlineChat extends EditorAction2 {
	constructor() {
		super({
			id: "inlineChat.focus",
			title: {
				value: localize("focus", "Focus Input"),
...
MenuRegistry.appendMenuItem(MENU_INLINE_CHAT_WIDGET_STATUS, {
	...
	when: ContextKeyExpr.and(
		...
		CTX_INLINE_CHAT_RESPONSE_TYPES.notEqualsTo(
	...
	constructor() {
		super({
			...
			menu: {
				...
				when: ContextKeyExpr.and(
					CTX_INLINE_CHAT_SUPPORT_ISSUE_REPORTING,
					CTX_INLINE_CHAT_LAST_RESPONSE_TYPE.notEqualsTo(undefined)
				),
				group: "2_feedback",
				order: 3,
			},
...
export class ApplyPreviewEdits extends AbstractInlineChatAction {
	constructor() {
		super({
			...
			title: {
				value: localize("apply1", "Accept Changes"),
		...
		super({
			...
			menu: {
				...
				when: ContextKeyExpr.or(
					...
					CTX_INLINE_CHAT_RESPONSE_TYPES.isEqualTo(
						InlineChateResponseTypes.OnlyMessages
					)
				),
				group: "0_main",
...
export class CopyRecordings extends AbstractInlineChatAction {
	...
	override async runInlineChatCommand(
		...
		const recordings = ieSessionService
			.recordings()
			.filter((r) => r.exchanges.length > 0);
		if (recordings.length === 0) {
			return;
		}

		const picks: (IQuickPickItem & { rec: Recording })[] = recordings.map(
			(rec) => {
				return {
					rec,
					label: localize(
...
export class ViewInChatAction extends AbstractInlineChatAction {
	constructor() {
		super({
			...
			menu: {
				id: MENU_INLINE_CHAT_WIDGET_STATUS,
				when: CTX_INLINE_CHAT_LAST_RESPONSE_TYPE.isEqualTo(
					InlineChatResponseType.Message
				),
				group: "0_main",
				order: 1,
			},
	...
	constructor() {
		super({
			...
			menu: {
				...
				when: ContextKeyExpr.and(
					CTX_INLINE_CHAT_LAST_RESPONSE_TYPE.isEqualTo("message"),
					CTX_INLINE_CHAT_MESSAGE_CROP_STATE.isEqualTo("cropped")
				),
	...
	constructor() {
		super({
			...
			menu: {
				...
				when: ContextKeyExpr.and(
					CTX_INLINE_CHAT_LAST_RESPONSE_TYPE.isEqualTo("message"),
					CTX_INLINE_CHAT_MESSAGE_CROP_STATE.isEqualTo("expanded")
				),
				group: "2_expandOrContract",
				order: 1,
			},
