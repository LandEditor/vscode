...
import type { Session } from "vs/workbench/contrib/inlineChat/browser/inlineChatSession";
...
export class InlineChatContentWidget implements IContentWidget {
	...
	constructor(
		...
		this._widget = scopedInstaService.createInstance(
			...
			{ resource: true },
			{
				...
				editorOverflowWidgetsDomNode:
					_editor.getOverflowWidgetsDomNode(),
				renderStyle: "compact",
		...
		this._store.add(
			this._widget.inputEditor.onDidContentSizeChange(() =>
	...
	beforeRender(): IDimension | null {
		const maxHeight =
			this._widget.input.inputEditor.getOption(EditorOption.lineHeight) *
			5;
	...
	show(position: IPosition) {
		if (!this._visible) {
			...
			this._editor.revealRangeNearTopIfOutsideViewport(
				Range.fromPositions(position),
				ScrollType.Immediate,
			);
			this._widget.inputEditor.setValue("");

			const wordInfo = this._editor
				.getModel()
				?.getWordAtPosition(position);

			this._position = wordInfo
				? new Position(position.lineNumber, wordInfo.startColumn)
				: position;
