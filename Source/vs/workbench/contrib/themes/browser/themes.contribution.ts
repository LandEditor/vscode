	...
	public async openQuickPick(
		...
		if (this.extensionGalleryService.isEnabled()) {
			if (
				...
				picks = [
					...picks,
					...configurationEntries(this.installMessage),
				];
			}
		}
				...
				quickpick.onDidAccept(async (_) => {
					...
					if (!theme || typeof theme.id === "undefined") {
						...
						if (marketplaceThemePicker) {
							...
							openExtensionViewlet(
								this.paneCompositeService,
								`${this.marketplaceTag} ${quickpick.value}`,
							);
						}
					} else {
	...
	class extends Action2 {
		constructor() {
			super({
				...
				title: {
					value: localize("selectTheme.label", "Color Theme"),
	...
	class extends Action2 {
		constructor() {
			super({
				...
				title: {
					value: localize("selectIconTheme.label", "File Icon Theme"),
