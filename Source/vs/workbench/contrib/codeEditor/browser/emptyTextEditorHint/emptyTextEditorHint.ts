...
export class EmptyTextEditorHintContribution implements IEditorContribution {
	...
	constructor(
		...
		@IInlineChatService
		protected readonly inlineChatService: IInlineChatService,
		...
		this.toDispose.push(
			inlineChatSessionService.onDidEndSession((editor) => {
				if (this.editor === editor) {
					this.update();
				}
			})
...
class EmptyTextEditorHintContentWidget implements IContentWidget {
	...
	constructor(
		...
		this.toDispose.add(
			this.inlineChatService.onDidChangeProviders(() =>
	...
	private onDidChangeModelContent(): void {
		if (
			!this.editor.getModel()?.getValueLength() &&
			!this.hasConflictingDecorations()
		) {
	...
	private _getHintInlineChat(providers: IInlineChatSessionProvider[]) {
		const providerName =
			(providers.length === 1 ? providers[0].label : undefined) ??
		...
		const handleClick = () => {
			...
			void this.commandService.executeCommand(inlineChatId, {
				from: "hint",
			});
		};
		...
		const keybindingHint =
			this.keybindingService.lookupKeybinding(inlineChatId);
	...
	private _getHintDefault() {
		...
		const chooseEditorOnClickOrTap = async (e: UIEvent) => {
			...
			const activeEditorInput =
				this.editorGroupsService.activeGroup.activeEditor;
			this.telemetryService.publicLog2<
				WorkbenchActionExecutedEvent,
				WorkbenchActionExecutedClassification
			>("workbenchActionExecuted", {
		...
		for (const anchor of hintElement.querySelectorAll("a")) {
			...
			const title =
				id && this.keybindingService.lookupKeybinding(id)?.getLabel();
			anchor.title = title ?? "";
		}
