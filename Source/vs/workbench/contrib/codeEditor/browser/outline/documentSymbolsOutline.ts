...
} from "vs/workbench/contrib/codeEditor/browser/outline/documentSymbolsTree";
...
class DocumentSymbolsOutline implements IOutline<DocumentSymbolItem> {
	...
	constructor(
		...
		const renderers = [
			...
			instantiationService.createInstance(DocumentSymbolRenderer, true),
		];
		...
		this._disposables.add(
			this._editor.onDidChangeModelContent((event) => {
				...
				if (model) {
					const timeout =
						_outlineModelService.getDebounceValue(model);
		...
		await this._codeEditorService.openCodeEditor(
			{
				...
				options: {
					...options,
					selection: Range.collapseToStart(
