...
} from "vs/editor/browser/widget/diffEditor/diffEditor.contribution";
...
function createScreenReaderHelp(): IDisposable {
	return AccessibilityHelpAction.addImplementation(
		105,
		"diff-editor",
		async (accessor) => {
			...
			const next = keybindingService
				.lookupKeybinding(AccessibleDiffViewerNext.id)
				?.getAriaLabel();
			const previous = keybindingService
				.lookupKeybinding(AccessibleDiffViewerPrev.id)
				?.getAriaLabel();

			if (
				!(
					editorService.activeTextEditorControl instanceof
					DiffEditorWidget
				)
			) {
				return;
