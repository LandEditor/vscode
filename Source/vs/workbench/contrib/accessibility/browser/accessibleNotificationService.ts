...
export class AccessibleNotificationService
	extends Disposable
	implements IAccessibleNotificationService
{
	declare readonly _serviceBrand: undefined;
	private _events: Map<
		AccessibleNotificationEvent,
		{ audioCue: AudioCue; alertMessage: string; alertSetting?: string }
	> = new Map();
	constructor(
	...
	private _notify(
		...
		const audioCueSetting: NotificationSetting =
			this._configurationService.getValue(audioCue.settingsKey);
		...
		const alertSettingValue: NotificationSetting =
			this._configurationService.getValue(alertSetting);
	...
	private _shouldNotify(
		...
		return (
			settingValue === "always" ||
			(settingValue === "userGesture" && userGesture === true)
		);
	}
}
type NotificationSetting = "never" | "always" | "userGesture";

export class TestAccessibleNotificationService
	extends Disposable
	implements IAccessibleNotificationService
{
	declare readonly _serviceBrand: undefined;
