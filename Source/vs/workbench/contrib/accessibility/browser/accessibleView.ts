...
export class AccessibleView extends Disposable {
	...
	show(
		provider?: IAccessibleContentProvider,
	...
	private _updateContextKeys(
		...
		const verbosityEnabled: boolean = this._configurationService.getValue(
	...
	private _render(
		...
		const verbose = this._configurationService.getValue(
			provider.verbositySettingKey
		);
		...
		this._getTextModel(
			URI.from({
				path: `accessible-view-${provider.verbositySettingKey}`,
		...
		disposableStore.add(
			this._editorWidget.onKeyDown((e) => {
				if (
					...
					shouldHide(e.browserEvent, this._keybindingService)
				) {
					hide(e);
				} else if (
	...
	private _updateLastProvider(): IAccessibleContentProvider | undefined {
		...
		lastProvider.provideContent =
			this._currentProvider.provideContent.bind(lastProvider);
	...
	private _getNavigationHint(): string {
		let hint = "";
		const nextKeybinding = this._keybindingService
			.lookupKeybinding(AccessibilityCommandId.ShowNext)
			?.getAriaLabel();
		const previousKeybinding = this._keybindingService
			.lookupKeybinding(AccessibilityCommandId.ShowPrevious)
			?.getAriaLabel();
	...
	private _getDisableVerbosityHint(
		verbositySettingKey: AccessibilityVerbositySettingId
	): string {
		...
		let hint = "";
		const disableKeybinding = this._keybindingService
			.lookupKeybinding(
	...
	private _getGoToSymbolHint(providerHasSymbols?: boolean): string {
		const goToSymbolKb = this._keybindingService
			.lookupKeybinding(AccessibilityCommandId.GoToSymbol)
			?.getAriaLabel();
		let goToSymbolHint = "";
