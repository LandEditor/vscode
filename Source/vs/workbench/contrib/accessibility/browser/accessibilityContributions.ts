			...
			AccessibleViewAction.addImplementation(
				...
				(accessor) => {
					...
					const editorHoverContent = editor
						? ModesHoverController.get(
								editor
						  )?.getWidgetContent() ?? undefined
						: undefined;
					if (!editor || !editorHoverContent) {
						return false;
					}
					this._options.language =
						editor?.getModel()?.getLanguageId() ?? undefined;
					accessibleViewService.show({
			...
			AccessibleViewAction.addImplementation(
				...
				(accessor) => {
					...
					const contextViewElement =
						contextViewService.getContextViewElement();
			...
			AccessibleViewAction.addImplementation(
				...
				(accessor) => {
					...
					function renderAccessibleView(): boolean {
						const notification =
							getNotificationFromContext(listService);
						if (!notification) {
							return false;
						}
						...
						const message =
							notification.message.original.toString();
						if (!message) {
							return false;
						}
