...
import type { TaskQuickPickEntryType } from "vs/workbench/contrib/tasks/browser/abstractTaskService";
import {
	ITaskService,
...
export class TaskQuickPick extends Disposable {
	...
	public async show(
		...
		const picker: IQuickPick<ITaskTwoLevelQuickPickEntry> =
		...
		picker.onDidTriggerItemButton(async (context) => {
			...
			if (
				...
				const key =
					task && !Types.isString(task) ? task.getKey() : undefined;
				if (key) {
	...
	private async _doPickerFirstLevel(
		picker: IQuickPick<ITaskTwoLevelQuickPickEntry>,
		...
		>((resolve) => {
			Event.once(picker.onDidAccept)(async () => {
				resolve(
					picker.selectedItems ? picker.selectedItems[0] : undefined,
				);
			});
		});
