...
export namespace ConfigureTaskAction {
	...
	export const TEXT = nls.localize(
	...
	implements ITaskService
{
	...
	private _getTaskSystemInfo(key: string): ITaskSystemInfo | undefined {
		const infos = this._taskSystemInfos.get(key);
		return infos?.length ? infos[0] : undefined;
	}
	...
	private async _handleExecuteResult(
		...
		if (executeResult.kind === TaskExecuteKind.Active) {
			...
			if (
				(active?.same && runSource === TaskRunSource.FolderOpen) ||
				runSource === TaskRunSource.Reconnect
			) {
	...
	private async _getGroupedTasks(filter?: ITaskFilter): Promise<TaskMap> {
		...
		try {
			...
			const customTasksPromises = customTasksKeyValuePairs.map(
				async ([key, folderTasks]) => {
					...
					if (
						...
						if (configurations || legacyTaskConfigurations) {
							const unUsedConfigurations: Set<string> =
								new Set<string>();
							if (configurations) {
								Object.keys(
							...
							const unUsedConfigurationPromises =
								unUsedConfigurationsAsArray.map(
									async (value) => {
										const configuringTask =
										...
										for (const [handle, provider] of this
											._providers) {
											const providerType =
												this._providerTypes.get(handle);
											if (
												configuringTask.type ===
												providerType
											) {
												if (
													providerType &&
	...
	private async _createTaskQuickPickEntries(
		...
		const TaskQuickPickEntry = (task: Task): ITaskQuickPickEntry => {
			...
			if (encounteredTasks[task._id]) {
				...
				newEntry.label = `${newEntry.label} (${(
					encounteredTasks[task._id].length + 1
				).toString()})`;
			} else {
	...
	private async _showQuickPick(
		...
		const picker: IQuickPick<ITaskQuickPickEntry> =
		...
		return new Promise<ITaskQuickPickEntry | undefined | null>(
			(resolve) => {
				this._register(
					picker.onDidAccept(async () => {
						const selectedEntry = picker.selectedItems
				...
				if (workspaceFolder) {
					...
					if (configuredTasks) {
						...
						if (globTasksDetected) {
							...
							groupTasks = await this._findWorkspaceTasks(
								(task) => {
			...
			this.tasks().then((tasks) => {
				...
				this._showIgnoredFoldersMessage().then(() => {
					for (const task of tasks) {
						...
						if (
							taskGroup?.isDefault &&
							taskGroup._id === TaskGroup.Build._id
						) {
