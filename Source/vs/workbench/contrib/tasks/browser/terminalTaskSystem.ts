...
import type { IModelService } from "vs/editor/common/services/model";
...
import {
	...
	VSCodeOscPt,
	VSCodeSequence,
} from "vs/workbench/contrib/terminal/browser/terminalEscapeSequences";
import { TerminalProcessExtHostProxy } from "vs/workbench/contrib/terminal/browser/terminalProcessExtHostProxy";
import {
	type ITerminalProfileResolverService,
	TERMINAL_VIEW_ID,
} from "vs/workbench/contrib/terminal/common/terminal";
import type { IConfigurationResolverService } from "vs/workbench/services/configurationResolver/common/configurationResolver";
import type { IWorkbenchEnvironmentService } from "vs/workbench/services/environment/common/environmentService";
import type { IOutputService } from "vs/workbench/services/output/common/output";
import type { IPaneCompositePartService } from "vs/workbench/services/panecomposite/browser/panecomposite";
import type { IPathService } from "vs/workbench/services/path/common/pathService";
import type { IViewsService } from "vs/workbench/services/views/common/viewsService";
	...
	taskShellIntegrationStartSequence(cwd: string | URI | undefined): string {
		return (
			...
			(cwd
				? VSCodeSequence(
						VSCodeOscPt.Property,
						`${VSCodeOscProperty.Cwd}=${
							typeof cwd === "string" ? cwd : cwd.fsPath
						}`,
					)
				: "") +
					...
					shellLaunchConfig.initialText =
						...
						formatMessageForTerminal(
							nls.localize(
								...
								`${
									shellLaunchConfig.executable
								} ${getArgsToEcho(shellLaunchConfig.args)}`,
							),
							{ excludeLeadingNewLine: true },
						) +
						this.taskShellIntegrationOutputSequence;
