...
} from "vs/workbench/contrib/interactive/browser/interactiveDocumentService";
...
} from "vs/workbench/contrib/interactive/browser/interactiveHistoryService";
...
const interactiveWindowCategory: ILocalizedString = {
	value: localize("interactiveWindow", "Interactive Window"),
	original: "Interactive Window",
};
...
export class InteractiveEditorSerializer implements IEditorSerializer {
	...
	canSerialize(editor: EditorInput): boolean {
		...
		return (
			URI.isUri(interactiveEditorInput?.primary?.resource) &&
			URI.isUri(interactiveEditorInput?.inputResource)
		);
	}
	...
	class extends Action2 {
		constructor() {
			super({
				...
				title: {
					value: localize("interactive.execute", "Execute Code"),
		...
		async run(
			...
			if (
				...
				if (notebookDocument && textModel) {
					...
					const collapseState =
						editorControl.notebookEditor.notebookOptions.getLayoutConfiguration()
...
registerThemingParticipant((theme) => {
	...
	registerColor(
		...
		{
			dark:
				theme.getColor(listInactiveSelectionBackground) ??
				transparent(listInactiveSelectionBackground, 1),
			light:
				theme.getColor(listInactiveSelectionBackground) ??
				transparent(listInactiveSelectionBackground, 1),
			hcDark: PANEL_BORDER,
