...
const LOCAL_HISTORY_CATEGORY = {
	value: localize("localHistory.category", "Local History"),
...
export const COMPARE_WITH_FILE_LABEL = {
	value: localize("localHistory.compareWithFile", "Compare with File"),
	...
	class extends Action2 {
		constructor() {
			super({
				...
				title: {
					value: localize("localHistory.open", "Show Contents"),
...
const RESTORE_CONTENTS_LABEL = {
	value: localize("localHistory.restore", "Restore Contents"),
...
registerAction2(
	class extends Action2 {
		...
		async run(accessor: ServicesAccessor): Promise<void> {
			...
			resourcePicker.items = resources
				.map((resource) => ({
					resource,
				...
				.sort((r1, r2) =>
					r1.resource.fsPath < r2.resource.fsPath ? -1 : 1
				);

			await Event.toPromise(resourcePicker.onDidAccept);
			resourcePicker.dispose();

			const resource = firstOrDefault(resourcePicker.selectedItems)
				?.resource;
			if (!resource) {
				return;
			}
			...
			entryPicker.items = Array.from(entries)
				...
				.map((entry) => ({
					entry,
					label: `$(circle-outline) ${SaveSourceRegistry.getSourceLabel(
						entry.source
					)}`,
...
MenuRegistry.appendMenuItem(MenuId.TimelineTitle, {
	command: {
		...
		title: {
			...
			original: "Local History: Find Entry to Restore...",
		},
	},
	group: "submenu",
	order: 1,
	...
	class extends Action2 {
		constructor() {
			super({
				...
				title: {
					value: localize("localHistory.rename", "Rename"),
					original: "Rename",
				},
				menu: {
					id: MenuId.TimelineItemContext,
	...
	class extends Action2 {
		constructor() {
			super({
				...
				title: {
					value: localize("localHistory.delete", "Delete"),
					original: "Delete",
				},
				menu: {
					id: MenuId.TimelineItemContext,
	...
	class extends Action2 {
		constructor() {
			super({
				...
				title: {
					value: localize("localHistory.deleteAll", "Delete All"),
					original: "Delete All",
				},
				f1: true,
				category: LOCAL_HISTORY_CATEGORY,
	...
	class extends Action2 {
		constructor() {
			super({
				...
				title: {
					value: localize("localHistory.create", "Create Entry"),
...
function toLocalHistoryEntryDateLabel(timestamp: number): string {
	return `${getLocalHistoryDateFormatter()
		.format(timestamp)
		.replace(SEP, "-")}`; // preserving `/` will break editor labels, so replace it with a non-path symbol
}
