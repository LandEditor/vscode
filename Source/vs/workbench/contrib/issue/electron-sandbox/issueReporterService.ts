...
export class IssueReporter extends Disposable {
	...
	private async updateIssueReporterUri(
		extension: OldIssueReporterExtensionData,
	): Promise<void> {
		try {
			if (extension.uri) {
	...
	private setEventHandlers(): void {
		this.addEventListener("issue-type", "change", (event: Event) => {
			...
			if (
				issueType === OldIssueType.PerformanceIssue &&
	...
	private setUpTypes(): void {
		const makeOption = (issueType: OldIssueType, description: string) =>
	...
	private updateExtensionSelector(
		extensions: OldIssueReporterExtensionData[],
	): void {
		interface IOption {
			name: string;
			id: string;
		...
		const makeOption = (
			extension: IOption,
			selectedExtension?: OldIssueReporterExtensionData,
		): HTMLOptionElement => {
			const selected =
	...
	private removeLoading(element: HTMLElement, fromReporter = false) {
