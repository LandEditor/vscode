...
export class ShowAudioCueHelp extends Action2 {
	...
	override async run(accessor: ServicesAccessor): Promise<void> {
		...
		const items: (IQuickPickItem & { audioCue: AudioCue })[] =
		...
		const quickPick = quickPickService.pick<
			IQuickPickItem & { audioCue: AudioCue }
		>(items, {
			activeItem: items[0],
			onDidFocus: (item) => {
			...
			onDidTriggerItemButton: (context) => {
				preferencesService.openSettings({
					query: context.item.audioCue.settingsKey,
				});
			},
