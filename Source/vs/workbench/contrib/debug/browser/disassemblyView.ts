...
import {
	...
	topStackFrameColor,
} from "vs/workbench/contrib/debug/browser/callStackEditorContribution";
...
export class DisassemblyView extends EditorPane {
	...
	constructor(
		...
		this._fontInfo = BareFontInfo.createFromRawSettings(
			_configurationService.getValue("editor"),
			PixelRatio.value
		);
		this._register(
		...
		if (session && resultEntries && this._disassembledInstructions) {
			...
			for (let i = 0; i < resultEntries.length; i++) {
				...
				try {
					...
					console.error(
						`Could not parse disassembly address ${
							instruction.address
	...
	implements IListAccessibilityProvider<IDisassembledInstructionEntry>
{
	...
	getAriaLabel(element: IDisassembledInstructionEntry): string | null {
		...
		const instruction = element.instruction;
		if (instruction.address !== "-1") {
			label += `${localize("instructionAddress", "Address")}: ${
				instruction.address
			}`;
		}
		if (instruction.instructionBytes) {
			label += `, ${localize("instructionBytes", "Bytes")}: ${
