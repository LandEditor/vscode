		...
		>
{
	...
	private doRenderElement(
		...
		if (stoppedDetails) {
			...
			data.session.title = `${session.getLabel()}: ${stoppedText(
				stoppedDetails
			)}`;
		...
		>
{
	...
	renderElement(
		...
		data.file.title = stackFrame.source.inMemory
