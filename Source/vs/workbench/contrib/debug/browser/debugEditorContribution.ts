...
export class DebugEditorContribution implements IDebugEditorContribution {
	...
	private _wordToLineNumbersMap: Map<string, number[]> | undefined =
		undefined;
	private get wordToLineNumbersMap(): Map<string, number[]> {
		if (!this._wordToLineNumbersMap) {
			this._wordToLineNumbersMap = getWordToLineNumbersMap(
	...
	private applyHoverConfiguration(
		...
		if (
			...
			const ownerDocument =
				this.editor.getContainerDomNode().ownerDocument;
			if (this.altListener) {
					...
					if (standardKeyboardEvent.keyCode === KeyCode.Alt) {
						...
						const listener = Event.any<KeyboardEvent | boolean>(
							...
							if (isKeyboardEvent(keyupEvent)) {
								standardKeyboardEvent =
									new StandardKeyboardEvent(keyupEvent);
							}
							if (
								!standardKeyboardEvent ||
	...
	private showEditorHover(position: Position, focus: boolean) {
		const hoverController =
			this.editor.getContribution<ModesHoverController>(
	...
	private hideHoverWidget(): void {
		if (
			...
			this.hoverWidget.willBeVisible()
		) {
	...
	private onEditorMouseMove(mouseEvent: IEditorMouseEvent): void {
		...
		if (target.type === MouseTargetType.CONTENT_TEXT) {
			if (
				target.position &&
				!Position.equals(target.position, this.hoverPosition)
			) {
	...
	private async updateInlineValueDecorations(
		...
		if (this.languageFeaturesService.inlineValuesProvider.has(model)) {
			...
			allDecorations = distinct(
				...
				(decoration) =>
					`${decoration.range.startLineNumber}:${decoration?.options.after?.content}`
			);
		}
