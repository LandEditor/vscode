...
export class VariablesView extends ViewPane {
	...
	private onMouseDblClick(e: ITreeMouseEvent<IExpression | IScope>): void {
		...
		if (
			...
			!e.element.presentationHint?.attributes?.includes("readOnly") &&
			!e.element.presentationHint?.lazy
		) {
			this.debugService
				.getViewModel()
 ...
 */
function getContextForVariableMenu(
	parentContext: IContextKeyService,
	variable: Variable,
...
class VariablesDataSource
	implements IAsyncDataSource<IStackFrame | null, IExpression | IScope>
{
...
export class VariablesRenderer extends AbstractExpressionsRenderer {
	...
	protected override renderActionBar(
		...
		const contextKeyService = getContextForVariableMenu(
			this.contextKeyService,
			variable,
		);
						...
						try {
							await progressService.withProgress(
								...
								async () => {
									...
									while (
										!(await extensionService.getExtension(
											HEX_EDITOR_EXTENSION_ID,
										))
									) {
										await timeout(30);
	...
	handler: async (accessor: ServicesAccessor) => {
		...
		if (dataBreakpointInfoResponse) {
			await debugService.addDataBreakpoint(
				dataBreakpointInfoResponse.description,
				dataBreakpointInfoResponse.dataId!,
				!!dataBreakpointInfoResponse.canPersist,
				dataBreakpointInfoResponse.accessTypes,
				"write",
			);
		}
	},
