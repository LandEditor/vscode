		...
		this._register(
			this.debugService.getViewModel().onDidSelectExpression((e) => {
				...
				if (
					...
					(expression instanceof Variable && e?.settingWatch)
				) {
...
class WatchExpressionsDataSource
	implements IAsyncDataSource<IDebugService, IExpression>
{
...
class WatchExpressionsDragAndDrop implements ITreeDragAndDrop<IExpression> {
	...
	drop(data: IDragAndDropData, targetElement: IExpression): void {
		...
		const position =
			targetElement instanceof Expression
				? watches.indexOf(targetElement)
				: watches.length - 1;
