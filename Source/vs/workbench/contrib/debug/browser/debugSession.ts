...
export class DebugSession implements IDebugSession, IDisposable {
	...
	get autoExpandLazyVariables(): boolean {
		...
		return this.configurationService.getValue<IDebugConfiguration>("debug")
	...
	async sendBreakpoints(
		...
		const response = await this.raw.setBreakpoints({
			...
			breakpoints: breakpointsToSend.map((bp) => ({
				...
				column: bp.sessionAgnosticData.column,
	...
	> {
		...
		const response = await this.raw.dataBreakpointInfo({
			name,
			variablesReference,
		});
	...
	private registerListeners(): void {
		...
		this.rawListeners.add(
			this.raw.onDidStop(async (event) => {
						...
						if (thread) {
							...
							const focus = async () => {
								if (
									...
									if (
										!focusedStackFrame ||
										focusedStackFrame.thread.session ===
											this
									) {
							...
							if (
								...
								focusedStackFrame.source.presentationHint ===
									"deemphasize" ||
								focusedStackFrame.presentationHint ===
									"deemphasize"
							) {
			...
			this.raw.onDidInvalidated(async (event) => {
				if (
					!(
						...
						(event.body.areas[0] === "variables" ||
							event.body.areas[0] === "watch")
					)
				) {
