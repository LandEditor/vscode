...
import {
	DebugTaskRunner,
	TaskRunResult,
} from "vs/workbench/contrib/debug/browser/debugTaskRunner";
import {
	CALLSTACK_VIEW_ID,
...
export class DebugService implements IDebugService {
	...
	async updateBreakpoints(
		uri: uri,
		data: Map<string, DebugProtocol.Breakpoint>,
	...
	async removeBreakpoints(id?: string): Promise<void> {
		...
		const urisToClear = distinct(toRemove, (bp) =>
			bp.originalUri.toString()
		).map((bp) => bp.originalUri);
