...
} from "vs/workbench/contrib/debug/browser/debugIcons";
...
} from "vs/workbench/contrib/debug/browser/replViewer";
import {
	CONTEXT_DEBUG_STATE,
...
export class Repl extends FilterViewPane implements IHistoryNavigationWidget {
	...
	constructor(
		...
		this.replOptions = this._register(
			this.instantiationService.createInstance(ReplOptions, this.id, () =>
	...
	private registerListeners(): void {
		...
		this._register(
			this.debugService.onDidEndSession(async (session) => {
		...
		if (this.model && this.tree) {
			...
			const traverseAndAppend = (
				...
				node.children.forEach((child) => {
					if (child.visible) {
						text +=
							child.element.toString().trimRight() +
							lineDelimiter;
	...
	private createReplTree(): void {
		...
		>this.instantiationService.createInstance(
			...
			new ReplDataSource() as IAsyncDataSource<
				IDebugSession,
				IReplElement
			>,
			{
	...
	class extends ViewAction<Repl> {
		constructor() {
			super({
				...
				viewId: REPL_VIEW_ID,
				title: {
					value: localize("clearRepl", "Clear Console"),
		...
		runInView(_accessor: ServicesAccessor, view: Repl): void {
			const accessibleNotificationService = _accessor.get(
				IAccessibleNotificationService
			);
			view.clearRepl();
