...
suite("Terminal history", () => {
	...
	suite("fetchBashHistory", () => {
		...
		let remoteConnection: Pick<
			...
			"remoteAuthority"
		> | null = null;
		let remoteEnvironment: Pick<IRemoteAgentEnvironment, "os"> | null =
			null;
		...
		setup(() => {
			...
			instantiationService.stub(IFileService, {
				async readFile(resource: URI) {
					...
					if (
						resource.scheme !== expected.scheme ||
						resource.fsPath !== expected.fsPath
					) {
						fail(
							`Unexpected file scheme/path ${resource.scheme} ${resource.fsPath}`
						);
					}
					return { value: VSBuffer.fromString(fileContent) };
				},
			...
			suite("local (overriden path)", () => {
				...
				teardown(() => {
					if (originalEnvValues["XDG_DATA_HOME"] === undefined) {
						...
						env["XDG_DATA_HOME"] =
							originalEnvValues["XDG_DATA_HOME"];
					}
				});
				test("current OS", async () => {
					filePath = "/home/user/data-home/fish/fish_history";
