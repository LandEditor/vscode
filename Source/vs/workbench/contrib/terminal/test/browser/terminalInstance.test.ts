...
import {
	...
	parseExitResult,
} from "vs/workbench/contrib/terminal/browser/terminalInstance";
import {
...
import {
	fixPath,
	getUri,
} from "vs/workbench/services/search/test/browser/queryBuilder.test";
...
suite("Workbench - TerminalInstance", () => {
		...
		test("should return friendly message when executable is specified for non-zero exit codes", () => {
			...
			deepStrictEqual(
				...
				),
				{
					code: 1,
					message:
						'The terminal process "foo" terminated with exit code: 1.',
				}
			);
			deepStrictEqual(
				parseExitResult(
	...
	suite("TerminalLabelComputer", () => {
		...
		setup(async () => {
			...
			mockMultiRootWorkspace = new Workspace("multi-root-workspace", [
				toWorkspaceFolder(ROOT_1_URI),
				toWorkspaceFolder(ROOT_2_URI),
			]);
		...
		test('should resolve to "" when the template variables are empty', () => {
			...
			terminalLabelComputer = store.add(
				new TerminalLabelComputer(
					configHelper,
					new TestFileService(),
					mockContextService
				)
