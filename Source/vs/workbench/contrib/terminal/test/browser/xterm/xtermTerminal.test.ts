...
class TestXtermTerminal extends XtermTerminal {
	webglAddonPromise: Promise<typeof WebglAddon> =
...
suite("XtermTerminal", () => {
	...
	setup(async () => {
		...
		configHelper = store.add(
			instantiationService.createInstance(TerminalConfigHelper)
		);
		XTermBaseCtor = (
		...
		xterm = store.add(
			instantiationService.createInstance(
				...
				capabilityStore,
				"",
				new MockContextKeyService().createKey("", true)!,
				true
			)
		);
