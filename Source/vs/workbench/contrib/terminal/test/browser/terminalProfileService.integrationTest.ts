...
} from "vs/workbench/contrib/terminal/browser/terminalProfileQuickpick";
...
suite("TerminalProfileService", () => {
	...
	setup(async () => {
		...
		const terminalContributionService =
			new TestTerminalContributionService();
		...
		terminalProfileService = instantiationService.createInstance(
	...
	suite("Profiles Quickpick", () => {
		...
		test("createInstance", async () => {
			...
			const result =
				await terminalProfileQuickpick.showAndGetResult(
					"createInstance"
				);
