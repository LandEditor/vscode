...
export class TerminalEditor extends EditorPane {
	...
	override async setInput(
		...
		this._editorInput.terminalInstance?.setVisible(
			this.isVisible() &&
				this._workbenchLayoutService.isVisible(Parts.EDITOR_PART)
		);
			...
			dom.addDisposableListener(
				...
				async (event: MouseEvent) => {
					...
					if (event.which === 2 && isLinux) {
						...
						const terminal =
							this._terminalEditorService.activeInstance;
						terminal?.focus();
					} else if (event.which === 3) {
						const rightClickBehavior =
							this._terminalService.configHelper.config
								.rightClickBehavior;
	...
	override setVisible(visible: boolean, group?: IEditorGroup): void {
		...
		this._editorInput?.terminalInstance?.setVisible(
			visible && this._workbenchLayoutService.isVisible(Parts.EDITOR_PART)
		);
	}
