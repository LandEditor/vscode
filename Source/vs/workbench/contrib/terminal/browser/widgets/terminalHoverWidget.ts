...
import type { ITerminalWidget } from "vs/workbench/contrib/terminal/browser/widgets/widgets";
...
class CellHoverTarget extends Widget implements IHoverTarget {
	...
	constructor(
		...
		topTarget.style.bottom = `${
			(this._options.terminalDimensions.height -
				this._options.viewportRange.start.y -
				1) *
			this._options.cellDimensions.height
		}px`;
		...
		if (rowCount > 2) {
			...
			middleTarget.style.width = `${
				this._options.terminalDimensions.width *
				this._options.cellDimensions.width
			}px`;
			middleTarget.style.height = `${
				(rowCount - 2) * this._options.cellDimensions.height
			}px`;
		...
		if (rowCount > 1) {
			...
			bottomTarget.style.width = `${
				(this._options.viewportRange.end.x + 1) *
				this._options.cellDimensions.width
			}px`;
