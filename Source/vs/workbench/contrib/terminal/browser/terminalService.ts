...
export class TerminalService extends Disposable implements ITerminalService {
	...
	get defaultLocation(): TerminalLocation {
		return this.configHelper.config.defaultLocation ===
	...
	@memoize get onAnyInstanceDataInput() {
		return this.createOnInstanceEvent((e) => e.onDidInputData);
	}
	@memoize get onAnyInstanceIconChange() {
		return this.createOnInstanceEvent((e) => e.onIconChanged);
	}
	@memoize get onAnyInstanceMaximumDimensionsChange() {
		return this.createOnInstanceEvent((e) =>
			Event.map(e.onMaximumDimensionsChanged, () => e, this._store)
		);
	}
	...
	@memoize get onAnyInstanceProcessIdReady() {
		return this.createOnInstanceEvent((e) => e.onProcessIdReady);
	}
	@memoize get onAnyInstanceSelectionChange() {
		return this.createOnInstanceEvent((e) => e.onDidChangeSelection);
	}
	@memoize get onAnyInstanceTitleChange() {
		return this.createOnInstanceEvent((e) => e.onTitleChanged);
	}
	...
	constructor(
		...
		this._configHelper = this._register(
			this._instantiationService.createInstance(TerminalConfigHelper)
		);
	...
	async safeDisposeTerminal(instance: ITerminalInstance): Promise<void> {
		...
		if (
			...
			(this.configHelper.config.confirmOnKill === "panel" ||
				this.configHelper.config.confirmOnKill === "always")
		) {
	...
	private async _onBeforeShutdownAsync(
		...
		try {
			...
			const shouldPersistProcesses =
				this._configHelper.config.enablePersistentSessions &&
				reason === ShutdownReason.RELOAD;
			if (!shouldPersistProcesses) {
				const hasDirtyInstances =
					(this.configHelper.config.confirmOnExit === "always" &&
	...
	async createTerminal(
		...
		let contributedProfile =
			config && "extensionIdentifier" in config ? config : undefined;

		// Get the default profile as a contributed profile if it exists
		if (!contributedProfile && (!options || !options.config)) {
			contributedProfile =
				await this._terminalProfileService.getContributedDefaultProfile(
					shellLaunchConfig
				);
		}
	...
	private _splitTerminal(
		...
		if (
			...
			if (!group) {
				throw new Error(
					`Cannot split a terminal without a group ${parent}`
				);
			}
...
class TerminalEditorStyle extends Themable {
	...
	constructor(
		...
		this._register(
			toDisposable(() => container.removeChild(this._styleElement))
		);
		this.updateStyles();
	}
				...
				if (iconContribution) {
					...
					if (def) {
						css +=
							...
							`{content: '${
								def.fontCharacter
							}' !important; font-family: ${dom.asCSSPropertyValue(
								def.font?.id ?? "codicon"
							)} !important;}`;
					}
				}
