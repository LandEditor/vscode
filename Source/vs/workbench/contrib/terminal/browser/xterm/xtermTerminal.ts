	...
	implements IXtermTerminal, IDetachedXtermTerminal, IInternalXtermTerminal
{
	...
	private static _suggestedRendererType: "canvas" | "dom" | undefined =
		undefined;
	...
	get textureAtlas(): Promise<ImageBitmap> | undefined {
		const canvas =
			this._webglAddon?.textureAtlas || this._canvasAddon?.textureAtlas;
		if (!canvas) {
			return undefined;
		}
	...
	constructor(
		...
		if (this._terminalSuggestWidgetVisibleContextKey) {
			this._suggestAddon = this._register(
	...
	*getBufferReverseIterator(): IterableIterator<string> {
	...
	private _updateSmoothScrolling() {
		this.raw.options.smoothScrollDuration =
			this._configHelper.config.smoothScrolling &&
	...
	private _shouldLoadWebgl(): boolean {
		return (
			(!isSafari &&
				this._configHelper.config.gpuAcceleration === "auto" &&
				XtermTerminal._suggestedRendererType === undefined) ||
	...
	private _shouldLoadCanvas(): boolean {
		return (
			...
			this._configHelper.config.gpuAcceleration === "canvas"
		);
	}
	...
	private async _enableWebglRenderer(): Promise<void> {
		...
		try {
			...
			if (
				...
				this._configHelper.config.gpuAcceleration !== "off"
			) {
	...
	protected async _getCanvasAddonConstructor(): Promise<
		typeof CanvasAddonType
	> {
		if (!CanvasAddon) {
			CanvasAddon = (
	...
	private async _refreshImageAddon(): Promise<void> {
		...
		if (
			this._configHelper.config.enableImages &&
			(this._canvasAddon || this._webglAddon)
		) {
			if (!this._imageAddon) {
	...
	private async _measureRenderTime(): Promise<void> {
		...
		const textRenderLayer =
			this._core._renderService._renderer.value._renderLayers[0];
		const originalOnGridChanged = textRenderLayer?.onGridChanged;
		const evaluateCanvasRenderer = () => {
			// Discard first frame time as it's normal to take longer
			frameTimes.shift();

			const medianTime = frameTimes.sort((a, b) => a - b)[
	...
	private _getXtermTheme(theme?: IColorTheme): ITheme {
		...
		const backgroundColor =
			this._backgroundColorProvider.getBackgroundColor(theme);
		const cursorColor =
	...
	private async _updateUnicodeVersion(): Promise<void> {
		if (
			...
			this._configHelper.config.unicodeVersion === "11"
		) {
...
export function getXtermScaledDimensions(
	...
	const scaledCharWidth =
		font.charWidth * window.devicePixelRatio + font.letterSpacing;
	const cols = Math.max(
	...
	const scaledCharHeight = Math.ceil(
		font.charHeight * window.devicePixelRatio
	);
