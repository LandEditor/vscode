...
export class TerminalConfigHelper
	extends Disposable
	implements IBrowserTerminalConfigHelper
{
		...
		if (rect && rect.width && rect.height) {
			...
			if (this.config.gpuAcceleration === "off") {
				...
				this._lastFontMeasurement.charWidth =
					cssCellWidth -
					Math.round(letterSpacing) / window.devicePixelRatio;
			}
		}
	...
	async showRecommendations(
		...
		if (
			...
			basename(shellLaunchConfig.executable).toLowerCase() === "wsl.exe"
		) {
