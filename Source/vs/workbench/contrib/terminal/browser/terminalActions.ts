...
export function registerTerminalActions() {
	...
	registerActiveInstanceAction({
		...
		precondition: ContextKeyExpr.and(
			...
			TerminalContextKeys.terminalEditorActive.toNegated(),
			TerminalContextKeys.viewShowing
		),
	...
	registerTerminalAction({
		...
		title: {
			value: localize("quickAccessTerminal", "Switch Active Terminal"),
	...
	registerActiveInstanceAction({
		...
		run: (activeInstance, c) =>
			c.groupService.unsplitInstance(activeInstance),
	});
	...
	registerTerminalAction({
		...
		title: {
			value: localize("workbench.action.terminal.join", "Join Terminals"),
			original: "Join Terminals",
		},
		precondition: ContextKeyExpr.and(
			ContextKeyExpr.or(
	...
	registerActiveInstanceAction({
		...
		title: {
			value: localize("workbench.action.terminal.clear", "Clear"),
			original: "Clear",
		},
		precondition: ContextKeyExpr.or(
			TerminalContextKeys.processSupported,
	...
	registerActiveInstanceAction({
		...
		run: (instancactiveInstance) =>
			instancactiveInstance.toggleSizeToContentWidth(),
	});
	...
	registerTerminalAction({
		...
		run: (c, accessor) =>
			getSelectedInstances(accessor)?.[0].toggleSizeToContentWidth(),
	});
	...
	registerActiveInstanceAction({
		...
		run: (insactiveInstanceance) =>
			insactiveInstanceance.selectNextSuggestion(),
	});
...
function getSelectedInstances(
	...
	for (const selection of selections) {
		instances.push(
			terminalService.getInstanceFromIndex(selection) as ITerminalInstance
		);
	}
	return instances;
}
