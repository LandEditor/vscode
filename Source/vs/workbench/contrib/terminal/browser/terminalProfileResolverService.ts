	...
	implements ITerminalProfileResolverService
{
	...
	private _getUnresolvedAutomationShellProfile(
		...
		const automationProfile = this._configurationService.getValue(
			`terminal.integrated.automationProfile.${this._getOsKey(
				options.os
			)}`
		);
	...
	private _isValidShellArgs(
		shellArgs: unknown,
		os: OperatingSystem
	): shellArgs is string | string[] | undefined {
		...
		if (
			...
			shellArgs.every((e) => typeof e === "string")
		) {
			return true;
		}
		return false;
	...
	async createProfileFromShellAndShellArgs(
		shell?: unknown,
		shellArgs?: unknown
	): Promise<ITerminalProfile | string> {
		const detectedProfile =
			this._terminalProfileService.availableProfiles?.find((p) => {
				if (p.path !== shell) {
					return false;
				}
				...
				return (
					p.path === shell &&
					equals(p.args, (shellArgs || []) as string[])
				);
			});
		...
		const args = this._isValidShellArgs(shellArgs, this._primaryBackendOs!)
			? shellArgs
			: profile.args;
		...
		if (
			detectedProfile &&
			detectedProfile.profileName === createdProfile.profileName &&
			detectedProfile.path === createdProfile.path &&
			terminalProfileArgsMatch(detectedProfile.args, createdProfile.args)
		) {
