...
import type { ITreeNode, ITreeSorter } from "vs/base/browser/ui/tree/tree";
...
import type { IPrefixTreeNode } from "vs/base/common/prefixTree";
		...
		>
{
	...
	private doRender(
		...
		templateData.icon.className = `computed-state ${ThemeIcon.asClassName(
			icon!,
		)}`;
...
registerAction2(
	class TestCoverageChangePerTestFilterAction extends Action2 {
		...
		override run(accessor: ServicesAccessor): void {
			...
			const tests = [...coverage.perTestCoverageIDs].map(
				TestId.fromString,
			);
