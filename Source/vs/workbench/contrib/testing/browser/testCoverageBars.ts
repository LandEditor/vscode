...
import type { AbstractFileCoverage } from "vs/workbench/contrib/testing/common/testCoverage";
...
export class ManagedTestCoverageBars extends Disposable {
	...
	private attachHover(
		...
		) => string | IUpdatableHoverTooltipMarkdownString | undefined,
	) {
		this._register(
	...
	public setCoverageInfo(coverage: CoverageBarSource | undefined) {
		...
		if (!this._coverage) {
			...
			ds.add(
				toDisposable(() => this.options.container.removeChild(root)),
			);
	...
	private doRender(coverage: CoverageBarSource) {
		...
		const overallStat = calculateDisplayedStat(
			coverage,
			getTestingConfiguration(
				this.configurationService,
				TestingConfigKeys.CoveragePercent,
