...
} from "vs/workbench/contrib/codeEditor/browser/editorLineNumberMenu";
...
import {
	...
	TestDecorations,
} from "vs/workbench/contrib/testing/common/testingDecorations";
...
const createRunTestDecoration = (
	...
	let glyphMarginClassName =
		ThemeIcon.asClassName(icon) + " testing-run-glyph";
	if (retired) {
...
abstract class RunTestDecoration {
	...
	public replaceOptions(
		...
		this.editorDecoration.options = createRunTestDecoration(
	...
	implements ITestDecoration
{
	override getContextMenuActions() {
		...
		if (
			...
			allActions.push(
				new Action(
					"testing.gutter.runAll",
					localize("run all test", "Run All Tests"),
					undefined,
					undefined,
					() => this.defaultRun()
				)
		...
		if (
			...
			allActions.push(
				new Action(
					...
					localize("debug all test", "Debug All Tests"),
					undefined,
					undefined,
					() => this.defaultDebug()
				)
...
class TestMessageDecoration implements ITestDecoration {
	...
	constructor(
		...
		let inlineText = renderStringAsPlaintext(message).replace(
			lineBreakRe,
			" "
		);
		...
		options.after = {
			...
			inlineClassName: `test-message-inline-content test-message-inline-content-s${severity} ${
				this.contentIdClass
			} ${messageUri ? "test-message-inline-content-clickable" : ""}`,
		};
