	...
	implements IEditorContribution
{
	...
	private static readonly fileCoverageDecorations = new WeakMap<
		FileCoverage,
		...
		if (!CodeCoverageDecorations.showInline.get()) {
			...
			model.changeDecorations((e) => {
				for (const id of toEnable) {
					const { applyHoverOptions, options } =
						this.decorationIds.get(id)!;
					const dup = { ...options };
					...
					const applyHoverOptions = (
						...
						if (showMissIndicator) {
							target.after = {
								...
								inlineClassName: `coverage-deco-branch-miss-indicator ${ThemeIcon.asClassName(
									testingCoverageMissingBranch,
								)}`,
	...
	private async loadDetails(coverage: FileCoverage, textModel: ITextModel) {
		const existing =
			CodeCoverageDecorations.fileCoverageDecorations.get(coverage);
		if (existing) {
			return existing;
		}
		...
		try {
			const details = await coverage.details(
			...
			CodeCoverageDecorations.fileCoverageDecorations.set(
				coverage,
				model,
			);
			return model;
...
class CoverageToolbarWidget extends Disposable implements IOverlayWidget {
	...
	private setActions() {
		...
		const toggleAction = new ActionWithIcon(
			...
			undefined,
			() =>
				CodeCoverageDecorations.showInline.set(
					!CodeCoverageDecorations.showInline.get(),
					undefined,
				),
		);
		...
		if (coverage.isForTest) {
			const testItem = coverage.fromResult.getTestById(
				coverage.isForTest.id.toString(),
			);
	...
	private rerunTest() {
		...
		if (current) {
			...
			this.setActions();
			this.testService
				.runResolvedTests(current.fromResult.request)
				.finally(() => {
...
registerAction2(
	class FilterCoverageToTestInEditor extends Action2 {
		...
		run(
			...
			if (
				!coverage ||
				!(coverage.isForTest || coverage.perTestData?.size)
			) {
				return;
			}

			const options =
				coverage?.perTestData ??
				coverage?.isForTest?.parent.perTestData;
			if (!options) {
				return;
			}
			...
			const items: QuickPickInput<TItem>[] = [
				...
				...tests.map((item) => ({
					...
					description: coverUtils.labels.percentCoverage(item.tpc),
					item,
				})),
			];
