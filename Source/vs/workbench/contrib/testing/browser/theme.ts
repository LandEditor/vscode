...
export const testMessageSeverityColors: {
	...
	[TestMessageType.Output]: {
		decorationForeground: registerColor(
			...
			{
				dark: transparent(editorForeground, 0.5),
				light: transparent(editorForeground, 0.5),
				hcDark: transparent(editorForeground, 0.5),
				hcLight: transparent(editorForeground, 0.5),
			},
			localize(
				"testing.message.info.decorationForeground",
...
registerThemingParticipant((theme, collector) => {
	...
	.coverage-deco-inline.coverage-deco-hit.coverage-deco-hovered {
		background: ${theme
			.getColor(testingCoveredBackground)
			?.transparent(1.3)};
