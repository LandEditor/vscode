...
export class TestingExplorerView extends ViewPane {
	...
	private discoveryProgress = this._register(
		...
		const ab = this._register(
			new ActionBar(this.elements.rerun, {
				actionViewItemProvider: (action) =>
					createActionViewItem(instantiationService, action),
			})
		);
		ab.push(
...
class TestingExplorerViewModel extends Disposable {
	...
	public projection = this._register(
		...
		const onEditorChange = () => {
			if (editorService.activeEditor instanceof DiffEditorInput) {
				this.filter.filterToDocumentUri(
					editorService.activeEditor.primary.resource
				);
			} else {
				this.filter.filterToDocumentUri(
					editorService.activeEditor?.resource
				);
			}
