...
} from "vs/workbench/contrib/testing/browser/theme";
import {
	AutoOpenPeekViewWhen,
	TestingConfigKeys,
	getTestingConfiguration,
} from "vs/workbench/contrib/testing/common/configuration";
...
import {
	...
	maxCountPriority,
	resultItemParents,
} from "vs/workbench/contrib/testing/common/testResult";
import {
	ITestResultService,
	ResultChangeEvent,
} from "vs/workbench/contrib/testing/common/testResultService";
...
import {
	IShowResultOptions,
	ITestingPeekOpener,
} from "vs/workbench/contrib/testing/common/testingPeekOpener";
import {
	cmpPriority,
	isFailedState,
} from "vs/workbench/contrib/testing/common/testingStates";
import {
	ParsedTestUri,
	TestUriType,
	buildTestUri,
	...
	implements ITestingPeekOpener
{
	...
	/** @inheritdoc */
	public readonly historyVisible = MutableObservableValue.stored(
		this._register(
	...
	private getFailedCandidateMessage(test: TestResultItem) {
		let best:
			| { taskId: number; index: number; message: ITestMessage }
			| undefined;
...
class TestResultsViewContent extends Disposable {
	...
	public fillBody(containerElement: HTMLElement): void {
		...
		const messageContainer = (this.messageContainer = dom.append(
			containerElement,
			dom.$(".test-output-peek-message-container")
		));
		...
		this.messageContextKeyService = this._register(
			this.contextKeyService.createScoped(this.messageContainer)
		);
...
class TestResultsPeek extends PeekViewWidget {
	...
	private applyTheme(theme: IColorTheme) {
		const borderColor =
			theme.getColor(testingPeekBorder) || Color.transparent;
		const headerBg =
			theme.getColor(testingPeekHeaderBackground) || Color.transparent;
		this.style({
	...
	protected override _fillContainer(container: HTMLElement): void {
		if (!this.scopedContextKeyService) {
			...
			TestingContextKeys.isInPeek
				...
				.set(true);
			const instaService = this.instantiationService.createChild(
				new ServiceCollection([
					IContextKeyService,
	...
	public setModel(subject: InspectSubject): Promise<void> {
		...
		this.show(
			subject.revealLocation.range,
			TestResultsPeek.lastHeightInLines || hintMessagePeekHeight(message)
		);
		this.editor.revealPositionNearTop(
			subject.revealLocation.range.getStartPosition(),
			ScrollType.Smooth
		);
	...
	public async showInPlace(subject: InspectSubject) {
		if (subject instanceof MessageSubject) {
			const message = subject.message;
			this.setTitle(
				firstLine(renderStringAsPlaintext(message.message)),
...
export class TestResultsView extends ViewPane {
	private readonly content = this._register(
		this.instantiationService.createInstance(
			TestResultsViewContent,
			undefined,
			{
	...
	protected override renderBody(container: HTMLElement): void {
		...
		this.content.onDidRequestReveal((subject) =>
...
class ScrollableMarkdownMessage extends Disposable {
	...
	constructor(
		...
		this.scrollable = this._register(
			new DomScrollableElement(rendered.element, {
	...
	implements IPeekOutputRenderer
{
	...
	private readonly textPreview = this._register(
		new MutableDisposable<ScrollableMarkdownMessage>()
	);
...
class TerminalMessagePeek extends Disposable implements IPeekOutputRenderer {
	...
	private async updateForTestSubject(
		...
		const terminal = await this.updateGenerically<ITaskRawOutput>({
			...
			*doInitialWrite(output, results) {
			...
			doListenForMoreData: (output, result, { xterm }) =>
				result.onChange((e) => {
					if (
	...
	private updateForTaskSubject(subject: TaskSubject) {
		return this.updateGenerically<ITestRunTaskResults>({
			...
			doListenForMoreData: (task, _result, { xterm }) =>
				task.output.onDidWriteData((e) => xterm.write(e.buffer)),
		});
	}
	...
	private async updateGenerically<T>(opts: {
		...
		doListenForMoreData: (
			target: T,
			result: LiveTestResult,
			terminal: IDetachedTerminalInstance
		...
		this.outputDataListener.value =
			result instanceof LiveTestResult
				? opts.doListenForMoreData(target, result, terminal)
				: undefined;
...
class TaskElement implements ITreeElement {
	...
	constructor(
		...
		this.label =
			this.task.name ?? localize("testUnnamedTask", "Unnamed Task");
	}
}
	...
	constructor(
		...
		const getTaskChildren = (
			...
			const tests = Iterable.filter(
				taskElem.results.tests,
				(test) =>
					test.tasks[taskElem.index].state >=
						TestResultState.Running ||
					test.tasks[taskElem.index].messages.length > 0
			);

			return Iterable.map(tests, (test) => ({
				element: taskElem.itemsCache.getOrCreate(
					test,
					() =>
						new TestCaseElement(
							taskElem.results,
		...
		ICompressibleTreeRenderer<ITreeElement, FuzzyScore, TemplateData>
{
	...
	private doRenderInner(
		...
		templateData.icon.className = `computed-state ${
...
class TreeActionsProvider {
	...
	public provideActionBar(element: ITreeElement) {
		...
		const capabilities = test
			? this.testProfileService.capabilitiesForTest(test)
			: 0;
