...
import {
	...
	testStubs,
} from "vs/workbench/contrib/testing/test/common/testStubs";
...
suite("Workbench - Test Results Service", () => {
	...
	setup(async () => {
		...
		const ok = await Promise.race([
			Promise.resolve(tests.expand(tests.root.id, Infinity)).then(
				() => true
			),
	...
	suite("LiveTestResult", () => {
		test("is empty if no tests are yet present", async () => {
			assert.deepStrictEqual(
				getLabelsIn(
					new TestLiveTestResult("foo", false, defaultOpts(["id-a"]))
						.tests
				),
				[]
			);
	...
	suite("service", () => {
		...
		class TestTestResultService extends TestResultService {
			protected override persistScheduler = {
		...
		test("clears results but keeps ongoing tests", async () => {
			...
			const r2 = results.push(
				new LiveTestResult("", false, defaultOpts([]))
			);
			results.clear();
