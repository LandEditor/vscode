...
import { type ITransaction, observableSignal } from "vs/base/common/observable";
import {
	type IPrefixTreeNode,
	WellDefinedPrefixTree,
} from "vs/base/common/prefixTree";
import { URI } from "vs/base/common/uri";
import type { IUriIdentityService } from "vs/platform/uriIdentity/common/uriIdentity";
import type { TestId } from "vs/workbench/contrib/testing/common/testId";
import type { LiveTestResult } from "vs/workbench/contrib/testing/common/testResult";
import {
				...
				if (chain.length === canonical.length) {
					...
					if (isPerTestCoverage) {
						const v = (node.value ??= new FileCoverage(
							IFileCoverage.empty(String(incId++), coverage.uri),
							result,
							this.accessor,
						));
						assert(
							v instanceof FileCoverage,
							"coverage is unexpectedly computed",
						);
						v.perTestData ??= new Map();
						const perTest = new FileCoverage(
							coverage,
							result,
							this.accessor,
						);
	...
	public filterTreeForTest(testId: TestId) {
		...
		for (const node of this.tree.values()) {
			if (node instanceof FileCoverage) {
				...
				const chain: IPrefixTreeNode<AbstractFileCoverage>[] = [];
				tree.mutatePath(
					this.treePathForUri(fileData.uri, /* canonical = */ false),
					(node) => {
						chain.push(node);
						...
						if (chain.length === canonical.length) {
							...
							node.value ??= new BypassedFileCoverage(
								this.treePathToUri(
									canonical.slice(0, chain.length),
								),
								fileData.fromResult,
