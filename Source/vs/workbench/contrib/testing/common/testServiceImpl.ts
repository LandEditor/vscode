...
export class TestService extends Disposable implements ITestService {
	 ...
	 * @inheritdoc
	 */
	public readonly showInlineOutput = MutableObservableValue.stored(
		this._register(
	...
	constructor(
		...
		this.providerCount =
			TestingContextKeys.providerCount.bindTo(contextKeyService);
		this.canRefreshTests =
			TestingContextKeys.canRefreshTests.bindTo(contextKeyService);
	...
	public async runTests(
		...
		for (const profile of this.testProfiles.getGroupDefaultProfiles(
			...
			const testIds = req.tests
				.filter((t) => canUseProfileWithTest(profile, t))
				.map((t) => t.item.extId);
			if (testIds.length) {
	...
	public async syncTests(): Promise<void> {
		...
		try {
			await Promise.all(
				[...this.testControllers.values()].map((c) =>
	...
	public registerTestController(
		...
		disposable.add(
			controller.canRefresh.onDidChange(this.updateCanRefresh, this)
		);
	...
	private updateCanRefresh() {
		this.canRefreshTests.set(
			Iterable.some(
				this.testControllers.values(),
				(t) => t.canRefresh.value
			)
		);
	}
}
