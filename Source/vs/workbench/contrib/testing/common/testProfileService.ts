	...
	implements ITestProfileService
{
	...
	private readonly preferredDefaults: StoredValue<{
		[K in TestRunProfileBitset]?: {
			controllerId: string;
			profileId: number;
		}[];
	}>;
	...
	constructor(
		...
		this.preferredDefaults = this._register(
			new StoredValue(
				{
			...
			?.map(
				(p) =>
					this.controllerProfiles
						...
						?.profiles.find(
							(c) =>
								c.profileId === p.profileId && c.group === group
						)
			)
			.filter(isDefined);
	...
	public setGroupDefaultProfiles(
		...
		if (
			...
			for (const [ctrlId, { profiles: ctrlProfiles }] of this
				.controllerProfiles) {
				const labels = new Set(
					profiles
						.filter((p) => p.controllerId === ctrlId)
						.map((p) => p.label)
				);
				const nextByLabels = ctrlProfiles.filter(
					(p) => labels.has(p.label) && p.group === otherGroup
				);
				if (nextByLabels.length) {
					newDefaults = newDefaults.filter(
						(p) => p.controllerId !== ctrlId
					);
					newDefaults.push(
						...nextByLabels.map((p) => ({
