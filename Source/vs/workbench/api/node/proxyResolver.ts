...
export function connectProxyResolver(
	...
	patchGlobalFetch(
		...
		initData,
		resolveProxyURL,
		params.lookupProxyAuthorization!,
		getOrLoadAdditionalCertificates.bind(undefined, params),
		disposables,
	);
	...
	if (!(globalThis as any).__vscodeOriginalFetch) {
		...
		globalThis.fetch = async function fetch(
			...
			function getRequestProperty(
				name: keyof Request & keyof RequestInit,
			) {
		...
		const modifiedInit = {
			...
			dispatcher: new undici.ProxyAgent({
				...
				clientFactory: (
					origin: URL,
					opts: object,
				): undiciType.Dispatcher =>
					(new undici.Pool(origin, opts) as any).compose(
						(dispatch: undiciType.Dispatcher["dispatch"]) => {
							class ProxyAuthHandler extends undici.DecoratorHandler {
								...
								constructor(
									...
									private handler: undiciType.Dispatcher.DispatchHandlers,
								) {
									super(handler);
								}
									...
									(async () => {
										try {
											const proxyAuthorization =
												await lookupProxyAuthorization(
													proxyURL!,
											...
											if (proxyAuthorization) {
												if (!this.options.headers) {
													...
													const i =
														this.options.headers.findIndex(
															(value, index) =>
																index % 2 ===
																	0 &&
																value.toLowerCase() ===
												...
												this.handler.onError?.(
													new undici.errors.RequestAbortedError(
														`Proxy response (407) ?.== 200 when HTTP Tunneling`,
													),
												); // Mimick undici's behavior
											}
										} catch (err) {
...
function patchUndici(undici: typeof undiciType) {
	...
	const patchedAgent = function PatchedAgent(
		...
		(agent as any)[agentOptions] = {
			...opts,
			...(opts?.connect && typeof opts?.connect === "object"
	...
	const patchedProxyAgent = function PatchedProxyAgent(
		...
		(proxyAgent as any)[proxyAgentOptions] =
			typeof opts === "string"
				? opts
				: {
						...opts,
	...
	if (originalAgentOptions) {
		...
		requestCA =
			(originalAgentOptions.connect &&
				...
				"ca" in originalAgentOptions.connect &&
