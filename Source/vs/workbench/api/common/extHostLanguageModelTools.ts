	...
	implements ExtHostLanguageModelToolsShape
{
	...
	async $invokeTool(
		...
		const options: vscode.LanguageModelToolInvocationOptions<Object> = {
			...
			requestedContentTypes: dto.requestedContentTypes,
		};
		...
		for (const key of Object.keys(extensionResult)) {
			...
			if (value instanceof Promise) {
				...
				!options.requestedContentTypes.includes(key) &&
				key !== "toString"
			) {
				...
				throw new Error(
					`Tool result for '${key}' was not requested from ${dto.toolId}.`,
				);
			}
		}
	...
	async $prepareToolInvocation(
		...
		const result = await item.tool.prepareToolInvocation(
			{ participantName, parameters },
			token,
		);
		if (!result) {
			return undefined;
