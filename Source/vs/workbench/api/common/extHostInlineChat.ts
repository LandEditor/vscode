...
class SessionWrapper {
	...
	constructor(readonly session: vscode.InteractiveEditorSession) {}
}
...
export class ExtHostInteractiveEditor implements ExtHostInlineChatShape {
	...
	registerProvider(
		...
		this._proxy.$registerInteractiveEditorProvider(
			wrapper.handle,
			metadata?.label ?? extension.displayName ?? extension.name,
			extension.identifier.value,
			typeof provider.handleInteractiveEditorResponseFeedback ===
				"function",
			typeof provider.provideFollowups === "function",
			metadata?.supportReportIssue ?? false
		);
		return toDisposable(() => {
	...
	async $prepareSession(
		...
		if (!entry) {
			this._logService.warn(
				"CANNOT prepare session because the PROVIDER IS GONE"
			);
			return undefined;
		}
	...
	async $provideResponse(
		...
		const progress: vscode.Progress<vscode.InteractiveEditorProgressItem> =
			{
				report: async (value) => {
					if (!request.live && value.edits?.length) {
						throw new Error(
							"Progress reporting is only supported for live sessions"
						);
					}
		...
		const task = Promise.resolve(
			entry.provider.provideInteractiveEditorResponse(
				sessionData.session,
				apiRequest,
				progress,
				token
		...
		const message =
			contents !== undefined
				? typeConvert.MarkdownString.from(contents)
				: undefined;
		if (edits instanceof extHostTypes.WorkspaceEdit) {
			...
			return {
				...
				id,
				type: InlineChatResponseType.EditorEdit,
				edits: (<vscode.TextEdit[]>edits).map(
					typeConvert.TextEdit.from
				),
				message,
			};
		}
	...
	async $provideFollowups(
		...
		if (entry && response && entry.provider.provideFollowups) {
			const task = Promise.resolve(
				entry.provider.provideFollowups(
					sessionData.session,
					response,
					token
				)
	...
	$handleFeedback(
		...
		if (entry && response) {
			const apiKind =
				typeConvert.InteractiveEditorResponseFeedbackKind.to(kind);
			entry.provider.handleInteractiveEditorResponseFeedback?.(
				sessionData.session,
				response,
				apiKind
			);
	...
	private static _isEditResponse(
		thing: any
	): thing is vscode.InteractiveEditorResponse {
		return (
			typeof thing === "object" &&
			typeof (<vscode.InteractiveEditorResponse>thing).edits === "object"
		);
	}
}
