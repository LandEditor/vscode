...
class TestRunTracker extends Disposable {
	...
	private readonly coverageEmitter = this._register(
		new Emitter<{
			runId: string;
			taskId: string;
			coverage: TestRunCoverageBearer | undefined;
		}>(),
	);
...
export class TestRunCoordinator {
	...
	private getTracker(
		...
		const coverageListener = tracker.onDidCoverage(
			({ runId, taskId, coverage }) => {
				if (!coverageReports) {
					coverageReports = { runId, coverage: new Map() };
					this.coverageReports.unshift(coverageReports);
					if (
						this.coverageReports.length >
						KEEP_N_LAST_COVERAGE_REPORTS
					) {
	...
	public async provideFileCoverage(
		...
		if (!this.fileCoverage) {
			this.fileCoverage = (async () =>
				this.provider.provideFileCoverage(token))();
		}
	...
	public async resolveFileCoverage(
		...
		if (!file.detailedCoverage) {
			file = fileCoverage[index] =
				(await this.provider.resolveFileCoverage?.(file, token)) ??
				file;
		}
