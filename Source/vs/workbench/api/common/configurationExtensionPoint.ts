			...
			(extension) => {
				...
				for (const key of Object.keys(overrides)) {
					if (!OVERRIDE_PROPERTY_REGEX.test(key)) {
						const registeredPropertyScheme =
							registeredProperties[key];
						if (
		...
		if (properties) {
			...
			for (const key in properties) {
				...
				if (propertyConfiguration.scope) {
					if (
						propertyConfiguration.scope.toString() === "application"
					) {
						propertyConfiguration.scope =
							ConfigurationScope.APPLICATION;
					} else if (
						propertyConfiguration.scope.toString() === "machine"
					) {
						propertyConfiguration.scope =
							ConfigurationScope.MACHINE;
					} else if (
						propertyConfiguration.scope.toString() === "resource"
					) {
						propertyConfiguration.scope =
							ConfigurationScope.RESOURCE;
					} else if (
						propertyConfiguration.scope.toString() ===
						"machine-overridable"
					) {
						propertyConfiguration.scope =
							ConfigurationScope.MACHINE_OVERRIDABLE;
