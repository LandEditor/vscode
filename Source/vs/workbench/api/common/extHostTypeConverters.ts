...
export namespace MarkdownString {
	...
	export function from(
		...
		renderer.image = (href) =>
			typeof href === "string"
				? collectUri(htmlContent.parseHrefAndDimensions(href).href)
				: "";
				...
				(subArr) =>
					...
					subArr.every(
						(docRef) =>
							...
							"uri" in docRef &&
							URI.isUri(docRef.uri) &&
							"version" in docRef &&
							typeof docRef.version === "number" &&
							"ranges" in docRef &&
	...
	export function fromFile(coverage: vscode.FileCoverage): IFileCoverage {
		return {
			...
			branch:
				...
				fromCoveredCount(coverage.branchCoverage),
			function:
				coverage.functionCoverage &&
				fromCoveredCount(coverage.functionCoverage),
...
export namespace ChatReplyFollowup {
	...
	export function from(
		followup: vscode.InteractiveSessionReplyFollowup
	): IChatReplyFollowup {
		return {
			kind: "reply",
			message: followup.message,
...
export namespace ChatFollowup {
	export function from(
		followup: string | vscode.InteractiveSessionFollowup
	): IChatFollowup {
...
export namespace ChatMessageRole {
	export function to(
		...
		switch (role) {
			...
			case chatProvider.ChatMessageRole.Function:
...
export namespace ChatResponseProgress {
	export function from(
		extension: IExtensionDescription,
		progress: vscode.InteractiveProgress | vscode.ChatAgentExtendedProgress
	): extHostProtocol.IChatResponseProgressDto {
		if ("placeholder" in progress && "resolvedContent" in progress) {
			...
			return {
				inlineReference:
					"uri" in progress.inlineReference
						? {
