...
export class ExtHostAuthentication implements ExtHostAuthenticationShape {
	...
	$setProviders(
		providers: vscode.AuthenticationProviderInformation[],
	): Promise<void> {
	...
	async getSessions(
		...
		const sortedScopes = [...scopes].sort().join(" ");
		return await this._getSessionsTaskSingler.getOrCreate(
			`${extensionId} ${sortedScopes}`,
			async () => {
				await this._proxy.$ensureProvider(providerId);
				const extensionName =
					requestingExtension.displayName || requestingExtension.name;
				return this._proxy.$getSessions(
					providerId,
					scopes,
					extensionId,
					extensionName,
	...
	$createSession(
		providerId: string,
		scopes: string[],
		options: vscode.AuthenticationProviderCreateSessionOptions,
	): Promise<vscode.AuthenticationSession> {
		const providerData = this._authenticationProviders.get(providerId);
		if (providerData) {
			return Promise.resolve(
				providerData.provider.createSession(scopes, options),
			);
		}
	...
	$removeSession(providerId: string, sessionId: string): Promise<void> {
		...
		if (providerData) {
			return Promise.resolve(
				providerData.provider.removeSession(sessionId),
			);
		}
