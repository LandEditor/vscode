	...
	implements ExtHostTaskShape, IExtHostTask
{
	...
	protected async getTaskExecution(
		...
		const createdResult: Promise<TaskExecutionImpl> = new Promise(
			(resolve, reject) => {
				function resolvePromiseWithCreatedTask(
					that: ExtHostTaskBase,
					execution: tasks.ITaskExecutionDTO,
					taskToCreate: vscode.Task | types.Task | undefined,
				) {
					if (!taskToCreate) {
		...
		return createdResult.then(
			(executionCreatedResult) => {
