...
class CodeLensAdapter {
	...
	async resolveCodeLens(
		...
		symbol.command = this._commands.toInternal(
			resolvedLens.command ?? CodeLensAdapter._badCmd,
			disposables
		);
		return symbol;
	}
...
class DocumentPasteEditProvider {
	...
	async providePasteEdits(
		...
		dataTransferDto: extHostProtocol.DataTransferDTO,
		token: CancellationToken
	): Promise<undefined | extHostProtocol.IPasteEditDto> {
		...
		const edit = await this._provider.provideDocumentPasteEdits(
			doc,
			vscodeRanges,
			dataTransfer,
			token
		...
		return {
			...
			yieldTo: edit.yieldTo?.map((yTo) => {
				return "mimeType" in yTo
					? yTo
					: {
							providerId:
								DocumentPasteEditProvider.toInternalProviderId(
									yTo.extensionId,
									yTo.providerId
								),
					  };
...
class RenameAdapter {
	...
	async provideRenameEdits(
		...
		try {
			...
			if (rejectReason) {
				return <extHostProtocol.IWorkspaceEditDto>{
					rejectReason,
					edits: undefined!,
				};
			} else {
	...
	async resolveRenameLocation(
		...
		try {
			...
			const rejectReason = RenameAdapter._asMessage(err);
			if (rejectReason) {
				return <languages.RenameLocation & languages.Rejection>{
					rejectReason,
					range: undefined!,
					text: undefined!,
				};
	...
	async provideInlayHints(
		...
		if (!Array.isArray(hints) || hints.length === 0) {
			...
			this._logService.trace(
				`[InlayHints] NO inlay hints from '${this._extension.identifier.value}' for ${ran}`
			);
			return undefined;
		}
		if (token.isCancellationRequested) {
		...
		this._logService.trace(
			`[InlayHints] ${result.hints.length} inlay hints from '${this._extension.identifier.value}' for ${ran}`
		);
		return result;
	}
	...
	private _convertInlayHint(
		...
		if (typeof hint.label === "string") {
			...
			result.label = hint.label.map((part) => {
				const result: languages.InlayHintLabelPart = {
					label: part.value,
				};
				result.tooltip = typeConvert.MarkdownString.fromStrict(
					part.tooltip
				);
				...
				if (part.command) {
					result.command = this._commands.toInternal(
						part.command,
						disposables
					);
				}
...
class DocumentOnDropEditAdapter {
	...
	async provideDocumentOnDropEdits(
		...
		dataTransferDto: extHostProtocol.DataTransferDTO,
		token: CancellationToken
	): Promise<extHostProtocol.IDocumentOnDropEditDto | undefined> {
		const doc = this._documents.getDocument(uri);
		const pos = typeConvert.Position.to(position);
		const dataTransfer = typeConvert.DataTransfer.toDataTransfer(
			dataTransferDto,
		...
		const edit = await this._provider.provideDocumentDropEdits(
			doc,
			pos,
			dataTransfer,
			token
		...
		return {
			...
			yieldTo: edit.yieldTo?.map((yTo) => {
				return "mimeType" in yTo
					...
					: {
							providerId:
								DocumentOnDropEditAdapter.toInternalProviderId(
									yTo.extensionId,
									yTo.providerId
								),
					  };
	...
	implements extHostProtocol.ExtHostLanguageFeaturesShape
{
			...
			.finally(() => {
				if (!doNotLog) {
					this._logService.trace(
						`[${
							data.extension.identifier.value
						}] provider DONE after ${Date.now() - t1}ms`
					);
				}
			});
	...
	$provideHover(
		...
		return this._withAdapter(
			...
			HoverAdapter,
			(adapter) =>
				adapter.provideHover(URI.revive(resource), position, token),
			undefined,
			token
		);
	}
	...
	registerCodeActionProvider(
		...
		this._proxy.$registerQuickFixSupport(
			handle,
			this._transformDocumentSelector(selector, extension),
			{
	...
	registerDocumentOnDropEditProvider(
		...
		const id =
			isProposedApiEnabled(extension, "dropMetadata") && metadata
				? DocumentOnDropEditAdapter.toInternalProviderId(
	...
	registerDocumentPasteEditProvider(
		...
		const internalId = DocumentPasteEditProvider.toInternalProviderId(
			extension.identifier.value,
			metadata.id
		);
