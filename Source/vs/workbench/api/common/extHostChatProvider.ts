...
export class ExtHostChatProvider implements ExtHostChatProviderShape {
	...
	registerProvider(
		...
		this._proxy.$registerProvider(handle, identifier, {
			extension,
			displayName: metadata.name ?? extension.value,
		});
	...
	async $provideChatResponse(
		...
		const progress = new Progress<vscode.ChatResponseFragment>(
			async (fragment) => {
	...
	private readonly _chatAccessAllowList = new ExtensionIdentifierMap<
		Promise<unknown>
	>();
	...
	async requestChatResponseProvider(
		...
		return {
			...
			makeRequest(messages, options, token) {
				...
				const requestPromise = that._proxy.$fetchResponse(
					from,
					identifier,
					requestId,
