	...
	implements MainThreadDebugServiceShape, IDebugAdapterFactory
{
	...
	constructor(
		...
		this._toDispose.add(
			this.debugService
				.getViewModel()
				.onDidFocusThread(({ thread, explicit, session }) => {
					if (session) {
		...
		this._toDispose.add(
			this.debugService
				.getViewModel()
				.onDidFocusStackFrame(({ stackFrame, explicit, session }) => {
					if (session) {
				...
				const rawbps = dto.lines.map(
					(l) =>
						<IBreakpointData>{
							...
							lineNumber: l.line + 1,
							column:
								l.character > 0 ? l.character + 1 : undefined, // a column value of 0 results in an omitted column attribute; see #46784
	...
	public $acceptDAError(
		...
		this.getDebugAdapter(handle).fireError(
			handle,
