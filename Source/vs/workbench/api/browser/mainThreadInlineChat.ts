...
export class MainThreadInlineChat implements MainThreadInlineChatShape {
	...
	private readonly _progresses = new Map<
		string,
		IProgress<IInlineChatProgressItem>
	>();
	...
	async $registerInteractiveEditorProvider(
		handle: number,
		label: string,
		debugName: string,
		...
		const unreg = this._inlineChatService.addProvider({
			...
			prepareInlineChatSession: async (model, range, token) => {
				const session = await this._proxy.$prepareSession(
					handle,
					model.uri,
					range,
					token
			...
			provideResponse: async (item, request, progress, token) => {
				...
				try {
					const result = await this._proxy.$provideResponse(
						handle,
						item,
						request,
						token
