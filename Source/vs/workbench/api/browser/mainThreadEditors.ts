...
export class MainThreadTextEditors implements MainThreadTextEditorsShape {
	...
	constructor(
		...
		private readonly _dirtyDiffModelService: IDirtyDiffModelService,
		@IUriIdentityService
	...
	private _getTextEditorDiffInformation(
		...
		const editorChangesObs = derived<
			IObservable<
				{ original: URI; modified: URI; changes: IChange[] } | undefined
			>
		>((reader) => {
			...
			return observableFromEvent(this, dirtyDiffModel.onDidChange, () => {
				const scmQuickDiff = dirtyDiffModel.quickDiffs.find(
					(diff) => diff.isSCM === true,
				);
				...
				const scmQuickDiffChanges =
					dirtyDiffModel.mapChanges.get(scmQuickDiff.label) ?? [];

				const changes = scmQuickDiffChanges.map(
					(index) => dirtyDiffModel.changes[index].change,
				);
		...
		return derivedOpts(
			...
			(reader) => {
				...
				const changes: ITextEditorChange[] = editorChanges.changes.map(
					(change) => [
