					...
					return [
						{
							...
							lineRangeMappings: changes.map(
								(change) => change as LineRangeMapping,
							),
						},
					];
				});
		...
		return derivedOpts(
			...
			(reader) => {
				...
				return editorChanges.map((change) => {
					const changes: ITextEditorChange[] =
						change.lineRangeMappings.map((change) => [
