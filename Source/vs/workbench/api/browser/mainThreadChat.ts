...
export class MainThreadChat extends Disposable implements MainThreadChatShape {
	...
	private readonly _activeRequestProgressCallbacks = new Map<
		string,
		(
			progress: IChatProgress
		) => DeferredPromise<string | IMarkdownString> | void
	>();
	...
	$transferChatSession(sessionId: number, toWorkspace: UriComponents): void {
		...
		const widget =
			this._chatWidgetService.getWidgetBySessionId(sessionIdStr);
	...
	async $registerChatProvider(handle: number, id: string): Promise<void> {
		...
		const unreg = this._chatService.registerProvider({
			...
			prepareSession: async (initialState, token) => {
				const session = await this._proxy.$prepareChat(
					handle,
					initialState,
					token
				);
			...
			provideReply: async (request, progress, token) => {
				...
				try {
					...
					const dto = await this._proxy.$provideReply(
						handle,
						request.session.id,
						requestDto,
						token
			...
			provideSlashCommands: (session, token) => {
				return this._proxy.$provideSlashCommands(
					handle,
					session.id,
					token
				);
			...
			removeRequest: (session, requestId) => {
				return this._proxy.$removeRequest(
					handle,
					session.id,
					requestId
				);
	...
	async $acceptResponseProgress(
		handle: number,
		sessionId: number,
		progress: IChatResponseProgressDto,
		...
		if ("placeholder" in progress) {
			...
			this._activeRequestProgressCallbacks.get(id)?.({
				...progress,
