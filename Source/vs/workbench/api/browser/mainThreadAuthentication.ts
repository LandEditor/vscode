	...
	implements IAuthenticationProvider
{
	...
	public manageTrustedExtensions(accountName: string) {
		...
		const quickPick =
			this.quickInputService.createQuickPick<TrustedExtensionsQuickPickItem>();
	...
	implements MainThreadAuthenticationShape
{
	...
	private async doGetSession(
		...
		if (options.createIfNone || options.forceNewSession) {
			...
			if (sessions?.length && !options.forceNewSession) {
				session = supportsMultipleAccounts
					? await this.authenticationService.selectSession(
							providerId,
							extensionId,
							extensionName,
							scopes,
				...
				if (
					typeof options.forceNewSession === "object" &&
					options.forceNewSession.sessionToRecreate
				) {
					sessionToRecreate = options.forceNewSession
						.sessionToRecreate as AuthenticationSession;
				} else {
		...
		if (!options.silent) {
			...
			sessions.length
				? this.authenticationService.requestSessionAccess(
						providerId,
						extensionId,
						extensionName,
						scopes,
