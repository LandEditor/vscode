...
export class MainThreadTesting
	extends Disposable
	implements MainThreadTestingShape, ITestRootProvider
{
	...
	$addTestsToRun(
		...
		this.withLiveRun(runId, (r) =>
			r.addTestChainToRun(controllerId, tests.map(ITestItem.deserialize))
		);
	}
	...
	$signalCoverageAvailable(runId: string, taskId: string): void {
		this.withLiveRun(runId, (run) => {
			...
			(task.coverage as MutableObservableValue<TestCoverage>).value =
				new TestCoverage({
					provideFileCoverage: async (token) =>
						revive<IFileCoverage[]>(
							await this.proxy.$provideFileCoverage(
								runId,
								taskId,
								token
							)
					...
					resolveFileCoverage: (i, token) =>
						this.proxy.$resolveFileCoverage(
							runId,
							taskId,
							i,
							token
		...
		if (r && r instanceof LiveTestResult) {
			for (const message of messages) {
				r.appendMessage(
					testId,
					taskId,
					ITestMessage.deserialize(message)
				);
			}
		}
	}
	...
	public $publishDiff(
		...
		this.testService.publishDiff(
			controllerId,
			diff.map(TestsDiffOp.deserialize)
		);
	}
