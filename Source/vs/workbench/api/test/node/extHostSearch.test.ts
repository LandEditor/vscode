...
suite("ExtHostSearch", () => {
	...
	async function registerTestTextSearchProvider(
		...
		disposables.add(
			extHostSearch.registerTextSearchProvider(scheme, provider)
		);
	...
	async function registerTestFileSearchProvider(
		...
		disposables.add(
			extHostSearch.registerFileSearchProvider(scheme, provider)
		);
		await rpcProtocol.sync();
	}
	...
	async function runTextSearch(
		...
		await rpcProtocol.sync();
		const results = (<IRawFileMatch2[]>mockMainThreadSearch.results).map(
			(r) => ({
				...r,
				...{
		...
		test("all provider calls get global include/excludes", async () => {
			await registerTestFileSearchProvider({
				provideFileSearchResults(
					...
					assert(
						options.excludes.length === 2 &&
							options.includes.length === 2,
				...
				provideFileSearchResults(
					...
					if (options.folder.fsPath === rootFolderA.fsPath) {
						reportedResults = [
							...
							joinPath(rootFolderA, relativePath)
						);
					} else {
						reportedResults = [
							"fileB.ts",
							...
							joinPath(rootFolderB, relativePath)
						);
					}
				...
				results
					.sort((a, b) => {
						const compareKeyA =
							a.uri.toString() +
							": " +
							(extensionResultIsMatch(a)
								? a.preview.text
								: a.text);
						const compareKeyB =
							b.uri.toString() +
							": " +
							(extensionResultIsMatch(b)
								? b.preview.text
								: b.text);
