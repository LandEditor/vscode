...
suite("ExtHostTreeView", function () {
	...
	test("construct node tree", () => {
		return testObject
			.$getChildren("testNodeTreeProvider")
			.then((elements) => {
		...
		store.add(
			target.onRefresh.event(() => {
				testObject
					.$getChildren("testNodeTreeProvider")
					.then((elements) => {
						assert.deepStrictEqual(elements?.map((e) => e.handle), [
							"0/0:a//0:b",
						]);
						done();
					});
		...
		return testObject
			...
			.then(() =>
				treeView.reveal({ key: "aa" }).then(() => {
					...
					assert.deepStrictEqual(
						expected.parentChain,
						(<Array<any>>revealTarget.args[0][1]!.parentChain).map(
	...
	function loadCompleteTree(treeId: string, element?: string): Promise<null> {
		return testObject
			.$getChildren(treeId, element)
			.then(
				(elements) =>
					elements?.map((e) => loadCompleteTree(treeId, e.handle))
			)
			.then(() => null);
	}
	...
	function aNodeTreeDataProvider(): TreeDataProvider<{ key: string }> {
		return {
			getChildren: (element: { key: string }): { key: string }[] => {
				return getChildren(element ? element.key : undefined).map(
					(key) => getNode(key)
				);
			},
	...
	function aCompleteNodeTreeDataProvider(): TreeDataProvider<{
		key: string;
	}> {
		return {
			getChildren: (element: { key: string }): { key: string }[] => {
