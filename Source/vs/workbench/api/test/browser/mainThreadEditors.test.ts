...
suite("MainThreadEditors", () => {
	...
	test(`applyWorkspaceEdit returns false if model is changed by user`, () => {
		...
		return bulkEdits
			.$tryApplyWorkspaceEdit({ edits: [workspaceResourceEdit] })
			.then((result) => {
	...
	test(`issue #54773: applyWorkspaceEdit checks model version in race situation`, () => {
		...
		const p1 = bulkEdits
			.$tryApplyWorkspaceEdit({ edits: [workspaceResourceEdit1] })
			.then((result) => {
				// first edit request succeeds
				assert.strictEqual(result, true);
			});
		const p2 = bulkEdits
			.$tryApplyWorkspaceEdit({ edits: [workspaceResourceEdit2] })
			.then((result) => {
