...
suite("ExtHostLanguageFeatureCommands", function () {
	...
	suiteSetup(() => {
		...
		services.set(
			...
			new (class extends mock<IExtensionService>() {
				...
				override activationEventIsDone(
		...
		return rpcProtocol.sync().then(() => {
			return commands
				...
				.then((list) => {
					...
					assert.strictEqual(
						second.textEdit!.range.start.character,
						4
					);
					...
					assert.strictEqual(
						third.textEdit!.range.start.character,
						1
					);
							...
							signatures: [
								{
									...
									documentation: `${
										context.triggerKind ===
										1 /* vscode.SignatureHelpTriggerKind.Invoke */
											? "invoked"
											: "unknown"
	...
	test("Color provider", function () {
		...
		return rpcProtocol
			...
			.then(() => {
				return commands
					.executeCommand<vscode.ColorInformation[]>(
			...
			.then(() => {
				...
				return commands
					.executeCommand<vscode.ColorPresentation[]>(
	...
	test("TypeHierarchy, back and forth", async function () {
		...
		>("vscode.provideSupertypes", root[0]);
		assert.strictEqual(incoming.length, 1);
		assert.strictEqual(incoming[0].name, "SUPER");

		const outgoing = await commands.executeCommand<
			vscode.TypeHierarchyItem[]
		>("vscode.provideSubtypes", root[0]);
		assert.strictEqual(outgoing.length, 1);
