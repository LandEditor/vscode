...
suite("SnippetParser", () => {
	...
	test("Marker, toTextmateString() <-> identity", function () {
		function assertIdent(input: string): void {
			...
			function checkCheckChildren(marker1: Marker, marker2: Marker) {
				assert.ok(
					marker1 instanceof
						Object.getPrototypeOf(marker2).constructor
				);
				assert.ok(
					marker2 instanceof
						Object.getPrototypeOf(marker1).constructor
				);
	...
	test("Parser, choise marker", () => {
		...
		assert.strictEqual(
			(<Choice>placeholders[0].children[0]).options.length,
			3
		);
	...
	test("Parser, transform example", () => {
		...
		assert.strictEqual(
			(<FormatString>transform.children[0]).ifValue,
			" :="
		);
	...
	test("Parser, default placeholder values", () => {
		...
		assert.strictEqual(
			(<Text>(<Placeholder>p1).children[0]).toString(),
			"err"
		);
		...
		assert.strictEqual(
			(<Text>(<Placeholder>p2).children[0]).toString(),
			"err"
		);
	});
	...
	test("Parser, default placeholder values and one transform", () => {
		...
		assert.strictEqual(
			(<Text>(<Placeholder>p3).children[0]).toString(),
			"err"
		);
		...
		assert.strictEqual(
			(<Text>(<Placeholder>p4).children[0]).toString(),
			"err"
		);
