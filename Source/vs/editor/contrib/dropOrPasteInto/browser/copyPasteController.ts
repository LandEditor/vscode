		...
		const p = createCancelablePromise(async (token) => {
			...
			try {
				...
				if (preference) {
					...
					type ItemWithEdit = IQuickPickItem & {
					...
					const selected =
						await this._quickInputService.pick<ItemWithEdit>(
							[
								...
								...(CopyPasteController._configureDefaultAction
									? [
											upcast<IQuickPickSeparator>({
												type: "separator",
											}),
											{
												label: CopyPasteController
