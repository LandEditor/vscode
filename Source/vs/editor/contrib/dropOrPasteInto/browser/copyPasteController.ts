...
import {
	...
	sortEditsByYieldTo,
} from "vs/editor/contrib/dropOrPasteInto/browser/edit";
import {
	CodeEditorStateFlag,
	...
	implements IEditorContribution
{
	...
	private isPasteAsEnabled(): boolean {
		return (
			this._editor.getOption(EditorOption.pasteAs).enabled &&
		...
		const p = createCancelablePromise(async (token) => {
			...
			try {
				...
				const supportedProviders = allProviders.filter((provider) =>
					isSupportedPasteProvider(provider, dataTransfer)
				);
				if (
					!supportedProviders.length ||
					(supportedProviders.length === 1 &&
						supportedProviders[0].id === "text") // Only our default text provider is active
				) {
				...
				const providerEdits = await this.getPasteEdits(
					supportedProviders,
					dataTransfer,
					model,
					selections,
				...
				if (
					providerEdits.length === 1 &&
					providerEdits[0].providerId === "text"
				) {
					await this.applyDefaultPasteHandler(
						dataTransfer,
						metadata,
	...
	private async mergeInDataFromCopy(
		...
		if (
			metadata?.id &&
			this._currentCopyOperation?.handle === metadata.id
		) {
			const toMergeDataTransfer =
				await this._currentCopyOperation.dataTransferPromise;
	...
	private async getPasteEdits(
		...
		selections: readonly Selection[],
		token: CancellationToken
	): Promise<Array<DocumentPasteEdit & { providerId: string }>> {
		const results = await raceCancellation(
			Promise.all(
				providers.map(async (provider) => {
					try {
						const edit = await provider.provideDocumentPasteEdits?.(
							model,
							selections,
							dataTransfer,
							token
...
function isSupportedPasteProvider(
