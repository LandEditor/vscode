...
} from "vs/editor/contrib/inlineCompletions/browser/inlineCompletionsSource";
...
export class InlineCompletionsModel extends Disposable {
	...
	private readonly _isActive = observableValue<
		boolean,
		InlineCompletionTriggerKind | void
	>(this, false);
	readonly _forceUpdateSignal =
		observableSignal<InlineCompletionTriggerKind>("forceUpdate");
	...
	constructor(
		...
		public readonly textModelVersionId: IObservable<
			number,
		...
		this._register(
			recomputeInitiallyAndOnChange(this._fetchInlineCompletions)
		);
	...
	private readonly _fetchInlineCompletions = derivedHandleChanges(
		{
			...
			handleChange: (ctx, changeSummary) => {
				...
				if (
					...
					this._preserveCurrentCompletionReasons.has(ctx.change)
				) {
	...
	public async triggerExplicitly(tx?: ITransaction): Promise<void> {
		subtransaction(tx, (tx) => {
			this._isActive.set(true, tx);
			this._forceUpdateSignal.trigger(
				tx,
	...
	private readonly _filteredInlineCompletionItems = derived(
		this,
		(reader) => {
	...
	>(
		{
			...
			equalityComparer: (a, b) => {
				...
				return (
					ghostTextOrReplacementEquals(a.ghostText, b.ghostText) &&
		...
		(reader) => {
			...
			const suggestItem = this.selectedSuggestItem.read(reader);
			if (suggestItem) {
				const suggestCompletion = suggestItem
					.toSingleTextEdit()
					.removeCommonPrefix(model);
				const augmentedCompletion = this._computeAugmentedCompletion(
					suggestCompletion,
					reader,
				);
				...
				const editPreviewLength = augmentedCompletion
					? augmentedCompletion.edit.text.length -
					  suggestCompletion.text.length
					: 0;

				const mode = this._suggestPreviewMode.read(reader);
				const cursor = this.cursorPosition.read(reader);
				const newGhostText = edit.computeGhostText(
					model,
					mode,
					cursor,
					editPreviewLength,
				...
				const ghostText =
					newGhostText ?? new GhostText(edit.range.endLineNumber, []);
				return {
					ghostText,
					inlineCompletion: augmentedCompletion?.completion,
					suggestItem,
				};
			} else {
				...
				const cursor = this.cursorPosition.read(reader);
				const ghostText = replacement.computeGhostText(
					model,
					mode,
					cursor,
				);
	...
	private _computeAugmentedCompletion(
	...
	public handleSuggestAccepted(item: SuggestItemInfo) {
		const itemEdit = item
			.toSingleTextEdit()
			.removeCommonPrefix(this.textModel);
		const augmentedCompletion = this._computeAugmentedCompletion(
			itemEdit,
			undefined,
		);
