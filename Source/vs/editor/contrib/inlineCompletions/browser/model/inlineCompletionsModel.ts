...
export class InlineCompletionsModel extends Disposable {
	...
	private readonly _onlyShowWhenCloseToCursor = this._editorObs
		.getOption(EditorOption.inlineSuggest)
		.map((v) => !!v.edits.experimental.onlyShowWhenCloseToCursor);
	private readonly _suggestPreviewEnabled = this._editorObs
		.getOption(EditorOption.suggest)
		.map((v) => v.preview);
	private readonly _suggestPreviewMode = this._editorObs
		.getOption(EditorOption.suggest)
		.map((v) => v.previewMode);
	private readonly _inlineSuggestMode = this._editorObs
		.getOption(EditorOption.inlineSuggest)
		.map((v) => v.mode);
	private readonly _inlineEditsEnabled = this._editorObs
		.getOption(EditorOption.inlineSuggest)
	...
	private readonly _inlineCompletionItems = derivedOpts(
		...
		(reader) => {
			...
			let inlineEdit: InlineCompletionWithUpdatedRange | undefined =
				undefined;
		...
		(reader) => {
			...
			if (item?.inlineEdit) {
				...
				const cursorDist = LineRange.fromRange(
					edit.range,
				).distanceToLine(this.primaryPosition.read(reader).lineNumber);

				if (
					this._onlyShowWhenCloseToCursor.read(reader) &&
					cursorDist > 3 &&
					!item.inlineEdit.request.isExplicitRequest &&
				...
				const currentItemIsCollapsed =
					...
					this._collapsedInlineEditId.read(reader) ===
						item.inlineEdit.semanticId;

				const commands =
					item.inlineEdit.inlineCompletion.source.inlineCompletions
						.commands;
				...
				const inlineEdit = new InlineEdit(
					...
					renderExplicitly,
					commands ?? [],
					item.inlineEdit.inlineCompletion,
				);

				return {
					kind: "inlineEdit",
					inlineEdit,
					inlineCompletion: item.inlineEdit,
					edits: [edit],
					cursorAtInlineEdit,
				};
			}
	...
	public async handleInlineCompletionShown(
