...
export class SingleTextEdit {
	...
	augments(base: SingleTextEdit): boolean {
		...
		return (
			this.text.startsWith(base.text) &&
			rangeExtends(this.range, base.range)
		);
	}
	...
	computeGhostText(
		...
		const suggestionTouchesIndentation =
			edit.range.startColumn - 1 <= sourceIndentationLength;
		...
		if (mode === "prefix") {
			...
			if (
				...
				(filteredChanges.length === 1 &&
					filteredChanges[0].originalStart !==
						valueToBeReplaced.length)
			) {
		...
		const previewStartInCompletionText =
			edit.text.length - previewSuffixLength;

		for (const c of changes) {
			const insertColumn =
				edit.range.startColumn + c.originalStart + c.originalLength;
