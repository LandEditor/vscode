...
export class InlineCompletionWithUpdatedRange {
	...
	public isVisible(
		...
		const originalValue = model
			.getValueInRange(minimizedReplacement.range, EndOfLinePreference.LF)
			.toLowerCase();
	...
	private _isSmallerThanOriginal(reader: IReader | undefined): boolean {
		return length(this._getUpdatedRange(reader)).isBefore(
			length(this.inlineCompletion.range)
		);
	}
...
function length(range: Range): Position {
	if (range.startLineNumber === range.endLineNumber) {
		...
		return new Position(
			1 + range.endLineNumber - range.startLineNumber,
			range.endColumn
		);
	}
}
