...
export class InlineEditsSideBySideDiff extends Disposable {
	...
	constructor(
		...
		this._register(
			this._editorObs.createOverlayWidget({
				...
				minContentWidthInPx: derived((reader) => {
					const x =
						this._previewEditorLayoutInfo.read(
							reader,
						)?.maxContentWidth;
					if (x === undefined) {
						return 0;
		...
		this._editorContainerTopLeft.set(
			this._previewEditorLayoutInfo.map((i) => i?.edit1),
			undefined,
		);
	}
	...
	private readonly _editorContainerTopLeft = observableValue<
		IObservable<Point | undefined> | undefined
	>(this, undefined);

	private readonly _editorContainer = n
		.div(
			{
	...
	public readonly previewEditor = this._register(
		this._instantiationService.createInstance(
	...
	private readonly _updatePreviewEditor = derived((reader) => {
		...
		if (this._overflowView) {
			this._overflowView.element.style.display =
				this._display.read(reader);
		}
	...
	private readonly _originalVerticalStartPosition = this._editorObs
		.observePosition(this._originalStartPosition, this._store)
		.map((p) => p?.y);
	private readonly _originalVerticalEndPosition = this._editorObs
	...
	private readonly _previewEditorLayoutInfo = derived(this, (reader) => {
		...
		const editorContentMaxWidthInRange = maxContentWidthInRange(
			this._editorObs,
		...
		const clientContentAreaRight =
			...
			this._editor.getContainerDomNode().getBoundingClientRect().left;
		const remainingWidthRightOfContent =
			getWindow(this._editor.getContainerDomNode()).outerWidth -
		...
		const previewEditorLeftInTextArea = Math.min(
			editorContentMaxWidthInRange + 20,
			maxPreviewEditorLeft,
		);

		const previewEditorLeft =
			editorLayout.contentLeft + previewEditorLeftInTextArea;
		const maxContentWidth =
			editorContentMaxWidthInRange + 20 + previewWidth + 70;

		const dist = maxPreviewEditorLeft - previewEditorLeftInTextArea;

		const left = Math.max(
			editorLayout.contentLeft,
			previewEditorLeft - horizontalScrollOffset,
		);
		...
		const codeEditDistRange =
			...
			inlineEdit.originalLineRange.length
	...
	private _stickyScrollController = StickyScrollController.get(
	...
	private readonly _extendedModifiedPath = derived((reader) => {
		...
		if (layoutInfo.edit2.y !== layoutInfo.code2.y) {
			extendedModifiedPathBuilder.curveTo2(
	...
	private readonly _backgroundSvg = n
		.svg(
			{
			...
			[
				n.svgElem("path", {
					...
					d: this._previewEditorLayoutInfo.map(
						(layoutInfo) =>
							layoutInfo &&
							new PathBuilder()
								.moveTo(layoutInfo.code1)
	...
	private readonly _foregroundBackgroundSvg = n
		.svg(
			{
				transform: "translate(-0.5 -0.5)",
				style: {
	...
	private readonly _foregroundSvg = n
		.svg(
			{
				transform: "translate(-0.5 -0.5)",
				style: {
			...
			derived((reader) => {
				...
				return [
					n.svgElem("path", {
						...
						d: layoutInfoObs.map((layoutInfo) =>
							new PathBuilder()
					...
					...(!layoutInfoObs
						...
						: [
									...
									n.svgElem(
										...
										[
											n.svgElem("stop", {
												...
												style: {
													stopColor:
														"var(--vscode-inlineEdit-modifiedBorder)",
													stopOpacity: "0",
												},
											}),
											n.svgElem("stop", {
								...
								n.svgElem("rect", {
									...
									x: layoutInfoObs.map(
										(layoutInfo) =>
											layoutInfo.code1.x - shadowWidth,
									),
									y: layoutInfoObs.map(
										(layoutInfo) => layoutInfo.code1.y,
									),
									width: shadowWidth,
									height: layoutInfoObs.map(
										(layoutInfo) =>
											layoutInfo.code2.y -
											layoutInfo.code1.y,
									),
	...
	private readonly _overflowView = n
		.div(
			{
				class: "inline-edits-view",
				style: {
