...
export class InlineEditsViewAndDiffProducer extends Disposable {
	...
	>(this, (reader, token) => {
		...
		this._originalModel
			.get()
			.setValue(this._editor.getModel()!.getValueInRange(range));
...
export class InlineEditWithChanges {
	public readonly originalLineRange = LineRange.fromRangeInclusive(
		RangeMapping.fromEditJoin(this.diffedTextEdit).originalRange,
	);
	public readonly modifiedLineRange = LineRange.fromRangeInclusive(
		RangeMapping.fromEditJoin(this.diffedTextEdit).modifiedRange,
	);

	constructor(public readonly diffedTextEdit: TextEdit) {}
}
...
export class InlineEditsView extends Disposable {
	...
	constructor(
		...
		const visible = derived(
			this,
			(reader) => this._edit.read(reader) !== undefined,
		);
		this._register(
	...
	private readonly _uiState = derived(this, (reader) => {
		...
		let newText = edit.diffedTextEdit.apply(
			new TextModelText(this._editor.getModel()!),
		);
		...
		const minIndent = findFirstMin(
			edit.modifiedLineRange.mapToLineArray((l) =>
				getIndentationLength(newLines[l - 1]),
			),
			numberComparator,
		)!;
