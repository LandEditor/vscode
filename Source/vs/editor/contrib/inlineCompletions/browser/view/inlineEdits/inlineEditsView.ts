...
export class InlineEditsView extends Disposable {
		...
		this._register(
			autorun((reader) => {
				...
				if (layoutInfo.shouldShowShadow) {
					...
					stop1.setAttribute(
						"style",
						"stop-color:var(--vscode-inlineEdit-border);stop-opacity:0",
					);
					...
					stop2.setAttribute(
						"style",
						"stop-color:var(--vscode-inlineEdit-border);stop-opacity:1",
					);
					...
					rect.setAttribute(
						"height",
						`${layoutInfo.code2.y - layoutInfo.code1.y}`,
					);
	...
	private readonly _uiState = derived(this, (reader) => {
		...
		if (edit.isCollapsed) {
			...
			(this._useMixedLinesDiff.read(reader) === "whenPossible" ||
				(edit.userJumpedToIt &&
					this._useMixedLinesDiff.read(reader) ===
						"afterJumpWhenPossible"))
		) {
			state = "mixedLines";
		} else if (
			this._useInterleavedLinesDiff.read(reader) === "always" ||
			(edit.userJumpedToIt &&
				this._useInterleavedLinesDiff.read(reader) === "afterJump")
		) {
	...
	private readonly _previewEditorLayoutInfo = derived(this, (reader) => {
		...
		const editorContentAreaWidth =
			...
			editorLayout.minimap.minimapWidth -
			editorLayout.verticalScrollbarWidth;
