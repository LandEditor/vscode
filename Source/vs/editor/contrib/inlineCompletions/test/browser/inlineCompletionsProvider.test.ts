...
} from "vs/editor/contrib/inlineCompletions/test/browser/utils";
import {
	ITestCodeEditor,
	...
	suite("inlineCompletionToGhostText", () => {
		function getOutput(text: string, suggestion: string): unknown {
			...
			for (const option of options) {
				result[option] = new SingleTextEdit(range, suggestion)
					.computeGhostText(tempModel, option)
		...
		test("Multi Part Diffing", () => {
			...
			assert.deepStrictEqual(
				getOutput("[(y ===)]", "(y === 1) { f(); }"),
				{ prefix: undefined, subword: "(y ===[ 1])[ { f(); }]" }
			);
			assert.deepStrictEqual(
				getOutput("[(y ==)]", "(y === 1) { f(); }"),
				{ prefix: undefined, subword: "(y ==[= 1])[ { f(); }]" }
		...
		test("Multi Part Diffing 1", () => {
			assert.deepStrictEqual(
				getOutput("[if () ()]", "if (1 == f()) ()"),
				{ prefix: undefined, subword: "if ([1 == f()]) ()" }
			);
		});
