...
suite("ParameterHintsModel", () => {
	...
	setup(() => {
		disposables.clear();
		registry =
			new LanguageFeatureRegistry<languages.SignatureHelpProvider>();
	});
	...
	function getNextHint(model: ParameterHintsModel) {
		return new Promise<languages.SignatureHelpResult | undefined>(
			(resolve) => {
				const sub = disposables.add(
					model.onChangedHints((e) => {
						sub.dispose();
						return resolve(
							e ? { value: e, dispose: () => {} } : undefined
						);
					})
				);
			}
				...
				try {
					...
					if (count === 0) {
						hintsModel.trigger(
							{
								triggerKind:
									languages.SignatureHelpTriggerKind.Invoke,
							},
							0
						);
					}

					return new Promise<languages.SignatureHelpResult>(
						(resolve) => {
							setTimeout(() => {
								resolve({
									value: {
