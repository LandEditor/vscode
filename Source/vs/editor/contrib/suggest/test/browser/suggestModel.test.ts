							...
							suggestions: [
								{
									...
									range: Range.fromPositions(
										pos.delta(
											0,
											doc.getLineContent(pos.lineNumber)[
												pos.column - 2
											] === "."
												? 0
												: -1
				...
				(event) => {
					...
					return assertEvent(
						...
						(event) => {
							...
							assert.strictEqual(
								event.completionModel.getIncompleteProvider()
									.size,
								1
							);
							assert.strictEqual(
			...
			registry.register(
				{ scheme: "test" },
				{
					...
					provideCompletionItems(doc, pos) {
						countB += 1;
						if (
							!doc.getWordUntilPosition(pos).word.startsWith("a")
						) {
							return;
						}
						return {
					...
					provideCompletionItems(doc, pos, ctx) {
						return {
							suggestions: [
								{
									label:
										doc.getWordUntilPosition(pos).word ||
										"hello",
									kind: CompletionItemKind.Property,
									insertText: "foofoo",
									range: getDefaultSuggestRange(doc, pos),
