...
export class ColorHoverParticipant
...
export class StandaloneColorPickerParticipant {
	...
	public async createColorHover(
		...
		return {
			colorHover: await _createColorHover(
				this,
				this._editor.getModel(),
				colorInfo,
				colorProvider
...
async function _createColorHover<
	T extends ColorHoverParticipant | StandaloneColorPickerParticipant,
>(
	participant: T,
...
function _updateEditorModel(
	...
	if (model.presentation.textEdit) {
		...
		newRange =
			editor.getModel()!._getTrackedRange(trackedRange) || newRange;
	} else {
		textEdits = [
			{ range, text: model.presentation.label, forceMoveMarkers: false },
		];
...
async function _updateColorPresentations(
	editorModel: ITextModel,
