...
export class ContentHoverController extends Disposable {
	...
	private _startHoverOperationIfNecessary(
		...
		this._computer.insistOnKeepingHoverVisible =
	...
	private _renderMessages(anchor: HoverAnchor, messages: IHoverPart[]): void {
		...
		const context: IEditorHoverRenderContext = {
			...
			setMinimumDimensions: (dimensions: dom.Dimension) =>
...
class HoverResult {
	...
	public filter(anchor: HoverAnchor): HoverResult {
		...
		return new FilteredHoverResult(
			this,
			this.anchor,
			filteredMessages,
			this.isComplete,
...
export class ContentHoverWidget extends ResizableContentWidget {
	...
	private _hasHorizontalScrollbar(): boolean {
		...
		const hasHorizontalScrollbar =
			scrollDimensions.scrollWidth > scrollDimensions.width;
	...
	override getPosition(): IContentWidgetPosition | null {
		...
		return {
			...
			positionAffinity: this._visibleData.isBeforeContent
	...
	public hide(): void {
		...
		const stoleFocus =
			this._visibleData.stoleFocus || this._hoverFocusedKey.get();
		this._setHoverData(undefined);
...
class ContentHoverComputer implements IHoverComputer<IHoverPart> {
	...
	public set shouldFocus(value: boolean) {
		this._shouldFocus = value;
	}

	private _source: HoverStartSource = HoverStartSource.Mouse;
	public get source(): HoverStartSource {
		return this._source;
	}
	public set source(value: HoverStartSource) {
		this._source = value;
	}
	...
	public set insistOnKeepingHoverVisible(value: boolean) {
