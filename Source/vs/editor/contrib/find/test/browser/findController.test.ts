...
} from "vs/editor/contrib/find/browser/findController";
	...
	test("issue #38232: Find Next Selection, regex enabled", async () => {
		await withAsyncTestCodeEditor(
			...
			async (editor) => {
				...
				const nextSelectionMatchFindAction =
					new NextSelectionMatchFindAction();
	...
	test("issue #47400, CMD+E supports feeding multiple line of text into the find widget", async () => {
		await withAsyncTestCodeEditor(
			...
			async (editor, _, instantiationService) => {
				...
				const startFindWithSelectionAction =
					new StartFindWithSelectionAction();
