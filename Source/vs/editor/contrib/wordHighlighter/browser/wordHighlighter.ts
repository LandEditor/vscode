...
export function getOccurrencesAcrossMultipleModels(
	...
	return first<ResourceMap<DocumentHighlight[]> | null | undefined>(
		...
		(
			t: ResourceMap<DocumentHighlight[]> | null | undefined
		): t is ResourceMap<DocumentHighlight[]> =>
	...
	protected _compute(
		...
		return timeout(250, token).then(() => {
			...
			for (const otherModel of allModels) {
				...
				const matches = otherModel.findMatches(
					wordResult.word,
					true,
					false,
					true,
...
function computeOccurencesAtPosition(
	...
	return new TextualOccurenceRequest(
		model,
		selection,
		word,
		wordSeparators,
...
class WordHighlighter {
	...
	constructor(
		...
		this.multiDocumentOccurrencesHighlight = this.editor.getOption(
			EditorOption.multiDocumentOccurrencesHighlight
		);
		...
		this.toUnhook.add(
			editor.onDidChangeConfiguration((e) => {
				...
				const newMultiDocumentValue = this.editor.getOption(
					EditorOption.multiDocumentOccurrencesHighlight
				);
				if (
					this.multiDocumentOccurrencesHighlight !==
					newMultiDocumentValue
				) {
					this.multiDocumentOccurrencesHighlight =
						newMultiDocumentValue;
					this._stopAll();
	...
	private _removeSingleDecorations(): void {
		...
		const currentDecorationIDs = WordHighlighter.storedDecorations.get(
	...
	private _stopSingular(): void {
		...
		if (this.editor.hasWidgetFocus()) {
			if (
				...
				WordHighlighter.query?.modelInfo?.model.uri.scheme !==
					Schemas.vscodeNotebookCell
			) {
	...
	private _run(): void {
		...
		if (workerRequestIsValid) {
			...
			const sendWord =
				(!WordHighlighter.query.modelInfo &&
			...
			if (
				...
				this.workerRequest = this.computeWithModel(
					...
					sendWord ? WordHighlighter.query.word : null,
		...
		for (const editor of currentEditors) {
			...
			if (uri && this.workerRequestValue.has(uri)) {
				const oldDecorationIDs: string[] | undefined =
					WordHighlighter.storedDecorations.get(uri);
				...
				WordHighlighter.storedDecorations =
					WordHighlighter.storedDecorations.set(
						uri,
						newDecorationIDs
					);
