...
export class TestDecorationProvider {
	...
	getDecorations(): ExpectedDecoration[] {
		...
		for (const decoration of decorations) {
			if (
				decoration.options === TestDecorationProvider.hiddenDecoration
			) {
				res.push({
				...
				decoration.options ===
				TestDecorationProvider.collapsedDecoration
			) {
				res.push({
					line: decoration.range.startLineNumber,
					type: "collapsed",
...
suite("Folding Model", () => {
	...
	test("getRegionsInsideWithLevel", () => {
		...
		try {
			...
			assertRegions(
				foldingModel.getRegionsInside(region2, (r, level) =>
					r.hidesLine(9)
				),
				[r3, r5],
				"7"
	...
	test("setCollapseStateRecursivly", () => {
		...
		try {
			...
			setCollapseStateLevelsDown(foldingModel, true, Number.MAX_VALUE, [
				4,
			]);
			assertFoldedRanges(foldingModel, [r3, r4, r5], "1");

			setCollapseStateLevelsDown(foldingModel, false, Number.MAX_VALUE, [
				8,
			]);
