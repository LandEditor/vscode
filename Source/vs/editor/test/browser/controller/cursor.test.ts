...
suite("Editor Controller", () => {
	...
	function setupAutoClosingLanguageTokenization() {
		...
		const encodedLanguageId =
			languageService.languageIdCodec.encodeLanguageId(
	...
	test("issue #832: word right", () => {
		usingCursor(
			...
			(editor, model, viewModel) => {
				...
				assertWordRight(
					49,
					"   /* Just some   more   text a+= 3 +5-3 + 7 */ ".length +
						1
				);
				assertWordRight(
					50,
	...
	test("issue #122714: tabSize=1 prevent typing a string matching decreaseIndentPattern in an empty file", () => {
		...
		const model = createTextModel("\\end", latextLanguageId, {
			tabSize: 1,
		});
		...
		usingCursor(
			...
			(editor, model, viewModel) => {
				...
				for (let i = 0, len = autoClosePositions.length; i < len; i++) {
					...
					for (
						let column = 1;
						column < autoCloseColumns.length;
						column++
					) {
						model.tokenization.forceTokenization(lineNumber);
						if (
							autoCloseColumns[column] ===
							AutoClosingColumnType.Special1
						) {
							assertType(
								editor,
								model,
				...
				for (let i = 0, len = autoClosePositions.length; i < len; i++) {
					...
					for (
						...
						if (
							...
							autoCloseColumns[column] ===
							AutoClosingColumnType.Special2
						) {
							assertType(
								editor,
								model,
