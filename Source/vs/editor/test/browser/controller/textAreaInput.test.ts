	...
	async function simulateInteraction(
		...
		const wrapper = disposables.add(
			new (class extends Disposable implements ICompleteTextAreaWrapper {
				private _onKeyDown = this._register(
					new Emitter<KeyboardEvent>()
				);
				readonly onKeyDown = this._onKeyDown.event;

				private _onKeyPress = this._register(
					new Emitter<KeyboardEvent>()
				);
				...
				private _onBeforeInput = this._register(
					new Emitter<InputEvent>()
				);
				...
				public _dispatchRecordedEvent(event: IRecordedEvent): void {
					...
					this._state.selectionDirection =
						event.state.selectionDirection;
				...
				setValue(reason: string, value: string): void {
					if (
						this._currDispatchingEvent?.type === "compositionstart"
					) {
						assert.fail(
