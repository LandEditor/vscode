...
import {
	...
	TextChange,
} from "vs/editor/common/core/textChange";
...
suite("TextChangeCompressor", () => {
	...
	function assertCompression(
		...
		const compressedDoTextEdits: IGeneratedEdit[] =
			compressedTextChanges.map((change) => {
				return {
		...
		const compressedUndoTextEdits: IGeneratedEdit[] =
			compressedTextChanges.map((change) => {
				return {
	...
	class GeneratedTest {
		...
		public print(): void {
			console.log(
				`assertCompression(${JSON.stringify(
					this._content
				)}, ${JSON.stringify(this._edits1)}, ${JSON.stringify(
					this._edits2
				)});`
			);
		}
