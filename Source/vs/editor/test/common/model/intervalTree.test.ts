		...
		public acceptOp(op: IOperation): void {
			if (op.type === "insert") {
				...
				const actual = actualNodes.map(
					(n) =>
						new Interval(n.cachedAbsoluteStart, n.cachedAbsoluteEnd)
				);
				const expected = this._oracle.search(
					new Interval(op.begin, op.end)
				);
...
function _printTree(
	...
	out.push(
		`${indent}[${getNodeColor(n) === NodeColor.Red ? "R" : "B"},${
			n.delta
		}, ${n.start}->${n.end}, ${n.maxEnd}] : {${delta + n.start}->${
			delta + n.end
		}}, maxEnd: ${n.maxEnd + delta}\n`
	);
	if (n.left !== SENTINEL) {
