...
export class GlyphMarginWidgets extends ViewPart {
	...
	private _collectDecorationBasedGlyphRenderRequest(
		...
		for (const d of decorations) {
			...
			const lane = Math.min(
				d.options.glyphMargin?.position ?? 1,
				this._glyphMarginDecorationLaneCount
			);
	...
	public prepareRender(ctx: RenderingContext): void {
		...
		while (requests.length > 0) {
			...
			const requestsAtLocation = requests.takeWhile(
				(el) =>
					el.lineNumber === first.lineNumber && el.lane === first.lane
			);
	...
	public render(ctx: RestrictedRenderingContext): void {
		...
		for (const widget of Object.values(this._widgets)) {
			if (!widget.renderInfo) {
				...
				const left =
					...
					(widget.renderInfo.lane - 1) * this._lineHeight;
		...
		for (let i = 0; i < this._decorationGlyphsToRender.length; i++) {
			...
			const left =
				this._glyphMarginLeft + (dec.lane - 1) * this._lineHeight;
