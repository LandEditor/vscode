	...
	implements IGpuRenderStrategy
{
	...
	constructor(
		...
		const bufferSize =
			this._viewGpuContext.maxGpuLines *
	...
	public override onScrollChanged(e?: ViewScrollChangedEvent): boolean {
		...
		this._scrollOffsetValueBuffer[0] =
			(e?.scrollLeft ?? this._context.viewLayout.getCurrentScrollLeft()) *
			dpr;

		this._scrollOffsetValueBuffer[1] =
			(e?.scrollTop ?? this._context.viewLayout.getCurrentScrollTop()) *
			dpr;
	...
	update(
		...
		const lineIndexCount =
			this._viewGpuContext.maxGpuCols * Constants.IndicesPerCell;
		...
		for (
			...
			for (
				...
				for (x = tokenStartIndex; x < tokenEndIndex; x++) {
					...
					cellBuffer[cellIndex + CellBufferInfo.Offset_Y] =
						absoluteOffsetY;
			...
			fillStartIndex =
				((y - 1) * this._viewGpuContext.maxGpuCols + tokenEndIndex) *
				Constants.IndicesPerCell;

			fillEndIndex =
				y * this._viewGpuContext.maxGpuCols * Constants.IndicesPerCell;
