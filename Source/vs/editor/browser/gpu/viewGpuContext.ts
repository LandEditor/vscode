				...
				(e) => {
					if (
						...
						const verticalScrollbarSize =
							configurationService.getValue<IEditorOptions>(
								"editor",
	...
	public canRenderDetailed(
		...
		if (data.inlineDecorations.length > 0) {
			...
			if (problemRules.length > 0) {
				reasons.push(
					`inlineDecorations with unsupported CSS rules (\`${problemRules.join(", ")}\`)`,
				);
			}
			if (problemSelectors.length > 0) {
				reasons.push(
					`inlineDecorations with unsupported CSS selectors (\`${problemSelectors.join(", ")}\`)`,
				);
			}
		}
 ...
 */
const gpuSupportedDecorationCssRules = ["color"];
