...
} from "vs/editor/browser/widget/diffEditor/utils";
...
export class DiffEditorWidget extends DelegatingEditor implements IDiffEditor {
	...
	private readonly elements = h(
		...
		[
			h(
				"div.noModificationsOverlay@overlay",
				{
					style: {
						position: "absolute",
						height: "100%",
	...
	private readonly _diffModel = this._register(
		disposableObservableValue<DiffEditorViewModel | undefined>(
			"diffModel",
			undefined
		)
	);
	...
	private readonly movedBlocksLinesPart = observableValue<
		MovedBlocksLinesPart | undefined
	>(this, undefined);
	...
	constructor(
		...
		const isEmbeddedDiffEditorKey =
			EditorContextKeys.isEmbeddedDiffEditor.bindTo(
				this._contextKeyService
			);
		this._register(
			autorun((reader) => {
				/** @description update isEmbeddedDiffEditorKey */
				isEmbeddedDiffEditorKey.set(
					this._options.isInEmbeddedEditor.read(reader)
				);
			})
		);

		const comparingMovedCodeKey =
			EditorContextKeys.comparingMovedCode.bindTo(
				this._contextKeyService
			);
		this._register(
			autorun((reader) => {
				/** @description update comparingMovedCodeKey */
				comparingMovedCodeKey.set(
					!!this._diffModel
						.read(reader)
		...
		const diffEditorRenderSideBySideInlineBreakpointReachedContextKeyValue =
			EditorContextKeys.diffEditorRenderSideBySideInlineBreakpointReached.bindTo(
				this._contextKeyService
			);
		this._register(
			autorun((reader) => {
				/** @description update accessibleDiffViewerVisible context key */
				diffEditorRenderSideBySideInlineBreakpointReachedContextKeyValue.set(
		...
		this._sash = derivedWithStore(this, (reader, store) => {
			...
			const result = store.add(
				new DiffEditorSash(this._options, this.elements.root, {
		...
		this._register(
			autorunWithStore((reader, store) => {
				...
				this._accessibleDiffViewer = store.add(
					this._register(
						this._instantiationService.createInstance(
							readHotReloadableExport(
							...
							(visible, tx) =>
								this._accessibleDiffViewerShouldBeVisible.set(
									visible,
									tx
								),
							this._options.onlyShowAccessibleDiffViewer.map(
								(v) => !v
							),
							this._rootSizeObserver.width,
							this._rootSizeObserver.height,
		...
		this._register(
			autorunWithStore((reader, store) => {
				this.movedBlocksLinesPart.set(
					store.add(
						new (readHotReloadableExport(
							MovedBlocksLinesPart,
							reader
						))(
							this.elements.root,
							this._diffModel,
		...
		const isDiffUpToDate = this._diffModel.map(
			(m, reader) => m?.isDiffUpToDate.read(reader)
		);
		this._register(
			autorunWithStore((reader, store) => {
	...
	private readonly _layoutInfo = derived(this, (reader) => {
		...
		const originalWidth =
			sashLeft ??
			Math.max(5, this._editors.original.getLayoutInfo().decorationsLeft);
		const modifiedWidth =
			width -
			originalWidth -
			(this._options.renderOverviewRuler.read(reader)
		...
		const movedBlocksLinesWidth =
			this.movedBlocksLinesPart.read(reader)?.width.read(reader) ?? 0;
