...
import {
	...
	RenderOptions,
	renderLines,
} from "vs/editor/browser/widget/diffEditor/renderLines";
import {
	animatedObservable,
	joinCombine,
	...
	constructor(
		...
		this._register(
			this._editors.original.onDidChangeViewZones((_args) => {
				if (
					!isChangingViewZones &&
					!this._canIgnoreViewZoneUpdateEvent()
				) {
		...
		const alignmentViewZones = derived<{
			orig: IViewZoneWithZoneId[];
			mod: IViewZoneWithZoneId[];
		}>((reader) => {
							...
							for (let i = 0; i < result.heightInLines; i++) {
								...
								marginElement.setAttribute(
									...
									`position:absolute;top:${
										...
										renderOptions.lineDecorationsWidth
									}px;height:${modLineHeight}px;right:0;`
								);
