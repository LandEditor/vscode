...
export class MovedBlocksLinesPart extends Disposable {
	...
	constructor(
		...
		this._register(
			autorun((reader) => {
				...
				this._element.style.left = `${
					info.width - info.verticalScrollbarWidth
				}px`;
		...
		this._register(
			applyViewZones(
				...
				movedBlockViewZones.map((zones) => zones.map((z) => z.original))
			)
		);
		this._register(
			applyViewZones(
				this._editors.modified,
				movedBlockViewZones.map((zones) => zones.map((z) => z.modified))
			)
		);
	...
	private readonly _state = derivedWithStore((reader, store) => {
		...
		const width =
			lineAreaLeft +
			lineAreaWidth +
			(infoMod.contentLeft - MovedBlocksLinesPart.movedCodeBlockPadding);
