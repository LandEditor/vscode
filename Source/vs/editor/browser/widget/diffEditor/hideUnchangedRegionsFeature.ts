...
import {
	DiffEditorViewModel,
	UnchangedRegion,
} from "vs/editor/browser/widget/diffEditor/diffEditorViewModel";
...
export class HideUnchangedRegionsFeature extends Disposable {
	...
	public get isUpdatingViewZones(): boolean {
		return this._isUpdatingViewZones;
	}

	private readonly _modifiedOutlineSource = derivedWithStore(
		this,
		(reader, store) => {
			...
			return store.add(
				new OutlineSource(this._languageFeaturesService, m)
			);
		}
	);
	...
	constructor(
		...
		const unchangedRegions = this._diffModel.map((m, reader) =>
			m?.diff.read(reader)?.mappings.length === 0
				? []
				: m?.unchangedRegions.read(reader) ?? []
		);

		const viewZones = derivedWithStore(this, (reader, store) => {
			/** @description view Zones */
			const modifiedOutlineSource =
				this._modifiedOutlineSource.read(reader);
		...
		this._register(
			applyObservableDecorations(
				...
				derived((reader) => {
					...
					const curUnchangedRegions = unchangedRegions.read(reader);
					const result =
						curUnchangedRegions.map<IModelDeltaDecoration>((r) => ({
		...
		this._register(
			applyViewZones(
				this._editors.original,
				viewZones.map((v) => v.origViewZones),
				(v) => (this._isUpdatingViewZones = v)
			)
		);
		this._register(
			applyViewZones(
				this._editors.modified,
				viewZones.map((v) => v.modViewZones),
				(v) => (this._isUpdatingViewZones = v)
			)
		);
		...
		this._register(
			autorun((reader) => {
				...
				this._editors.original.setHiddenAreas(
					curUnchangedRegions
						.map((r) =>
							r.getHiddenOriginalRange(reader).toInclusiveRange()
						)
						.filter(isDefined)
				);
				this._editors.modified.setHiddenAreas(
					curUnchangedRegions
						.map((r) =>
							r.getHiddenModifiedRange(reader).toInclusiveRange()
						)
						.filter(isDefined)
				);
			})
			...
			this._editors.modified.onMouseUp((event) => {
				if (
					...
					const region = model.unchangedRegions
						.get()
						.find((r) =>
							r.modifiedUnchangedRange.includes(lineNumber)
						);
					if (!region) {
						return;
					}
			...
			this._editors.original.onMouseUp((event) => {
				if (
					...
					const region = model.unchangedRegions
						.get()
						.find((r) =>
							r.originalUnchangedRange.includes(lineNumber)
						);
					if (!region) {
						return;
					}
...
class CollapsedCodeOverlayWidget extends ViewZoneOverlayWidget {
			...
			h(
				...
				[
					$(
						"a",
						{
							title: localize("showAll", "Show all"),
							role: "button",
							onclick: () => {
	...
	constructor(
		...
		const layoutInfo = observableFromEvent(
