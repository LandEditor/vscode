...
export class OverviewRulerPart extends Disposable {
	...
	constructor(
		...
		const scrollTopObservable = observableFromEvent(
			this._editors.modified.onDidScrollChange,
			() => this._editors.modified.getScrollTop()
		);
		const scrollHeightObservable = observableFromEvent(
			this._editors.modified.onDidScrollChange,
			() => this._editors.modified.getScrollHeight()
		);
			...
			autorunWithStore((reader, store) => {
				...
				store.add(
					autorunWithStore((reader, store) => {
						...
						const origHiddenRangesChanged =
							observableSignalFromEvent(
								"hiddenRangesChanged",
								this._editors.original.onDidChangeHiddenAreas
							);
						const modHiddenRangesChanged =
							observableSignalFromEvent(
								"hiddenRangesChanged",
							...
							autorun((reader) => {
								...
								if (layoutInfo) {
									const freeSpace =
										...
										2 *
											OverviewRulerPart.ONE_OVERVIEW_WIDTH;
									...
									const scrollTop =
										scrollTopObservable.read(reader);
									const scrollHeight =
										scrollHeightObservable.read(reader);
