	...
	private _setSelectionChangeListener(
		...
		return addDisposableListener(
			...
			() => {
				...
				const startPositionOfScreenReaderContentWithinEditor =
					this._screenReaderSupport.startPositionOfScreenReaderContentWithinEditor();
				if (!startPositionOfScreenReaderContentWithinEditor) {
					return;
				}
				const model = this._context.viewModel.model;
				const offsetOfStartOfScreenReaderContent = model.getOffsetAt(
					startPositionOfScreenReaderContentWithinEditor,
				);
				const offsetOfSelectionStart =
					range.startOffset + offsetOfStartOfScreenReaderContent;
				const offsetOfSelectionEnd =
