...
export class PagedScreenReaderStrategy {
	...
	public static fromEditorSelection(
		...
		const endOfLinePreference = useLineFeed
			? EndOfLinePreference.LF
			: EndOfLinePreference.TextDefined;
		if (
			trimLongText &&
			model.getValueLengthInRange(pretextRange, endOfLinePreference) >
				LIMIT_CHARS
		) {
		...
		if (
			trimLongText &&
			model.getValueLengthInRange(posttextRange, endOfLinePreference) >
				LIMIT_CHARS
		) {
