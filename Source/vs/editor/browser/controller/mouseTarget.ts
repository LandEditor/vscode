...
import {
	...
	Direction,
} from "vs/editor/common/cursor/cursorAtomicMoveOperations";
...
class HitTestRequest extends BareHitTestRequest {
	...
	public override toString(): string {
		return `pos(${this.pos.x},${this.pos.y}), editorPos(${
			this.editorPos.x
		},${this.editorPos.y}), relativePos(${this.relativePos.x},${
			this.relativePos.y
		}), mouseVerticalOffset: ${
			...
			this.target ? (<HTMLElement>this.target).outerHTML : null
		}`;
	}
...
export class MouseTargetFactory {
	...
	public createMouseTarget(
		...
		try {
			const r = MouseTargetFactory._createMouseTarget(
				ctx,
				request,
				false
			);
