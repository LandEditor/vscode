...
export class ObservableCodeEditor extends Disposable {
	...
	public readonly isTextFocused = observableFromEvent(
		this,
		(e) => {
	...
	public createOverlayWidget(widget: IObservableOverlayWidget): IDisposable {
		const overlayWidgetId =
			"observableOverlayWidget" + this._widgetCounter++;
	...
	public observePosition(
		...
		const result = observableValueOpts<Point | null>(
			{ owner: this, equalsFn: equalsIfDefined(Point.equals) },
			new Point(0, 0),
		);
		const contentWidgetId =
			`observablePositionWidget` + this._widgetCounter++;
		...
		const w: IContentWidget = {
			...
			afterRender(position, coordinate) {
				result.set(
					coordinate
						? new Point(coordinate.left, coordinate.top)
						: null,
					undefined,
				);
			},
