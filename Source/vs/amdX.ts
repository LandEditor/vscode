...
class AMDModuleImporter {
	...
	private _initialize(): void {
		...
		if (this._isRenderer) {
			...
			this._amdPolicy = (<any>globalThis).trustedTypes?.createPolicy(
				"amdLoader",
				{
	...
	public async load<T>(scriptSrc: string): Promise<T> {
		...
		if (
			Array.isArray(defineCall.dependencies) &&
			defineCall.dependencies.length > 0
		) {
			throw new Error(
	...
	private async _nodeJSLoadScript(
		...
		try {
			const fs = <typeof import("fs")>(
				globalThis._VSCODE_NODE_MODULES["fs"]
			);
			const vm = <typeof import("vm")>(
				globalThis._VSCODE_NODE_MODULES["vm"]
			);
