		...
		return this.transaction(connection, () => {
			...
			if (toInsert && toInsert.size > 0) {
				...
				keysValuesChunks.forEach((keysValuesChunk) => {
					this.prepare(
						connection,
						`INSERT INTO ItemTable VALUES ${new Array(
							keysValuesChunk.length / 2,
						)
							.fill("(?,?)")
							.join(",")}`,
			...
			if (toDelete?.size) {
				...
				keysChunks.forEach((keysChunk) => {
					this.prepare(
						connection,
						`DELETE FROM ItemTable WHERE key IN (${new Array(
							keysChunk.length,
						)
							.fill("?")
							.join(",")})`,
	...
	private prepare(
		...
		const statementErrorListener = (error: Error) => {
			this.handleSQLiteError(
				...
				`[storage ${
					this.name
				}] prepare(): ${error} (${sql}). Details: ${errorDetails()}`,
			);
		};
