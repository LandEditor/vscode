...
suite("MarkdownRenderer", () => {
	...
	suite("ThemeIcons Support On", () => {
		...
		test("render icon in link", () => {
			...
			assert.strictEqual(
				result.innerHTML,
				`<p><a data-href="#link" href="" title="#link"><span class="codicon codicon-zap"></span>-link</a></p>`
			);
		});
...
`
	...
	test("Should render command links in trusted strings", () => {
		...
		assert.strictEqual(
			result.innerHTML,
			`<p><a data-href="command:doFoo" href="" title="command:doFoo">command1</a> <a data-href="command:doFoo" href="">command2</a></p>`
		);
	});
	...
	suite("fillInIncompleteTokens", () => {
		...
		suite("table", () => {
			...
			test("incomplete header one column", () => {
				...
				const tokens = marked.lexer(incompleteTable);
				const completeTableTokens = marked.lexer(
					incompleteTable + "|\n| --- |"
				);
		...
		suite("codeblock", () => {
			...
			test("code block header only", () => {
				...
				const completeCodeblockTokens = marked.lexer(
					incompleteCodeblock + "\n```"
				);
		...
		function simpleMarkdownTestSuite(
			name: string,
			delimiter: string
		): void {
		...
		suite("link", () => {
			test("incomplete link text", () => {
				...
				const completeTokens = marked.lexer(
					incomplete + "](about:blank)"
				);
				assert.deepStrictEqual(newTokens, completeTokens);
			});
