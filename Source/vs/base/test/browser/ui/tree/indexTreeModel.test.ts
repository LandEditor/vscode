...
suite("IndexTreeModel", () => {
	...
	test("smart diff consistency", () => {
		...
		for (let i = 0; i < times; i++) {
			...
			for (
				...
				changes.push(
					`splice(${spliceIndex}, ${deleteCount}, [${inserts
						.map((e) => e.element)
						.join(", ")}]) -> ${listElements.join(", ")}`
				);

				assert.deepStrictEqual(
					expected,
					listElements,
					`Expected ${listElements.join(
						", "
	...
	test("recursive filter", () => {
		...
		const model = new IndexTreeModel<string>("test", toList(list), "root", {
			filter,
		});
