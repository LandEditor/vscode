...
((global, factory) => {
	...
	const Tokenizer = /*#__PURE__*/ (() => {
			...
			if (cap) {
				...
				for (i = 0; i < l; i++) {
					...
					const hasMultipleLineBreaks = spacers.every((t) => {
						...
						for (
							let _iterator =
									_createForOfIteratorHelperLoose(chars),
								_step;
							!(_step = _iterator()).done;
						) {
							const _char = _step.value;
		...
		_proto.emStrong = function emStrong(src, maskedSrc, prevChar) {
			...
			if (!match) {
				return; // _ can't be between two alphanumerics. \p{L}\p{N} includes non-english alphabet/numbers as well
			}
			...
			if (
				...
				while ((match = endReg.exec(maskedSrc)) != null) {
					...
					if (Math.min(lLength, rLength) % 2) {
						const _text = src.slice(
							1,
							lLength + match.index + rLength,
						);
		...
		_proto.autolink = function autolink(src, mangle) {
			...
			if (cap) {
				...
				if (cap[2] === "@") {
					text = escape(
						this.options.mangle ? mangle(cap[1]) : cap[1],
					);
					href = `mailto:${text}`;
				} else {
					text = escape(cap[1]);
		...
		_proto.url = function url(src, mangle) {
			...
			if ((cap = this.rules.inline.url.exec(src))) {
				...
				if (cap[2] === "@") {
					text = escape(
						this.options.mangle ? mangle(cap[0]) : cap[0],
					);
					href = `mailto:${text}`;
				} else {
	...
	const Lexer = /*#__PURE__*/ (() => {
		...
		_proto.blockTokens = function blockTokens(src, tokens) {
			...
			while (src) {
				...
				if (src) {
					const errMsg = `Infinite loop on byte: ${src.charCodeAt(
						0,
					)}`;
		...
		_proto.inlineTokens = function inlineTokens(src, tokens) {
			...
			let token;
			let lastToken;
			let cutSrc; // String with links masked to avoid interference with em and strong

			let maskedSrc = src;
			let match;
			let keepPrevChar;
			let prevChar; // Mask out reflinks
	...
	const Renderer = /*#__PURE__*/ (() => {
		...
		const _proto = Renderer.prototype;
		...
		_proto.code = function code(_code, infostring, escaped) {
			...
			return `<pre><code class="${this.options.langPrefix}${escape(
				lang,
				true,
			)}">${escaped ? _code : escape(_code, true)}</code></pre>\n`;
		};
		/**
		 * @param {string} quote
		 */
	 ...
	 */
	const TextRenderer = /*#__PURE__*/ (() => {
		function TextRenderer() {}

		const _proto = TextRenderer.prototype;
	...
	const Slugger = /*#__PURE__*/ (() => {
		...
		const _proto = Slugger.prototype;
		...
		_proto.getNextSafeSlug = function getNextSafeSlug(
			originalSlug,
			isDryRun,
		) {
	...
	function marked(src, opt, callback) {
		...
		try {
			const _tokens = Lexer.lex(src, opt);
	...
	marked.use = () => {
		...
		const opts = merge.apply(void 0, [{}].concat(args));
		...
		args.forEach((pack) => {
			...
			if (pack.tokenizer) {
				(() => {
					const tokenizer =
						marked.defaults.tokenizer || new Tokenizer();

					const _loop2 = function _loop2(prop) {
						...
						tokenizer[prop] = () => {
							...
							let ret = pack.tokenizer[prop].apply(
								tokenizer,
								args,
							);
			...
			if (pack.walkTokens) {
				const _walkTokens = marked.defaults.walkTokens;
	...
	marked.walkTokens = (tokens, callback) => {
		...
		const _loop3 = function _loop3() {
			...
			switch (token.type) {
				case "table": {
					for (
						let _iterator2 = _createForOfIteratorHelperLoose(
								token.header,
							),
							_step2;
					...
					for (
						let _iterator3 = _createForOfIteratorHelperLoose(
								token.rows,
							),
							_step3;
						...
						for (
							let _iterator4 =
									_createForOfIteratorHelperLoose(row),
								_step4;
							!(_step4 = _iterator4()).done;
						) {
							const _cell = _step4.value;
							values = values.concat(
				...
				default: {
					if (marked.defaults.extensions?.childTokens?.[token.type]) {
						// Walk any extensions
		...
		for (
			let _iterator = _createForOfIteratorHelperLoose(tokens), _step;
			!(_step = _iterator()).done;
		) {
			_loop3();
		}
