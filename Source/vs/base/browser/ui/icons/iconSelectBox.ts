...
export class IconSelectBox extends Disposable {
	...
	private renderIcons(
		...
		if (icons.length) {
			...
			dom.append(
				container,
				dom.$(".icon-no-results", undefined, noResults),
			);
			alert(noResults);
		}
	...
	private reveal(index: number): void {
		...
		if (element.offsetTop + this.iconContainerHeight > scrollTop + height) {
			this.scrollableElement.setScrollPosition({
				scrollTop:
					element.offsetTop + this.iconContainerHeight - height,
			});
	...
	layout(dimension: dom.Dimension): void {
		...
		if (this.iconsContainer) {
			this.iconsContainer.style.paddingLeft = `${Math.floor(
				containerPadding / 2,
			)}px`;
			this.iconsContainer.style.paddingRight = `${Math.ceil(
				containerPadding / 2,
			)}px`;
		}

		if (this.scrollableElement) {
			this.scrollableElement.getDomNode().style.height = `${
				this.iconIdElement
					? dimension.height - 80
					: dimension.height - 40
			}px`;
	...
	focusPreviousRow(): void {
		let previousRowIndex =
			this.focusedItemIndex - this.numberOfElementsPerRow;
		if (previousRowIndex < 0) {
			...
			previousRowIndex =
				...
				this.numberOfElementsPerRow * numberOfRows -
				1;
			previousRowIndex =
				previousRowIndex < 0
