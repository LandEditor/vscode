...
import {
	type ISashEvent as IBaseSashEvent,
	Orientation,
	Sash,
	SashState,
} from "../sash/sash.js";
	...
	constructor(
		...
		if (options.descriptor) {
			...
			options.descriptor.views.forEach((viewDescriptor, index) => {
				const sizing =
					types.isUndefined(viewDescriptor.visible) ||
		...
		try {
			...
			if (this.viewItems.length > 1) {
				...
				const sash =
					this.orientation === Orientation.VERTICAL
						? new Sash(
								this.sashContainer,
								{
									getHorizontalSashTop: (s) =>
										this.getSashPosition(s),
									getHorizontalSashWidth:
										this.getSashOrthogonalSize,
								},
								{
									...opts,
				...
				const onDidResetDisposable = sash.onDidReset(() => {
					const index = this.sashItems.findIndex(
						(item) => item.sash === sash,
					);
					const upIndexes = range(index, -1);
					const downIndexes = range(index + 1, this.viewItems.length);
	...
	private resize(
		...
		const maxDeltaDown =
			downIndexes.length === 0
				...
				: downIndexes.reduce(
						(r, i) =>
							r + (sizes[i] - this.viewItems[i].minimumSize),
						0,
					);
		const minDeltaDown =
			downIndexes.length === 0
				...
				: downIndexes.reduce(
						(r, i) =>
							r + (sizes[i] - this.viewItems[i].maximumSize),
						0,
					);
