...
import type { MarkdownRenderOptions } from "../../markdownRenderer.js";
...
export class InputBox extends Widget {
	...
	public set width(width: number) {
		if (this.options.flexibleHeight && this.options.flexibleWidth) {
			...
			if (this.mirror) {
				const paddingLeft =
					Number.parseFloat(this.mirror.style.paddingLeft || "") || 0;
				const paddingRight =
					Number.parseFloat(this.mirror.style.paddingRight || "") ||
					0;
	...
	implements IHistoryNavigationWidget
{
	...
	constructor(
		...
		this.onblur(this.input, () => {
			...
			if (
				!resetPlaceholder(NLS_PLACEHOLDER_HISTORY_HINT_SUFFIX_IN_PARENS)
			) {
	...
	public showNextValue(): void {
		...
		aria.status(
			this.value
				? this.value
				: nls.localize("clearedInput", "Cleared Input"),
		);
	}
	...
	public showPreviousValue(): void {
		...
		if (previous) {
			previous =
				previous === this.value ? this.getPreviousValue() : previous;
		}
