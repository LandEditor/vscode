...
} from "vs/base/browser/ui/toggle/toggle";
import {
	getVisibleState,
	isFilterResult,
...
class FindWidget<T, TFilterData> extends Disposable {
	...
	constructor(
		...
		this._register(
			toDisposable(() => container.removeChild(this.elements.root))
		);
...
class StickyScrollController<T, TFilterData, TRef> extends Disposable {
	...
	private findStickyState(
		...
		while (
			...
			firstVisibleNodeUnderWidget = this.getNextVisibleNode(
				firstVisibleNodeUnderWidget
			);
		...
		return stickyNodes.length
			? new StickyScrollState(stickyNodes)
			: undefined;
	}
	...
	private calculateStickyNodePosition(
		...
		if (
			stickyRowPositionTop > topOfLastChild &&
...
class StickyScrollWidget<T, TFilterData, TRef> implements IDisposable {
	...
	setState(state: StickyScrollState<T, TFilterData, TRef> | undefined): void {
		...
		for (
			...
			const previousStickyNode = stickyIndex
				? state.stickyNodes[stickyIndex - 1]
				: undefined;
			const currentWidgetHieght = previousStickyNode
		...
		this._rootDomNode.style.height = `${
	...
	private createElement(
		...
		const mouseListenerDisposable = this.registerMouseListeners(
			stickyElement,
			stickyNode,
			currentWidgetHeight
		);
	...
	private registerMouseListeners(
		...
		return addDisposableListener(
			...
			(e: MouseEvent) => {
				...
				if (
					isMonacoCustomToggle(e.target as HTMLElement) ||
					isActionItem(e.target as HTMLElement)
				) {
					return;
				}
				...
				this._mouseUpTimerDisposable.value = disposableTimeout(() => {
					const elementTop = this.view.getElementTop(
						stickyNode.startIndex
					);
	...
	implements IDisposable
{
	...
	constructor(
		...
		const activeNodes = this.disposables.add(
			new EventCollection(onDidChangeActiveNodes.event)
		);
		...
		onDidChangeCollapseStateRelay.input =
		...
		const activeNodesEmitter = this.disposables.add(
	...
	style(styles: IListStyles): void {
		...
		if (styles.listBackground) {
			content.push(
				`.monaco-list${suffix} .monaco-scrollable-element .monaco-tree-sticky-container { background-color: ${styles.listBackground}; }`
			);
			content.push(
				`.monaco-list${suffix} .monaco-scrollable-element .monaco-tree-sticky-container .monaco-tree-sticky-row { background-color: ${styles.listBackground}; }`
			);
		}
