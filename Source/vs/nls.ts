...
function createScopedLocalize(scope: string[]): IBoundLocalizeFunc {
	return (idx: number, defaultValue: null) => {
...
export function create(
	...
	return {
		...
		getConfiguredDefaultLocale:
			data.getConfiguredDefaultLocale ?? ((_: string) => undefined),
	};
}
...
export function load(
	...
	const messagesLoaded = (messages: string[] | IBundledStrings) => {
		if (Array.isArray(messages)) {
			(messages as any as IConsumerAPI).localize =
				createScopedLocalize(messages);
			(messages as any as IConsumerAPI).localize2 =
	...
	if (typeof pluginConfig.loadBundle === "function") {
		(pluginConfig.loadBundle as BundleLoader)(
			name,
			language,
			(err: Error, messages) => {
