	...
	implements IExtensionManagementService
{
	...
	async canInstall(
		...
		if (
			extension.allTargetPlatforms.some((targetPlatform) =>
				isTargetPlatformCompatible(
					targetPlatform,
		...
		return new MarkdownString(
			`${nls.localize("incompatible platform", "The '{0}' extension is not available in {1} for {2}.", extension.displayName ?? extension.identifier.id, this.productService.nameLong, TargetPlatformToString(currentTargetPlatform))} [${nls.localize("learn more", "Learn More")}](https://aka.ms/vscode-platform-specific-extensions)`,
		);
	}
