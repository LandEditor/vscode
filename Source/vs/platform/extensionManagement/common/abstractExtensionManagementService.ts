...
export type InstallExtensionTaskOptions = InstallOptions &
	InstallVSIXOptions & { readonly profileLocation: URI };
	...
	implements IExtensionManagementService
{
	...
	protected async installExtensions(
		...
		await Promise.allSettled(
			extensions.map(async (e) => {
				try {
					const result = await this.installExtension(
						e,
						(
	...
	private async installExtension(
		...
		const installExtensionTaskOptions: InstallExtensionTaskOptions = {
			...
			installOnlyNewlyAddedFromExtensionPack: URI.isUri(extension)
				...
				: true /* always true for gallery extensions */,
			isApplicationScoped,
		...
		const installResults: (InstallExtensionResult & {
		...
		try {
			if (installExtensionTaskOptions.donotIncludePackAndDependencies) {
				...
				try {
					...
					const options: InstallExtensionTaskOptions = {
						...
						context: {
							...installExtensionTaskOptions.context,
					...
					for (const { gallery, manifest } of distinct(
						...
						const existingInstallingExtension =
							this.installingExtensions.get(key);
						if (existingInstallingExtension) {
							...
							this.installingExtensions.set(key, {
								task,
								waitingTasks: [installExtensionTask],
							});
							this._onInstallExtension.fire({
			...
			const extensionsToInstallMap = allInstallExtensionTasks.reduce(
				(result, { task, manifest }) => {
					result.set(task.identifier.id.toLowerCase(), {
						task,
						manifest,
					});
					return result;
			...
			while (extensionsToInstallMap.size) {
				...
				const extensionsWithoutDepsToInstall = [
					...extensionsToInstallMap.values(),
				].filter(
					({ manifest }) =>
						!manifest.extensionDependencies?.some((id) =>
					...
					extensionsToInstall.map(async ({ task }) => {
						...
						try {
							...
							installResults.push({
								...
								profileLocation: task.profileLocation,
			...
			return allInstallExtensionTasks.map(({ task }) => ({
				identifier: task.identifier,
				operation: InstallOperation.Install,
				source: task.source,
	...
	private async uninstallExtension(
		...
		const postUninstallExtension = (
			...
			} else if (uninstallOptions.profileLocation) {
				this.logService.info(
...
export function joinErrors(
	...
	return errors.reduce<Error>(
		(previousValue: Error, currentValue: Error | string) => {
			return new Error(
