	...
	implements IExtensionManagementService
{
	...
	async canInstall(
		...
		return new MarkdownString(
			`${localize("incompatible platform", "The '{0}' extension is not available in {1} for {2}.", extension.displayName ?? extension.identifier.id, this.productService.nameLong, TargetPlatformToString(currentTargetPlatform))} [${localize("learn more", "Learn More")}](https://aka.ms/vscode-platform-specific-extensions)`,
		);
	}
