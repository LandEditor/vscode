...
type AllowedExtensionsConfigValueType = IStringDictionary<
	...
	implements IAllowedExtensionsService
{
	...
	readonly onDidChangeAllowedExtensions =
	...
	constructor(
		...
		for (const key in productService.extensionPublisherMappings) {
			this.publisherMappings[key.toLowerCase()] =
				productService.extensionPublisherMappings[key].toLowerCase();
		}
	...
	isAllowed(
		...
		publisher =
			this.publisherMappings[publisher]?.toLowerCase() ?? publisher;
