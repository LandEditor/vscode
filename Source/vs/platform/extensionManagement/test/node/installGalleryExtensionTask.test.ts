...
class TestExtensionsScanner extends mock<ExtensionsScanner>() {
	override async scanExtensions(): Promise<ILocalExtension[]> {
		return [];
	}
}
...
class TestInstallGalleryExtensionTask extends InstallGalleryExtensionTask {
	...
	constructor(
		...
		const logService = instantiationService.stub(
			ILogService,
			new NullLogService()
		);
		const fileService = instantiationService.stub(
			IFileService,
			disposables.add(new FileService(logService))
		);
		...
		const extensionsProfileScannerService = instantiationService.stub(
		...
		const extensionsScannerService = instantiationService.stub(
		...
		super(
			...
			{
				profileLocation:
					userDataProfilesService.defaultProfile.extensionsResource,
			},
			extensionDownloader,
...
suite("InstallGalleryExtensionTask Tests", () => {
	...
	test("if verification is enabled by default, the task completes", async () => {
		const testObject = new TestInstallGalleryExtensionTask(
			aGalleryExtension("a", { isSigned: true }),
			anExtensionsDownloader({
	...
	function anExtensionsDownloader(options: {
		...
		instantiationService.stub(INativeEnvironmentService, <
			Partial<INativeEnvironmentService>
		>{
			...
			new TestConfigurationService(
				isBoolean(options.isSignatureVerificationEnabled)
					? {
							extensions: {
								verifySignature:
									options.isSignatureVerificationEnabled,
							},
					  }
					: undefined
			)
