...
suite("NativeExtensionsScanerService Test", () => {
	...
	test("scan user extension", async () => {
		const manifest: Partial<IScannedExtensionManifest> =
			anExtensionManifest({
				"name": "name",
				"publisher": "pub",
				__metadata: { id: "uuid" },
	...
	test("scan exclude uninstalled extensions", async () => {
		...
		await instantiationService
			...
			.writeFile(
				joinPath(
					URI.file(
						instantiationService.get(INativeEnvironmentService)
							.extensionsPath
					),
					".obsolete"
				),
				VSBuffer.fromString(JSON.stringify({ "pub.name2-1.0.0": true }))
			);
		const testObject: IExtensionsScannerService = disposables.add(
			instantiationService.createInstance(ExtensionsScannerService)
		);
	...
	async function anExtension(
		...
		const extensionLocation = joinPath(
			...
			`${manifest.publisher}.${manifest.name}-${manifest.version}-${
				manifest.__metadata?.targetPlatform ?? TargetPlatform.UNDEFINED
			}`
		);
