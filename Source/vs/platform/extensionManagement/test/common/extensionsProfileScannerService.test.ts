...
suite("ExtensionsProfileScannerService", () => {
	...
	test("extensions in mixed format is read and migrated", async () => {
		...
		const extension3 = aExtension(
			"pub.c",
			joinPath(extensionsLocation, "pub.c-1.0.0")
		);
		...
		assert.deepStrictEqual(
			...
			[
				{
					identifier: extension1.identifier,
					location: extension1.location.toJSON(),
					version: extension1.manifest.version,
					metadata: undefined,
				},
				{
	...
	test("add extension trigger events", async () => {
		...
		assert.deepStrictEqual(
			(<ProfileExtensionsEvent>target1.args[0][0]).extensions[0]
				.identifier,
		...
		assert.deepStrictEqual(
			(<ProfileExtensionsEvent>target2.args[0][0]).extensions[0]
				.identifier,
			extension.identifier
		);
		assert.deepStrictEqual(
