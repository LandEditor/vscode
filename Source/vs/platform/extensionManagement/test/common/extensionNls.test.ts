...
suite("Localize Manifest", () => {
	...
	test("replaces template strings", function () {
		...
		assert.strictEqual(
			(localizedManifest.contributes?.configuration as IConfiguration)
				.title,
			"Test Configuration"
		);
	});
	...
	test("replaces template strings - command title & categories become ILocalizedString", function () {
		...
		const title = localizedManifest.contributes?.commands?.[0]
			.title as ILocalizedString;
		const category = localizedManifest.contributes?.commands?.[0]
			.category as ILocalizedString;
