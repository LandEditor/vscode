...
export class CodeWindow extends BaseWindow implements ICodeWindow {
	...
	private joinNativeFullScreenTransition: DeferredPromise<void> | undefined =
		undefined;
		...
		//#region create browser window
		{
			...
			if (useCustomTitleStyle) {
				...
				if (useWindowControlsOverlay(this.configurationService)) {
					...
					const titleBarColor =
						this.themeMainService.getWindowSplash()?.colorInfo
							.titleBarBackground ??
			...
			if (
				...
				const cachedWindowControlHeight =
					this.stateService.getItem<number>(
				...
				this._win.hookWindowMessage(WM_INITMENU, () => {
					...
					const shouldTriggerDefaultSystemContextMenu = () => {
						...
						if (
							...
							cy <=
								Math.max(
									this._win.getBounds().height * 0.15,
									35
								)
						) {
							return false;
		...
		if (!e || e.affectsConfiguration("http.proxy")) {
			...
			const newNoProxy =
				(
					...
					process.env["NO_PROXY"] ||
					""
				).trim() || undefined; // Not standardized.
			if (
	...
	updateWindowControls(options: {
		...
		// macOS: traffic lights
			...
			if (!verticalOffset) {
				...
				this._win.setWindowButtonPosition({
					x: verticalOffset,
					y: verticalOffset,
				});
			}
	...
	private useNativeFullScreen(): boolean {
		...
		if (
			!windowConfig ||
			typeof windowConfig.nativeFullScreen !== "boolean"
		) {
			return true; // default
		}
