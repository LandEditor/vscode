...
import {
	...
	watchFileContents,
} from "vs/platform/files/node/watcher/nodejs/nodejsWatcherLib";
...
(process.env["BUILD_SOURCEVERSION"] || process.env["CI"]
	...
	(isMacintosh /* macOS: does not seem to report this */ ? test.skip : test)(
		"deleting watched path is handled properly (folder watch)",
		async function () {
			const watchedPath = join(testDir, "deep");

			const watcher = new TestNodeJSFileWatcherLibrary(
				{ path: watchedPath, excludes: [], recursive: false },
				(changes) => {}
			);
			await watcher.ready;
