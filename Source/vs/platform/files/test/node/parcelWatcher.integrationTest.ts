...
(process.env["BUILD_SOURCEVERSION"] || process.env["CI"]
	? suite.skip
	...
	(!isWindows /* UNC is windows only */ ? test.skip : test)(
		...
		async function () {
			...
			const uncPath = `\\\\localhost\\${getDriveLetter(
				testDir
			)?.toLowerCase()}$\\${ltrim(
				testDir.substr(testDir.indexOf(":") + 1),
				"\\"
			)}`;
	...
	test("should not exclude roots that do not overlap", () => {
		if (isWindows) {
			assert.deepStrictEqual(watcher.testNormalizePaths(["C:\\a"]), [
				"C:\\a",
			]);
			assert.deepStrictEqual(
				watcher.testNormalizePaths(["C:\\a", "C:\\b"]),
				["C:\\a", "C:\\b"]
			);
			assert.deepStrictEqual(
				watcher.testNormalizePaths(["C:\\a", "C:\\b", "C:\\c\\d\\e"]),
				["C:\\a", "C:\\b", "C:\\c\\d\\e"]
			);
		} else {
			assert.deepStrictEqual(watcher.testNormalizePaths(["/a"]), ["/a"]);
			assert.deepStrictEqual(watcher.testNormalizePaths(["/a", "/b"]), [
				"/a",
				"/b",
			]);
			assert.deepStrictEqual(
				watcher.testNormalizePaths(["/a", "/b", "/c/d/e"]),
				["/a", "/b", "/c/d/e"]
			);
		}
	});
	...
	test("should remove sub-folders of other paths", () => {
		if (isWindows) {
			assert.deepStrictEqual(
				watcher.testNormalizePaths(["C:\\a", "C:\\a\\b"]),
				["C:\\a"]
			);
			assert.deepStrictEqual(
				watcher.testNormalizePaths(["C:\\a", "C:\\b", "C:\\a\\b"]),
				["C:\\a", "C:\\b"]
			);
			assert.deepStrictEqual(
				watcher.testNormalizePaths([
					"C:\\b\\a",
					"C:\\a",
					"C:\\b",
					"C:\\a\\b",
			...
			assert.deepStrictEqual(watcher.testNormalizePaths(["/a", "/a/b"]), [
				"/a",
			]);
			assert.deepStrictEqual(
				watcher.testNormalizePaths(["/a", "/b", "/a/b"]),
				["/a", "/b"]
			);
			assert.deepStrictEqual(
				watcher.testNormalizePaths(["/b/a", "/a", "/b", "/a/b"]),
				["/a", "/b"]
			);
			assert.deepStrictEqual(
				watcher.testNormalizePaths(["/a", "/a/b", "/a/c/d"]),
				["/a"]
			);
		}
	});
