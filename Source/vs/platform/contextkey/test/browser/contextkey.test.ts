...
import {
	ContextKeyService,
	setContext,
} from "vs/platform/contextkey/browser/contextKeyService";
import {
	ContextKeyExpr,
	IContextKeyService,
	...
	test("updateParent", () => {
		...
		testDisposables.add(
			child.onDidChangeContext((e) => {
				try {
					assert.ok(
						e.affectsSome(new Set(["testA"])),
						"testA changed"
					);
					assert.ok(
						e.affectsSome(new Set(["testB"])),
						"testB changed"
					);
					assert.ok(
						e.affectsSome(new Set(["testC"])),
						"testC changed"
					);
					assert.ok(
						!e.affectsSome(new Set(["testD"])),
