...
import {
	...
	IOSProperties,
	IOSStatistics,
} from "vs/platform/native/common/native";
	...
	implements INativeHostMainService
{
	...
	readonly onDidOpenWindow = Event.map(
		this.windowsMainService.onDidOpenWindow,
		(window) => window.id
	);
	readonly onDidTriggerSystemContextMenu = Event.filter(
		Event.map(
			this.windowsMainService.onDidTriggerSystemContextMenu,
			({ window, x, y }) => {
				return { windowId: window.id, x, y };
			}
		),
		({ windowId }) => !!this.windowsMainService.getWindowById(windowId)
	);

	readonly onDidMaximizeWindow = Event.filter(
	...
	readonly onDidUnmaximizeWindow = Event.filter(
		Event.fromNodeEventEmitter(
			app,
	...
	readonly onDidBlurWindow = Event.filter(
		Event.fromNodeEventEmitter(
			app,
			"browser-window-blur",
			(event, window: BrowserWindow) => window.id
	...
	async moveWindowTop(
		windowId: number | undefined,
		options?: { targetWindowId?: number }
	): Promise<void> {
		const window =
			this.windowById(options?.targetWindowId) ??
	...
	async positionWindow(
		firstArg: number | undefined,
		position: IRectangle,
		options?: { targetWindowId?: number | undefined } | undefined
	): Promise<void> {
		const window =
			this.windowById(options?.targetWindowId) ??
			this.codeWindowById(firstArg);
		if (window?.win) {
	...
	async focusWindow(
		windowId: number | undefined,
		options?: { targetWindowId?: number; force?: boolean }
	): Promise<void> {
		const window =
			this.windowById(options?.targetWindowId) ??
			this.codeWindowById(windowId);
	...
	async closeWindowById(
		...
		const window =
			this.windowById(targetWindowId) ??
			this.codeWindowById(targetWindowId);
	...
	private windowById(
		...
		return (
			this.codeWindowById(windowId) ?? this.auxiliaryWindowById(windowId)
		);
	}
	...
	private getTargetWindow(
		...
		let window = this.instantiationService.invokeFunction(
			getFocusedOrLastActiveWindow
		);
		if (!window) {
