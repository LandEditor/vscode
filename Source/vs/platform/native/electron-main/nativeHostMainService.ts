	...
	implements INativeHostMainService
{
	...
	readonly onDidTriggerMainWindowSystemContextMenu = Event.filter(
		Event.map(
	...
	readonly onDidMaximizeMainWindow = Event.filter(
	...
	readonly onDidUnmaximizeMainWindow = Event.filter(
		Event.fromNodeEventEmitter(
			app,
		...
		Event.map(
			Event.filter(
				Event.fromNodeEventEmitter(
					app,
					"browser-window-blur",
					(event, window: BrowserWindow) => window
				),
				(window) => isAuxiliaryWindow(window.webContents)
			),
			(window) => window.id
		)
	);
	...
	async focusWindow(
		windowId: number | undefined,
		options?: INativeOptions & { force?: boolean }
	): Promise<void> {
		const window = this.windowById(options?.targetWindowId, windowId);
