...
suite("DefaultConfiguration", () => {
	...
	function reset() {
		...
		const configurationDefaultsOverrides =
			configurationRegistry.getConfigurationDefaultsOverrides();
		configurationRegistry.deregisterDefaultConfigurations(
			[...configurationDefaultsOverrides.keys()].map((key) => ({
				extensionId: configurationDefaultsOverrides.get(key)?.source,
				overrides: {
					[key]: configurationDefaultsOverrides.get(key)?.value,
				},
			}))
		);
	}
	...
	test("Test registering a normal property and override identifier", async () => {
		...
		assert.ok(equals(actual.getValue("[a]"), { "b": true }));
		assert.ok(
			equals(actual.contents, { "b": false, "[a]": { "b": true } })
		);
		assert.ok(
			equals(actual.overrides, [
				{ contents: { "b": true }, identifiers: ["a"], keys: ["b"] },
	...
	test("Test deregistering an override identifier", async () => {
		...
		assert.ok(
			equals(testObject.configurationModel.contents, { "b": false })
		);
