...
class ListElement implements IListElement {
	...
	private readonly _onChecked: Emitter<{
...
class ListElementRenderer
	implements IListRenderer<IListElement, IListElementTemplateData>
{
	...
	renderElement(
		...
		const mainItem: QuickPickItem = element.item
			? element.item
			: element.separator!;
		...
		if (element.item?.iconPath) {
			const icon = isDark(this.themeService.getColorTheme().type)
				? element.item.iconPath.dark
				: element.item.iconPath.light ?? element.item.iconPath.dark;
			const iconUrl = URI.revive(icon);
			data.icon.className = "quick-input-list-icon";
			data.icon.style.backgroundImage = dom.asCSSUrl(iconUrl);
		} else {
			data.icon.style.backgroundImage = "";
			data.icon.className = element.item?.iconClass
				? `quick-input-list-icon ${element.item.iconClass}`
				: "";
		}
...
export class QuickInputList {
	...
	private readonly _listElementChecked = new Emitter<{
		listElement: IListElement;
		checked: boolean;
	}>();
	...
	constructor(
		...
		this.list = options.createList(
			...
			this.container,
			delegate,
			[new ListElementRenderer(themeService)],
			{
				identityProvider: {
					getId: (element) => {
			...
			this.disposables.push(
				this.list.onMouseOver(async (e) => {
					...
					if (
						...
						!(
							e.browserEvent.relatedTarget instanceof
							HTMLAnchorElement
						) &&
	...
	toggleHover() {
		const element: IListElement | undefined =
...
class QuickInputAccessibilityProvider
	implements IListAccessibilityProvider<IListElement>
{
