 ...
 */
export enum ShellIntegrationOscPs {
	/**
	...
	implements IShellIntegration, ITerminalAddon
{
	...
	private _hasUpdatedTelemetry = false;
	...
	private _doHandleFinalTermSequence(data: string): boolean {
		...
		switch (command) {
			...
			case FinalTermOscPt.CommandFinished: {
				const exitCode =
					args.length === 1 ? Number.parseInt(args[0]) : undefined;
				this._createOrGetCommandDetection(
					this._terminal,
	...
	private _doHandleVSCodeSequence(data: string): boolean {
		...
		switch (sequenceCommand) {
			...
			case VSCodeOscPt.CommandFinished: {
				const arg0 = args[0];
				const exitCode =
					arg0 !== undefined ? Number.parseInt(arg0) : undefined;
				this._createOrGetCommandDetection(
					this._terminal,
				).handleCommandFinished(exitCode);
				return true;
