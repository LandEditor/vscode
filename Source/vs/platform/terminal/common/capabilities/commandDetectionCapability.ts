	...
	implements ICommandDetectionCapability
{
	...
	private get _isInputting(): boolean {
		return !!(
			...
			!this._currentCommand.commandExecutedMarker
		);
	}
	...
	get hasInput(): boolean | undefined {
		...
		if (
			...
			const line = this._terminal.buffer.active
				.getLine(this._terminal.buffer.active.cursorY)
				?.translateToString(true, this._currentCommand.commandStartX);
			if (line === undefined) {
				return undefined;
			}
	...
	handleCommandStart(options?: IHandleCommandOptions): void {
		...
		this._onCommandStarted.fire({
			marker: options?.marker || this._currentCommand.commandStartMarker,
	...
	private async _handleCommandStartWindows(): Promise<void> {
		...
		if (!this._cursorOnNextLine() || !prompt) {
			...
			for (; i < 20; i++) {
				...
				if (
					...
					!this._windowsPromptPollingInProcess ||
					(this._cursorOnNextLine() && prompt)
				) {
					if (!this._windowsPromptPollingInProcess) {
						this._logService.debug(
							"CommandDetectionCapability#_handleCommandStartWindows polling cancelled"
						);
					}
					break;
				}
			...
			if (i === 20) {
				this._logService.debug(
					"CommandDetectionCapability#_handleCommandStartWindows reached max attempts, ",
					this._cursorOnNextLine(),
					this._getWindowsPrompt()
				);
			} else if (prompt) {
		...
		if (!this._currentCommand.commandExecutedMarker) {
			this._onCursorMoveListener = this._terminal.onCursorMove(() => {
				if (
					this._commandMarkers.length === 0 ||
					this._commandMarkers[this._commandMarkers.length - 1]
		...
		if (this._currentCommand.commandStartMarker) {
			...
			if (line) {
				this._currentCommand.commandStartLineContent =
					line.translateToString(true);
			}
		}
		this._onCommandStarted.fire({
	...
	handleCommandExecuted(options?: IHandleCommandOptions): void {
		...
		this._currentCommand.commandExecutedMarker =
			options?.marker || this._terminal.registerMarker(0);
		this._currentCommand.commandExecutedX =
			this._terminal.buffer.active.cursorX;
		this._logService.debug(
			"CommandDetectionCapability#handleCommandExecuted",
			this._currentCommand.commandExecutedX,
			this._currentCommand.commandExecutedMarker?.line
		);
		...
		if (
			!this._currentCommand.commandStartMarker ||
			!this._currentCommand.commandExecutedMarker ||
			this._currentCommand.commandStartX === undefined
		) {
			return;
		}

		// Calculate the command
		this._currentCommand.command = this.__isCommandStorageDisabled
			? ""
			: this._terminal.buffer.active
					.getLine(this._currentCommand.commandStartMarker.line)
					?.translateToString(
						true,
						this._currentCommand.commandStartX,
						this._currentCommand.commandRightPromptStartX
		...
		const commandExecutedLine =
			this._currentCommand.commandExecutedMarker.line;
		for (; y < commandExecutedLine; y++) {
			const line = this._terminal.buffer.active.getLine(y);
			if (line) {
				const continuation = this._currentCommand.continuations?.find(
					(e) => e.marker.line === y
				);
				if (continuation) {
				...
				this._currentCommand.command += line.translateToString(
					true,
					startColumn
				);
			}
	...
	handleCommandFinished(
		...
		this._logService.debug(
			...
			this._terminal.buffer.active.cursorX,
			this._currentCommand.commandFinishedMarker?.line,
			this._currentCommand.command,
		...
		if (
			...
			this._handleCommandStartOptions?.ignoreCommandLine
		) {
			...
			const newCommand: ITerminalCommand = {
				command: this._handleCommandStartOptions?.ignoreCommandLine
					? ""
					: command || "",
				...
				commandStartLineContent:
					this._currentCommand.commandStartLineContent,
				hasOutput: () =>
					!executedMarker?.isDisposed &&
					!endMarker?.isDisposed &&
					!!(
						executedMarker &&
						endMarker &&
				...
				getOutput: () =>
					getOutputForCommand(executedMarker, endMarker, buffer),
				getOutputMatch: (outputMatcher: ITerminalOutputMatcher) =>
		...
		this._currentCommand.previousCommandMarker =
			this._currentCommand.commandStartMarker;
		this._currentCommand = {};
	...
	private _evaluateCommandMarkersWindows(): void {
		...
		this._commandMarkers = this._commandMarkers.sort(
			(a, b) => a.line - b.line
		);
	...
	serialize(): ISerializedCommandDetectionCapability {
		const commands: ISerializedTerminalCommand[] = this.commands.map(
			(e) => {
				return {
	...
	deserialize(serialized: ISerializedCommandDetectionCapability): void {
		...
		for (const e of serialized.commands) {
			...
			const endMarker =
				e.endLine !== undefined
					? this._terminal.registerMarker(
							e.endLine - (buffer.baseY + buffer.cursorY)
					  )
					: undefined;
			const executedMarker =
				e.executedLine !== undefined
					? this._terminal.registerMarker(
							e.executedLine - (buffer.baseY + buffer.cursorY)
					  )
					: undefined;
			const newCommand: ITerminalCommand = {
				command: this.__isCommandStorageDisabled ? "" : e.command,
...
function getOutputMatchForCommand(
	executedMarker: IMarker | undefined,
	endMarker: IMarker | undefined,
	buffer: IBuffer,
	cols: number,
