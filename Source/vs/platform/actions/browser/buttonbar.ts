...
export class MenuWorkbenchButtonBar extends ButtonBar {
	...
	constructor(
		...
		const conifgProvider: IButtonConfigProvider =
			options?.buttonConfigProvider ?? (() => ({ showLabel: true }));
		...
		const update = () => {
			...
			for (let i = 0; i < actions.length; i++) {
				...
				if (
					actionOrSubmenu instanceof SubmenuItemAction &&
					...
					btn = this.addButtonWithDropdown({
						secondary:
							conifgProvider(action)?.isSecondary ?? secondary,
						actionRunner,
						actions: rest,
				...
				if (
					conifgProvider(action)?.showIcon &&
