...
export class UserDataAutoSyncChannel implements IServerChannel {
	...
	call(context: any, command: string, args?: any): Promise<any> {
		switch (command) {
			case "triggerSync":
				return this.service.triggerSync(args[0], args[1], args[2]);
			case "turnOn":
				return this.service.turnOn();
			case "turnOff":
				return this.service.turnOff(args[0]);
		}
...
export class UserDataSycnUtilServiceChannel implements IServerChannel {
	...
	call(context: any, command: string, args?: any): Promise<any> {
		switch (command) {
			case "resolveDefaultIgnoredSettings":
				return this.service.resolveDefaultIgnoredSettings();
			case "resolveUserKeybindings":
...
export class UserDataSyncMachinesServiceChannel implements IServerChannel {
	...
	listen(_: unknown, event: string): Event<any> {
		switch (event) {
			case "onDidChange":
				return this.service.onDidChange;
		}
		throw new Error(`Event not found: ${event}`);
	}
	...
	async call(context: any, command: string, args?: any): Promise<any> {
		switch (command) {
			...
			case "addCurrentMachine":
				return this.service.addCurrentMachine();
			case "removeCurrentMachine":
				return this.service.removeCurrentMachine();
			case "renameMachine":
				return this.service.renameMachine(args[0], args[1]);
			case "setEnablements":
				return this.service.setEnablements(args);
		}
		throw new Error("Invalid call");
	}
}
