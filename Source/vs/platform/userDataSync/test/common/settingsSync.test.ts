...
suite("SettingsSync - Auto", () => {
	...
	test("when settings file is empty and remote has no changes", () =>
		runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
			...
			const remoteUserData = await testObject.getRemoteUserData(null);
			assert.strictEqual(
				parseSettingsSyncContent(lastSyncUserData!.syncData!.content!)
					?.settings,
				"{}"
			);
			assert.strictEqual(
				parseSettingsSyncContent(remoteUserData!.syncData!.content!)
					?.settings,
				"{}"
			);
			assert.strictEqual(
...
async function updateSettings(
	...
	await client.instantiationService
		...
		.writeFile(
			(
				profile ??
				client.instantiationService.get(IUserDataProfilesService)
					.defaultProfile
			).settingsResource,
			VSBuffer.fromString(content)
		);
