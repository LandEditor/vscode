...
export class UserDataSyncClient extends Disposable {
	...
	async setUp(empty: boolean = false): Promise<void> {
		...
		this.instantiationService.stub(
			...
			this._register(
				new TestUserDataProfileStorageService(storageService)
			)
		);
		...
		>{
			...
			onDidInstallExtensions: new Emitter<
				readonly InstallExtensionResult[]
			>().event,
			onDidUninstallExtension: new Emitter<DidUninstallExtensionEvent>()
				.event,
		});
		this.instantiationService.stub(
			IGlobalExtensionEnablementService,
	...
	getSynchronizer(source: SyncResource): IUserDataSynchroniser {
		return (
			this.instantiationService.get(
				IUserDataSyncService
			) as UserDataSyncService
		)
			.getOrCreateActiveProfileSynchronizer(
				this.instantiationService.get(IUserDataProfilesService)
					.defaultProfile,
				undefined
			)
		...
		if (this.session) {
			...
			if (this.collectionCounter) {
				...
				for (
					...
					if (collectionData) {
						const latest: Record<ServerResource, string> =
							Object.create({});
						collectionData.forEach(
							(value, key) => (latest[key] = value.ref)
						);
