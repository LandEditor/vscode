...
<div id="issue-reporter">
	<div id="english" class="input-group hidden">${escape(
		localize("completeInEnglish", "Please complete the form in English.")
	)}</div>
	...
	<div class="section">
		<div class="input-group">
			<label class="inline-label" for="issue-type">${escape(
				localize("issueTypeLabel", "This is a")
			)}</label>
		...
		<div class="input-group" id="problem-source">
			<label class="inline-label" for="issue-source">${escape(
				localize("issueSourceLabel", "File on")
			...
			<div id="problem-source-help-text" class="instructions hidden">${escape(
				...
				"{0}",
				() =>
					`<span tabIndex=0 role="button" id="disableExtensions" class="workbenchCommand">${escape(
						localize(
							"disableExtensions",
			...
			<div id="extension-selection">
				<label class="inline-label" for="extension-selector">${escape(
					localize("chooseExtension", "Extension")
				)} <span class="required-input">*</span></label>
	...
	<div class="input-group description-section">
		...
		<div class="block-info-text">
			<textarea name="description" id="description" placeholder="${escape(
				localize("details", "Please enter details.")
			)}" required></textarea>
		</div>
		<div id="description-empty-error" class="validation-error hidden" role="alert">${escape(
			localize("descriptionEmptyValidation", "A description is required.")
		)}</div>
	</div>
		...
		<div class="block block-extension-data">
			...
			<label class="extension-caption" id="extension-caption" for="includeExtensionData">
				...
				<span class="ext-parens" hidden>(</span><a href="#" class="showInfo" id="extension-id">${escape(
					localize("show", "show")
				)}</a><span class="ext-parens" hidden>)</span>
			</label>
