...
VS Code version: ${
			this._data.versionInfo && this._data.versionInfo.vscodeVersion
		}
...
<!-- generated by issue reporter -->`;
	...
	private getInfos(): string {
		...
		if (
			...
			if (
				!this._data.fileOnMarketplace &&
				this._data.includeExtensionData &&
		...
		if (
			...
			if (
				!this._data.fileOnMarketplace &&
				this._data.includeSystemInfo &&
				this._data.systemInfo
			) {
		...
		if (this._data.issueType === IssueType.PerformanceIssue) {
			...
			if (
				!this._data.fileOnMarketplace &&
				this._data.includeWorkspaceInfo
			) {
		...
		if (
			...
			if (
				!this._data.fileOnMarketplace &&
				!this._data.fileOnExtension &&
				this._data.includeExtensions
			) {
...
---|---|---`;
		const table = this._data.enabledNonThemeExtesions
			.map((e) => {
				return `${e.name}|${e.publisher?.substr(0, 3) ?? "N/A"}|${
					e.version
				}`;
			})
			.join("\n");
