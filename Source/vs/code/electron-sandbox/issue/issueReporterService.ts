...
import {
	...
	IssueReporterModel,
} from "vs/code/electron-sandbox/issue/issueReporterModel";
...
import {
	...
	IssueReporterStyles,
	IssueReporterWindowConfiguration,
	IssueType,
...
export class IssueReporter extends Disposable {
		...
		this.issueReporterModel = new IssueReporterModel({
			...
			versionInfo: {
				vscodeVersion: `${configuration.product.nameShort} ${
					...
					configuration.product.date || "Date unknown"
				})`,
				os: `${this.configuration.os.type} ${
					this.configuration.os.arch
				} ${this.configuration.os.release}${
					isLinuxSnap ? " snap" : ""
				}`,
			},
	...
	setInitialFocus() {
		...
		if (fileOnExtension) {
			const issueTitle =
				mainWindow.document.getElementById("issue-title");
			issueTitle?.focus();
		} else {
	...
	private handleExtensionData(extensions: IssueReporterExtensionData[]) {
		...
		if (
			this.configuration.disableExtensions ||
	...
	private async updateIssueReporterUri(
		...
		try {
			const uri = await this.issueMainService.$getIssueReporterUri(
				extension.id,
			);
	...
	private async getIssueDataFromExtension(
		extension: IssueReporterExtensionData,
	): Promise<string> {
		try {
			const data = await this.issueMainService.$getIssueReporterData(
				extension.id,
			);
	...
	private async getIssueTemplateFromExtension(
		extension: IssueReporterExtensionData,
	): Promise<string> {
		try {
			const data = await this.issueMainService.$getIssueReporterTemplate(
				extension.id,
			);
	...
	private async getReporterStatus(
		extension: IssueReporterExtensionData,
	): Promise<boolean[]> {
		try {
			const data = await this.issueMainService.$getReporterStatus(
				extension.id,
				extension.name,
			);
			return data;
	...
	private setEventHandlers(): void {
		...
		const showInfoElements =
			mainWindow.document.getElementsByClassName("showInfo");
	...
	private isPreviewEnabled() {
		...
		if (
			this.issueReporterModel.getData().selectedExtension
				?.hasIssueDataProviders &&
			!this.receivedExtensionData
		) {
			return false;
		}
	...
	private getExtensionData(): string | undefined {
		return this.issueReporterModel.getData().selectedExtension
			?.extensionData;
	}
	...
	private setUpTypes(): void {
		...
		reset(
			...
			makeOption(
				...
				localize("performanceIssue", "Performance Issue"),
			),
		);
	...
	private setSourceOptions(): void {
		...
		const { issueType, fileOnExtension, selectedExtension } =
			this.issueReporterModel.getData();
		...
		sourceSelect.append(
			this.makeOption(
				"extension",
				localize("extension", "An extension"),
				false,
			),
		);
	...
	private renderBlocks(): void {
		...
		const processBlock =
			mainWindow.document.querySelector(".block-process");
		const workspaceBlock =
			mainWindow.document.querySelector(".block-workspace");
		const extensionsBlock =
			mainWindow.document.querySelector(".block-extensions");
		const experimentsBlock =
			mainWindow.document.querySelector(".block-experiments");
		...
		if (fileOnExtension && selectedExtension?.hasIssueDataProviders) {
			...
			if (data) {
				(extensionDataTextArea as HTMLElement).innerText =
					data.toString();
			}
	...
	private async createIssue(): Promise<boolean> {
		...
		if (!this.validateInputs()) {
			...
			const invalidInput =
				mainWindow.document.getElementsByClassName("invalid-input");
	...
	private updateSystemInfo(state: IssueReporterModelData) {
		const target = mainWindow.document.querySelector<HTMLElement>(
				...
				async (e: Event) => {
					...
					if (matches.length) {
						...
						if (matches[0].hasIssueUriRequestHandler) {
							...
							const template =
								await this.getIssueTemplateFromExtension(
									matches[0],
								);
							const descriptionTextArea =
								this.getElementById("description")!;
							const descriptionText = (
								descriptionTextArea as HTMLTextAreaElement
							).value;
							if (
								descriptionText === "" ||
								!descriptionText.includes(template)
							) {
								const fullTextArea =
									descriptionText +
									(descriptionText === "" ? "" : "\n") +
	...
	private getElementById<T extends HTMLElement = HTMLElement>(
		...
		const element = mainWindow.document.getElementById(elementId) as
			| T
			| undefined;
		if (element) {
			return element;
