...
} from "vs/platform/diagnostics/electron-main/diagnosticsMainService";
import {
	DialogMainService,
	IDialogMainService,
} from "vs/platform/dialogs/electron-main/dialogMainService";
...
} from "vs/platform/externalTerminal/node/externalTerminalService";
...
} from "vs/platform/keyboardLayout/electron-main/keyboardLayoutMainService";
import {
	ILaunchMainService,
	LaunchMainService,
} from "vs/platform/launch/electron-main/launchMainService";
import {
...
import {
	IMenubarMainService,
	MenubarMainService,
} from "vs/platform/menubar/electron-main/menubarMainService";
import {
	INativeHostMainService,
	NativeHostMainService,
} from "vs/platform/native/electron-main/nativeHostMainService";
...
import {
	...
	URLHandlerRouter,
} from "vs/platform/url/common/urlIpc";
...
} from "vs/platform/workspaces/electron-main/workspacesHistoryMainService";
...
import {
	resolveMachineId,
	resolveSqmId,
} from "vs/platform/telemetry/electron-main/telemetryUtils";
...
import {
	...
	IProtocolUrl,
} from "vs/platform/url/electron-main/url";
import {
	IUtilityProcessWorkerMainService,
	UtilityProcessWorkerMainService,
} from "vs/platform/utilityProcess/electron-main/utilityProcessWorkerMainService";
...
export class CodeApplication extends Disposable {
	...
	private registerListeners(): void {
		...
		app.on("web-contents-created", (event, contents) => {
			...
			const isChildWindow = contents?.opener?.url.startsWith(
			...
			contents.setWindowOpenHandler((handler) => {
				...
				if (handler.url === "about:blank") {
					this.logService.trace(
						"webContents#setWindowOpenHandler: Allowing about:blank window to open"
					);
				...
				// Any other URL: delegate to OS
					...
					this.logService.trace(
						`webContents#setWindowOpenHandler: Prevented opening window with URL ${handler.url}}`
					);
		...
		validatedIpcMain.handle("vscode:getWindowId", (event) =>
			Promise.resolve(event.sender.id)
		);
		...
		validatedIpcMain.handle(
			"vscode:writeNlsFile",
			(event, path: unknown, data: unknown) => {
	...
	private validateNlsPath(pathSegments: unknown[]): URI | undefined {
		...
		if (
			...
			!isEqualOrParent(
				path,
				this.environmentMainService.cachedLanguagesPath,
				!isLinux
			)
		) {
			return undefined;
	...
	async startup(): Promise<void> {
		...
		const { sharedProcessReady, sharedProcessClient } =
			this.setupSharedProcess(machineId, sqmId);
		...
		// Auth Handler
		this._register(
			appInstantiationService.createInstance(ProxyAuthHandler)
		);
	...
	private async shouldBlockURI(
		uri: URI,
		windowsMainService: IWindowsMainService,
		dialogMainService: IDialogMainService
	): Promise<boolean> {
		if (
			uri.authority !== Schemas.file &&
			uri.authority !== Schemas.vscodeRemote
		) {
			// !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
			//
		...
		if (checkboxChecked) {
			...
			const request = {
				...
				args: uri.authority === Schemas.file ? "local" : "remote",
			};
	...
	private initChannels(
		...
		const fileSystemProviderChannel = new DiskFileSystemProviderChannel(
		...
		const storageChannel = this._register(
		...
		const profileStorageListener = this._register(
	...
	private async openFirstWindow(
		...
		return windowsMainService.open({
			...
			cli: args,
			forceNewWindow:
				args["new-window"] || (!hasCliArgs && args["unity-launch"]),
			diffMode: args.diff,
